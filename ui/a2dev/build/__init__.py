...
def _set_rc_key(target_path, key, value_string):
    ...
    """
    error, current = subprocess.getstatusoutput([Paths.rcedit, target_path, '--get-version-string', key])
    if error:
        current = ''
    ...
    current = subprocess.check_output([Paths.rcedit, target_path, '--get-version-string', key]).decode()
    if current == value_string:
        print(f'{CHK_MK}')
    else:
