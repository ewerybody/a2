#############################################################################
##
## Copyright (C) 2021 The Qt Company Ltd.
## Contact: https://www.qt.io/licensing/
##
## This file is part of Qt for Python.
##
## $QT_BEGIN_LICENSE:LGPL$
## Commercial License Usage
## Licensees holding valid commercial Qt licenses may use this file in
## accordance with the commercial license agreement provided with the
## Software or, alternatively, in accordance with the terms contained in
## a written agreement between you and The Qt Company. For licensing terms
## and conditions see https://www.qt.io/terms-conditions. For further
## information use the contact form at https://www.qt.io/contact-us.
##
## GNU Lesser General Public License Usage
## Alternatively, this file may be used under the terms of the GNU Lesser
## General Public License version 3 as published by the Free Software
## Foundation and appearing in the file LICENSE.LGPL3 included in the
## packaging of this file. Please review the following information to
## ensure the GNU Lesser General Public License version 3 requirements
## will be met: https://www.gnu.org/licenses/lgpl-3.0.html.
##
## GNU General Public License Usage
## Alternatively, this file may be used under the terms of the GNU
## General Public License version 2.0 or (at your option) the GNU General
## Public license version 3 or any later version approved by the KDE Free
## Qt Foundation. The licenses are as published by the Free Software
## Foundation and appearing in the file LICENSE.GPL2 and LICENSE.GPL3
## included in the packaging of this file. Please review the following
## information to ensure the GNU General Public License requirements will
## be met: https://www.gnu.org/licenses/gpl-2.0.html and
## https://www.gnu.org/licenses/gpl-3.0.html.
##
## $QT_END_LICENSE$
##
#############################################################################
"""
This file contains the exact signatures for all functions in module
PySide6.QtGui, except for defaults which are replaced by "...".
"""

# Module `PySide6.QtGui`

from shiboken6 import Shiboken

import os
from enum import Enum
from typing import Any, Callable, Optional, Tuple, Type, Union, Iterable, Sequence, Dict, List, Set, overload, Text

from a2qt import QtCore, QtGui


class QAbstractFileIconProvider(Shiboken.Object):

    Computer                 : QAbstractFileIconProvider.IconType = ... # 0x0
    Desktop                  : QAbstractFileIconProvider.IconType = ... # 0x1
    Trashcan                 : QAbstractFileIconProvider.IconType = ... # 0x2
    Network                  : QAbstractFileIconProvider.IconType = ... # 0x3
    Drive                    : QAbstractFileIconProvider.IconType = ... # 0x4
    Folder                   : QAbstractFileIconProvider.IconType = ... # 0x5
    File                     : QAbstractFileIconProvider.IconType = ... # 0x6
    DontUseCustomDirectoryIcons: QAbstractFileIconProvider.Option = ... # 0x1

    class IconType(Enum):

        Computer                 : QAbstractFileIconProvider.IconType # 0x0
        Desktop                  : QAbstractFileIconProvider.IconType # 0x1
        Trashcan                 : QAbstractFileIconProvider.IconType # 0x2
        Network                  : QAbstractFileIconProvider.IconType # 0x3
        Drive                    : QAbstractFileIconProvider.IconType # 0x4
        Folder                   : QAbstractFileIconProvider.IconType # 0x5
        File                     : QAbstractFileIconProvider.IconType # 0x6

    class Option(Enum):

        DontUseCustomDirectoryIcons: QAbstractFileIconProvider.Option # 0x1

    class Options(object): ...


    def __init__(self) -> None: ...

    @overload
    def icon(self, arg__1:QtCore.QFileInfo) -> QtGui.QIcon: ...
    @overload
    def icon(self, arg__1:QtGui.QAbstractFileIconProvider.IconType) -> QtGui.QIcon: ...
    def options(self) -> QtGui.QAbstractFileIconProvider.Options: ...
    def setOptions(self, arg__1:QtGui.QAbstractFileIconProvider.Options) -> None: ...
    def type(self, arg__1:QtCore.QFileInfo) -> str: ...


class QAbstractTextDocumentLayout(QtCore.QObject):

    class PaintContext(Shiboken.Object):

        @overload
        def __init__(self) -> None: ...
        @overload
        def __init__(self, PaintContext:QtGui.QAbstractTextDocumentLayout.PaintContext) -> None: ...

        @staticmethod
        def __copy__() -> None: ...

    class Selection(Shiboken.Object):

        @overload
        def __init__(self) -> None: ...
        @overload
        def __init__(self, Selection:QtGui.QAbstractTextDocumentLayout.Selection) -> None: ...

        @staticmethod
        def __copy__() -> None: ...


    def __init__(self, doc:QtGui.QTextDocument) -> None: ...

    def anchorAt(self, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> str: ...
    def blockBoundingRect(self, block:QtGui.QTextBlock) -> QtCore.QRectF: ...
    def blockWithMarkerAt(self, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> QtGui.QTextBlock: ...
    def document(self) -> QtGui.QTextDocument: ...
    def documentChanged(self, from_:int, charsRemoved:int, charsAdded:int) -> None: ...
    def documentSize(self) -> QtCore.QSizeF: ...
    def draw(self, painter:QtGui.QPainter, context:QtGui.QAbstractTextDocumentLayout.PaintContext) -> None: ...
    def drawInlineObject(self, painter:QtGui.QPainter, rect:Union[QtCore.QRectF, QtCore.QRect], object:QtGui.QTextInlineObject, posInDocument:int, format:QtGui.QTextFormat) -> None: ...
    def format(self, pos:int) -> QtGui.QTextCharFormat: ...
    def formatAt(self, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> QtGui.QTextFormat: ...
    def formatIndex(self, pos:int) -> int: ...
    def frameBoundingRect(self, frame:QtGui.QTextFrame) -> QtCore.QRectF: ...
    def handlerForObject(self, objectType:int) -> QtGui.QTextObjectInterface: ...
    def hitTest(self, point:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], accuracy:QtCore.Qt.HitTestAccuracy) -> int: ...
    def imageAt(self, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> str: ...
    def pageCount(self) -> int: ...
    def paintDevice(self) -> QtGui.QPaintDevice: ...
    def positionInlineObject(self, item:QtGui.QTextInlineObject, posInDocument:int, format:QtGui.QTextFormat) -> None: ...
    def registerHandler(self, objectType:int, component:QtCore.QObject) -> None: ...
    def resizeInlineObject(self, item:QtGui.QTextInlineObject, posInDocument:int, format:QtGui.QTextFormat) -> None: ...
    def setPaintDevice(self, device:QtGui.QPaintDevice) -> None: ...
    def unregisterHandler(self, objectType:int, component:Optional[QtCore.QObject]=...) -> None: ...


class QAccessible(Shiboken.Object):

    SoundPlayed              : QAccessible.Event = ... # 0x1
    Alert                    : QAccessible.Event = ... # 0x2
    ForegroundChanged        : QAccessible.Event = ... # 0x3
    MenuStart                : QAccessible.Event = ... # 0x4
    MenuEnd                  : QAccessible.Event = ... # 0x5
    PopupMenuStart           : QAccessible.Event = ... # 0x6
    PopupMenuEnd             : QAccessible.Event = ... # 0x7
    ContextHelpStart         : QAccessible.Event = ... # 0xc
    ContextHelpEnd           : QAccessible.Event = ... # 0xd
    DragDropStart            : QAccessible.Event = ... # 0xe
    DragDropEnd              : QAccessible.Event = ... # 0xf
    DialogStart              : QAccessible.Event = ... # 0x10
    DialogEnd                : QAccessible.Event = ... # 0x11
    ScrollingStart           : QAccessible.Event = ... # 0x12
    ScrollingEnd             : QAccessible.Event = ... # 0x13
    MenuCommand              : QAccessible.Event = ... # 0x18
    ActionChanged            : QAccessible.Event = ... # 0x101
    ActiveDescendantChanged  : QAccessible.Event = ... # 0x102
    AttributeChanged         : QAccessible.Event = ... # 0x103
    DocumentContentChanged   : QAccessible.Event = ... # 0x104
    DocumentLoadComplete     : QAccessible.Event = ... # 0x105
    DocumentLoadStopped      : QAccessible.Event = ... # 0x106
    DocumentReload           : QAccessible.Event = ... # 0x107
    HyperlinkEndIndexChanged : QAccessible.Event = ... # 0x108
    HyperlinkNumberOfAnchorsChanged: QAccessible.Event = ... # 0x109
    HyperlinkSelectedLinkChanged: QAccessible.Event = ... # 0x10a
    HypertextLinkActivated   : QAccessible.Event = ... # 0x10b
    HypertextLinkSelected    : QAccessible.Event = ... # 0x10c
    HyperlinkStartIndexChanged: QAccessible.Event = ... # 0x10d
    HypertextChanged         : QAccessible.Event = ... # 0x10e
    HypertextNLinksChanged   : QAccessible.Event = ... # 0x10f
    ObjectAttributeChanged   : QAccessible.Event = ... # 0x110
    PageChanged              : QAccessible.Event = ... # 0x111
    SectionChanged           : QAccessible.Event = ... # 0x112
    TableCaptionChanged      : QAccessible.Event = ... # 0x113
    TableColumnDescriptionChanged: QAccessible.Event = ... # 0x114
    TableColumnHeaderChanged : QAccessible.Event = ... # 0x115
    TableModelChanged        : QAccessible.Event = ... # 0x116
    TableRowDescriptionChanged: QAccessible.Event = ... # 0x117
    TableRowHeaderChanged    : QAccessible.Event = ... # 0x118
    TableSummaryChanged      : QAccessible.Event = ... # 0x119
    TextAttributeChanged     : QAccessible.Event = ... # 0x11a
    TextCaretMoved           : QAccessible.Event = ... # 0x11b
    TextColumnChanged        : QAccessible.Event = ... # 0x11d
    TextInserted             : QAccessible.Event = ... # 0x11e
    TextRemoved              : QAccessible.Event = ... # 0x11f
    TextUpdated              : QAccessible.Event = ... # 0x120
    TextSelectionChanged     : QAccessible.Event = ... # 0x121
    VisibleDataChanged       : QAccessible.Event = ... # 0x122
    ObjectCreated            : QAccessible.Event = ... # 0x8000
    ObjectDestroyed          : QAccessible.Event = ... # 0x8001
    ObjectShow               : QAccessible.Event = ... # 0x8002
    ObjectHide               : QAccessible.Event = ... # 0x8003
    ObjectReorder            : QAccessible.Event = ... # 0x8004
    Focus                    : QAccessible.Event = ... # 0x8005
    Selection                : QAccessible.Event = ... # 0x8006
    SelectionAdd             : QAccessible.Event = ... # 0x8007
    SelectionRemove          : QAccessible.Event = ... # 0x8008
    SelectionWithin          : QAccessible.Event = ... # 0x8009
    StateChanged             : QAccessible.Event = ... # 0x800a
    LocationChanged          : QAccessible.Event = ... # 0x800b
    NameChanged              : QAccessible.Event = ... # 0x800c
    DescriptionChanged       : QAccessible.Event = ... # 0x800d
    ValueChanged             : QAccessible.Event = ... # 0x800e
    ParentChanged            : QAccessible.Event = ... # 0x800f
    HelpChanged              : QAccessible.Event = ... # 0x80a0
    DefaultActionChanged     : QAccessible.Event = ... # 0x80b0
    AcceleratorChanged       : QAccessible.Event = ... # 0x80c0
    InvalidEvent             : QAccessible.Event = ... # 0x80c1
    TextInterface            : QAccessible.InterfaceType = ... # 0x0
    EditableTextInterface    : QAccessible.InterfaceType = ... # 0x1
    ValueInterface           : QAccessible.InterfaceType = ... # 0x2
    ActionInterface          : QAccessible.InterfaceType = ... # 0x3
    ImageInterface           : QAccessible.InterfaceType = ... # 0x4
    TableInterface           : QAccessible.InterfaceType = ... # 0x5
    TableCellInterface       : QAccessible.InterfaceType = ... # 0x6
    HyperlinkInterface       : QAccessible.InterfaceType = ... # 0x7
    AllRelations             : QAccessible.RelationFlag = ... # -0x1
    Label                    : QAccessible.RelationFlag = ... # 0x1
    Labelled                 : QAccessible.RelationFlag = ... # 0x2
    Controller               : QAccessible.RelationFlag = ... # 0x4
    Controlled               : QAccessible.RelationFlag = ... # 0x8
    NoRole                   : QAccessible.Role = ... # 0x0
    TitleBar                 : QAccessible.Role = ... # 0x1
    MenuBar                  : QAccessible.Role = ... # 0x2
    ScrollBar                : QAccessible.Role = ... # 0x3
    Grip                     : QAccessible.Role = ... # 0x4
    Sound                    : QAccessible.Role = ... # 0x5
    Cursor                   : QAccessible.Role = ... # 0x6
    Caret                    : QAccessible.Role = ... # 0x7
    AlertMessage             : QAccessible.Role = ... # 0x8
    Window                   : QAccessible.Role = ... # 0x9
    Client                   : QAccessible.Role = ... # 0xa
    PopupMenu                : QAccessible.Role = ... # 0xb
    MenuItem                 : QAccessible.Role = ... # 0xc
    ToolTip                  : QAccessible.Role = ... # 0xd
    Application              : QAccessible.Role = ... # 0xe
    Document                 : QAccessible.Role = ... # 0xf
    Pane                     : QAccessible.Role = ... # 0x10
    Chart                    : QAccessible.Role = ... # 0x11
    Dialog                   : QAccessible.Role = ... # 0x12
    Border                   : QAccessible.Role = ... # 0x13
    Grouping                 : QAccessible.Role = ... # 0x14
    Separator                : QAccessible.Role = ... # 0x15
    ToolBar                  : QAccessible.Role = ... # 0x16
    StatusBar                : QAccessible.Role = ... # 0x17
    Table                    : QAccessible.Role = ... # 0x18
    ColumnHeader             : QAccessible.Role = ... # 0x19
    RowHeader                : QAccessible.Role = ... # 0x1a
    Column                   : QAccessible.Role = ... # 0x1b
    Row                      : QAccessible.Role = ... # 0x1c
    Cell                     : QAccessible.Role = ... # 0x1d
    Link                     : QAccessible.Role = ... # 0x1e
    HelpBalloon              : QAccessible.Role = ... # 0x1f
    Assistant                : QAccessible.Role = ... # 0x20
    List                     : QAccessible.Role = ... # 0x21
    ListItem                 : QAccessible.Role = ... # 0x22
    Tree                     : QAccessible.Role = ... # 0x23
    TreeItem                 : QAccessible.Role = ... # 0x24
    PageTab                  : QAccessible.Role = ... # 0x25
    PropertyPage             : QAccessible.Role = ... # 0x26
    Indicator                : QAccessible.Role = ... # 0x27
    Graphic                  : QAccessible.Role = ... # 0x28
    StaticText               : QAccessible.Role = ... # 0x29
    EditableText             : QAccessible.Role = ... # 0x2a
    Button                   : QAccessible.Role = ... # 0x2b
    PushButton               : QAccessible.Role = ... # 0x2b
    CheckBox                 : QAccessible.Role = ... # 0x2c
    RadioButton              : QAccessible.Role = ... # 0x2d
    ComboBox                 : QAccessible.Role = ... # 0x2e
    ProgressBar              : QAccessible.Role = ... # 0x30
    Dial                     : QAccessible.Role = ... # 0x31
    HotkeyField              : QAccessible.Role = ... # 0x32
    Slider                   : QAccessible.Role = ... # 0x33
    SpinBox                  : QAccessible.Role = ... # 0x34
    Canvas                   : QAccessible.Role = ... # 0x35
    Animation                : QAccessible.Role = ... # 0x36
    Equation                 : QAccessible.Role = ... # 0x37
    ButtonDropDown           : QAccessible.Role = ... # 0x38
    ButtonMenu               : QAccessible.Role = ... # 0x39
    ButtonDropGrid           : QAccessible.Role = ... # 0x3a
    Whitespace               : QAccessible.Role = ... # 0x3b
    PageTabList              : QAccessible.Role = ... # 0x3c
    Clock                    : QAccessible.Role = ... # 0x3d
    Splitter                 : QAccessible.Role = ... # 0x3e
    LayeredPane              : QAccessible.Role = ... # 0x80
    Terminal                 : QAccessible.Role = ... # 0x81
    Desktop                  : QAccessible.Role = ... # 0x82
    Paragraph                : QAccessible.Role = ... # 0x83
    WebDocument              : QAccessible.Role = ... # 0x84
    Section                  : QAccessible.Role = ... # 0x85
    Notification             : QAccessible.Role = ... # 0x86
    ColorChooser             : QAccessible.Role = ... # 0x404
    Footer                   : QAccessible.Role = ... # 0x40e
    Form                     : QAccessible.Role = ... # 0x410
    Heading                  : QAccessible.Role = ... # 0x414
    Note                     : QAccessible.Role = ... # 0x41b
    ComplementaryContent     : QAccessible.Role = ... # 0x42c
    UserRole                 : QAccessible.Role = ... # 0xffff
    Name                     : QAccessible.Text = ... # 0x0
    Description              : QAccessible.Text = ... # 0x1
    Value                    : QAccessible.Text = ... # 0x2
    Help                     : QAccessible.Text = ... # 0x3
    Accelerator              : QAccessible.Text = ... # 0x4
    DebugDescription         : QAccessible.Text = ... # 0x5
    UserText                 : QAccessible.Text = ... # 0xffff
    CharBoundary             : QAccessible.TextBoundaryType = ... # 0x0
    WordBoundary             : QAccessible.TextBoundaryType = ... # 0x1
    SentenceBoundary         : QAccessible.TextBoundaryType = ... # 0x2
    ParagraphBoundary        : QAccessible.TextBoundaryType = ... # 0x3
    LineBoundary             : QAccessible.TextBoundaryType = ... # 0x4
    NoBoundary               : QAccessible.TextBoundaryType = ... # 0x5

    class Event(Enum):

        SoundPlayed              : QAccessible.Event # 0x1
        Alert                    : QAccessible.Event # 0x2
        ForegroundChanged        : QAccessible.Event # 0x3
        MenuStart                : QAccessible.Event # 0x4
        MenuEnd                  : QAccessible.Event # 0x5
        PopupMenuStart           : QAccessible.Event # 0x6
        PopupMenuEnd             : QAccessible.Event # 0x7
        ContextHelpStart         : QAccessible.Event # 0xc
        ContextHelpEnd           : QAccessible.Event # 0xd
        DragDropStart            : QAccessible.Event # 0xe
        DragDropEnd              : QAccessible.Event # 0xf
        DialogStart              : QAccessible.Event # 0x10
        DialogEnd                : QAccessible.Event # 0x11
        ScrollingStart           : QAccessible.Event # 0x12
        ScrollingEnd             : QAccessible.Event # 0x13
        MenuCommand              : QAccessible.Event # 0x18
        ActionChanged            : QAccessible.Event # 0x101
        ActiveDescendantChanged  : QAccessible.Event # 0x102
        AttributeChanged         : QAccessible.Event # 0x103
        DocumentContentChanged   : QAccessible.Event # 0x104
        DocumentLoadComplete     : QAccessible.Event # 0x105
        DocumentLoadStopped      : QAccessible.Event # 0x106
        DocumentReload           : QAccessible.Event # 0x107
        HyperlinkEndIndexChanged : QAccessible.Event # 0x108
        HyperlinkNumberOfAnchorsChanged: QAccessible.Event # 0x109
        HyperlinkSelectedLinkChanged: QAccessible.Event # 0x10a
        HypertextLinkActivated   : QAccessible.Event # 0x10b
        HypertextLinkSelected    : QAccessible.Event # 0x10c
        HyperlinkStartIndexChanged: QAccessible.Event # 0x10d
        HypertextChanged         : QAccessible.Event # 0x10e
        HypertextNLinksChanged   : QAccessible.Event # 0x10f
        ObjectAttributeChanged   : QAccessible.Event # 0x110
        PageChanged              : QAccessible.Event # 0x111
        SectionChanged           : QAccessible.Event # 0x112
        TableCaptionChanged      : QAccessible.Event # 0x113
        TableColumnDescriptionChanged: QAccessible.Event # 0x114
        TableColumnHeaderChanged : QAccessible.Event # 0x115
        TableModelChanged        : QAccessible.Event # 0x116
        TableRowDescriptionChanged: QAccessible.Event # 0x117
        TableRowHeaderChanged    : QAccessible.Event # 0x118
        TableSummaryChanged      : QAccessible.Event # 0x119
        TextAttributeChanged     : QAccessible.Event # 0x11a
        TextCaretMoved           : QAccessible.Event # 0x11b
        TextColumnChanged        : QAccessible.Event # 0x11d
        TextInserted             : QAccessible.Event # 0x11e
        TextRemoved              : QAccessible.Event # 0x11f
        TextUpdated              : QAccessible.Event # 0x120
        TextSelectionChanged     : QAccessible.Event # 0x121
        VisibleDataChanged       : QAccessible.Event # 0x122
        ObjectCreated            : QAccessible.Event # 0x8000
        ObjectDestroyed          : QAccessible.Event # 0x8001
        ObjectShow               : QAccessible.Event # 0x8002
        ObjectHide               : QAccessible.Event # 0x8003
        ObjectReorder            : QAccessible.Event # 0x8004
        Focus                    : QAccessible.Event # 0x8005
        Selection                : QAccessible.Event # 0x8006
        SelectionAdd             : QAccessible.Event # 0x8007
        SelectionRemove          : QAccessible.Event # 0x8008
        SelectionWithin          : QAccessible.Event # 0x8009
        StateChanged             : QAccessible.Event # 0x800a
        LocationChanged          : QAccessible.Event # 0x800b
        NameChanged              : QAccessible.Event # 0x800c
        DescriptionChanged       : QAccessible.Event # 0x800d
        ValueChanged             : QAccessible.Event # 0x800e
        ParentChanged            : QAccessible.Event # 0x800f
        HelpChanged              : QAccessible.Event # 0x80a0
        DefaultActionChanged     : QAccessible.Event # 0x80b0
        AcceleratorChanged       : QAccessible.Event # 0x80c0
        InvalidEvent             : QAccessible.Event # 0x80c1

    class InterfaceType(Enum):

        TextInterface            : QAccessible.InterfaceType # 0x0
        EditableTextInterface    : QAccessible.InterfaceType # 0x1
        ValueInterface           : QAccessible.InterfaceType # 0x2
        ActionInterface          : QAccessible.InterfaceType # 0x3
        ImageInterface           : QAccessible.InterfaceType # 0x4
        TableInterface           : QAccessible.InterfaceType # 0x5
        TableCellInterface       : QAccessible.InterfaceType # 0x6
        HyperlinkInterface       : QAccessible.InterfaceType # 0x7

    class Relation(object): ...

    class RelationFlag(Enum):

        AllRelations             : QAccessible.RelationFlag # -0x1
        Label                    : QAccessible.RelationFlag # 0x1
        Labelled                 : QAccessible.RelationFlag # 0x2
        Controller               : QAccessible.RelationFlag # 0x4
        Controlled               : QAccessible.RelationFlag # 0x8

    class Role(Enum):

        NoRole                   : QAccessible.Role # 0x0
        TitleBar                 : QAccessible.Role # 0x1
        MenuBar                  : QAccessible.Role # 0x2
        ScrollBar                : QAccessible.Role # 0x3
        Grip                     : QAccessible.Role # 0x4
        Sound                    : QAccessible.Role # 0x5
        Cursor                   : QAccessible.Role # 0x6
        Caret                    : QAccessible.Role # 0x7
        AlertMessage             : QAccessible.Role # 0x8
        Window                   : QAccessible.Role # 0x9
        Client                   : QAccessible.Role # 0xa
        PopupMenu                : QAccessible.Role # 0xb
        MenuItem                 : QAccessible.Role # 0xc
        ToolTip                  : QAccessible.Role # 0xd
        Application              : QAccessible.Role # 0xe
        Document                 : QAccessible.Role # 0xf
        Pane                     : QAccessible.Role # 0x10
        Chart                    : QAccessible.Role # 0x11
        Dialog                   : QAccessible.Role # 0x12
        Border                   : QAccessible.Role # 0x13
        Grouping                 : QAccessible.Role # 0x14
        Separator                : QAccessible.Role # 0x15
        ToolBar                  : QAccessible.Role # 0x16
        StatusBar                : QAccessible.Role # 0x17
        Table                    : QAccessible.Role # 0x18
        ColumnHeader             : QAccessible.Role # 0x19
        RowHeader                : QAccessible.Role # 0x1a
        Column                   : QAccessible.Role # 0x1b
        Row                      : QAccessible.Role # 0x1c
        Cell                     : QAccessible.Role # 0x1d
        Link                     : QAccessible.Role # 0x1e
        HelpBalloon              : QAccessible.Role # 0x1f
        Assistant                : QAccessible.Role # 0x20
        List                     : QAccessible.Role # 0x21
        ListItem                 : QAccessible.Role # 0x22
        Tree                     : QAccessible.Role # 0x23
        TreeItem                 : QAccessible.Role # 0x24
        PageTab                  : QAccessible.Role # 0x25
        PropertyPage             : QAccessible.Role # 0x26
        Indicator                : QAccessible.Role # 0x27
        Graphic                  : QAccessible.Role # 0x28
        StaticText               : QAccessible.Role # 0x29
        EditableText             : QAccessible.Role # 0x2a
        Button                   : QAccessible.Role # 0x2b
        PushButton               : QAccessible.Role # 0x2b
        CheckBox                 : QAccessible.Role # 0x2c
        RadioButton              : QAccessible.Role # 0x2d
        ComboBox                 : QAccessible.Role # 0x2e
        ProgressBar              : QAccessible.Role # 0x30
        Dial                     : QAccessible.Role # 0x31
        HotkeyField              : QAccessible.Role # 0x32
        Slider                   : QAccessible.Role # 0x33
        SpinBox                  : QAccessible.Role # 0x34
        Canvas                   : QAccessible.Role # 0x35
        Animation                : QAccessible.Role # 0x36
        Equation                 : QAccessible.Role # 0x37
        ButtonDropDown           : QAccessible.Role # 0x38
        ButtonMenu               : QAccessible.Role # 0x39
        ButtonDropGrid           : QAccessible.Role # 0x3a
        Whitespace               : QAccessible.Role # 0x3b
        PageTabList              : QAccessible.Role # 0x3c
        Clock                    : QAccessible.Role # 0x3d
        Splitter                 : QAccessible.Role # 0x3e
        LayeredPane              : QAccessible.Role # 0x80
        Terminal                 : QAccessible.Role # 0x81
        Desktop                  : QAccessible.Role # 0x82
        Paragraph                : QAccessible.Role # 0x83
        WebDocument              : QAccessible.Role # 0x84
        Section                  : QAccessible.Role # 0x85
        Notification             : QAccessible.Role # 0x86
        ColorChooser             : QAccessible.Role # 0x404
        Footer                   : QAccessible.Role # 0x40e
        Form                     : QAccessible.Role # 0x410
        Heading                  : QAccessible.Role # 0x414
        Note                     : QAccessible.Role # 0x41b
        ComplementaryContent     : QAccessible.Role # 0x42c
        UserRole                 : QAccessible.Role # 0xffff

    class State(Shiboken.Object):

        @overload
        def __init__(self) -> None: ...
        @overload
        def __init__(self, State:QtGui.QAccessible.State) -> None: ...

        @staticmethod
        def __copy__() -> None: ...

    class Text(Enum):

        Name                     : QAccessible.Text # 0x0
        Description              : QAccessible.Text # 0x1
        Value                    : QAccessible.Text # 0x2
        Help                     : QAccessible.Text # 0x3
        Accelerator              : QAccessible.Text # 0x4
        DebugDescription         : QAccessible.Text # 0x5
        UserText                 : QAccessible.Text # 0xffff

    class TextBoundaryType(Enum):

        CharBoundary             : QAccessible.TextBoundaryType # 0x0
        WordBoundary             : QAccessible.TextBoundaryType # 0x1
        SentenceBoundary         : QAccessible.TextBoundaryType # 0x2
        ParagraphBoundary        : QAccessible.TextBoundaryType # 0x3
        LineBoundary             : QAccessible.TextBoundaryType # 0x4
        NoBoundary               : QAccessible.TextBoundaryType # 0x5


    def __init__(self, QAccessible:QtGui.QAccessible) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    @staticmethod
    def accessibleInterface(uniqueId:int) -> QtGui.QAccessibleInterface: ...
    @staticmethod
    def cleanup() -> None: ...
    @staticmethod
    def deleteAccessibleInterface(uniqueId:int) -> None: ...
    @staticmethod
    def installFactory(arg__1:Callable) -> None: ...
    @staticmethod
    def isActive() -> bool: ...
    @staticmethod
    def qAccessibleTextBoundaryHelper(cursor:QtGui.QTextCursor, boundaryType:QtGui.QAccessible.TextBoundaryType) -> Tuple[int, int]: ...
    @staticmethod
    def queryAccessibleInterface(arg__1:QtCore.QObject) -> QtGui.QAccessibleInterface: ...
    @staticmethod
    def registerAccessibleInterface(iface:QtGui.QAccessibleInterface) -> int: ...
    @staticmethod
    def setActive(active:bool) -> None: ...
    @staticmethod
    def setRootObject(object:QtCore.QObject) -> None: ...
    @staticmethod
    def uniqueId(iface:QtGui.QAccessibleInterface) -> int: ...
    @staticmethod
    def updateAccessibility(event:QtGui.QAccessibleEvent) -> None: ...


class QAccessibleActionInterface(Shiboken.Object):

    def __init__(self) -> None: ...

    def actionNames(self) -> List[str]: ...
    @staticmethod
    def decreaseAction() -> str: ...
    def doAction(self, actionName:str) -> None: ...
    @staticmethod
    def increaseAction() -> str: ...
    def keyBindingsForAction(self, actionName:str) -> List[str]: ...
    def localizedActionDescription(self, name:str) -> str: ...
    def localizedActionName(self, name:str) -> str: ...
    @staticmethod
    def nextPageAction() -> str: ...
    @staticmethod
    def pressAction() -> str: ...
    @staticmethod
    def previousPageAction() -> str: ...
    @staticmethod
    def scrollDownAction() -> str: ...
    @staticmethod
    def scrollLeftAction() -> str: ...
    @staticmethod
    def scrollRightAction() -> str: ...
    @staticmethod
    def scrollUpAction() -> str: ...
    @staticmethod
    def setFocusAction() -> str: ...
    @staticmethod
    def showMenuAction() -> str: ...
    @staticmethod
    def toggleAction() -> str: ...


class QAccessibleEditableTextInterface(Shiboken.Object):

    def __init__(self) -> None: ...

    def deleteText(self, startOffset:int, endOffset:int) -> None: ...
    def insertText(self, offset:int, text:str) -> None: ...
    def replaceText(self, startOffset:int, endOffset:int, text:str) -> None: ...


class QAccessibleEvent(Shiboken.Object):

    @overload
    def __init__(self, iface:QtGui.QAccessibleInterface, typ:QtGui.QAccessible.Event) -> None: ...
    @overload
    def __init__(self, obj:QtCore.QObject, typ:QtGui.QAccessible.Event) -> None: ...

    def accessibleInterface(self) -> QtGui.QAccessibleInterface: ...
    def child(self) -> int: ...
    def object(self) -> QtCore.QObject: ...
    def setChild(self, chld:int) -> None: ...
    def type(self) -> QtGui.QAccessible.Event: ...
    def uniqueId(self) -> int: ...


class QAccessibleInterface(Shiboken.Object):

    def __init__(self) -> None: ...

    def actionInterface(self) -> QtGui.QAccessibleActionInterface: ...
    def backgroundColor(self) -> QtGui.QColor: ...
    def child(self, index:int) -> QtGui.QAccessibleInterface: ...
    def childAt(self, x:int, y:int) -> QtGui.QAccessibleInterface: ...
    def childCount(self) -> int: ...
    def editableTextInterface(self) -> QtGui.QAccessibleEditableTextInterface: ...
    def focusChild(self) -> QtGui.QAccessibleInterface: ...
    def foregroundColor(self) -> QtGui.QColor: ...
    def indexOfChild(self, arg__1:QtGui.QAccessibleInterface) -> int: ...
    def interface_cast(self, arg__1:QtGui.QAccessible.InterfaceType) -> int: ...
    def isValid(self) -> bool: ...
    def object(self) -> QtCore.QObject: ...
    def parent(self) -> QtGui.QAccessibleInterface: ...
    def rect(self) -> QtCore.QRect: ...
    def relations(self, match:QtGui.QAccessible.Relation=...) -> List[Tuple[QtGui.QAccessibleInterface, QtGui.QAccessible.Relation]]: ...
    def role(self) -> QtGui.QAccessible.Role: ...
    def setText(self, t:QtGui.QAccessible.Text, text:str) -> None: ...
    def state(self) -> QtGui.QAccessible.State: ...
    def tableCellInterface(self) -> QtGui.QAccessibleTableCellInterface: ...
    def text(self, t:QtGui.QAccessible.Text) -> str: ...
    def textInterface(self) -> QtGui.QAccessibleTextInterface: ...
    def valueInterface(self) -> QtGui.QAccessibleValueInterface: ...
    def virtual_hook(self, id:int, data:int) -> None: ...
    def window(self) -> QtGui.QWindow: ...


class QAccessibleObject(QtGui.QAccessibleInterface):

    def __init__(self, object:QtCore.QObject) -> None: ...

    def childAt(self, x:int, y:int) -> QtGui.QAccessibleInterface: ...
    def isValid(self) -> bool: ...
    def object(self) -> QtCore.QObject: ...
    def rect(self) -> QtCore.QRect: ...
    def setText(self, t:QtGui.QAccessible.Text, text:str) -> None: ...


class QAccessibleStateChangeEvent(QtGui.QAccessibleEvent):

    @overload
    def __init__(self, iface:QtGui.QAccessibleInterface, state:QtGui.QAccessible.State) -> None: ...
    @overload
    def __init__(self, obj:QtCore.QObject, state:QtGui.QAccessible.State) -> None: ...

    def changedStates(self) -> QtGui.QAccessible.State: ...


class QAccessibleTableCellInterface(Shiboken.Object):

    def __init__(self) -> None: ...

    def columnExtent(self) -> int: ...
    def columnHeaderCells(self) -> List[QtGui.QAccessibleInterface]: ...
    def columnIndex(self) -> int: ...
    def isSelected(self) -> bool: ...
    def rowExtent(self) -> int: ...
    def rowHeaderCells(self) -> List[QtGui.QAccessibleInterface]: ...
    def rowIndex(self) -> int: ...
    def table(self) -> QtGui.QAccessibleInterface: ...


class QAccessibleTableModelChangeEvent(QtGui.QAccessibleEvent):

    ModelReset               : QAccessibleTableModelChangeEvent.ModelChangeType = ... # 0x0
    DataChanged              : QAccessibleTableModelChangeEvent.ModelChangeType = ... # 0x1
    RowsInserted             : QAccessibleTableModelChangeEvent.ModelChangeType = ... # 0x2
    ColumnsInserted          : QAccessibleTableModelChangeEvent.ModelChangeType = ... # 0x3
    RowsRemoved              : QAccessibleTableModelChangeEvent.ModelChangeType = ... # 0x4
    ColumnsRemoved           : QAccessibleTableModelChangeEvent.ModelChangeType = ... # 0x5

    class ModelChangeType(Enum):

        ModelReset               : QAccessibleTableModelChangeEvent.ModelChangeType # 0x0
        DataChanged              : QAccessibleTableModelChangeEvent.ModelChangeType # 0x1
        RowsInserted             : QAccessibleTableModelChangeEvent.ModelChangeType # 0x2
        ColumnsInserted          : QAccessibleTableModelChangeEvent.ModelChangeType # 0x3
        RowsRemoved              : QAccessibleTableModelChangeEvent.ModelChangeType # 0x4
        ColumnsRemoved           : QAccessibleTableModelChangeEvent.ModelChangeType # 0x5


    @overload
    def __init__(self, iface:QtGui.QAccessibleInterface, changeType:QtGui.QAccessibleTableModelChangeEvent.ModelChangeType) -> None: ...
    @overload
    def __init__(self, obj:QtCore.QObject, changeType:QtGui.QAccessibleTableModelChangeEvent.ModelChangeType) -> None: ...

    def firstColumn(self) -> int: ...
    def firstRow(self) -> int: ...
    def lastColumn(self) -> int: ...
    def lastRow(self) -> int: ...
    def modelChangeType(self) -> QtGui.QAccessibleTableModelChangeEvent.ModelChangeType: ...
    def setFirstColumn(self, col:int) -> None: ...
    def setFirstRow(self, row:int) -> None: ...
    def setLastColumn(self, col:int) -> None: ...
    def setLastRow(self, row:int) -> None: ...
    def setModelChangeType(self, changeType:QtGui.QAccessibleTableModelChangeEvent.ModelChangeType) -> None: ...


class QAccessibleTextCursorEvent(QtGui.QAccessibleEvent):

    @overload
    def __init__(self, iface:QtGui.QAccessibleInterface, cursorPos:int) -> None: ...
    @overload
    def __init__(self, obj:QtCore.QObject, cursorPos:int) -> None: ...

    def cursorPosition(self) -> int: ...
    def setCursorPosition(self, position:int) -> None: ...


class QAccessibleTextInsertEvent(QtGui.QAccessibleTextCursorEvent):

    @overload
    def __init__(self, iface:QtGui.QAccessibleInterface, position:int, text:str) -> None: ...
    @overload
    def __init__(self, obj:QtCore.QObject, position:int, text:str) -> None: ...

    def changePosition(self) -> int: ...
    def textInserted(self) -> str: ...


class QAccessibleTextInterface(Shiboken.Object):

    def __init__(self) -> None: ...

    def addSelection(self, startOffset:int, endOffset:int) -> None: ...
    def attributes(self, offset:int) -> Tuple[str, int, int]: ...
    def characterCount(self) -> int: ...
    def characterRect(self, offset:int) -> QtCore.QRect: ...
    def cursorPosition(self) -> int: ...
    def offsetAtPoint(self, point:QtCore.QPoint) -> int: ...
    def removeSelection(self, selectionIndex:int) -> None: ...
    def scrollToSubstring(self, startIndex:int, endIndex:int) -> None: ...
    def selection(self, selectionIndex:int) -> Tuple[int, int]: ...
    def selectionCount(self) -> int: ...
    def setCursorPosition(self, position:int) -> None: ...
    def setSelection(self, selectionIndex:int, startOffset:int, endOffset:int) -> None: ...
    def text(self, startOffset:int, endOffset:int) -> str: ...
    def textAfterOffset(self, offset:int, boundaryType:QtGui.QAccessible.TextBoundaryType) -> Tuple[str, int, int]: ...
    def textAtOffset(self, offset:int, boundaryType:QtGui.QAccessible.TextBoundaryType) -> Tuple[str, int, int]: ...
    def textBeforeOffset(self, offset:int, boundaryType:QtGui.QAccessible.TextBoundaryType) -> Tuple[str, int, int]: ...


class QAccessibleTextRemoveEvent(QtGui.QAccessibleTextCursorEvent):

    @overload
    def __init__(self, iface:QtGui.QAccessibleInterface, position:int, text:str) -> None: ...
    @overload
    def __init__(self, obj:QtCore.QObject, position:int, text:str) -> None: ...

    def changePosition(self) -> int: ...
    def textRemoved(self) -> str: ...


class QAccessibleTextSelectionEvent(QtGui.QAccessibleTextCursorEvent):

    @overload
    def __init__(self, iface:QtGui.QAccessibleInterface, start:int, end:int) -> None: ...
    @overload
    def __init__(self, obj:QtCore.QObject, start:int, end:int) -> None: ...

    def selectionEnd(self) -> int: ...
    def selectionStart(self) -> int: ...
    def setSelection(self, start:int, end:int) -> None: ...


class QAccessibleTextUpdateEvent(QtGui.QAccessibleTextCursorEvent):

    @overload
    def __init__(self, iface:QtGui.QAccessibleInterface, position:int, oldText:str, text:str) -> None: ...
    @overload
    def __init__(self, obj:QtCore.QObject, position:int, oldText:str, text:str) -> None: ...

    def changePosition(self) -> int: ...
    def textInserted(self) -> str: ...
    def textRemoved(self) -> str: ...


class QAccessibleValueChangeEvent(QtGui.QAccessibleEvent):

    @overload
    def __init__(self, iface:QtGui.QAccessibleInterface, val:Any) -> None: ...
    @overload
    def __init__(self, obj:QtCore.QObject, val:Any) -> None: ...

    def setValue(self, val:Any) -> None: ...
    def value(self) -> Any: ...


class QAccessibleValueInterface(Shiboken.Object):

    def __init__(self) -> None: ...

    def currentValue(self) -> Any: ...
    def maximumValue(self) -> Any: ...
    def minimumStepSize(self) -> Any: ...
    def minimumValue(self) -> Any: ...
    def setCurrentValue(self, value:Any) -> None: ...


class QAction(QtCore.QObject):
    triggered: QtCore.Signal = ...

    Trigger                  : QAction.ActionEvent = ... # 0x0
    Hover                    : QAction.ActionEvent = ... # 0x1
    NoRole                   : QAction.MenuRole = ... # 0x0
    TextHeuristicRole        : QAction.MenuRole = ... # 0x1
    ApplicationSpecificRole  : QAction.MenuRole = ... # 0x2
    AboutQtRole              : QAction.MenuRole = ... # 0x3
    AboutRole                : QAction.MenuRole = ... # 0x4
    PreferencesRole          : QAction.MenuRole = ... # 0x5
    QuitRole                 : QAction.MenuRole = ... # 0x6
    LowPriority              : QAction.Priority = ... # 0x0
    NormalPriority           : QAction.Priority = ... # 0x80
    HighPriority             : QAction.Priority = ... # 0x100

    class ActionEvent(Enum):

        Trigger                  : QAction.ActionEvent # 0x0
        Hover                    : QAction.ActionEvent # 0x1

    class MenuRole(Enum):

        NoRole                   : QAction.MenuRole # 0x0
        TextHeuristicRole        : QAction.MenuRole # 0x1
        ApplicationSpecificRole  : QAction.MenuRole # 0x2
        AboutQtRole              : QAction.MenuRole # 0x3
        AboutRole                : QAction.MenuRole # 0x4
        PreferencesRole          : QAction.MenuRole # 0x5
        QuitRole                 : QAction.MenuRole # 0x6

    class Priority(Enum):

        LowPriority              : QAction.Priority # 0x0
        NormalPriority           : QAction.Priority # 0x80
        HighPriority             : QAction.Priority # 0x100


    @overload
    def __init__(self, icon:Union[QtGui.QIcon, QtGui.QPixmap], text:str, parent:Optional[QtCore.QObject]=...) -> None: ...
    @overload
    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...
    @overload
    def __init__(self, text:str, parent:Optional[QtCore.QObject]=...) -> None: ...

    def actionGroup(self) -> QtGui.QActionGroup: ...
    def activate(self, event:QtGui.QAction.ActionEvent) -> None: ...
    def associatedObjects(self) -> List[QtCore.QObject]: ...
    def autoRepeat(self) -> bool: ...
    def data(self) -> Any: ...
    def event(self, arg__1:QtCore.QEvent) -> bool: ...
    def font(self) -> QtGui.QFont: ...
    def hover(self) -> None: ...
    def icon(self) -> QtGui.QIcon: ...
    def iconText(self) -> str: ...
    def isCheckable(self) -> bool: ...
    def isChecked(self) -> bool: ...
    def isEnabled(self) -> bool: ...
    def isIconVisibleInMenu(self) -> bool: ...
    def isSeparator(self) -> bool: ...
    def isShortcutVisibleInContextMenu(self) -> bool: ...
    def isVisible(self) -> bool: ...
    def menuRole(self) -> QtGui.QAction.MenuRole: ...
    def priority(self) -> QtGui.QAction.Priority: ...
    def resetEnabled(self) -> None: ...
    def setActionGroup(self, group:QtGui.QActionGroup) -> None: ...
    def setAutoRepeat(self, arg__1:bool) -> None: ...
    def setCheckable(self, arg__1:bool) -> None: ...
    def setChecked(self, arg__1:bool) -> None: ...
    def setData(self, var:Any) -> None: ...
    def setDisabled(self, b:bool) -> None: ...
    def setEnabled(self, arg__1:bool) -> None: ...
    def setFont(self, font:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    def setIcon(self, icon:Union[QtGui.QIcon, QtGui.QPixmap]) -> None: ...
    def setIconText(self, text:str) -> None: ...
    def setIconVisibleInMenu(self, visible:bool) -> None: ...
    def setMenuRole(self, menuRole:QtGui.QAction.MenuRole) -> None: ...
    def setPriority(self, priority:QtGui.QAction.Priority) -> None: ...
    def setSeparator(self, b:bool) -> None: ...
    @overload
    def setShortcut(self, arg__1:QtCore.Qt.Key) -> None: ...
    @overload
    def setShortcut(self, shortcut:Union[QtGui.QKeySequence, QtCore.QKeyCombination, QtGui.QKeySequence.StandardKey, str, int]) -> None: ...
    def setShortcutContext(self, context:QtCore.Qt.ShortcutContext) -> None: ...
    def setShortcutVisibleInContextMenu(self, show:bool) -> None: ...
    @overload
    def setShortcuts(self, arg__1:QtGui.QKeySequence.StandardKey) -> None: ...
    @overload
    def setShortcuts(self, shortcuts:Sequence[QtGui.QKeySequence]) -> None: ...
    def setStatusTip(self, statusTip:str) -> None: ...
    def setText(self, text:str) -> None: ...
    def setToolTip(self, tip:str) -> None: ...
    def setVisible(self, arg__1:bool) -> None: ...
    def setWhatsThis(self, what:str) -> None: ...
    def shortcut(self) -> QtGui.QKeySequence: ...
    def shortcutContext(self) -> QtCore.Qt.ShortcutContext: ...
    def shortcuts(self) -> List[QtGui.QKeySequence]: ...
    def showStatusText(self, object:Optional[QtCore.QObject]=...) -> bool: ...
    def statusTip(self) -> str: ...
    def text(self) -> str: ...
    def toggle(self) -> None: ...
    def toolTip(self) -> str: ...
    def trigger(self) -> None: ...
    def whatsThis(self) -> str: ...


class QActionEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QActionEvent) -> None: ...
    @overload
    def __init__(self, type:int, action:QtGui.QAction, before:Optional[QtGui.QAction]=...) -> None: ...

    def action(self) -> QtGui.QAction: ...
    def before(self) -> QtGui.QAction: ...
    def clone(self) -> QtGui.QActionEvent: ...


class QActionGroup(QtCore.QObject):

    class ExclusionPolicy(Enum):

        None_                    : QActionGroup.ExclusionPolicy # 0x0
        Exclusive                : QActionGroup.ExclusionPolicy # 0x1
        ExclusiveOptional        : QActionGroup.ExclusionPolicy # 0x2


    def __init__(self, parent:QtCore.QObject) -> None: ...

    def actions(self) -> List[QtGui.QAction]: ...
    @overload
    def addAction(self, a:QtGui.QAction) -> QtGui.QAction: ...
    @overload
    def addAction(self, icon:Union[QtGui.QIcon, QtGui.QPixmap], text:str) -> QtGui.QAction: ...
    @overload
    def addAction(self, text:str) -> QtGui.QAction: ...
    def checkedAction(self) -> QtGui.QAction: ...
    def exclusionPolicy(self) -> QtGui.QActionGroup.ExclusionPolicy: ...
    def isEnabled(self) -> bool: ...
    def isExclusive(self) -> bool: ...
    def isVisible(self) -> bool: ...
    def removeAction(self, a:QtGui.QAction) -> None: ...
    def setDisabled(self, b:bool) -> None: ...
    def setEnabled(self, arg__1:bool) -> None: ...
    def setExclusionPolicy(self, policy:QtGui.QActionGroup.ExclusionPolicy) -> None: ...
    def setExclusive(self, arg__1:bool) -> None: ...
    def setVisible(self, arg__1:bool) -> None: ...


class QBackingStore(Shiboken.Object):

    def __init__(self, window:QtGui.QWindow) -> None: ...

    def beginPaint(self, arg__1:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> None: ...
    def endPaint(self) -> None: ...
    def flush(self, region:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect], window:Optional[QtGui.QWindow]=..., offset:QtCore.QPoint=...) -> None: ...
    def hasStaticContents(self) -> bool: ...
    def paintDevice(self) -> QtGui.QPaintDevice: ...
    def resize(self, size:QtCore.QSize) -> None: ...
    def scroll(self, area:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect], dx:int, dy:int) -> bool: ...
    def setStaticContents(self, region:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> None: ...
    def size(self) -> QtCore.QSize: ...
    def staticContents(self) -> QtGui.QRegion: ...
    def window(self) -> QtGui.QWindow: ...


class QBitmap(QtGui.QPixmap):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QBitmap:Union[QtGui.QBitmap, str]) -> None: ...
    @overload
    def __init__(self, arg__1:QtCore.QSize) -> None: ...
    @overload
    def __init__(self, arg__1:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    @overload
    def __init__(self, fileName:str, format:Optional[bytes]=...) -> None: ...
    @overload
    def __init__(self, w:int, h:int) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def clear(self) -> None: ...
    @staticmethod
    def fromData(size:QtCore.QSize, bits:bytes, monoFormat:QtGui.QImage.Format=...) -> QtGui.QBitmap: ...
    @staticmethod
    def fromImage(image:Union[QtGui.QImage, str], flags:QtCore.Qt.ImageConversionFlags=...) -> QtGui.QBitmap: ...
    @staticmethod
    def fromPixmap(pixmap:Union[QtGui.QPixmap, QtGui.QImage, str]) -> QtGui.QBitmap: ...
    def swap(self, other:Union[QtGui.QBitmap, str]) -> None: ...
    def transformed(self, matrix:QtGui.QTransform) -> QtGui.QBitmap: ...


class QBrush(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    @overload
    def __init__(self, bs:QtCore.Qt.BrushStyle) -> None: ...
    @overload
    def __init__(self, color:QtCore.Qt.GlobalColor, bs:QtCore.Qt.BrushStyle=...) -> None: ...
    @overload
    def __init__(self, color:QtCore.Qt.GlobalColor, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    @overload
    def __init__(self, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int], bs:QtCore.Qt.BrushStyle=...) -> None: ...
    @overload
    def __init__(self, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int], pixmap:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    @overload
    def __init__(self, gradient:Union[QtGui.QGradient, QtGui.QGradient.Preset]) -> None: ...
    @overload
    def __init__(self, image:Union[QtGui.QImage, str]) -> None: ...
    @overload
    def __init__(self, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def color(self) -> QtGui.QColor: ...
    def gradient(self) -> QtGui.QGradient: ...
    def isOpaque(self) -> bool: ...
    @overload
    def setColor(self, color:QtCore.Qt.GlobalColor) -> None: ...
    @overload
    def setColor(self, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    def setStyle(self, arg__1:QtCore.Qt.BrushStyle) -> None: ...
    def setTexture(self, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    def setTextureImage(self, image:Union[QtGui.QImage, str]) -> None: ...
    def setTransform(self, arg__1:QtGui.QTransform) -> None: ...
    def style(self) -> QtCore.Qt.BrushStyle: ...
    def swap(self, other:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def texture(self) -> QtGui.QPixmap: ...
    def textureImage(self) -> QtGui.QImage: ...
    def transform(self) -> QtGui.QTransform: ...


class QClipboard(QtCore.QObject):

    Clipboard                : QClipboard.Mode = ... # 0x0
    Selection                : QClipboard.Mode = ... # 0x1
    FindBuffer               : QClipboard.Mode = ... # 0x2
    LastMode                 : QClipboard.Mode = ... # 0x2

    class Mode(Enum):

        Clipboard                : QClipboard.Mode # 0x0
        Selection                : QClipboard.Mode # 0x1
        FindBuffer               : QClipboard.Mode # 0x2
        LastMode                 : QClipboard.Mode # 0x2


    def clear(self, mode:QtGui.QClipboard.Mode=...) -> None: ...
    def image(self, mode:QtGui.QClipboard.Mode=...) -> QtGui.QImage: ...
    def mimeData(self, mode:QtGui.QClipboard.Mode=...) -> QtCore.QMimeData: ...
    def ownsClipboard(self) -> bool: ...
    def ownsFindBuffer(self) -> bool: ...
    def ownsSelection(self) -> bool: ...
    def pixmap(self, mode:QtGui.QClipboard.Mode=...) -> QtGui.QPixmap: ...
    def setImage(self, arg__1:Union[QtGui.QImage, str], mode:QtGui.QClipboard.Mode=...) -> None: ...
    def setMimeData(self, data:QtCore.QMimeData, mode:QtGui.QClipboard.Mode=...) -> None: ...
    def setPixmap(self, arg__1:Union[QtGui.QPixmap, QtGui.QImage, str], mode:QtGui.QClipboard.Mode=...) -> None: ...
    def setText(self, arg__1:str, mode:QtGui.QClipboard.Mode=...) -> None: ...
    def supportsFindBuffer(self) -> bool: ...
    def supportsSelection(self) -> bool: ...
    @overload
    def text(self, mode:QtGui.QClipboard.Mode=...) -> str: ...
    @overload
    def text(self, subtype:str, mode:QtGui.QClipboard.Mode=...) -> Tuple: ...


class QCloseEvent(QtCore.QEvent):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:QtGui.QCloseEvent) -> None: ...


class QColor(Shiboken.Object):

    HexRgb                   : QColor.NameFormat = ... # 0x0
    HexArgb                  : QColor.NameFormat = ... # 0x1
    Invalid                  : QColor.Spec = ... # 0x0
    Rgb                      : QColor.Spec = ... # 0x1
    Hsv                      : QColor.Spec = ... # 0x2
    Cmyk                     : QColor.Spec = ... # 0x3
    Hsl                      : QColor.Spec = ... # 0x4
    ExtendedRgb              : QColor.Spec = ... # 0x5

    class NameFormat(Enum):

        HexRgb                   : QColor.NameFormat # 0x0
        HexArgb                  : QColor.NameFormat # 0x1

    class Spec(Enum):

        Invalid                  : QColor.Spec # 0x0
        Rgb                      : QColor.Spec # 0x1
        Hsv                      : QColor.Spec # 0x2
        Cmyk                     : QColor.Spec # 0x3
        Hsl                      : QColor.Spec # 0x4
        ExtendedRgb              : QColor.Spec # 0x5


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:Any) -> None: ...
    @overload
    def __init__(self, color:QtCore.Qt.GlobalColor) -> None: ...
    @overload
    def __init__(self, name:str) -> None: ...
    @overload
    def __init__(self, r:int, g:int, b:int, a:int=...) -> None: ...
    @overload
    def __init__(self, rgb:int) -> None: ...
    @overload
    def __init__(self, rgba64:QtGui.QRgba64) -> None: ...
    @overload
    def __init__(self, spec:QtGui.QColor.Spec, a1:int, a2:int, a3:int, a4:int, a5:int=...) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __setstate__(self, arg__1:object) -> object: ...
    def __str__(self) -> object: ...
    def alpha(self) -> int: ...
    def alphaF(self) -> float: ...
    def black(self) -> int: ...
    def blackF(self) -> float: ...
    def blue(self) -> int: ...
    def blueF(self) -> float: ...
    @staticmethod
    def colorNames() -> List[str]: ...
    def convertTo(self, colorSpec:QtGui.QColor.Spec) -> QtGui.QColor: ...
    def cyan(self) -> int: ...
    def cyanF(self) -> float: ...
    def darker(self, f:int=...) -> QtGui.QColor: ...
    @staticmethod
    def fromCmyk(c:int, m:int, y:int, k:int, a:int=...) -> QtGui.QColor: ...
    @staticmethod
    def fromCmykF(c:float, m:float, y:float, k:float, a:float=...) -> QtGui.QColor: ...
    @staticmethod
    def fromHsl(h:int, s:int, l:int, a:int=...) -> QtGui.QColor: ...
    @staticmethod
    def fromHslF(h:float, s:float, l:float, a:float=...) -> QtGui.QColor: ...
    @staticmethod
    def fromHsv(h:int, s:int, v:int, a:int=...) -> QtGui.QColor: ...
    @staticmethod
    def fromHsvF(h:float, s:float, v:float, a:float=...) -> QtGui.QColor: ...
    @overload
    @staticmethod
    def fromRgb(r:int, g:int, b:int, a:int=...) -> QtGui.QColor: ...
    @overload
    @staticmethod
    def fromRgb(rgb:int) -> QtGui.QColor: ...
    @staticmethod
    def fromRgbF(r:float, g:float, b:float, a:float=...) -> QtGui.QColor: ...
    @staticmethod
    def fromRgba(rgba:int) -> QtGui.QColor: ...
    @overload
    @staticmethod
    def fromRgba64(r:int, g:int, b:int, a:int=...) -> QtGui.QColor: ...
    @overload
    @staticmethod
    def fromRgba64(rgba:QtGui.QRgba64) -> QtGui.QColor: ...
    def getCmyk(self) -> Tuple[int, int, int, int, int]: ...
    def getCmykF(self) -> Tuple[float, float, float, float, float]: ...
    def getHsl(self) -> Tuple[int, int, int, int]: ...
    def getHslF(self) -> Tuple[float, float, float, float]: ...
    def getHsv(self) -> Tuple[int, int, int, int]: ...
    def getHsvF(self) -> Tuple[float, float, float, float]: ...
    def getRgb(self) -> Tuple[int, int, int, int]: ...
    def getRgbF(self) -> Tuple[float, float, float, float]: ...
    def green(self) -> int: ...
    def greenF(self) -> float: ...
    def hslHue(self) -> int: ...
    def hslHueF(self) -> float: ...
    def hslSaturation(self) -> int: ...
    def hslSaturationF(self) -> float: ...
    def hsvHue(self) -> int: ...
    def hsvHueF(self) -> float: ...
    def hsvSaturation(self) -> int: ...
    def hsvSaturationF(self) -> float: ...
    def hue(self) -> int: ...
    def hueF(self) -> float: ...
    def isValid(self) -> bool: ...
    @overload
    @staticmethod
    def isValidColor(arg__1:str) -> bool: ...
    @overload
    @staticmethod
    def isValidColor(name:str) -> bool: ...
    def lighter(self, f:int=...) -> QtGui.QColor: ...
    def lightness(self) -> int: ...
    def lightnessF(self) -> float: ...
    def magenta(self) -> int: ...
    def magentaF(self) -> float: ...
    def name(self, format:QtGui.QColor.NameFormat=...) -> str: ...
    def red(self) -> int: ...
    def redF(self) -> float: ...
    def rgb(self) -> int: ...
    def rgba(self) -> int: ...
    def rgba64(self) -> QtGui.QRgba64: ...
    def saturation(self) -> int: ...
    def saturationF(self) -> float: ...
    def setAlpha(self, alpha:int) -> None: ...
    def setAlphaF(self, alpha:float) -> None: ...
    def setBlue(self, blue:int) -> None: ...
    def setBlueF(self, blue:float) -> None: ...
    def setCmyk(self, c:int, m:int, y:int, k:int, a:int=...) -> None: ...
    def setCmykF(self, c:float, m:float, y:float, k:float, a:float=...) -> None: ...
    def setGreen(self, green:int) -> None: ...
    def setGreenF(self, green:float) -> None: ...
    def setHsl(self, h:int, s:int, l:int, a:int=...) -> None: ...
    def setHslF(self, h:float, s:float, l:float, a:float=...) -> None: ...
    def setHsv(self, h:int, s:int, v:int, a:int=...) -> None: ...
    def setHsvF(self, h:float, s:float, v:float, a:float=...) -> None: ...
    def setNamedColor(self, name:str) -> None: ...
    def setRed(self, red:int) -> None: ...
    def setRedF(self, red:float) -> None: ...
    @overload
    def setRgb(self, r:int, g:int, b:int, a:int=...) -> None: ...
    @overload
    def setRgb(self, rgb:int) -> None: ...
    def setRgbF(self, r:float, g:float, b:float, a:float=...) -> None: ...
    def setRgba(self, rgba:int) -> None: ...
    def setRgba64(self, rgba:QtGui.QRgba64) -> None: ...
    def spec(self) -> QtGui.QColor.Spec: ...
    def toCmyk(self) -> QtGui.QColor: ...
    def toExtendedRgb(self) -> QtGui.QColor: ...
    def toHsl(self) -> QtGui.QColor: ...
    def toHsv(self) -> QtGui.QColor: ...
    def toRgb(self) -> QtGui.QColor: ...
    def toTuple(self) -> object: ...
    def value(self) -> int: ...
    def valueF(self) -> float: ...
    def yellow(self) -> int: ...
    def yellowF(self) -> float: ...


class QColorConstants(Shiboken.Object):

    class Svg(Shiboken.Object): ...


class QColorSpace(Shiboken.Object):

    SRgb                     : QColorSpace.NamedColorSpace = ... # 0x1
    SRgbLinear               : QColorSpace.NamedColorSpace = ... # 0x2
    AdobeRgb                 : QColorSpace.NamedColorSpace = ... # 0x3
    DisplayP3                : QColorSpace.NamedColorSpace = ... # 0x4
    ProPhotoRgb              : QColorSpace.NamedColorSpace = ... # 0x5

    class NamedColorSpace(Enum):

        SRgb                     : QColorSpace.NamedColorSpace # 0x1
        SRgbLinear               : QColorSpace.NamedColorSpace # 0x2
        AdobeRgb                 : QColorSpace.NamedColorSpace # 0x3
        DisplayP3                : QColorSpace.NamedColorSpace # 0x4
        ProPhotoRgb              : QColorSpace.NamedColorSpace # 0x5

    class Primaries(Enum):

        Custom                   : QColorSpace.Primaries # 0x0
        SRgb                     : QColorSpace.Primaries # 0x1
        AdobeRgb                 : QColorSpace.Primaries # 0x2
        DciP3D65                 : QColorSpace.Primaries # 0x3
        ProPhotoRgb              : QColorSpace.Primaries # 0x4

    class TransferFunction(Enum):

        Custom                   : QColorSpace.TransferFunction # 0x0
        Linear                   : QColorSpace.TransferFunction # 0x1
        Gamma                    : QColorSpace.TransferFunction # 0x2
        SRgb                     : QColorSpace.TransferFunction # 0x3
        ProPhotoRgb              : QColorSpace.TransferFunction # 0x4


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, colorSpace:Union[QtGui.QColorSpace, QtGui.QColorSpace.NamedColorSpace]) -> None: ...
    @overload
    def __init__(self, namedColorSpace:QtGui.QColorSpace.NamedColorSpace) -> None: ...
    @overload
    def __init__(self, primaries:QtGui.QColorSpace.Primaries, gamma:float) -> None: ...
    @overload
    def __init__(self, primaries:QtGui.QColorSpace.Primaries, transferFunction:QtGui.QColorSpace.TransferFunction, gamma:float=...) -> None: ...
    @overload
    def __init__(self, primaries:QtGui.QColorSpace.Primaries, transferFunctionTable:Sequence[int]) -> None: ...
    @overload
    def __init__(self, whitePoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], redPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], greenPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], bluePoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], redTransferFunctionTable:Sequence[int], greenTransferFunctionTable:Sequence[int], blueTransferFunctionTable:Sequence[int]) -> None: ...
    @overload
    def __init__(self, whitePoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], redPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], greenPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], bluePoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], transferFunction:QtGui.QColorSpace.TransferFunction, gamma:float=...) -> None: ...
    @overload
    def __init__(self, whitePoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], redPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], greenPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], bluePoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], transferFunctionTable:Sequence[int]) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def description(self) -> str: ...
    @staticmethod
    def fromIccProfile(iccProfile:Union[QtCore.QByteArray, bytes]) -> QtGui.QColorSpace: ...
    def gamma(self) -> float: ...
    def iccProfile(self) -> QtCore.QByteArray: ...
    def isValid(self) -> bool: ...
    def primaries(self) -> QtGui.QColorSpace.Primaries: ...
    def setDescription(self, description:str) -> None: ...
    @overload
    def setPrimaries(self, primariesId:QtGui.QColorSpace.Primaries) -> None: ...
    @overload
    def setPrimaries(self, whitePoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], redPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], greenPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], bluePoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def setTransferFunction(self, transferFunction:QtGui.QColorSpace.TransferFunction, gamma:float=...) -> None: ...
    @overload
    def setTransferFunction(self, transferFunctionTable:Sequence[int]) -> None: ...
    def setTransferFunctions(self, redTransferFunctionTable:Sequence[int], greenTransferFunctionTable:Sequence[int], blueTransferFunctionTable:Sequence[int]) -> None: ...
    def swap(self, colorSpace:Union[QtGui.QColorSpace, QtGui.QColorSpace.NamedColorSpace]) -> None: ...
    def transferFunction(self) -> QtGui.QColorSpace.TransferFunction: ...
    def transformationToColorSpace(self, colorspace:Union[QtGui.QColorSpace, QtGui.QColorSpace.NamedColorSpace]) -> QtGui.QColorTransform: ...
    @overload
    def withTransferFunction(self, transferFunction:QtGui.QColorSpace.TransferFunction, gamma:float=...) -> QtGui.QColorSpace: ...
    @overload
    def withTransferFunction(self, transferFunctionTable:Sequence[int]) -> QtGui.QColorSpace: ...
    def withTransferFunctions(self, redTransferFunctionTable:Sequence[int], greenTransferFunctionTable:Sequence[int], blueTransferFunctionTable:Sequence[int]) -> QtGui.QColorSpace: ...


class QColorTransform(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, colorTransform:QtGui.QColorTransform) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    @overload
    def map(self, argb:int) -> int: ...
    @overload
    def map(self, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> QtGui.QColor: ...
    @overload
    def map(self, rgba64:QtGui.QRgba64) -> QtGui.QRgba64: ...
    def swap(self, other:QtGui.QColorTransform) -> None: ...


class QConicalGradient(QtGui.QGradient):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QConicalGradient:QtGui.QConicalGradient) -> None: ...
    @overload
    def __init__(self, center:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], startAngle:float) -> None: ...
    @overload
    def __init__(self, cx:float, cy:float, startAngle:float) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def angle(self) -> float: ...
    def center(self) -> QtCore.QPointF: ...
    def setAngle(self, angle:float) -> None: ...
    @overload
    def setCenter(self, center:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def setCenter(self, x:float, y:float) -> None: ...


class QContextMenuEvent(QtGui.QInputEvent):

    Mouse                    : QContextMenuEvent.Reason = ... # 0x0
    Keyboard                 : QContextMenuEvent.Reason = ... # 0x1
    Other                    : QContextMenuEvent.Reason = ... # 0x2

    class Reason(Enum):

        Mouse                    : QContextMenuEvent.Reason # 0x0
        Keyboard                 : QContextMenuEvent.Reason # 0x1
        Other                    : QContextMenuEvent.Reason # 0x2


    @overload
    def __init__(self, arg__1:QtGui.QContextMenuEvent) -> None: ...
    @overload
    def __init__(self, reason:QtGui.QContextMenuEvent.Reason, pos:QtCore.QPoint) -> None: ...
    @overload
    def __init__(self, reason:QtGui.QContextMenuEvent.Reason, pos:QtCore.QPoint, globalPos:QtCore.QPoint, modifiers:QtCore.Qt.KeyboardModifiers=...) -> None: ...

    def clone(self) -> QtGui.QContextMenuEvent: ...
    def globalPos(self) -> QtCore.QPoint: ...
    def globalX(self) -> int: ...
    def globalY(self) -> int: ...
    def pos(self) -> QtCore.QPoint: ...
    def reason(self) -> QtGui.QContextMenuEvent.Reason: ...
    def x(self) -> int: ...
    def y(self) -> int: ...


class QCursor(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, bitmap:Union[QtGui.QBitmap, str], mask:Union[QtGui.QBitmap, str], hotX:int=..., hotY:int=...) -> None: ...
    @overload
    def __init__(self, cursor:Union[QtGui.QCursor, QtCore.Qt.CursorShape, QtGui.QPixmap]) -> None: ...
    @overload
    def __init__(self, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str], hotX:int=..., hotY:int=...) -> None: ...
    @overload
    def __init__(self, shape:QtCore.Qt.CursorShape) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, outS:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, inS:QtCore.QDataStream) -> QtCore.QDataStream: ...
    @overload
    def bitmap(self) -> QtGui.QBitmap: ...
    @overload
    def bitmap(self, arg__1:QtCore.Qt.ReturnByValueConstant) -> QtGui.QBitmap: ...
    def hotSpot(self) -> QtCore.QPoint: ...
    @overload
    def mask(self) -> QtGui.QBitmap: ...
    @overload
    def mask(self, arg__1:QtCore.Qt.ReturnByValueConstant) -> QtGui.QBitmap: ...
    def pixmap(self) -> QtGui.QPixmap: ...
    @overload
    @staticmethod
    def pos() -> QtCore.QPoint: ...
    @overload
    @staticmethod
    def pos(screen:QtGui.QScreen) -> QtCore.QPoint: ...
    @overload
    @staticmethod
    def setPos(p:QtCore.QPoint) -> None: ...
    @overload
    @staticmethod
    def setPos(screen:QtGui.QScreen, p:QtCore.QPoint) -> None: ...
    @overload
    @staticmethod
    def setPos(screen:QtGui.QScreen, x:int, y:int) -> None: ...
    @overload
    @staticmethod
    def setPos(x:int, y:int) -> None: ...
    def setShape(self, newShape:QtCore.Qt.CursorShape) -> None: ...
    def shape(self) -> QtCore.Qt.CursorShape: ...
    def swap(self, other:Union[QtGui.QCursor, QtCore.Qt.CursorShape, QtGui.QPixmap]) -> None: ...


class QDesktopServices(Shiboken.Object):

    def __init__(self) -> None: ...

    @staticmethod
    def openUrl(url:Union[QtCore.QUrl, str]) -> bool: ...
    @staticmethod
    def setUrlHandler(scheme:str, receiver:QtCore.QObject, method:bytes) -> None: ...
    @staticmethod
    def unsetUrlHandler(scheme:str) -> None: ...


class QDoubleValidator(QtGui.QValidator):

    StandardNotation         : QDoubleValidator.Notation = ... # 0x0
    ScientificNotation       : QDoubleValidator.Notation = ... # 0x1

    class Notation(Enum):

        StandardNotation         : QDoubleValidator.Notation # 0x0
        ScientificNotation       : QDoubleValidator.Notation # 0x1


    @overload
    def __init__(self, bottom:float, top:float, decimals:int, parent:Optional[QtCore.QObject]=...) -> None: ...
    @overload
    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...

    def bottom(self) -> float: ...
    def decimals(self) -> int: ...
    def notation(self) -> QtGui.QDoubleValidator.Notation: ...
    def setBottom(self, arg__1:float) -> None: ...
    def setDecimals(self, arg__1:int) -> None: ...
    def setNotation(self, arg__1:QtGui.QDoubleValidator.Notation) -> None: ...
    def setRange(self, bottom:float, top:float, decimals:int=...) -> None: ...
    def setTop(self, arg__1:float) -> None: ...
    def top(self) -> float: ...
    def validate(self, arg__1:str, arg__2:int) -> object: ...


class QDrag(QtCore.QObject):

    def __init__(self, dragSource:QtCore.QObject) -> None: ...

    @staticmethod
    def cancel() -> None: ...
    def defaultAction(self) -> QtCore.Qt.DropAction: ...
    def dragCursor(self, action:QtCore.Qt.DropAction) -> QtGui.QPixmap: ...
    @overload
    def exec(self, supportedActions:QtCore.Qt.DropActions, defaultAction:QtCore.Qt.DropAction) -> QtCore.Qt.DropAction: ...
    @overload
    def exec(self, supportedActions:QtCore.Qt.DropActions=...) -> QtCore.Qt.DropAction: ...
    @overload
    def exec_(self, arg__1:QtCore.Qt.DropActions, arg__2:QtCore.Qt.DropAction) -> QtCore.Qt.DropAction: ...
    @overload
    def exec_(self, supportedActions:QtCore.Qt.DropActions=...) -> QtCore.Qt.DropAction: ...
    def hotSpot(self) -> QtCore.QPoint: ...
    def mimeData(self) -> QtCore.QMimeData: ...
    def pixmap(self) -> QtGui.QPixmap: ...
    def setDragCursor(self, cursor:Union[QtGui.QPixmap, QtGui.QImage, str], action:QtCore.Qt.DropAction) -> None: ...
    def setHotSpot(self, hotspot:QtCore.QPoint) -> None: ...
    def setMimeData(self, data:QtCore.QMimeData) -> None: ...
    def setPixmap(self, arg__1:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    def source(self) -> QtCore.QObject: ...
    def supportedActions(self) -> QtCore.Qt.DropActions: ...
    def target(self) -> QtCore.QObject: ...


class QDragEnterEvent(QtGui.QDragMoveEvent):

    @overload
    def __init__(self, arg__1:QtGui.QDragEnterEvent) -> None: ...
    @overload
    def __init__(self, pos:QtCore.QPoint, actions:QtCore.Qt.DropActions, data:QtCore.QMimeData, buttons:QtCore.Qt.MouseButtons, modifiers:QtCore.Qt.KeyboardModifiers) -> None: ...


class QDragLeaveEvent(QtCore.QEvent):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:QtGui.QDragLeaveEvent) -> None: ...


class QDragMoveEvent(QtGui.QDropEvent):

    @overload
    def __init__(self, arg__1:QtGui.QDragMoveEvent) -> None: ...
    @overload
    def __init__(self, pos:QtCore.QPoint, actions:QtCore.Qt.DropActions, data:QtCore.QMimeData, buttons:QtCore.Qt.MouseButtons, modifiers:QtCore.Qt.KeyboardModifiers, type:QtCore.QEvent.Type=...) -> None: ...

    @overload
    def accept(self) -> None: ...
    @overload
    def accept(self, r:QtCore.QRect) -> None: ...
    def answerRect(self) -> QtCore.QRect: ...
    def clone(self) -> QtGui.QDragMoveEvent: ...
    @overload
    def ignore(self) -> None: ...
    @overload
    def ignore(self, r:QtCore.QRect) -> None: ...


class QDropEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QDropEvent) -> None: ...
    @overload
    def __init__(self, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], actions:QtCore.Qt.DropActions, data:QtCore.QMimeData, buttons:QtCore.Qt.MouseButtons, modifiers:QtCore.Qt.KeyboardModifiers, type:QtCore.QEvent.Type=...) -> None: ...

    def acceptProposedAction(self) -> None: ...
    def buttons(self) -> QtCore.Qt.MouseButtons: ...
    def clone(self) -> QtGui.QDropEvent: ...
    def dropAction(self) -> QtCore.Qt.DropAction: ...
    def keyboardModifiers(self) -> QtCore.Qt.KeyboardModifiers: ...
    def mimeData(self) -> QtCore.QMimeData: ...
    def modifiers(self) -> QtCore.Qt.KeyboardModifiers: ...
    def mouseButtons(self) -> QtCore.Qt.MouseButtons: ...
    def pos(self) -> QtCore.QPoint: ...
    def posF(self) -> QtCore.QPointF: ...
    def position(self) -> QtCore.QPointF: ...
    def possibleActions(self) -> QtCore.Qt.DropActions: ...
    def proposedAction(self) -> QtCore.Qt.DropAction: ...
    def setDropAction(self, action:QtCore.Qt.DropAction) -> None: ...
    def source(self) -> QtCore.QObject: ...


class QEnterEvent(QtGui.QSinglePointEvent):

    @overload
    def __init__(self, arg__1:QtGui.QEnterEvent) -> None: ...
    @overload
    def __init__(self, localPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], scenePos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], globalPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], device:QtGui.QPointingDevice=...) -> None: ...

    def clone(self) -> QtGui.QEnterEvent: ...
    def globalPos(self) -> QtCore.QPoint: ...
    def globalX(self) -> int: ...
    def globalY(self) -> int: ...
    def localPos(self) -> QtCore.QPointF: ...
    def pos(self) -> QtCore.QPoint: ...
    def screenPos(self) -> QtCore.QPointF: ...
    def windowPos(self) -> QtCore.QPointF: ...
    def x(self) -> int: ...
    def y(self) -> int: ...


class QEventPoint(Shiboken.Object):

    Unknown                  : QEventPoint.State = ... # 0x0
    Pressed                  : QEventPoint.State = ... # 0x1
    Updated                  : QEventPoint.State = ... # 0x2
    Stationary               : QEventPoint.State = ... # 0x4
    Released                 : QEventPoint.State = ... # 0x8

    class State(Enum):

        Unknown                  : QEventPoint.State # 0x0
        Pressed                  : QEventPoint.State # 0x1
        Updated                  : QEventPoint.State # 0x2
        Stationary               : QEventPoint.State # 0x4
        Released                 : QEventPoint.State # 0x8


    @overload
    def __init__(self, id:int=..., device:Optional[QtGui.QPointingDevice]=...) -> None: ...
    @overload
    def __init__(self, other:QtGui.QEventPoint) -> None: ...
    @overload
    def __init__(self, pointId:int, state:QtGui.QEventPoint.State, scenePosition:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], globalPosition:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def device(self) -> QtGui.QPointingDevice: ...
    def ellipseDiameters(self) -> QtCore.QSizeF: ...
    def globalGrabPosition(self) -> QtCore.QPointF: ...
    def globalLastPosition(self) -> QtCore.QPointF: ...
    def globalPosition(self) -> QtCore.QPointF: ...
    def globalPressPosition(self) -> QtCore.QPointF: ...
    def grabPosition(self) -> QtCore.QPointF: ...
    def id(self) -> int: ...
    def isAccepted(self) -> bool: ...
    def lastNormalizedPos(self) -> QtCore.QPointF: ...
    def lastPos(self) -> QtCore.QPointF: ...
    def lastPosition(self) -> QtCore.QPointF: ...
    def lastScenePos(self) -> QtCore.QPointF: ...
    def lastScreenPos(self) -> QtCore.QPointF: ...
    def lastTimestamp(self) -> int: ...
    def normalizedPos(self) -> QtCore.QPointF: ...
    def normalizedPosition(self) -> QtCore.QPointF: ...
    def pos(self) -> QtCore.QPointF: ...
    def position(self) -> QtCore.QPointF: ...
    def pressPosition(self) -> QtCore.QPointF: ...
    def pressTimestamp(self) -> int: ...
    def pressure(self) -> float: ...
    def rotation(self) -> float: ...
    def sceneGrabPosition(self) -> QtCore.QPointF: ...
    def sceneLastPosition(self) -> QtCore.QPointF: ...
    def scenePos(self) -> QtCore.QPointF: ...
    def scenePosition(self) -> QtCore.QPointF: ...
    def scenePressPosition(self) -> QtCore.QPointF: ...
    def screenPos(self) -> QtCore.QPointF: ...
    def setAccepted(self, accepted:bool=...) -> None: ...
    def startNormalizedPos(self) -> QtCore.QPointF: ...
    def startPos(self) -> QtCore.QPointF: ...
    def startScenePos(self) -> QtCore.QPointF: ...
    def startScreenPos(self) -> QtCore.QPointF: ...
    def state(self) -> QtGui.QEventPoint.State: ...
    def swap(self, other:QtGui.QEventPoint) -> None: ...
    def timeHeld(self) -> float: ...
    def timestamp(self) -> int: ...
    def uniqueId(self) -> QtGui.QPointingDeviceUniqueId: ...
    def velocity(self) -> QtGui.QVector2D: ...


class QExposeEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QExposeEvent) -> None: ...
    @overload
    def __init__(self, m_region:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> None: ...

    def clone(self) -> QtGui.QExposeEvent: ...
    def region(self) -> QtGui.QRegion: ...


class QFileOpenEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QFileOpenEvent) -> None: ...
    @overload
    def __init__(self, file:str) -> None: ...
    @overload
    def __init__(self, url:Union[QtCore.QUrl, str]) -> None: ...

    def clone(self) -> QtGui.QFileOpenEvent: ...
    def file(self) -> str: ...
    def url(self) -> QtCore.QUrl: ...


class QFocusEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QFocusEvent) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, reason:QtCore.Qt.FocusReason=...) -> None: ...

    def clone(self) -> QtGui.QFocusEvent: ...
    def gotFocus(self) -> bool: ...
    def lostFocus(self) -> bool: ...
    def reason(self) -> QtCore.Qt.FocusReason: ...


class QFont(Shiboken.Object):

    MixedCase                : QFont.Capitalization = ... # 0x0
    AllUppercase             : QFont.Capitalization = ... # 0x1
    AllLowercase             : QFont.Capitalization = ... # 0x2
    SmallCaps                : QFont.Capitalization = ... # 0x3
    Capitalize               : QFont.Capitalization = ... # 0x4
    PreferDefaultHinting     : QFont.HintingPreference = ... # 0x0
    PreferNoHinting          : QFont.HintingPreference = ... # 0x1
    PreferVerticalHinting    : QFont.HintingPreference = ... # 0x2
    PreferFullHinting        : QFont.HintingPreference = ... # 0x3
    PercentageSpacing        : QFont.SpacingType = ... # 0x0
    AbsoluteSpacing          : QFont.SpacingType = ... # 0x1
    AnyStretch               : QFont.Stretch = ... # 0x0
    UltraCondensed           : QFont.Stretch = ... # 0x32
    ExtraCondensed           : QFont.Stretch = ... # 0x3e
    Condensed                : QFont.Stretch = ... # 0x4b
    SemiCondensed            : QFont.Stretch = ... # 0x57
    Unstretched              : QFont.Stretch = ... # 0x64
    SemiExpanded             : QFont.Stretch = ... # 0x70
    Expanded                 : QFont.Stretch = ... # 0x7d
    ExtraExpanded            : QFont.Stretch = ... # 0x96
    UltraExpanded            : QFont.Stretch = ... # 0xc8
    StyleNormal              : QFont.Style = ... # 0x0
    StyleItalic              : QFont.Style = ... # 0x1
    StyleOblique             : QFont.Style = ... # 0x2
    Helvetica                : QFont.StyleHint = ... # 0x0
    SansSerif                : QFont.StyleHint = ... # 0x0
    Serif                    : QFont.StyleHint = ... # 0x1
    Times                    : QFont.StyleHint = ... # 0x1
    Courier                  : QFont.StyleHint = ... # 0x2
    TypeWriter               : QFont.StyleHint = ... # 0x2
    Decorative               : QFont.StyleHint = ... # 0x3
    OldEnglish               : QFont.StyleHint = ... # 0x3
    System                   : QFont.StyleHint = ... # 0x4
    AnyStyle                 : QFont.StyleHint = ... # 0x5
    Cursive                  : QFont.StyleHint = ... # 0x6
    Monospace                : QFont.StyleHint = ... # 0x7
    Fantasy                  : QFont.StyleHint = ... # 0x8
    PreferDefault            : QFont.StyleStrategy = ... # 0x1
    PreferBitmap             : QFont.StyleStrategy = ... # 0x2
    PreferDevice             : QFont.StyleStrategy = ... # 0x4
    PreferOutline            : QFont.StyleStrategy = ... # 0x8
    ForceOutline             : QFont.StyleStrategy = ... # 0x10
    PreferMatch              : QFont.StyleStrategy = ... # 0x20
    PreferQuality            : QFont.StyleStrategy = ... # 0x40
    PreferAntialias          : QFont.StyleStrategy = ... # 0x80
    NoAntialias              : QFont.StyleStrategy = ... # 0x100
    NoSubpixelAntialias      : QFont.StyleStrategy = ... # 0x800
    PreferNoShaping          : QFont.StyleStrategy = ... # 0x1000
    NoFontMerging            : QFont.StyleStrategy = ... # 0x8000
    Thin                     : QFont.Weight = ... # 0x64
    ExtraLight               : QFont.Weight = ... # 0xc8
    Light                    : QFont.Weight = ... # 0x12c
    Normal                   : QFont.Weight = ... # 0x190
    Medium                   : QFont.Weight = ... # 0x1f4
    DemiBold                 : QFont.Weight = ... # 0x258
    Bold                     : QFont.Weight = ... # 0x2bc
    ExtraBold                : QFont.Weight = ... # 0x320
    Black                    : QFont.Weight = ... # 0x384

    class Capitalization(Enum):

        MixedCase                : QFont.Capitalization # 0x0
        AllUppercase             : QFont.Capitalization # 0x1
        AllLowercase             : QFont.Capitalization # 0x2
        SmallCaps                : QFont.Capitalization # 0x3
        Capitalize               : QFont.Capitalization # 0x4

    class HintingPreference(Enum):

        PreferDefaultHinting     : QFont.HintingPreference # 0x0
        PreferNoHinting          : QFont.HintingPreference # 0x1
        PreferVerticalHinting    : QFont.HintingPreference # 0x2
        PreferFullHinting        : QFont.HintingPreference # 0x3

    class SpacingType(Enum):

        PercentageSpacing        : QFont.SpacingType # 0x0
        AbsoluteSpacing          : QFont.SpacingType # 0x1

    class Stretch(Enum):

        AnyStretch               : QFont.Stretch # 0x0
        UltraCondensed           : QFont.Stretch # 0x32
        ExtraCondensed           : QFont.Stretch # 0x3e
        Condensed                : QFont.Stretch # 0x4b
        SemiCondensed            : QFont.Stretch # 0x57
        Unstretched              : QFont.Stretch # 0x64
        SemiExpanded             : QFont.Stretch # 0x70
        Expanded                 : QFont.Stretch # 0x7d
        ExtraExpanded            : QFont.Stretch # 0x96
        UltraExpanded            : QFont.Stretch # 0xc8

    class Style(Enum):

        StyleNormal              : QFont.Style # 0x0
        StyleItalic              : QFont.Style # 0x1
        StyleOblique             : QFont.Style # 0x2

    class StyleHint(Enum):

        Helvetica                : QFont.StyleHint # 0x0
        SansSerif                : QFont.StyleHint # 0x0
        Serif                    : QFont.StyleHint # 0x1
        Times                    : QFont.StyleHint # 0x1
        Courier                  : QFont.StyleHint # 0x2
        TypeWriter               : QFont.StyleHint # 0x2
        Decorative               : QFont.StyleHint # 0x3
        OldEnglish               : QFont.StyleHint # 0x3
        System                   : QFont.StyleHint # 0x4
        AnyStyle                 : QFont.StyleHint # 0x5
        Cursive                  : QFont.StyleHint # 0x6
        Monospace                : QFont.StyleHint # 0x7
        Fantasy                  : QFont.StyleHint # 0x8

    class StyleStrategy(Enum):

        PreferDefault            : QFont.StyleStrategy # 0x1
        PreferBitmap             : QFont.StyleStrategy # 0x2
        PreferDevice             : QFont.StyleStrategy # 0x4
        PreferOutline            : QFont.StyleStrategy # 0x8
        ForceOutline             : QFont.StyleStrategy # 0x10
        PreferMatch              : QFont.StyleStrategy # 0x20
        PreferQuality            : QFont.StyleStrategy # 0x40
        PreferAntialias          : QFont.StyleStrategy # 0x80
        NoAntialias              : QFont.StyleStrategy # 0x100
        NoSubpixelAntialias      : QFont.StyleStrategy # 0x800
        PreferNoShaping          : QFont.StyleStrategy # 0x1000
        NoFontMerging            : QFont.StyleStrategy # 0x8000

    class Weight(Enum):

        Thin                     : QFont.Weight # 0x64
        ExtraLight               : QFont.Weight # 0xc8
        Light                    : QFont.Weight # 0x12c
        Normal                   : QFont.Weight # 0x190
        Medium                   : QFont.Weight # 0x1f4
        DemiBold                 : QFont.Weight # 0x258
        Bold                     : QFont.Weight # 0x2bc
        ExtraBold                : QFont.Weight # 0x320
        Black                    : QFont.Weight # 0x384


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, families:Sequence[str], pointSize:int=..., weight:int=..., italic:bool=...) -> None: ...
    @overload
    def __init__(self, family:str, pointSize:int=..., weight:int=..., italic:bool=...) -> None: ...
    @overload
    def __init__(self, font:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    @overload
    def __init__(self, font:Union[QtGui.QFont, str, Sequence[str]], pd:QtGui.QPaintDevice) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def bold(self) -> bool: ...
    @staticmethod
    def cacheStatistics() -> None: ...
    def capitalization(self) -> QtGui.QFont.Capitalization: ...
    @staticmethod
    def cleanup() -> None: ...
    def defaultFamily(self) -> str: ...
    def exactMatch(self) -> bool: ...
    def families(self) -> List[str]: ...
    def family(self) -> str: ...
    def fixedPitch(self) -> bool: ...
    def fromString(self, arg__1:str) -> bool: ...
    def hintingPreference(self) -> QtGui.QFont.HintingPreference: ...
    @staticmethod
    def initialize() -> None: ...
    @staticmethod
    def insertSubstitution(arg__1:str, arg__2:str) -> None: ...
    @staticmethod
    def insertSubstitutions(arg__1:str, arg__2:Sequence[str]) -> None: ...
    def isCopyOf(self, arg__1:Union[QtGui.QFont, str, Sequence[str]]) -> bool: ...
    def italic(self) -> bool: ...
    def kerning(self) -> bool: ...
    def key(self) -> str: ...
    def legacyWeight(self) -> int: ...
    def letterSpacing(self) -> float: ...
    def letterSpacingType(self) -> QtGui.QFont.SpacingType: ...
    def overline(self) -> bool: ...
    def pixelSize(self) -> int: ...
    def pointSize(self) -> int: ...
    def pointSizeF(self) -> float: ...
    @staticmethod
    def removeSubstitutions(arg__1:str) -> None: ...
    def resolve(self, arg__1:Union[QtGui.QFont, str, Sequence[str]]) -> QtGui.QFont: ...
    def resolveMask(self) -> int: ...
    def setBold(self, arg__1:bool) -> None: ...
    def setCapitalization(self, arg__1:QtGui.QFont.Capitalization) -> None: ...
    def setFamilies(self, arg__1:Sequence[str]) -> None: ...
    def setFamily(self, arg__1:str) -> None: ...
    def setFixedPitch(self, arg__1:bool) -> None: ...
    def setHintingPreference(self, hintingPreference:QtGui.QFont.HintingPreference) -> None: ...
    def setItalic(self, b:bool) -> None: ...
    def setKerning(self, arg__1:bool) -> None: ...
    def setLegacyWeight(self, legacyWeight:int) -> None: ...
    def setLetterSpacing(self, type:QtGui.QFont.SpacingType, spacing:float) -> None: ...
    def setOverline(self, arg__1:bool) -> None: ...
    def setPixelSize(self, arg__1:int) -> None: ...
    def setPointSize(self, arg__1:int) -> None: ...
    def setPointSizeF(self, arg__1:float) -> None: ...
    def setResolveMask(self, mask:int) -> None: ...
    def setStretch(self, arg__1:int) -> None: ...
    def setStrikeOut(self, arg__1:bool) -> None: ...
    def setStyle(self, style:QtGui.QFont.Style) -> None: ...
    def setStyleHint(self, arg__1:QtGui.QFont.StyleHint, strategy:QtGui.QFont.StyleStrategy=...) -> None: ...
    def setStyleName(self, arg__1:str) -> None: ...
    def setStyleStrategy(self, s:QtGui.QFont.StyleStrategy) -> None: ...
    def setUnderline(self, arg__1:bool) -> None: ...
    def setWeight(self, weight:QtGui.QFont.Weight) -> None: ...
    def setWordSpacing(self, spacing:float) -> None: ...
    def stretch(self) -> int: ...
    def strikeOut(self) -> bool: ...
    def style(self) -> QtGui.QFont.Style: ...
    def styleHint(self) -> QtGui.QFont.StyleHint: ...
    def styleName(self) -> str: ...
    def styleStrategy(self) -> QtGui.QFont.StyleStrategy: ...
    @staticmethod
    def substitute(arg__1:str) -> str: ...
    @staticmethod
    def substitutes(arg__1:str) -> List[str]: ...
    @staticmethod
    def substitutions() -> List[str]: ...
    def swap(self, other:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    def toString(self) -> str: ...
    def underline(self) -> bool: ...
    def weight(self) -> QtGui.QFont.Weight: ...
    def wordSpacing(self) -> float: ...


class QFontDatabase(Shiboken.Object):

    GeneralFont              : QFontDatabase.SystemFont = ... # 0x0
    FixedFont                : QFontDatabase.SystemFont = ... # 0x1
    TitleFont                : QFontDatabase.SystemFont = ... # 0x2
    SmallestReadableFont     : QFontDatabase.SystemFont = ... # 0x3
    Any                      : QFontDatabase.WritingSystem = ... # 0x0
    Latin                    : QFontDatabase.WritingSystem = ... # 0x1
    Greek                    : QFontDatabase.WritingSystem = ... # 0x2
    Cyrillic                 : QFontDatabase.WritingSystem = ... # 0x3
    Armenian                 : QFontDatabase.WritingSystem = ... # 0x4
    Hebrew                   : QFontDatabase.WritingSystem = ... # 0x5
    Arabic                   : QFontDatabase.WritingSystem = ... # 0x6
    Syriac                   : QFontDatabase.WritingSystem = ... # 0x7
    Thaana                   : QFontDatabase.WritingSystem = ... # 0x8
    Devanagari               : QFontDatabase.WritingSystem = ... # 0x9
    Bengali                  : QFontDatabase.WritingSystem = ... # 0xa
    Gurmukhi                 : QFontDatabase.WritingSystem = ... # 0xb
    Gujarati                 : QFontDatabase.WritingSystem = ... # 0xc
    Oriya                    : QFontDatabase.WritingSystem = ... # 0xd
    Tamil                    : QFontDatabase.WritingSystem = ... # 0xe
    Telugu                   : QFontDatabase.WritingSystem = ... # 0xf
    Kannada                  : QFontDatabase.WritingSystem = ... # 0x10
    Malayalam                : QFontDatabase.WritingSystem = ... # 0x11
    Sinhala                  : QFontDatabase.WritingSystem = ... # 0x12
    Thai                     : QFontDatabase.WritingSystem = ... # 0x13
    Lao                      : QFontDatabase.WritingSystem = ... # 0x14
    Tibetan                  : QFontDatabase.WritingSystem = ... # 0x15
    Myanmar                  : QFontDatabase.WritingSystem = ... # 0x16
    Georgian                 : QFontDatabase.WritingSystem = ... # 0x17
    Khmer                    : QFontDatabase.WritingSystem = ... # 0x18
    SimplifiedChinese        : QFontDatabase.WritingSystem = ... # 0x19
    TraditionalChinese       : QFontDatabase.WritingSystem = ... # 0x1a
    Japanese                 : QFontDatabase.WritingSystem = ... # 0x1b
    Korean                   : QFontDatabase.WritingSystem = ... # 0x1c
    Vietnamese               : QFontDatabase.WritingSystem = ... # 0x1d
    Other                    : QFontDatabase.WritingSystem = ... # 0x1e
    Symbol                   : QFontDatabase.WritingSystem = ... # 0x1e
    Ogham                    : QFontDatabase.WritingSystem = ... # 0x1f
    Runic                    : QFontDatabase.WritingSystem = ... # 0x20
    Nko                      : QFontDatabase.WritingSystem = ... # 0x21
    WritingSystemsCount      : QFontDatabase.WritingSystem = ... # 0x22

    class SystemFont(Enum):

        GeneralFont              : QFontDatabase.SystemFont # 0x0
        FixedFont                : QFontDatabase.SystemFont # 0x1
        TitleFont                : QFontDatabase.SystemFont # 0x2
        SmallestReadableFont     : QFontDatabase.SystemFont # 0x3

    class WritingSystem(Enum):

        Any                      : QFontDatabase.WritingSystem # 0x0
        Latin                    : QFontDatabase.WritingSystem # 0x1
        Greek                    : QFontDatabase.WritingSystem # 0x2
        Cyrillic                 : QFontDatabase.WritingSystem # 0x3
        Armenian                 : QFontDatabase.WritingSystem # 0x4
        Hebrew                   : QFontDatabase.WritingSystem # 0x5
        Arabic                   : QFontDatabase.WritingSystem # 0x6
        Syriac                   : QFontDatabase.WritingSystem # 0x7
        Thaana                   : QFontDatabase.WritingSystem # 0x8
        Devanagari               : QFontDatabase.WritingSystem # 0x9
        Bengali                  : QFontDatabase.WritingSystem # 0xa
        Gurmukhi                 : QFontDatabase.WritingSystem # 0xb
        Gujarati                 : QFontDatabase.WritingSystem # 0xc
        Oriya                    : QFontDatabase.WritingSystem # 0xd
        Tamil                    : QFontDatabase.WritingSystem # 0xe
        Telugu                   : QFontDatabase.WritingSystem # 0xf
        Kannada                  : QFontDatabase.WritingSystem # 0x10
        Malayalam                : QFontDatabase.WritingSystem # 0x11
        Sinhala                  : QFontDatabase.WritingSystem # 0x12
        Thai                     : QFontDatabase.WritingSystem # 0x13
        Lao                      : QFontDatabase.WritingSystem # 0x14
        Tibetan                  : QFontDatabase.WritingSystem # 0x15
        Myanmar                  : QFontDatabase.WritingSystem # 0x16
        Georgian                 : QFontDatabase.WritingSystem # 0x17
        Khmer                    : QFontDatabase.WritingSystem # 0x18
        SimplifiedChinese        : QFontDatabase.WritingSystem # 0x19
        TraditionalChinese       : QFontDatabase.WritingSystem # 0x1a
        Japanese                 : QFontDatabase.WritingSystem # 0x1b
        Korean                   : QFontDatabase.WritingSystem # 0x1c
        Vietnamese               : QFontDatabase.WritingSystem # 0x1d
        Other                    : QFontDatabase.WritingSystem # 0x1e
        Symbol                   : QFontDatabase.WritingSystem # 0x1e
        Ogham                    : QFontDatabase.WritingSystem # 0x1f
        Runic                    : QFontDatabase.WritingSystem # 0x20
        Nko                      : QFontDatabase.WritingSystem # 0x21
        WritingSystemsCount      : QFontDatabase.WritingSystem # 0x22


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QFontDatabase:QtGui.QFontDatabase) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    @staticmethod
    def addApplicationFont(fileName:str) -> int: ...
    @staticmethod
    def addApplicationFontFromData(fontData:Union[QtCore.QByteArray, bytes]) -> int: ...
    @staticmethod
    def applicationFontFamilies(id:int) -> List[str]: ...
    @staticmethod
    def bold(family:str, style:str) -> bool: ...
    @staticmethod
    def families(writingSystem:QtGui.QFontDatabase.WritingSystem=...) -> List[str]: ...
    @staticmethod
    def font(family:str, style:str, pointSize:int) -> QtGui.QFont: ...
    @staticmethod
    def hasFamily(family:str) -> bool: ...
    @staticmethod
    def isBitmapScalable(family:str, style:str=...) -> bool: ...
    @staticmethod
    def isFixedPitch(family:str, style:str=...) -> bool: ...
    @staticmethod
    def isPrivateFamily(family:str) -> bool: ...
    @staticmethod
    def isScalable(family:str, style:str=...) -> bool: ...
    @staticmethod
    def isSmoothlyScalable(family:str, style:str=...) -> bool: ...
    @staticmethod
    def italic(family:str, style:str) -> bool: ...
    @staticmethod
    def pointSizes(family:str, style:str=...) -> List[int]: ...
    @staticmethod
    def removeAllApplicationFonts() -> bool: ...
    @staticmethod
    def removeApplicationFont(id:int) -> bool: ...
    @staticmethod
    def smoothSizes(family:str, style:str) -> List[int]: ...
    @staticmethod
    def standardSizes() -> List[int]: ...
    @overload
    @staticmethod
    def styleString(font:Union[QtGui.QFont, str, Sequence[str]]) -> str: ...
    @overload
    @staticmethod
    def styleString(fontInfo:Union[QtGui.QFontInfo, QtGui.QFont]) -> str: ...
    @staticmethod
    def styles(family:str) -> List[str]: ...
    @staticmethod
    def systemFont(type:QtGui.QFontDatabase.SystemFont) -> QtGui.QFont: ...
    @staticmethod
    def weight(family:str, style:str) -> int: ...
    @staticmethod
    def writingSystemName(writingSystem:QtGui.QFontDatabase.WritingSystem) -> str: ...
    @staticmethod
    def writingSystemSample(writingSystem:QtGui.QFontDatabase.WritingSystem) -> str: ...
    @overload
    @staticmethod
    def writingSystems() -> List[QtGui.QFontDatabase.WritingSystem]: ...
    @overload
    @staticmethod
    def writingSystems(family:str) -> List[QtGui.QFontDatabase.WritingSystem]: ...


class QFontInfo(Shiboken.Object):

    @overload
    def __init__(self, arg__1:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    @overload
    def __init__(self, arg__1:Union[QtGui.QFontInfo, QtGui.QFont]) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def bold(self) -> bool: ...
    def exactMatch(self) -> bool: ...
    def family(self) -> str: ...
    def fixedPitch(self) -> bool: ...
    def italic(self) -> bool: ...
    def legacyWeight(self) -> int: ...
    def overline(self) -> bool: ...
    def pixelSize(self) -> int: ...
    def pointSize(self) -> int: ...
    def pointSizeF(self) -> float: ...
    def strikeOut(self) -> bool: ...
    def style(self) -> QtGui.QFont.Style: ...
    def styleHint(self) -> QtGui.QFont.StyleHint: ...
    def styleName(self) -> str: ...
    def swap(self, other:Union[QtGui.QFontInfo, QtGui.QFont]) -> None: ...
    def underline(self) -> bool: ...
    def weight(self) -> int: ...


class QFontMetrics(Shiboken.Object):

    @overload
    def __init__(self, arg__1:QtGui.QFontMetrics) -> None: ...
    @overload
    def __init__(self, arg__1:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    @overload
    def __init__(self, font:Union[QtGui.QFont, str, Sequence[str]], pd:QtGui.QPaintDevice) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def ascent(self) -> int: ...
    def averageCharWidth(self) -> int: ...
    @overload
    def boundingRect(self, r:QtCore.QRect, flags:int, text:str, tabstops:int, tabarray:object=...) -> QtCore.QRect: ...
    @overload
    def boundingRect(self, text:str) -> QtCore.QRect: ...
    @overload
    def boundingRect(self, x:int, y:int, w:int, h:int, flags:int, text:str, tabstops:int, tabarray:object=...) -> QtCore.QRect: ...
    def boundingRectChar(self, arg__1:int) -> QtCore.QRect: ...
    def capHeight(self) -> int: ...
    def descent(self) -> int: ...
    def elidedText(self, text:str, mode:QtCore.Qt.TextElideMode, width:int, flags:int=...) -> str: ...
    def fontDpi(self) -> float: ...
    def height(self) -> int: ...
    def horizontalAdvance(self, arg__1:str, len:int=...) -> int: ...
    def horizontalAdvanceChar(self, arg__1:int) -> int: ...
    def inFont(self, arg__1:str) -> bool: ...
    def inFontUcs4(self, ucs4:int) -> bool: ...
    def leading(self) -> int: ...
    def leftBearing(self, arg__1:str) -> int: ...
    def lineSpacing(self) -> int: ...
    def lineWidth(self) -> int: ...
    def maxWidth(self) -> int: ...
    def minLeftBearing(self) -> int: ...
    def minRightBearing(self) -> int: ...
    def overlinePos(self) -> int: ...
    def rightBearing(self, arg__1:str) -> int: ...
    def size(self, flags:int, str:str, tabstops:int, tabarray:object=...) -> QtCore.QSize: ...
    def strikeOutPos(self) -> int: ...
    def swap(self, other:QtGui.QFontMetrics) -> None: ...
    def tightBoundingRect(self, text:str) -> QtCore.QRect: ...
    def underlinePos(self) -> int: ...
    def xHeight(self) -> int: ...


class QFontMetricsF(Shiboken.Object):

    @overload
    def __init__(self, arg__1:QtGui.QFontMetrics) -> None: ...
    @overload
    def __init__(self, arg__1:Union[QtGui.QFontMetricsF, QtGui.QFontMetrics]) -> None: ...
    @overload
    def __init__(self, font:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    @overload
    def __init__(self, font:Union[QtGui.QFont, str, Sequence[str]], pd:QtGui.QPaintDevice) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def ascent(self) -> float: ...
    def averageCharWidth(self) -> float: ...
    @overload
    def boundingRect(self, r:Union[QtCore.QRectF, QtCore.QRect], flags:int, string:str, tabstops:int, tabarray:object=...) -> QtCore.QRectF: ...
    @overload
    def boundingRect(self, string:str) -> QtCore.QRectF: ...
    def boundingRectChar(self, arg__1:int) -> QtCore.QRectF: ...
    def capHeight(self) -> float: ...
    def descent(self) -> float: ...
    def elidedText(self, text:str, mode:QtCore.Qt.TextElideMode, width:float, flags:int=...) -> str: ...
    def fontDpi(self) -> float: ...
    def height(self) -> float: ...
    def horizontalAdvance(self, string:str, length:int=...) -> float: ...
    def horizontalAdvanceChar(self, arg__1:int) -> float: ...
    def inFont(self, arg__1:str) -> bool: ...
    def inFontUcs4(self, ucs4:int) -> bool: ...
    def leading(self) -> float: ...
    def leftBearing(self, arg__1:str) -> float: ...
    def lineSpacing(self) -> float: ...
    def lineWidth(self) -> float: ...
    def maxWidth(self) -> float: ...
    def minLeftBearing(self) -> float: ...
    def minRightBearing(self) -> float: ...
    def overlinePos(self) -> float: ...
    def rightBearing(self, arg__1:str) -> float: ...
    def size(self, flags:int, str:str, tabstops:int, tabarray:object=...) -> QtCore.QSizeF: ...
    def strikeOutPos(self) -> float: ...
    def swap(self, other:Union[QtGui.QFontMetricsF, QtGui.QFontMetrics]) -> None: ...
    def tightBoundingRect(self, text:str) -> QtCore.QRectF: ...
    def underlinePos(self) -> float: ...
    def xHeight(self) -> float: ...


class QGlyphRun(Shiboken.Object):

    Overline                 : QGlyphRun.GlyphRunFlag = ... # 0x1
    Underline                : QGlyphRun.GlyphRunFlag = ... # 0x2
    StrikeOut                : QGlyphRun.GlyphRunFlag = ... # 0x4
    RightToLeft              : QGlyphRun.GlyphRunFlag = ... # 0x8
    SplitLigature            : QGlyphRun.GlyphRunFlag = ... # 0x10

    class GlyphRunFlag(Enum):

        Overline                 : QGlyphRun.GlyphRunFlag # 0x1
        Underline                : QGlyphRun.GlyphRunFlag # 0x2
        StrikeOut                : QGlyphRun.GlyphRunFlag # 0x4
        RightToLeft              : QGlyphRun.GlyphRunFlag # 0x8
        SplitLigature            : QGlyphRun.GlyphRunFlag # 0x10

    class GlyphRunFlags(object): ...


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, other:QtGui.QGlyphRun) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def boundingRect(self) -> QtCore.QRectF: ...
    def clear(self) -> None: ...
    def flags(self) -> QtGui.QGlyphRun.GlyphRunFlags: ...
    def glyphIndexes(self) -> List[int]: ...
    def isEmpty(self) -> bool: ...
    def isRightToLeft(self) -> bool: ...
    def overline(self) -> bool: ...
    def positions(self) -> List[QtCore.QPointF]: ...
    def rawFont(self) -> QtGui.QRawFont: ...
    def setBoundingRect(self, boundingRect:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    def setFlag(self, flag:QtGui.QGlyphRun.GlyphRunFlag, enabled:bool=...) -> None: ...
    def setFlags(self, flags:QtGui.QGlyphRun.GlyphRunFlags) -> None: ...
    def setGlyphIndexes(self, glyphIndexes:Sequence[int]) -> None: ...
    def setOverline(self, overline:bool) -> None: ...
    def setPositions(self, positions:Sequence[QtCore.QPointF]) -> None: ...
    def setRawData(self, glyphIndexArray:int, glyphPositionArray:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], size:int) -> None: ...
    def setRawFont(self, rawFont:QtGui.QRawFont) -> None: ...
    def setRightToLeft(self, on:bool) -> None: ...
    def setStrikeOut(self, strikeOut:bool) -> None: ...
    def setUnderline(self, underline:bool) -> None: ...
    def strikeOut(self) -> bool: ...
    def swap(self, other:QtGui.QGlyphRun) -> None: ...
    def underline(self) -> bool: ...


class QGradient(Shiboken.Object):

    LogicalMode              : QGradient.CoordinateMode = ... # 0x0
    StretchToDeviceMode      : QGradient.CoordinateMode = ... # 0x1
    ObjectBoundingMode       : QGradient.CoordinateMode = ... # 0x2
    ObjectMode               : QGradient.CoordinateMode = ... # 0x3
    ColorInterpolation       : QGradient.InterpolationMode = ... # 0x0
    ComponentInterpolation   : QGradient.InterpolationMode = ... # 0x1
    WarmFlame                : Preset = ... # 0x1
    NightFade                : QGradient.Preset = ... # 0x2
    SpringWarmth             : QGradient.Preset = ... # 0x3
    JuicyPeach               : QGradient.Preset = ... # 0x4
    YoungPassion             : QGradient.Preset = ... # 0x5
    LadyLips                 : QGradient.Preset = ... # 0x6
    SunnyMorning             : QGradient.Preset = ... # 0x7
    RainyAshville            : QGradient.Preset = ... # 0x8
    FrozenDreams             : QGradient.Preset = ... # 0x9
    WinterNeva               : QGradient.Preset = ... # 0xa
    DustyGrass               : QGradient.Preset = ... # 0xb
    TemptingAzure            : QGradient.Preset = ... # 0xc
    HeavyRain                : QGradient.Preset = ... # 0xd
    AmyCrisp                 : QGradient.Preset = ... # 0xe
    MeanFruit                : QGradient.Preset = ... # 0xf
    DeepBlue                 : QGradient.Preset = ... # 0x10
    RipeMalinka              : QGradient.Preset = ... # 0x11
    CloudyKnoxville          : QGradient.Preset = ... # 0x12
    MalibuBeach              : QGradient.Preset = ... # 0x13
    NewLife                  : QGradient.Preset = ... # 0x14
    TrueSunset               : QGradient.Preset = ... # 0x15
    MorpheusDen              : QGradient.Preset = ... # 0x16
    RareWind                 : QGradient.Preset = ... # 0x17
    NearMoon                 : QGradient.Preset = ... # 0x18
    WildApple                : QGradient.Preset = ... # 0x19
    SaintPetersburg          : QGradient.Preset = ... # 0x1a
    PlumPlate                : QGradient.Preset = ... # 0x1c
    EverlastingSky           : QGradient.Preset = ... # 0x1d
    HappyFisher              : QGradient.Preset = ... # 0x1e
    Blessing                 : QGradient.Preset = ... # 0x1f
    SharpeyeEagle            : QGradient.Preset = ... # 0x20
    LadogaBottom             : QGradient.Preset = ... # 0x21
    LemonGate                : QGradient.Preset = ... # 0x22
    ItmeoBranding            : QGradient.Preset = ... # 0x23
    ZeusMiracle              : QGradient.Preset = ... # 0x24
    OldHat                   : QGradient.Preset = ... # 0x25
    StarWine                 : QGradient.Preset = ... # 0x26
    HappyAcid                : QGradient.Preset = ... # 0x29
    AwesomePine              : QGradient.Preset = ... # 0x2a
    NewYork                  : QGradient.Preset = ... # 0x2b
    ShyRainbow               : QGradient.Preset = ... # 0x2c
    MixedHopes               : QGradient.Preset = ... # 0x2e
    FlyHigh                  : QGradient.Preset = ... # 0x2f
    StrongBliss              : QGradient.Preset = ... # 0x30
    FreshMilk                : QGradient.Preset = ... # 0x31
    SnowAgain                : QGradient.Preset = ... # 0x32
    FebruaryInk              : QGradient.Preset = ... # 0x33
    KindSteel                : QGradient.Preset = ... # 0x34
    SoftGrass                : QGradient.Preset = ... # 0x35
    GrownEarly               : QGradient.Preset = ... # 0x36
    SharpBlues               : QGradient.Preset = ... # 0x37
    ShadyWater               : QGradient.Preset = ... # 0x38
    DirtyBeauty              : QGradient.Preset = ... # 0x39
    GreatWhale               : QGradient.Preset = ... # 0x3a
    TeenNotebook             : QGradient.Preset = ... # 0x3b
    PoliteRumors             : QGradient.Preset = ... # 0x3c
    SweetPeriod              : QGradient.Preset = ... # 0x3d
    WideMatrix               : QGradient.Preset = ... # 0x3e
    SoftCherish              : QGradient.Preset = ... # 0x3f
    RedSalvation             : QGradient.Preset = ... # 0x40
    BurningSpring            : QGradient.Preset = ... # 0x41
    NightParty               : QGradient.Preset = ... # 0x42
    SkyGlider                : QGradient.Preset = ... # 0x43
    HeavenPeach              : QGradient.Preset = ... # 0x44
    PurpleDivision           : QGradient.Preset = ... # 0x45
    AquaSplash               : QGradient.Preset = ... # 0x46
    SpikyNaga                : QGradient.Preset = ... # 0x48
    LoveKiss                 : QGradient.Preset = ... # 0x49
    CleanMirror              : QGradient.Preset = ... # 0x4b
    PremiumDark              : QGradient.Preset = ... # 0x4c
    ColdEvening              : QGradient.Preset = ... # 0x4d
    CochitiLake              : QGradient.Preset = ... # 0x4e
    SummerGames              : QGradient.Preset = ... # 0x4f
    PassionateBed            : QGradient.Preset = ... # 0x50
    MountainRock             : QGradient.Preset = ... # 0x51
    DesertHump               : QGradient.Preset = ... # 0x52
    JungleDay                : QGradient.Preset = ... # 0x53
    PhoenixStart             : QGradient.Preset = ... # 0x54
    OctoberSilence           : QGradient.Preset = ... # 0x55
    FarawayRiver             : QGradient.Preset = ... # 0x56
    AlchemistLab             : QGradient.Preset = ... # 0x57
    OverSun                  : QGradient.Preset = ... # 0x58
    PremiumWhite             : QGradient.Preset = ... # 0x59
    MarsParty                : QGradient.Preset = ... # 0x5a
    EternalConstance         : QGradient.Preset = ... # 0x5b
    JapanBlush               : QGradient.Preset = ... # 0x5c
    SmilingRain              : QGradient.Preset = ... # 0x5d
    CloudyApple              : QGradient.Preset = ... # 0x5e
    BigMango                 : QGradient.Preset = ... # 0x5f
    HealthyWater             : QGradient.Preset = ... # 0x60
    AmourAmour               : QGradient.Preset = ... # 0x61
    RiskyConcrete            : QGradient.Preset = ... # 0x62
    StrongStick              : QGradient.Preset = ... # 0x63
    ViciousStance            : QGradient.Preset = ... # 0x64
    PaloAlto                 : QGradient.Preset = ... # 0x65
    HappyMemories            : QGradient.Preset = ... # 0x66
    MidnightBloom            : QGradient.Preset = ... # 0x67
    Crystalline              : QGradient.Preset = ... # 0x68
    PartyBliss               : QGradient.Preset = ... # 0x6a
    ConfidentCloud           : QGradient.Preset = ... # 0x6b
    LeCocktail               : QGradient.Preset = ... # 0x6c
    RiverCity                : QGradient.Preset = ... # 0x6d
    FrozenBerry              : QGradient.Preset = ... # 0x6e
    ChildCare                : QGradient.Preset = ... # 0x70
    FlyingLemon              : QGradient.Preset = ... # 0x71
    NewRetrowave             : QGradient.Preset = ... # 0x72
    HiddenJaguar             : QGradient.Preset = ... # 0x73
    AboveTheSky              : QGradient.Preset = ... # 0x74
    Nega                     : QGradient.Preset = ... # 0x75
    DenseWater               : QGradient.Preset = ... # 0x76
    Seashore                 : QGradient.Preset = ... # 0x78
    MarbleWall               : QGradient.Preset = ... # 0x79
    CheerfulCaramel          : QGradient.Preset = ... # 0x7a
    NightSky                 : QGradient.Preset = ... # 0x7b
    MagicLake                : QGradient.Preset = ... # 0x7c
    YoungGrass               : QGradient.Preset = ... # 0x7d
    ColorfulPeach            : QGradient.Preset = ... # 0x7e
    GentleCare               : QGradient.Preset = ... # 0x7f
    PlumBath                 : QGradient.Preset = ... # 0x80
    HappyUnicorn             : QGradient.Preset = ... # 0x81
    AfricanField             : QGradient.Preset = ... # 0x83
    SolidStone               : QGradient.Preset = ... # 0x84
    OrangeJuice              : QGradient.Preset = ... # 0x85
    GlassWater               : QGradient.Preset = ... # 0x86
    NorthMiracle             : QGradient.Preset = ... # 0x88
    FruitBlend               : QGradient.Preset = ... # 0x89
    MillenniumPine           : QGradient.Preset = ... # 0x8a
    HighFlight               : QGradient.Preset = ... # 0x8b
    MoleHall                 : QGradient.Preset = ... # 0x8c
    SpaceShift               : QGradient.Preset = ... # 0x8e
    ForestInei               : QGradient.Preset = ... # 0x8f
    RoyalGarden              : QGradient.Preset = ... # 0x90
    RichMetal                : QGradient.Preset = ... # 0x91
    JuicyCake                : QGradient.Preset = ... # 0x92
    SmartIndigo              : QGradient.Preset = ... # 0x93
    SandStrike               : QGradient.Preset = ... # 0x94
    NorseBeauty              : QGradient.Preset = ... # 0x95
    AquaGuidance             : QGradient.Preset = ... # 0x96
    SunVeggie                : QGradient.Preset = ... # 0x97
    SeaLord                  : QGradient.Preset = ... # 0x98
    BlackSea                 : QGradient.Preset = ... # 0x99
    GrassShampoo             : QGradient.Preset = ... # 0x9a
    LandingAircraft          : QGradient.Preset = ... # 0x9b
    WitchDance               : QGradient.Preset = ... # 0x9c
    SleeplessNight           : QGradient.Preset = ... # 0x9d
    AngelCare                : QGradient.Preset = ... # 0x9e
    CrystalRiver             : QGradient.Preset = ... # 0x9f
    SoftLipstick             : QGradient.Preset = ... # 0xa0
    SaltMountain             : QGradient.Preset = ... # 0xa1
    PerfectWhite             : QGradient.Preset = ... # 0xa2
    FreshOasis               : QGradient.Preset = ... # 0xa3
    StrictNovember           : QGradient.Preset = ... # 0xa4
    MorningSalad             : QGradient.Preset = ... # 0xa5
    DeepRelief               : QGradient.Preset = ... # 0xa6
    SeaStrike                : QGradient.Preset = ... # 0xa7
    NightCall                : QGradient.Preset = ... # 0xa8
    SupremeSky               : QGradient.Preset = ... # 0xa9
    LightBlue                : QGradient.Preset = ... # 0xaa
    MindCrawl                : QGradient.Preset = ... # 0xab
    LilyMeadow               : QGradient.Preset = ... # 0xac
    SugarLollipop            : QGradient.Preset = ... # 0xad
    SweetDessert             : QGradient.Preset = ... # 0xae
    MagicRay                 : QGradient.Preset = ... # 0xaf
    TeenParty                : QGradient.Preset = ... # 0xb0
    FrozenHeat               : QGradient.Preset = ... # 0xb1
    GagarinView              : QGradient.Preset = ... # 0xb2
    FabledSunset             : QGradient.Preset = ... # 0xb3
    PerfectBlue              : QGradient.Preset = ... # 0xb4
    NumPresets               : QGradient.Preset = ... # 0xb5
    PadSpread                : QGradient.Spread = ... # 0x0
    ReflectSpread            : QGradient.Spread = ... # 0x1
    RepeatSpread             : QGradient.Spread = ... # 0x2
    LinearGradient           : QGradient.Type = ... # 0x0
    RadialGradient           : QGradient.Type = ... # 0x1
    ConicalGradient          : QGradient.Type = ... # 0x2
    NoGradient               : QGradient.Type = ... # 0x3

    class CoordinateMode(Enum):

        LogicalMode              : QGradient.CoordinateMode # 0x0
        StretchToDeviceMode      : QGradient.CoordinateMode # 0x1
        ObjectBoundingMode       : QGradient.CoordinateMode # 0x2
        ObjectMode               : QGradient.CoordinateMode # 0x3

    class InterpolationMode(Enum):

        ColorInterpolation       : QGradient.InterpolationMode # 0x0
        ComponentInterpolation   : QGradient.InterpolationMode # 0x1

    class Preset(Enum):

        WarmFlame                : QGradient.Preset # 0x1
        NightFade                : QGradient.Preset # 0x2
        SpringWarmth             : QGradient.Preset # 0x3
        JuicyPeach               : QGradient.Preset # 0x4
        YoungPassion             : QGradient.Preset # 0x5
        LadyLips                 : QGradient.Preset # 0x6
        SunnyMorning             : QGradient.Preset # 0x7
        RainyAshville            : QGradient.Preset # 0x8
        FrozenDreams             : QGradient.Preset # 0x9
        WinterNeva               : QGradient.Preset # 0xa
        DustyGrass               : QGradient.Preset # 0xb
        TemptingAzure            : QGradient.Preset # 0xc
        HeavyRain                : QGradient.Preset # 0xd
        AmyCrisp                 : QGradient.Preset # 0xe
        MeanFruit                : QGradient.Preset # 0xf
        DeepBlue                 : QGradient.Preset # 0x10
        RipeMalinka              : QGradient.Preset # 0x11
        CloudyKnoxville          : QGradient.Preset # 0x12
        MalibuBeach              : QGradient.Preset # 0x13
        NewLife                  : QGradient.Preset # 0x14
        TrueSunset               : QGradient.Preset # 0x15
        MorpheusDen              : QGradient.Preset # 0x16
        RareWind                 : QGradient.Preset # 0x17
        NearMoon                 : QGradient.Preset # 0x18
        WildApple                : QGradient.Preset # 0x19
        SaintPetersburg          : QGradient.Preset # 0x1a
        PlumPlate                : QGradient.Preset # 0x1c
        EverlastingSky           : QGradient.Preset # 0x1d
        HappyFisher              : QGradient.Preset # 0x1e
        Blessing                 : QGradient.Preset # 0x1f
        SharpeyeEagle            : QGradient.Preset # 0x20
        LadogaBottom             : QGradient.Preset # 0x21
        LemonGate                : QGradient.Preset # 0x22
        ItmeoBranding            : QGradient.Preset # 0x23
        ZeusMiracle              : QGradient.Preset # 0x24
        OldHat                   : QGradient.Preset # 0x25
        StarWine                 : QGradient.Preset # 0x26
        HappyAcid                : QGradient.Preset # 0x29
        AwesomePine              : QGradient.Preset # 0x2a
        NewYork                  : QGradient.Preset # 0x2b
        ShyRainbow               : QGradient.Preset # 0x2c
        MixedHopes               : QGradient.Preset # 0x2e
        FlyHigh                  : QGradient.Preset # 0x2f
        StrongBliss              : QGradient.Preset # 0x30
        FreshMilk                : QGradient.Preset # 0x31
        SnowAgain                : QGradient.Preset # 0x32
        FebruaryInk              : QGradient.Preset # 0x33
        KindSteel                : QGradient.Preset # 0x34
        SoftGrass                : QGradient.Preset # 0x35
        GrownEarly               : QGradient.Preset # 0x36
        SharpBlues               : QGradient.Preset # 0x37
        ShadyWater               : QGradient.Preset # 0x38
        DirtyBeauty              : QGradient.Preset # 0x39
        GreatWhale               : QGradient.Preset # 0x3a
        TeenNotebook             : QGradient.Preset # 0x3b
        PoliteRumors             : QGradient.Preset # 0x3c
        SweetPeriod              : QGradient.Preset # 0x3d
        WideMatrix               : QGradient.Preset # 0x3e
        SoftCherish              : QGradient.Preset # 0x3f
        RedSalvation             : QGradient.Preset # 0x40
        BurningSpring            : QGradient.Preset # 0x41
        NightParty               : QGradient.Preset # 0x42
        SkyGlider                : QGradient.Preset # 0x43
        HeavenPeach              : QGradient.Preset # 0x44
        PurpleDivision           : QGradient.Preset # 0x45
        AquaSplash               : QGradient.Preset # 0x46
        SpikyNaga                : QGradient.Preset # 0x48
        LoveKiss                 : QGradient.Preset # 0x49
        CleanMirror              : QGradient.Preset # 0x4b
        PremiumDark              : QGradient.Preset # 0x4c
        ColdEvening              : QGradient.Preset # 0x4d
        CochitiLake              : QGradient.Preset # 0x4e
        SummerGames              : QGradient.Preset # 0x4f
        PassionateBed            : QGradient.Preset # 0x50
        MountainRock             : QGradient.Preset # 0x51
        DesertHump               : QGradient.Preset # 0x52
        JungleDay                : QGradient.Preset # 0x53
        PhoenixStart             : QGradient.Preset # 0x54
        OctoberSilence           : QGradient.Preset # 0x55
        FarawayRiver             : QGradient.Preset # 0x56
        AlchemistLab             : QGradient.Preset # 0x57
        OverSun                  : QGradient.Preset # 0x58
        PremiumWhite             : QGradient.Preset # 0x59
        MarsParty                : QGradient.Preset # 0x5a
        EternalConstance         : QGradient.Preset # 0x5b
        JapanBlush               : QGradient.Preset # 0x5c
        SmilingRain              : QGradient.Preset # 0x5d
        CloudyApple              : QGradient.Preset # 0x5e
        BigMango                 : QGradient.Preset # 0x5f
        HealthyWater             : QGradient.Preset # 0x60
        AmourAmour               : QGradient.Preset # 0x61
        RiskyConcrete            : QGradient.Preset # 0x62
        StrongStick              : QGradient.Preset # 0x63
        ViciousStance            : QGradient.Preset # 0x64
        PaloAlto                 : QGradient.Preset # 0x65
        HappyMemories            : QGradient.Preset # 0x66
        MidnightBloom            : QGradient.Preset # 0x67
        Crystalline              : QGradient.Preset # 0x68
        PartyBliss               : QGradient.Preset # 0x6a
        ConfidentCloud           : QGradient.Preset # 0x6b
        LeCocktail               : QGradient.Preset # 0x6c
        RiverCity                : QGradient.Preset # 0x6d
        FrozenBerry              : QGradient.Preset # 0x6e
        ChildCare                : QGradient.Preset # 0x70
        FlyingLemon              : QGradient.Preset # 0x71
        NewRetrowave             : QGradient.Preset # 0x72
        HiddenJaguar             : QGradient.Preset # 0x73
        AboveTheSky              : QGradient.Preset # 0x74
        Nega                     : QGradient.Preset # 0x75
        DenseWater               : QGradient.Preset # 0x76
        Seashore                 : QGradient.Preset # 0x78
        MarbleWall               : QGradient.Preset # 0x79
        CheerfulCaramel          : QGradient.Preset # 0x7a
        NightSky                 : QGradient.Preset # 0x7b
        MagicLake                : QGradient.Preset # 0x7c
        YoungGrass               : QGradient.Preset # 0x7d
        ColorfulPeach            : QGradient.Preset # 0x7e
        GentleCare               : QGradient.Preset # 0x7f
        PlumBath                 : QGradient.Preset # 0x80
        HappyUnicorn             : QGradient.Preset # 0x81
        AfricanField             : QGradient.Preset # 0x83
        SolidStone               : QGradient.Preset # 0x84
        OrangeJuice              : QGradient.Preset # 0x85
        GlassWater               : QGradient.Preset # 0x86
        NorthMiracle             : QGradient.Preset # 0x88
        FruitBlend               : QGradient.Preset # 0x89
        MillenniumPine           : QGradient.Preset # 0x8a
        HighFlight               : QGradient.Preset # 0x8b
        MoleHall                 : QGradient.Preset # 0x8c
        SpaceShift               : QGradient.Preset # 0x8e
        ForestInei               : QGradient.Preset # 0x8f
        RoyalGarden              : QGradient.Preset # 0x90
        RichMetal                : QGradient.Preset # 0x91
        JuicyCake                : QGradient.Preset # 0x92
        SmartIndigo              : QGradient.Preset # 0x93
        SandStrike               : QGradient.Preset # 0x94
        NorseBeauty              : QGradient.Preset # 0x95
        AquaGuidance             : QGradient.Preset # 0x96
        SunVeggie                : QGradient.Preset # 0x97
        SeaLord                  : QGradient.Preset # 0x98
        BlackSea                 : QGradient.Preset # 0x99
        GrassShampoo             : QGradient.Preset # 0x9a
        LandingAircraft          : QGradient.Preset # 0x9b
        WitchDance               : QGradient.Preset # 0x9c
        SleeplessNight           : QGradient.Preset # 0x9d
        AngelCare                : QGradient.Preset # 0x9e
        CrystalRiver             : QGradient.Preset # 0x9f
        SoftLipstick             : QGradient.Preset # 0xa0
        SaltMountain             : QGradient.Preset # 0xa1
        PerfectWhite             : QGradient.Preset # 0xa2
        FreshOasis               : QGradient.Preset # 0xa3
        StrictNovember           : QGradient.Preset # 0xa4
        MorningSalad             : QGradient.Preset # 0xa5
        DeepRelief               : QGradient.Preset # 0xa6
        SeaStrike                : QGradient.Preset # 0xa7
        NightCall                : QGradient.Preset # 0xa8
        SupremeSky               : QGradient.Preset # 0xa9
        LightBlue                : QGradient.Preset # 0xaa
        MindCrawl                : QGradient.Preset # 0xab
        LilyMeadow               : QGradient.Preset # 0xac
        SugarLollipop            : QGradient.Preset # 0xad
        SweetDessert             : QGradient.Preset # 0xae
        MagicRay                 : QGradient.Preset # 0xaf
        TeenParty                : QGradient.Preset # 0xb0
        FrozenHeat               : QGradient.Preset # 0xb1
        GagarinView              : QGradient.Preset # 0xb2
        FabledSunset             : QGradient.Preset # 0xb3
        PerfectBlue              : QGradient.Preset # 0xb4
        NumPresets               : QGradient.Preset # 0xb5

    class Spread(Enum):

        PadSpread                : QGradient.Spread # 0x0
        ReflectSpread            : QGradient.Spread # 0x1
        RepeatSpread             : QGradient.Spread # 0x2

    class Type(Enum):

        LinearGradient           : QGradient.Type # 0x0
        RadialGradient           : QGradient.Type # 0x1
        ConicalGradient          : QGradient.Type # 0x2
        NoGradient               : QGradient.Type # 0x3


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QGradient:Union[QtGui.QGradient, QtGui.QGradient.Preset]) -> None: ...
    @overload
    def __init__(self, arg__1:QtGui.QGradient.Preset) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def coordinateMode(self) -> QtGui.QGradient.CoordinateMode: ...
    def interpolationMode(self) -> QtGui.QGradient.InterpolationMode: ...
    def setColorAt(self, pos:float, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    def setCoordinateMode(self, mode:QtGui.QGradient.CoordinateMode) -> None: ...
    def setInterpolationMode(self, mode:QtGui.QGradient.InterpolationMode) -> None: ...
    def setSpread(self, spread:QtGui.QGradient.Spread) -> None: ...
    def setStops(self, stops:Sequence[Tuple[float, QtGui.QColor]]) -> None: ...
    def spread(self) -> QtGui.QGradient.Spread: ...
    def stops(self) -> List[Tuple[float, QtGui.QColor]]: ...
    def type(self) -> QtGui.QGradient.Type: ...


class QGuiApplication(QtCore.QCoreApplication):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:Sequence[str]) -> None: ...

    @staticmethod
    def allWindows() -> List[QtGui.QWindow]: ...
    @staticmethod
    def applicationDisplayName() -> str: ...
    @staticmethod
    def applicationState() -> QtCore.Qt.ApplicationState: ...
    @staticmethod
    def changeOverrideCursor(arg__1:Union[QtGui.QCursor, QtCore.Qt.CursorShape, QtGui.QPixmap]) -> None: ...
    @staticmethod
    def clipboard() -> QtGui.QClipboard: ...
    @staticmethod
    def desktopFileName() -> str: ...
    @staticmethod
    def desktopSettingsAware() -> bool: ...
    def devicePixelRatio(self) -> float: ...
    def event(self, arg__1:QtCore.QEvent) -> bool: ...
    @staticmethod
    def exec() -> int: ...
    def exec_(self) -> int: ...
    @staticmethod
    def focusObject() -> QtCore.QObject: ...
    @staticmethod
    def focusWindow() -> QtGui.QWindow: ...
    @staticmethod
    def font() -> QtGui.QFont: ...
    @staticmethod
    def highDpiScaleFactorRoundingPolicy() -> QtCore.Qt.HighDpiScaleFactorRoundingPolicy: ...
    @staticmethod
    def inputMethod() -> QtGui.QInputMethod: ...
    @staticmethod
    def isLeftToRight() -> bool: ...
    @staticmethod
    def isRightToLeft() -> bool: ...
    def isSavingSession(self) -> bool: ...
    def isSessionRestored(self) -> bool: ...
    @staticmethod
    def keyboardModifiers() -> QtCore.Qt.KeyboardModifiers: ...
    @staticmethod
    def layoutDirection() -> QtCore.Qt.LayoutDirection: ...
    @staticmethod
    def modalWindow() -> QtGui.QWindow: ...
    @staticmethod
    def mouseButtons() -> QtCore.Qt.MouseButtons: ...
    def notify(self, arg__1:QtCore.QObject, arg__2:QtCore.QEvent) -> bool: ...
    @staticmethod
    def overrideCursor() -> QtGui.QCursor: ...
    @staticmethod
    def palette() -> QtGui.QPalette: ...
    @staticmethod
    def platformName() -> str: ...
    @staticmethod
    def primaryScreen() -> QtGui.QScreen: ...
    @staticmethod
    def queryKeyboardModifiers() -> QtCore.Qt.KeyboardModifiers: ...
    @staticmethod
    def quitOnLastWindowClosed() -> bool: ...
    def resolveInterface(self, name:bytes, revision:int) -> int: ...
    @staticmethod
    def restoreOverrideCursor() -> None: ...
    @staticmethod
    def screenAt(point:QtCore.QPoint) -> QtGui.QScreen: ...
    @staticmethod
    def screens() -> List[QtGui.QScreen]: ...
    def sessionId(self) -> str: ...
    def sessionKey(self) -> str: ...
    @staticmethod
    def setApplicationDisplayName(name:str) -> None: ...
    @staticmethod
    def setDesktopFileName(name:str) -> None: ...
    @staticmethod
    def setDesktopSettingsAware(on:bool) -> None: ...
    @staticmethod
    def setFont(arg__1:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    @staticmethod
    def setHighDpiScaleFactorRoundingPolicy(policy:QtCore.Qt.HighDpiScaleFactorRoundingPolicy) -> None: ...
    @staticmethod
    def setLayoutDirection(direction:QtCore.Qt.LayoutDirection) -> None: ...
    @staticmethod
    def setOverrideCursor(arg__1:Union[QtGui.QCursor, QtCore.Qt.CursorShape, QtGui.QPixmap]) -> None: ...
    @staticmethod
    def setPalette(pal:Union[QtGui.QPalette, QtCore.Qt.GlobalColor, QtGui.QColor]) -> None: ...
    @staticmethod
    def setQuitOnLastWindowClosed(quit:bool) -> None: ...
    @staticmethod
    def setWindowIcon(icon:Union[QtGui.QIcon, QtGui.QPixmap]) -> None: ...
    @staticmethod
    def styleHints() -> QtGui.QStyleHints: ...
    @staticmethod
    def sync() -> None: ...
    @staticmethod
    def topLevelAt(pos:QtCore.QPoint) -> QtGui.QWindow: ...
    @staticmethod
    def topLevelWindows() -> List[QtGui.QWindow]: ...
    @staticmethod
    def windowIcon() -> QtGui.QIcon: ...


class QHelpEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QHelpEvent) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, pos:QtCore.QPoint, globalPos:QtCore.QPoint) -> None: ...

    def clone(self) -> QtGui.QHelpEvent: ...
    def globalPos(self) -> QtCore.QPoint: ...
    def globalX(self) -> int: ...
    def globalY(self) -> int: ...
    def pos(self) -> QtCore.QPoint: ...
    def x(self) -> int: ...
    def y(self) -> int: ...


class QHideEvent(QtCore.QEvent):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:QtGui.QHideEvent) -> None: ...


class QHoverEvent(QtGui.QSinglePointEvent):

    @overload
    def __init__(self, arg__1:QtGui.QHoverEvent) -> None: ...
    @overload
    def __init__(
        self,
        type:QtCore.QEvent.Type,
        pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element],
        oldPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element],
        modifiers:QtCore.Qt.KeyboardModifiers=...,
        device:QtGui.QPointingDevice=...,
    ) -> None: ...

    def clone(self) -> QtGui.QHoverEvent: ...
    def isUpdateEvent(self) -> bool: ...
    def oldPos(self) -> QtCore.QPoint: ...
    def oldPosF(self) -> QtCore.QPointF: ...
    def pos(self) -> QtCore.QPoint: ...
    def posF(self) -> QtCore.QPointF: ...


class QIcon(Shiboken.Object):

    Normal                   : QIcon.Mode = ... # 0x0
    Disabled                 : QIcon.Mode = ... # 0x1
    Active                   : QIcon.Mode = ... # 0x2
    Selected                 : QIcon.Mode = ... # 0x3
    On                       : QIcon.State = ... # 0x0
    Off                      : QIcon.State = ... # 0x1

    class Mode(Enum):

        Normal                   : QIcon.Mode # 0x0
        Disabled                 : QIcon.Mode # 0x1
        Active                   : QIcon.Mode # 0x2
        Selected                 : QIcon.Mode # 0x3

    class State(Enum):

        On                       : QIcon.State # 0x0
        Off                      : QIcon.State # 0x1


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, engine:QtGui.QIconEngine) -> None: ...
    @overload
    def __init__(self, fileName:str) -> None: ...
    @overload
    def __init__(self, other:Union[QtGui.QIcon, QtGui.QPixmap]) -> None: ...
    @overload
    def __init__(self, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    @overload
    def actualSize(self, size:QtCore.QSize, mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> QtCore.QSize: ...
    @overload
    def actualSize(self, window:QtGui.QWindow, size:QtCore.QSize, mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> QtCore.QSize: ...
    def addFile(self, fileName:str, size:QtCore.QSize=..., mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> None: ...
    @overload
    def addPixmap(self, path:Union[str, bytes, os.PathLike]) -> None: ...
    @overload
    def addPixmap(self, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str], mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> None: ...
    def availableSizes(self, mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> List[QtCore.QSize]: ...
    def cacheKey(self) -> int: ...
    @staticmethod
    def fallbackSearchPaths() -> List[str]: ...
    @staticmethod
    def fallbackThemeName() -> str: ...
    @overload
    @staticmethod
    def fromTheme(name:str) -> QtGui.QIcon: ...
    @overload
    @staticmethod
    def fromTheme(name:str, fallback:Union[QtGui.QIcon, QtGui.QPixmap]) -> QtGui.QIcon: ...
    @staticmethod
    def hasThemeIcon(name:str) -> bool: ...
    def isMask(self) -> bool: ...
    def isNull(self) -> bool: ...
    def name(self) -> str: ...
    @overload
    def paint(self, painter:QtGui.QPainter, rect:QtCore.QRect, alignment:QtCore.Qt.Alignment=..., mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> None: ...
    @overload
    def paint(self, painter:QtGui.QPainter, x:int, y:int, w:int, h:int, alignment:QtCore.Qt.Alignment=..., mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> None: ...
    @overload
    def pixmap(self, extent:int, mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> QtGui.QPixmap: ...
    @overload
    def pixmap(self, size:QtCore.QSize, devicePixelRatio:float, mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> QtGui.QPixmap: ...
    @overload
    def pixmap(self, size:QtCore.QSize, mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> QtGui.QPixmap: ...
    @overload
    def pixmap(self, w:int, h:int, mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> QtGui.QPixmap: ...
    @overload
    def pixmap(self, window:QtGui.QWindow, size:QtCore.QSize, mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> QtGui.QPixmap: ...
    @staticmethod
    def setFallbackSearchPaths(paths:Sequence[str]) -> None: ...
    @staticmethod
    def setFallbackThemeName(name:str) -> None: ...
    def setIsMask(self, isMask:bool) -> None: ...
    @staticmethod
    def setThemeName(path:str) -> None: ...
    @staticmethod
    def setThemeSearchPaths(searchpath:Sequence[str]) -> None: ...
    def swap(self, other:Union[QtGui.QIcon, QtGui.QPixmap]) -> None: ...
    @staticmethod
    def themeName() -> str: ...
    @staticmethod
    def themeSearchPaths() -> List[str]: ...


class QIconDragEvent(QtCore.QEvent):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:QtGui.QIconDragEvent) -> None: ...


class QIconEngine(Shiboken.Object):

    IsNullHook               : QIconEngine.IconEngineHook = ... # 0x3
    ScaledPixmapHook         : QIconEngine.IconEngineHook = ... # 0x4

    class IconEngineHook(Enum):

        IsNullHook               : QIconEngine.IconEngineHook # 0x3
        ScaledPixmapHook         : QIconEngine.IconEngineHook # 0x4

    class ScaledPixmapArgument(Shiboken.Object):

        def __init__(self) -> None: ...


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, other:QtGui.QIconEngine) -> None: ...

    def actualSize(self, size:QtCore.QSize, mode:QtGui.QIcon.Mode, state:QtGui.QIcon.State) -> QtCore.QSize: ...
    def addFile(self, fileName:str, size:QtCore.QSize, mode:QtGui.QIcon.Mode, state:QtGui.QIcon.State) -> None: ...
    def addPixmap(self, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str], mode:QtGui.QIcon.Mode, state:QtGui.QIcon.State) -> None: ...
    def availableSizes(self, mode:QtGui.QIcon.Mode=..., state:QtGui.QIcon.State=...) -> List[QtCore.QSize]: ...
    def clone(self) -> QtGui.QIconEngine: ...
    def iconName(self) -> str: ...
    def isNull(self) -> bool: ...
    def key(self) -> str: ...
    def paint(self, painter:QtGui.QPainter, rect:QtCore.QRect, mode:QtGui.QIcon.Mode, state:QtGui.QIcon.State) -> None: ...
    def pixmap(self, size:QtCore.QSize, mode:QtGui.QIcon.Mode, state:QtGui.QIcon.State) -> QtGui.QPixmap: ...
    def read(self, in_:QtCore.QDataStream) -> bool: ...
    def scaledPixmap(self, size:QtCore.QSize, mode:QtGui.QIcon.Mode, state:QtGui.QIcon.State, scale:float) -> QtGui.QPixmap: ...
    def write(self, out:QtCore.QDataStream) -> bool: ...


class QImage(QtGui.QPaintDevice):

    Format_Invalid           : QImage.Format = ... # 0x0
    Format_Mono              : QImage.Format = ... # 0x1
    Format_MonoLSB           : QImage.Format = ... # 0x2
    Format_Indexed8          : QImage.Format = ... # 0x3
    Format_RGB32             : QImage.Format = ... # 0x4
    Format_ARGB32            : QImage.Format = ... # 0x5
    Format_ARGB32_Premultiplied: QImage.Format = ... # 0x6
    Format_RGB16             : QImage.Format = ... # 0x7
    Format_ARGB8565_Premultiplied: QImage.Format = ... # 0x8
    Format_RGB666            : QImage.Format = ... # 0x9
    Format_ARGB6666_Premultiplied: QImage.Format = ... # 0xa
    Format_RGB555            : QImage.Format = ... # 0xb
    Format_ARGB8555_Premultiplied: QImage.Format = ... # 0xc
    Format_RGB888            : QImage.Format = ... # 0xd
    Format_RGB444            : QImage.Format = ... # 0xe
    Format_ARGB4444_Premultiplied: QImage.Format = ... # 0xf
    Format_RGBX8888          : QImage.Format = ... # 0x10
    Format_RGBA8888          : QImage.Format = ... # 0x11
    Format_RGBA8888_Premultiplied: QImage.Format = ... # 0x12
    Format_BGR30             : QImage.Format = ... # 0x13
    Format_A2BGR30_Premultiplied: QImage.Format = ... # 0x14
    Format_RGB30             : QImage.Format = ... # 0x15
    Format_A2RGB30_Premultiplied: QImage.Format = ... # 0x16
    Format_Alpha8            : QImage.Format = ... # 0x17
    Format_Grayscale8        : QImage.Format = ... # 0x18
    Format_RGBX64            : QImage.Format = ... # 0x19
    Format_RGBA64            : QImage.Format = ... # 0x1a
    Format_RGBA64_Premultiplied: QImage.Format = ... # 0x1b
    Format_Grayscale16       : QImage.Format = ... # 0x1c
    Format_BGR888            : QImage.Format = ... # 0x1d
    Format_RGBX16FPx4        : QImage.Format = ... # 0x1e
    Format_RGBA16FPx4        : QImage.Format = ... # 0x1f
    Format_RGBA16FPx4_Premultiplied: QImage.Format = ... # 0x20
    Format_RGBX32FPx4        : QImage.Format = ... # 0x21
    Format_RGBA32FPx4        : QImage.Format = ... # 0x22
    Format_RGBA32FPx4_Premultiplied: QImage.Format = ... # 0x23
    NImageFormats            : QImage.Format = ... # 0x24
    InvertRgb                : QImage.InvertMode = ... # 0x0
    InvertRgba               : QImage.InvertMode = ... # 0x1

    class Format(Enum):

        Format_Invalid           : QImage.Format # 0x0
        Format_Mono              : QImage.Format # 0x1
        Format_MonoLSB           : QImage.Format # 0x2
        Format_Indexed8          : QImage.Format # 0x3
        Format_RGB32             : QImage.Format # 0x4
        Format_ARGB32            : QImage.Format # 0x5
        Format_ARGB32_Premultiplied: QImage.Format # 0x6
        Format_RGB16             : QImage.Format # 0x7
        Format_ARGB8565_Premultiplied: QImage.Format # 0x8
        Format_RGB666            : QImage.Format # 0x9
        Format_ARGB6666_Premultiplied: QImage.Format # 0xa
        Format_RGB555            : QImage.Format # 0xb
        Format_ARGB8555_Premultiplied: QImage.Format # 0xc
        Format_RGB888            : QImage.Format # 0xd
        Format_RGB444            : QImage.Format # 0xe
        Format_ARGB4444_Premultiplied: QImage.Format # 0xf
        Format_RGBX8888          : QImage.Format # 0x10
        Format_RGBA8888          : QImage.Format # 0x11
        Format_RGBA8888_Premultiplied: QImage.Format # 0x12
        Format_BGR30             : QImage.Format # 0x13
        Format_A2BGR30_Premultiplied: QImage.Format # 0x14
        Format_RGB30             : QImage.Format # 0x15
        Format_A2RGB30_Premultiplied: QImage.Format # 0x16
        Format_Alpha8            : QImage.Format # 0x17
        Format_Grayscale8        : QImage.Format # 0x18
        Format_RGBX64            : QImage.Format # 0x19
        Format_RGBA64            : QImage.Format # 0x1a
        Format_RGBA64_Premultiplied: QImage.Format # 0x1b
        Format_Grayscale16       : QImage.Format # 0x1c
        Format_BGR888            : QImage.Format # 0x1d
        Format_RGBX16FPx4        : QImage.Format # 0x1e
        Format_RGBA16FPx4        : QImage.Format # 0x1f
        Format_RGBA16FPx4_Premultiplied: QImage.Format # 0x20
        Format_RGBX32FPx4        : QImage.Format # 0x21
        Format_RGBA32FPx4        : QImage.Format # 0x22
        Format_RGBA32FPx4_Premultiplied: QImage.Format # 0x23
        NImageFormats            : QImage.Format # 0x24

    class InvertMode(Enum):

        InvertRgb                : QImage.InvertMode # 0x0
        InvertRgba               : QImage.InvertMode # 0x1


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:str, arg__2:int, arg__3:int, arg__4:QtGui.QImage.Format) -> None: ...
    @overload
    def __init__(self, arg__1:str, arg__2:int, arg__3:int, arg__4:int, arg__5:QtGui.QImage.Format) -> None: ...
    @overload
    def __init__(self, arg__1:Union[QtGui.QImage, str]) -> None: ...
    @overload
    def __init__(self, data:bytes, width:int, height:int, bytesPerLine:int, format:QtGui.QImage.Format, cleanupFunction:Optional[Callable]=..., cleanupInfo:Optional[int]=...) -> None: ...
    @overload
    def __init__(self, data:bytes, width:int, height:int, format:QtGui.QImage.Format, cleanupFunction:Optional[Callable]=..., cleanupInfo:Optional[int]=...) -> None: ...
    @overload
    def __init__(self, fileName:Union[str, bytes, os.PathLike], format:Optional[bytes]=...) -> None: ...
    @overload
    def __init__(self, size:QtCore.QSize, format:QtGui.QImage.Format) -> None: ...
    @overload
    def __init__(self, width:int, height:int, format:QtGui.QImage.Format) -> None: ...
    @overload
    def __init__(self, xpm:Iterable) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def allGray(self) -> bool: ...
    def applyColorTransform(self, transform:QtGui.QColorTransform) -> None: ...
    def bitPlaneCount(self) -> int: ...
    def bits(self) -> bytes: ...
    def bytesPerLine(self) -> int: ...
    def cacheKey(self) -> int: ...
    def color(self, i:int) -> int: ...
    def colorCount(self) -> int: ...
    def colorSpace(self) -> QtGui.QColorSpace: ...
    def colorTable(self) -> List[int]: ...
    def constBits(self) -> bytes: ...
    def constScanLine(self, arg__1:int) -> object: ...
    def convertTo(self, f:QtGui.QImage.Format, flags:QtCore.Qt.ImageConversionFlags=...) -> None: ...
    def convertToColorSpace(self, arg__1:Union[QtGui.QColorSpace, QtGui.QColorSpace.NamedColorSpace]) -> None: ...
    @overload
    def convertToFormat(self, f:QtGui.QImage.Format, colorTable:Sequence[int], flags:QtCore.Qt.ImageConversionFlags=...) -> QtGui.QImage: ...
    @overload
    def convertToFormat(self, f:QtGui.QImage.Format, flags:QtCore.Qt.ImageConversionFlags=...) -> QtGui.QImage: ...
    def convertToFormat_helper(self, format:QtGui.QImage.Format, flags:QtCore.Qt.ImageConversionFlags) -> QtGui.QImage: ...
    def convertToFormat_inplace(self, format:QtGui.QImage.Format, flags:QtCore.Qt.ImageConversionFlags) -> bool: ...
    def convertedTo(self, f:QtGui.QImage.Format, flags:QtCore.Qt.ImageConversionFlags=...) -> QtGui.QImage: ...
    def convertedToColorSpace(self, arg__1:Union[QtGui.QColorSpace, QtGui.QColorSpace.NamedColorSpace]) -> QtGui.QImage: ...
    @overload
    def copy(self, rect:QtCore.QRect=...) -> QtGui.QImage: ...
    @overload
    def copy(self, x:int, y:int, w:int, h:int) -> QtGui.QImage: ...
    def createAlphaMask(self, flags:QtCore.Qt.ImageConversionFlags=...) -> QtGui.QImage: ...
    def createHeuristicMask(self, clipTight:bool=...) -> QtGui.QImage: ...
    def createMaskFromColor(self, color:int, mode:QtCore.Qt.MaskMode=...) -> QtGui.QImage: ...
    def depth(self) -> int: ...
    def devType(self) -> int: ...
    def deviceIndependentSize(self) -> QtCore.QSizeF: ...
    def devicePixelRatio(self) -> float: ...
    def dotsPerMeterX(self) -> int: ...
    def dotsPerMeterY(self) -> int: ...
    @overload
    def fill(self, color:QtCore.Qt.GlobalColor) -> None: ...
    @overload
    def fill(self, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    @overload
    def fill(self, pixel:int) -> None: ...
    def format(self) -> QtGui.QImage.Format: ...
    @staticmethod
    def fromData(data:Union[QtCore.QByteArray, bytes], format:Optional[bytes]=...) -> QtGui.QImage: ...
    def hasAlphaChannel(self) -> bool: ...
    def height(self) -> int: ...
    def invertPixels(self, mode:QtGui.QImage.InvertMode=...) -> None: ...
    def isGrayscale(self) -> bool: ...
    def isNull(self) -> bool: ...
    @overload
    def load(self, device:QtCore.QIODevice, format:bytes) -> bool: ...
    @overload
    def load(self, fileName:str, format:Optional[bytes]=...) -> bool: ...
    def loadFromData(self, data:Union[QtCore.QByteArray, bytes], format:Optional[bytes]=...) -> bool: ...
    def metric(self, metric:QtGui.QPaintDevice.PaintDeviceMetric) -> int: ...
    def mirror(self, horizontally:bool=..., vertically:bool=...) -> None: ...
    def mirrored(self, horizontally:bool=..., vertically:bool=...) -> QtGui.QImage: ...
    def mirrored_helper(self, horizontal:bool, vertical:bool) -> QtGui.QImage: ...
    def mirrored_inplace(self, horizontal:bool, vertical:bool) -> None: ...
    def offset(self) -> QtCore.QPoint: ...
    def paintEngine(self) -> QtGui.QPaintEngine: ...
    @overload
    def pixel(self, pt:QtCore.QPoint) -> int: ...
    @overload
    def pixel(self, x:int, y:int) -> int: ...
    @overload
    def pixelColor(self, pt:QtCore.QPoint) -> QtGui.QColor: ...
    @overload
    def pixelColor(self, x:int, y:int) -> QtGui.QColor: ...
    def pixelFormat(self) -> QtGui.QPixelFormat: ...
    @overload
    def pixelIndex(self, pt:QtCore.QPoint) -> int: ...
    @overload
    def pixelIndex(self, x:int, y:int) -> int: ...
    def rect(self) -> QtCore.QRect: ...
    def reinterpretAsFormat(self, f:QtGui.QImage.Format) -> bool: ...
    def rgbSwap(self) -> None: ...
    def rgbSwapped(self) -> QtGui.QImage: ...
    def rgbSwapped_helper(self) -> QtGui.QImage: ...
    def rgbSwapped_inplace(self) -> None: ...
    @overload
    def save(self, device:QtCore.QIODevice, format:Optional[bytes]=..., quality:int=...) -> bool: ...
    @overload
    def save(self, fileName:str, format:Optional[bytes]=..., quality:int=...) -> bool: ...
    @overload
    def scaled(self, s:QtCore.QSize, aspectMode:QtCore.Qt.AspectRatioMode=..., mode:QtCore.Qt.TransformationMode=...) -> QtGui.QImage: ...
    @overload
    def scaled(self, w:int, h:int, aspectMode:QtCore.Qt.AspectRatioMode=..., mode:QtCore.Qt.TransformationMode=...) -> QtGui.QImage: ...
    def scaledToHeight(self, h:int, mode:QtCore.Qt.TransformationMode=...) -> QtGui.QImage: ...
    def scaledToWidth(self, w:int, mode:QtCore.Qt.TransformationMode=...) -> QtGui.QImage: ...
    def scanLine(self, arg__1:int) -> object: ...
    def setAlphaChannel(self, alphaChannel:Union[QtGui.QImage, str]) -> None: ...
    def setColor(self, i:int, c:int) -> None: ...
    def setColorCount(self, arg__1:int) -> None: ...
    def setColorSpace(self, arg__1:Union[QtGui.QColorSpace, QtGui.QColorSpace.NamedColorSpace]) -> None: ...
    def setColorTable(self, colors:Sequence[int]) -> None: ...
    def setDevicePixelRatio(self, scaleFactor:float) -> None: ...
    def setDotsPerMeterX(self, arg__1:int) -> None: ...
    def setDotsPerMeterY(self, arg__1:int) -> None: ...
    def setOffset(self, arg__1:QtCore.QPoint) -> None: ...
    @overload
    def setPixel(self, pt:QtCore.QPoint, index_or_rgb:int) -> None: ...
    @overload
    def setPixel(self, x:int, y:int, index_or_rgb:int) -> None: ...
    @overload
    def setPixelColor(self, pt:QtCore.QPoint, c:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    @overload
    def setPixelColor(self, x:int, y:int, c:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    def setText(self, key:str, value:str) -> None: ...
    def size(self) -> QtCore.QSize: ...
    def sizeInBytes(self) -> int: ...
    def smoothScaled(self, w:int, h:int) -> QtGui.QImage: ...
    def swap(self, other:Union[QtGui.QImage, str]) -> None: ...
    def text(self, key:str=...) -> str: ...
    def textKeys(self) -> List[str]: ...
    @staticmethod
    def toImageFormat(format:QtGui.QPixelFormat) -> QtGui.QImage.Format: ...
    @staticmethod
    def toPixelFormat(format:QtGui.QImage.Format) -> QtGui.QPixelFormat: ...
    def transformed(self, matrix:QtGui.QTransform, mode:QtCore.Qt.TransformationMode=...) -> QtGui.QImage: ...
    @staticmethod
    def trueMatrix(arg__1:QtGui.QTransform, w:int, h:int) -> QtGui.QTransform: ...
    @overload
    def valid(self, pt:QtCore.QPoint) -> bool: ...
    @overload
    def valid(self, x:int, y:int) -> bool: ...
    def width(self) -> int: ...


class QImageIOHandler(Shiboken.Object):

    Size                     : QImageIOHandler.ImageOption = ... # 0x0
    ClipRect                 : QImageIOHandler.ImageOption = ... # 0x1
    Description              : QImageIOHandler.ImageOption = ... # 0x2
    ScaledClipRect           : QImageIOHandler.ImageOption = ... # 0x3
    ScaledSize               : QImageIOHandler.ImageOption = ... # 0x4
    CompressionRatio         : QImageIOHandler.ImageOption = ... # 0x5
    Gamma                    : QImageIOHandler.ImageOption = ... # 0x6
    Quality                  : QImageIOHandler.ImageOption = ... # 0x7
    Name                     : QImageIOHandler.ImageOption = ... # 0x8
    SubType                  : QImageIOHandler.ImageOption = ... # 0x9
    IncrementalReading       : QImageIOHandler.ImageOption = ... # 0xa
    Endianness               : QImageIOHandler.ImageOption = ... # 0xb
    Animation                : QImageIOHandler.ImageOption = ... # 0xc
    BackgroundColor          : QImageIOHandler.ImageOption = ... # 0xd
    ImageFormat              : QImageIOHandler.ImageOption = ... # 0xe
    SupportedSubTypes        : QImageIOHandler.ImageOption = ... # 0xf
    OptimizedWrite           : QImageIOHandler.ImageOption = ... # 0x10
    ProgressiveScanWrite     : QImageIOHandler.ImageOption = ... # 0x11
    ImageTransformation      : QImageIOHandler.ImageOption = ... # 0x12
    TransformationNone       : QImageIOHandler.Transformation = ... # 0x0
    TransformationMirror     : QImageIOHandler.Transformation = ... # 0x1
    TransformationFlip       : QImageIOHandler.Transformation = ... # 0x2
    TransformationRotate180  : QImageIOHandler.Transformation = ... # 0x3
    TransformationRotate90   : QImageIOHandler.Transformation = ... # 0x4
    TransformationMirrorAndRotate90: QImageIOHandler.Transformation = ... # 0x5
    TransformationFlipAndRotate90: QImageIOHandler.Transformation = ... # 0x6
    TransformationRotate270  : QImageIOHandler.Transformation = ... # 0x7

    class ImageOption(Enum):

        Size                     : QImageIOHandler.ImageOption # 0x0
        ClipRect                 : QImageIOHandler.ImageOption # 0x1
        Description              : QImageIOHandler.ImageOption # 0x2
        ScaledClipRect           : QImageIOHandler.ImageOption # 0x3
        ScaledSize               : QImageIOHandler.ImageOption # 0x4
        CompressionRatio         : QImageIOHandler.ImageOption # 0x5
        Gamma                    : QImageIOHandler.ImageOption # 0x6
        Quality                  : QImageIOHandler.ImageOption # 0x7
        Name                     : QImageIOHandler.ImageOption # 0x8
        SubType                  : QImageIOHandler.ImageOption # 0x9
        IncrementalReading       : QImageIOHandler.ImageOption # 0xa
        Endianness               : QImageIOHandler.ImageOption # 0xb
        Animation                : QImageIOHandler.ImageOption # 0xc
        BackgroundColor          : QImageIOHandler.ImageOption # 0xd
        ImageFormat              : QImageIOHandler.ImageOption # 0xe
        SupportedSubTypes        : QImageIOHandler.ImageOption # 0xf
        OptimizedWrite           : QImageIOHandler.ImageOption # 0x10
        ProgressiveScanWrite     : QImageIOHandler.ImageOption # 0x11
        ImageTransformation      : QImageIOHandler.ImageOption # 0x12

    class Transformation(Enum):

        TransformationNone       : QImageIOHandler.Transformation # 0x0
        TransformationMirror     : QImageIOHandler.Transformation # 0x1
        TransformationFlip       : QImageIOHandler.Transformation # 0x2
        TransformationRotate180  : QImageIOHandler.Transformation # 0x3
        TransformationRotate90   : QImageIOHandler.Transformation # 0x4
        TransformationMirrorAndRotate90: QImageIOHandler.Transformation # 0x5
        TransformationFlipAndRotate90: QImageIOHandler.Transformation # 0x6
        TransformationRotate270  : QImageIOHandler.Transformation # 0x7

    class Transformations(object): ...


    def __init__(self) -> None: ...

    @staticmethod
    def allocateImage(size:QtCore.QSize, format:QtGui.QImage.Format, image:Union[QtGui.QImage, str]) -> bool: ...
    def canRead(self) -> bool: ...
    def currentImageNumber(self) -> int: ...
    def currentImageRect(self) -> QtCore.QRect: ...
    def device(self) -> QtCore.QIODevice: ...
    def format(self) -> QtCore.QByteArray: ...
    def imageCount(self) -> int: ...
    def jumpToImage(self, imageNumber:int) -> bool: ...
    def jumpToNextImage(self) -> bool: ...
    def loopCount(self) -> int: ...
    def nextImageDelay(self) -> int: ...
    def option(self, option:QtGui.QImageIOHandler.ImageOption) -> Any: ...
    def read(self, image:Union[QtGui.QImage, str]) -> bool: ...
    def setDevice(self, device:QtCore.QIODevice) -> None: ...
    def setFormat(self, format:Union[QtCore.QByteArray, bytes]) -> None: ...
    def setOption(self, option:QtGui.QImageIOHandler.ImageOption, value:Any) -> None: ...
    def supportsOption(self, option:QtGui.QImageIOHandler.ImageOption) -> bool: ...
    def write(self, image:Union[QtGui.QImage, str]) -> bool: ...


class QImageReader(Shiboken.Object):

    UnknownError             : QImageReader.ImageReaderError = ... # 0x0
    FileNotFoundError        : QImageReader.ImageReaderError = ... # 0x1
    DeviceError              : QImageReader.ImageReaderError = ... # 0x2
    UnsupportedFormatError   : QImageReader.ImageReaderError = ... # 0x3
    InvalidDataError         : QImageReader.ImageReaderError = ... # 0x4

    class ImageReaderError(Enum):

        UnknownError             : QImageReader.ImageReaderError # 0x0
        FileNotFoundError        : QImageReader.ImageReaderError # 0x1
        DeviceError              : QImageReader.ImageReaderError # 0x2
        UnsupportedFormatError   : QImageReader.ImageReaderError # 0x3
        InvalidDataError         : QImageReader.ImageReaderError # 0x4


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, device:QtCore.QIODevice, format:Union[QtCore.QByteArray, bytes]=...) -> None: ...
    @overload
    def __init__(self, fileName:str, format:Union[QtCore.QByteArray, bytes]=...) -> None: ...

    @staticmethod
    def allocationLimit() -> int: ...
    def autoDetectImageFormat(self) -> bool: ...
    def autoTransform(self) -> bool: ...
    def backgroundColor(self) -> QtGui.QColor: ...
    def canRead(self) -> bool: ...
    def clipRect(self) -> QtCore.QRect: ...
    def currentImageNumber(self) -> int: ...
    def currentImageRect(self) -> QtCore.QRect: ...
    def decideFormatFromContent(self) -> bool: ...
    def device(self) -> QtCore.QIODevice: ...
    def error(self) -> QtGui.QImageReader.ImageReaderError: ...
    def errorString(self) -> str: ...
    def fileName(self) -> str: ...
    def format(self) -> QtCore.QByteArray: ...
    def imageCount(self) -> int: ...
    @overload
    @staticmethod
    def imageFormat(device:QtCore.QIODevice) -> QtCore.QByteArray: ...
    @overload
    @staticmethod
    def imageFormat(fileName:str) -> QtCore.QByteArray: ...
    @overload
    def imageFormat(self) -> QtGui.QImage.Format: ...
    @staticmethod
    def imageFormatsForMimeType(mimeType:Union[QtCore.QByteArray, bytes]) -> List[QtCore.QByteArray]: ...
    def jumpToImage(self, imageNumber:int) -> bool: ...
    def jumpToNextImage(self) -> bool: ...
    def loopCount(self) -> int: ...
    def nextImageDelay(self) -> int: ...
    def quality(self) -> int: ...
    def read(self) -> QtGui.QImage: ...
    def scaledClipRect(self) -> QtCore.QRect: ...
    def scaledSize(self) -> QtCore.QSize: ...
    @staticmethod
    def setAllocationLimit(mbLimit:int) -> None: ...
    def setAutoDetectImageFormat(self, enabled:bool) -> None: ...
    def setAutoTransform(self, enabled:bool) -> None: ...
    def setBackgroundColor(self, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    def setClipRect(self, rect:QtCore.QRect) -> None: ...
    def setDecideFormatFromContent(self, ignored:bool) -> None: ...
    def setDevice(self, device:QtCore.QIODevice) -> None: ...
    def setFileName(self, fileName:str) -> None: ...
    def setFormat(self, format:Union[QtCore.QByteArray, bytes]) -> None: ...
    def setQuality(self, quality:int) -> None: ...
    def setScaledClipRect(self, rect:QtCore.QRect) -> None: ...
    def setScaledSize(self, size:QtCore.QSize) -> None: ...
    def size(self) -> QtCore.QSize: ...
    def subType(self) -> QtCore.QByteArray: ...
    @staticmethod
    def supportedImageFormats() -> List[QtCore.QByteArray]: ...
    @staticmethod
    def supportedMimeTypes() -> List[QtCore.QByteArray]: ...
    def supportedSubTypes(self) -> List[QtCore.QByteArray]: ...
    def supportsAnimation(self) -> bool: ...
    def supportsOption(self, option:QtGui.QImageIOHandler.ImageOption) -> bool: ...
    def text(self, key:str) -> str: ...
    def textKeys(self) -> List[str]: ...
    def transformation(self) -> QtGui.QImageIOHandler.Transformations: ...


class QImageWriter(Shiboken.Object):

    UnknownError             : QImageWriter.ImageWriterError = ... # 0x0
    DeviceError              : QImageWriter.ImageWriterError = ... # 0x1
    UnsupportedFormatError   : QImageWriter.ImageWriterError = ... # 0x2
    InvalidImageError        : QImageWriter.ImageWriterError = ... # 0x3

    class ImageWriterError(Enum):

        UnknownError             : QImageWriter.ImageWriterError # 0x0
        DeviceError              : QImageWriter.ImageWriterError # 0x1
        UnsupportedFormatError   : QImageWriter.ImageWriterError # 0x2
        InvalidImageError        : QImageWriter.ImageWriterError # 0x3


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, device:QtCore.QIODevice, format:Union[QtCore.QByteArray, bytes]) -> None: ...
    @overload
    def __init__(self, fileName:str, format:Union[QtCore.QByteArray, bytes]=...) -> None: ...

    def canWrite(self) -> bool: ...
    def compression(self) -> int: ...
    def device(self) -> QtCore.QIODevice: ...
    def error(self) -> QtGui.QImageWriter.ImageWriterError: ...
    def errorString(self) -> str: ...
    def fileName(self) -> str: ...
    def format(self) -> QtCore.QByteArray: ...
    @staticmethod
    def imageFormatsForMimeType(mimeType:Union[QtCore.QByteArray, bytes]) -> List[QtCore.QByteArray]: ...
    def optimizedWrite(self) -> bool: ...
    def progressiveScanWrite(self) -> bool: ...
    def quality(self) -> int: ...
    def setCompression(self, compression:int) -> None: ...
    def setDevice(self, device:QtCore.QIODevice) -> None: ...
    def setFileName(self, fileName:str) -> None: ...
    def setFormat(self, format:Union[QtCore.QByteArray, bytes]) -> None: ...
    def setOptimizedWrite(self, optimize:bool) -> None: ...
    def setProgressiveScanWrite(self, progressive:bool) -> None: ...
    def setQuality(self, quality:int) -> None: ...
    def setSubType(self, type:Union[QtCore.QByteArray, bytes]) -> None: ...
    def setText(self, key:str, text:str) -> None: ...
    def setTransformation(self, orientation:QtGui.QImageIOHandler.Transformations) -> None: ...
    def subType(self) -> QtCore.QByteArray: ...
    @staticmethod
    def supportedImageFormats() -> List[QtCore.QByteArray]: ...
    @staticmethod
    def supportedMimeTypes() -> List[QtCore.QByteArray]: ...
    def supportedSubTypes(self) -> List[QtCore.QByteArray]: ...
    def supportsOption(self, option:QtGui.QImageIOHandler.ImageOption) -> bool: ...
    def transformation(self) -> QtGui.QImageIOHandler.Transformations: ...
    def write(self, image:Union[QtGui.QImage, str]) -> bool: ...


class QInputDevice(QtCore.QObject):

    class Capabilities(object): ...

    class Capability(Enum):

        None_                    : QInputDevice.Capability # 0x0
        Position                 : QInputDevice.Capability # 0x1
        Area                     : QInputDevice.Capability # 0x2
        Pressure                 : QInputDevice.Capability # 0x4
        Velocity                 : QInputDevice.Capability # 0x8
        NormalizedPosition       : QInputDevice.Capability # 0x20
        MouseEmulation           : QInputDevice.Capability # 0x40
        PixelScroll              : QInputDevice.Capability # 0x80
        Scroll                   : QInputDevice.Capability # 0x100
        Hover                    : QInputDevice.Capability # 0x200
        Rotation                 : QInputDevice.Capability # 0x400
        XTilt                    : QInputDevice.Capability # 0x800
        YTilt                    : QInputDevice.Capability # 0x1000
        TangentialPressure       : QInputDevice.Capability # 0x2000
        ZPosition                : QInputDevice.Capability # 0x4000
        All                      : QInputDevice.Capability # 0x7fffffff

    class DeviceType(Enum):

        Unknown                  : QInputDevice.DeviceType # 0x0
        Mouse                    : QInputDevice.DeviceType # 0x1
        TouchScreen              : QInputDevice.DeviceType # 0x2
        TouchPad                 : QInputDevice.DeviceType # 0x4
        Puck                     : QInputDevice.DeviceType # 0x8
        Stylus                   : QInputDevice.DeviceType # 0x10
        Airbrush                 : QInputDevice.DeviceType # 0x20
        Keyboard                 : QInputDevice.DeviceType # 0x1000
        AllDevices               : QInputDevice.DeviceType # 0x7fffffff

    class DeviceTypes(object): ...


    @overload
    def __init__(self, name:str, systemId:int, type:QtGui.QInputDevice.DeviceType, seatName:str=..., parent:Optional[QtCore.QObject]=...) -> None: ...
    @overload
    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...

    def availableVirtualGeometry(self) -> QtCore.QRect: ...
    def capabilities(self) -> QtGui.QInputDevice.Capabilities: ...
    @staticmethod
    def devices() -> List[QtGui.QInputDevice]: ...
    def hasCapability(self, cap:QtGui.QInputDevice.Capability) -> bool: ...
    def name(self) -> str: ...
    @staticmethod
    def primaryKeyboard(seatName:str=...) -> QtGui.QInputDevice: ...
    def seatName(self) -> str: ...
    def systemId(self) -> int: ...
    def type(self) -> QtGui.QInputDevice.DeviceType: ...


class QInputEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QInputEvent) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, m_dev:QtGui.QInputDevice, modifiers:QtCore.Qt.KeyboardModifiers=...) -> None: ...

    def clone(self) -> QtGui.QInputEvent: ...
    def device(self) -> QtGui.QInputDevice: ...
    def deviceType(self) -> QtGui.QInputDevice.DeviceType: ...
    def modifiers(self) -> QtCore.Qt.KeyboardModifiers: ...
    def setModifiers(self, modifiers:QtCore.Qt.KeyboardModifiers) -> None: ...
    def setTimestamp(self, timestamp:int) -> None: ...
    def timestamp(self) -> int: ...


class QInputMethod(QtCore.QObject):

    Click                    : QInputMethod.Action = ... # 0x0
    ContextMenu              : QInputMethod.Action = ... # 0x1

    class Action(Enum):

        Click                    : QInputMethod.Action # 0x0
        ContextMenu              : QInputMethod.Action # 0x1


    def anchorRectangle(self) -> QtCore.QRectF: ...
    def commit(self) -> None: ...
    def cursorRectangle(self) -> QtCore.QRectF: ...
    def hide(self) -> None: ...
    def inputDirection(self) -> QtCore.Qt.LayoutDirection: ...
    def inputItemClipRectangle(self) -> QtCore.QRectF: ...
    def inputItemRectangle(self) -> QtCore.QRectF: ...
    def inputItemTransform(self) -> QtGui.QTransform: ...
    def invokeAction(self, a:QtGui.QInputMethod.Action, cursorPosition:int) -> None: ...
    def isAnimating(self) -> bool: ...
    def isVisible(self) -> bool: ...
    def keyboardRectangle(self) -> QtCore.QRectF: ...
    def locale(self) -> QtCore.QLocale: ...
    @staticmethod
    def queryFocusObject(query:QtCore.Qt.InputMethodQuery, argument:Any) -> Any: ...
    def reset(self) -> None: ...
    def setInputItemRectangle(self, rect:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    def setInputItemTransform(self, transform:QtGui.QTransform) -> None: ...
    def setVisible(self, visible:bool) -> None: ...
    def show(self) -> None: ...
    def update(self, queries:QtCore.Qt.InputMethodQueries) -> None: ...


class QInputMethodEvent(QtCore.QEvent):

    TextFormat               : QInputMethodEvent.AttributeType = ... # 0x0
    Cursor                   : QInputMethodEvent.AttributeType = ... # 0x1
    Language                 : QInputMethodEvent.AttributeType = ... # 0x2
    Ruby                     : QInputMethodEvent.AttributeType = ... # 0x3
    Selection                : QInputMethodEvent.AttributeType = ... # 0x4

    class Attribute(Shiboken.Object):

        @overload
        def __init__(self, Attribute:QtGui.QInputMethodEvent.Attribute) -> None: ...
        @overload
        def __init__(self, typ:QtGui.QInputMethodEvent.AttributeType, s:int, l:int) -> None: ...
        @overload
        def __init__(self, typ:QtGui.QInputMethodEvent.AttributeType, s:int, l:int, val:Any) -> None: ...

        @staticmethod
        def __copy__() -> None: ...

    class AttributeType(Enum):

        TextFormat               : QInputMethodEvent.AttributeType # 0x0
        Cursor                   : QInputMethodEvent.AttributeType # 0x1
        Language                 : QInputMethodEvent.AttributeType # 0x2
        Ruby                     : QInputMethodEvent.AttributeType # 0x3
        Selection                : QInputMethodEvent.AttributeType # 0x4


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:QtGui.QInputMethodEvent) -> None: ...
    @overload
    def __init__(self, preeditText:str, attributes:Sequence[QtGui.QInputMethodEvent.Attribute]) -> None: ...

    def attributes(self) -> List[QtGui.QInputMethodEvent.Attribute]: ...
    def clone(self) -> QtGui.QInputMethodEvent: ...
    def commitString(self) -> str: ...
    def preeditString(self) -> str: ...
    def replacementLength(self) -> int: ...
    def replacementStart(self) -> int: ...
    def setCommitString(self, commitString:str, replaceFrom:int=..., replaceLength:int=...) -> None: ...


class QInputMethodQueryEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QInputMethodQueryEvent) -> None: ...
    @overload
    def __init__(self, queries:QtCore.Qt.InputMethodQueries) -> None: ...

    def clone(self) -> QtGui.QInputMethodQueryEvent: ...
    def queries(self) -> QtCore.Qt.InputMethodQueries: ...
    def setValue(self, query:QtCore.Qt.InputMethodQuery, value:Any) -> None: ...
    def value(self, query:QtCore.Qt.InputMethodQuery) -> Any: ...


class QIntList(object): ...


class QIntValidator(QtGui.QValidator):

    @overload
    def __init__(self, bottom:int, top:int, parent:Optional[QtCore.QObject]=...) -> None: ...
    @overload
    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...

    def bottom(self) -> int: ...
    def fixup(self, input:str) -> None: ...
    def setBottom(self, arg__1:int) -> None: ...
    def setRange(self, bottom:int, top:int) -> None: ...
    def setTop(self, arg__1:int) -> None: ...
    def top(self) -> int: ...
    def validate(self, arg__1:str, arg__2:int) -> object: ...


class QKeyEvent(QtGui.QInputEvent):

    @overload
    def __init__(self, arg__1:QtGui.QKeyEvent) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, key:int, modifiers:QtCore.Qt.KeyboardModifiers, nativeScanCode:int, nativeVirtualKey:int, nativeModifiers:int, text:str=..., autorep:bool=..., count:int=..., device:QtGui.QInputDevice=...) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, key:int, modifiers:QtCore.Qt.KeyboardModifiers, text:str=..., autorep:bool=..., count:int=...) -> None: ...

    def clone(self) -> QtGui.QKeyEvent: ...
    def count(self) -> int: ...
    def isAutoRepeat(self) -> bool: ...
    def key(self) -> int: ...
    def keyCombination(self) -> QtCore.QKeyCombination: ...
    def matches(self, key:QtGui.QKeySequence.StandardKey) -> bool: ...
    def modifiers(self) -> QtCore.Qt.KeyboardModifiers: ...
    def nativeModifiers(self) -> int: ...
    def nativeScanCode(self) -> int: ...
    def nativeVirtualKey(self) -> int: ...
    def text(self) -> str: ...


class QKeySequence(Shiboken.Object):

    NativeText               : QKeySequence.SequenceFormat = ... # 0x0
    PortableText             : QKeySequence.SequenceFormat = ... # 0x1
    NoMatch                  : QKeySequence.SequenceMatch = ... # 0x0
    PartialMatch             : QKeySequence.SequenceMatch = ... # 0x1
    ExactMatch               : QKeySequence.SequenceMatch = ... # 0x2
    UnknownKey               : QKeySequence.StandardKey = ... # 0x0
    HelpContents             : QKeySequence.StandardKey = ... # 0x1
    WhatsThis                : QKeySequence.StandardKey = ... # 0x2
    Open                     : QKeySequence.StandardKey = ... # 0x3
    Close                    : QKeySequence.StandardKey = ... # 0x4
    Save                     : QKeySequence.StandardKey = ... # 0x5
    New                      : QKeySequence.StandardKey = ... # 0x6
    Delete                   : QKeySequence.StandardKey = ... # 0x7
    Cut                      : QKeySequence.StandardKey = ... # 0x8
    Copy                     : QKeySequence.StandardKey = ... # 0x9
    Paste                    : QKeySequence.StandardKey = ... # 0xa
    Undo                     : QKeySequence.StandardKey = ... # 0xb
    Redo                     : QKeySequence.StandardKey = ... # 0xc
    Back                     : QKeySequence.StandardKey = ... # 0xd
    Forward                  : QKeySequence.StandardKey = ... # 0xe
    Refresh                  : QKeySequence.StandardKey = ... # 0xf
    ZoomIn                   : QKeySequence.StandardKey = ... # 0x10
    ZoomOut                  : QKeySequence.StandardKey = ... # 0x11
    Print                    : QKeySequence.StandardKey = ... # 0x12
    AddTab                   : QKeySequence.StandardKey = ... # 0x13
    NextChild                : QKeySequence.StandardKey = ... # 0x14
    PreviousChild            : QKeySequence.StandardKey = ... # 0x15
    Find                     : QKeySequence.StandardKey = ... # 0x16
    FindNext                 : QKeySequence.StandardKey = ... # 0x17
    FindPrevious             : QKeySequence.StandardKey = ... # 0x18
    Replace                  : QKeySequence.StandardKey = ... # 0x19
    SelectAll                : QKeySequence.StandardKey = ... # 0x1a
    Bold                     : QKeySequence.StandardKey = ... # 0x1b
    Italic                   : QKeySequence.StandardKey = ... # 0x1c
    Underline                : QKeySequence.StandardKey = ... # 0x1d
    MoveToNextChar           : QKeySequence.StandardKey = ... # 0x1e
    MoveToPreviousChar       : QKeySequence.StandardKey = ... # 0x1f
    MoveToNextWord           : QKeySequence.StandardKey = ... # 0x20
    MoveToPreviousWord       : QKeySequence.StandardKey = ... # 0x21
    MoveToNextLine           : QKeySequence.StandardKey = ... # 0x22
    MoveToPreviousLine       : QKeySequence.StandardKey = ... # 0x23
    MoveToNextPage           : QKeySequence.StandardKey = ... # 0x24
    MoveToPreviousPage       : QKeySequence.StandardKey = ... # 0x25
    MoveToStartOfLine        : QKeySequence.StandardKey = ... # 0x26
    MoveToEndOfLine          : QKeySequence.StandardKey = ... # 0x27
    MoveToStartOfBlock       : QKeySequence.StandardKey = ... # 0x28
    MoveToEndOfBlock         : QKeySequence.StandardKey = ... # 0x29
    MoveToStartOfDocument    : QKeySequence.StandardKey = ... # 0x2a
    MoveToEndOfDocument      : QKeySequence.StandardKey = ... # 0x2b
    SelectNextChar           : QKeySequence.StandardKey = ... # 0x2c
    SelectPreviousChar       : QKeySequence.StandardKey = ... # 0x2d
    SelectNextWord           : QKeySequence.StandardKey = ... # 0x2e
    SelectPreviousWord       : QKeySequence.StandardKey = ... # 0x2f
    SelectNextLine           : QKeySequence.StandardKey = ... # 0x30
    SelectPreviousLine       : QKeySequence.StandardKey = ... # 0x31
    SelectNextPage           : QKeySequence.StandardKey = ... # 0x32
    SelectPreviousPage       : QKeySequence.StandardKey = ... # 0x33
    SelectStartOfLine        : QKeySequence.StandardKey = ... # 0x34
    SelectEndOfLine          : QKeySequence.StandardKey = ... # 0x35
    SelectStartOfBlock       : QKeySequence.StandardKey = ... # 0x36
    SelectEndOfBlock         : QKeySequence.StandardKey = ... # 0x37
    SelectStartOfDocument    : QKeySequence.StandardKey = ... # 0x38
    SelectEndOfDocument      : QKeySequence.StandardKey = ... # 0x39
    DeleteStartOfWord        : QKeySequence.StandardKey = ... # 0x3a
    DeleteEndOfWord          : QKeySequence.StandardKey = ... # 0x3b
    DeleteEndOfLine          : QKeySequence.StandardKey = ... # 0x3c
    InsertParagraphSeparator : QKeySequence.StandardKey = ... # 0x3d
    InsertLineSeparator      : QKeySequence.StandardKey = ... # 0x3e
    SaveAs                   : QKeySequence.StandardKey = ... # 0x3f
    Preferences              : QKeySequence.StandardKey = ... # 0x40
    Quit                     : QKeySequence.StandardKey = ... # 0x41
    FullScreen               : QKeySequence.StandardKey = ... # 0x42
    Deselect                 : QKeySequence.StandardKey = ... # 0x43
    DeleteCompleteLine       : QKeySequence.StandardKey = ... # 0x44
    Backspace                : QKeySequence.StandardKey = ... # 0x45
    Cancel                   : QKeySequence.StandardKey = ... # 0x46

    class SequenceFormat(Enum):

        NativeText               : QKeySequence.SequenceFormat # 0x0
        PortableText             : QKeySequence.SequenceFormat # 0x1

    class SequenceMatch(Enum):

        NoMatch                  : QKeySequence.SequenceMatch # 0x0
        PartialMatch             : QKeySequence.SequenceMatch # 0x1
        ExactMatch               : QKeySequence.SequenceMatch # 0x2

    class StandardKey(Enum):

        UnknownKey               : QKeySequence.StandardKey # 0x0
        HelpContents             : QKeySequence.StandardKey # 0x1
        WhatsThis                : QKeySequence.StandardKey # 0x2
        Open                     : QKeySequence.StandardKey # 0x3
        Close                    : QKeySequence.StandardKey # 0x4
        Save                     : QKeySequence.StandardKey # 0x5
        New                      : QKeySequence.StandardKey # 0x6
        Delete                   : QKeySequence.StandardKey # 0x7
        Cut                      : QKeySequence.StandardKey # 0x8
        Copy                     : QKeySequence.StandardKey # 0x9
        Paste                    : QKeySequence.StandardKey # 0xa
        Undo                     : QKeySequence.StandardKey # 0xb
        Redo                     : QKeySequence.StandardKey # 0xc
        Back                     : QKeySequence.StandardKey # 0xd
        Forward                  : QKeySequence.StandardKey # 0xe
        Refresh                  : QKeySequence.StandardKey # 0xf
        ZoomIn                   : QKeySequence.StandardKey # 0x10
        ZoomOut                  : QKeySequence.StandardKey # 0x11
        Print                    : QKeySequence.StandardKey # 0x12
        AddTab                   : QKeySequence.StandardKey # 0x13
        NextChild                : QKeySequence.StandardKey # 0x14
        PreviousChild            : QKeySequence.StandardKey # 0x15
        Find                     : QKeySequence.StandardKey # 0x16
        FindNext                 : QKeySequence.StandardKey # 0x17
        FindPrevious             : QKeySequence.StandardKey # 0x18
        Replace                  : QKeySequence.StandardKey # 0x19
        SelectAll                : QKeySequence.StandardKey # 0x1a
        Bold                     : QKeySequence.StandardKey # 0x1b
        Italic                   : QKeySequence.StandardKey # 0x1c
        Underline                : QKeySequence.StandardKey # 0x1d
        MoveToNextChar           : QKeySequence.StandardKey # 0x1e
        MoveToPreviousChar       : QKeySequence.StandardKey # 0x1f
        MoveToNextWord           : QKeySequence.StandardKey # 0x20
        MoveToPreviousWord       : QKeySequence.StandardKey # 0x21
        MoveToNextLine           : QKeySequence.StandardKey # 0x22
        MoveToPreviousLine       : QKeySequence.StandardKey # 0x23
        MoveToNextPage           : QKeySequence.StandardKey # 0x24
        MoveToPreviousPage       : QKeySequence.StandardKey # 0x25
        MoveToStartOfLine        : QKeySequence.StandardKey # 0x26
        MoveToEndOfLine          : QKeySequence.StandardKey # 0x27
        MoveToStartOfBlock       : QKeySequence.StandardKey # 0x28
        MoveToEndOfBlock         : QKeySequence.StandardKey # 0x29
        MoveToStartOfDocument    : QKeySequence.StandardKey # 0x2a
        MoveToEndOfDocument      : QKeySequence.StandardKey # 0x2b
        SelectNextChar           : QKeySequence.StandardKey # 0x2c
        SelectPreviousChar       : QKeySequence.StandardKey # 0x2d
        SelectNextWord           : QKeySequence.StandardKey # 0x2e
        SelectPreviousWord       : QKeySequence.StandardKey # 0x2f
        SelectNextLine           : QKeySequence.StandardKey # 0x30
        SelectPreviousLine       : QKeySequence.StandardKey # 0x31
        SelectNextPage           : QKeySequence.StandardKey # 0x32
        SelectPreviousPage       : QKeySequence.StandardKey # 0x33
        SelectStartOfLine        : QKeySequence.StandardKey # 0x34
        SelectEndOfLine          : QKeySequence.StandardKey # 0x35
        SelectStartOfBlock       : QKeySequence.StandardKey # 0x36
        SelectEndOfBlock         : QKeySequence.StandardKey # 0x37
        SelectStartOfDocument    : QKeySequence.StandardKey # 0x38
        SelectEndOfDocument      : QKeySequence.StandardKey # 0x39
        DeleteStartOfWord        : QKeySequence.StandardKey # 0x3a
        DeleteEndOfWord          : QKeySequence.StandardKey # 0x3b
        DeleteEndOfLine          : QKeySequence.StandardKey # 0x3c
        InsertParagraphSeparator : QKeySequence.StandardKey # 0x3d
        InsertLineSeparator      : QKeySequence.StandardKey # 0x3e
        SaveAs                   : QKeySequence.StandardKey # 0x3f
        Preferences              : QKeySequence.StandardKey # 0x40
        Quit                     : QKeySequence.StandardKey # 0x41
        FullScreen               : QKeySequence.StandardKey # 0x42
        Deselect                 : QKeySequence.StandardKey # 0x43
        DeleteCompleteLine       : QKeySequence.StandardKey # 0x44
        Backspace                : QKeySequence.StandardKey # 0x45
        Cancel                   : QKeySequence.StandardKey # 0x46


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, k1:int, k2:int=..., k3:int=..., k4:int=...) -> None: ...
    @overload
    def __init__(self, k1:Union[QtCore.QKeyCombination, QtCore.Qt.KeyboardModifiers, QtCore.Qt.Key], k2:Union[QtCore.QKeyCombination, QtCore.Qt.KeyboardModifiers, QtCore.Qt.Key]=..., k3:Union[QtCore.QKeyCombination, QtCore.Qt.KeyboardModifiers, QtCore.Qt.Key]=..., k4:Union[QtCore.QKeyCombination, QtCore.Qt.KeyboardModifiers, QtCore.Qt.Key]=...) -> None: ...
    @overload
    def __init__(self, key:QtGui.QKeySequence.StandardKey) -> None: ...
    @overload
    def __init__(self, key:str, format:QtGui.QKeySequence.SequenceFormat=...) -> None: ...
    @overload
    def __init__(self, ks:Union[QtGui.QKeySequence, QtCore.QKeyCombination, QtGui.QKeySequence.StandardKey, str, int]) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, in_:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def __rshift__(self, out:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def count(self) -> int: ...
    @staticmethod
    def fromString(str:str, format:QtGui.QKeySequence.SequenceFormat=...) -> QtGui.QKeySequence: ...
    def isEmpty(self) -> bool: ...
    @staticmethod
    def keyBindings(key:QtGui.QKeySequence.StandardKey) -> List[QtGui.QKeySequence]: ...
    @staticmethod
    def listFromString(str:str, format:QtGui.QKeySequence.SequenceFormat=...) -> List[QtGui.QKeySequence]: ...
    @staticmethod
    def listToString(list:Sequence[QtGui.QKeySequence], format:QtGui.QKeySequence.SequenceFormat=...) -> str: ...
    def matches(self, seq:Union[QtGui.QKeySequence, QtCore.QKeyCombination, QtGui.QKeySequence.StandardKey, str, int]) -> QtGui.QKeySequence.SequenceMatch: ...
    @staticmethod
    def mnemonic(text:str) -> QtGui.QKeySequence: ...
    def swap(self, other:Union[QtGui.QKeySequence, QtCore.QKeyCombination, QtGui.QKeySequence.StandardKey, str, int]) -> None: ...
    def toString(self, format:QtGui.QKeySequence.SequenceFormat=...) -> str: ...


class QLinearGradient(QtGui.QGradient):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QLinearGradient:QtGui.QLinearGradient) -> None: ...
    @overload
    def __init__(self, start:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], finalStop:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def __init__(self, xStart:float, yStart:float, xFinalStop:float, yFinalStop:float) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def finalStop(self) -> QtCore.QPointF: ...
    @overload
    def setFinalStop(self, stop:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def setFinalStop(self, x:float, y:float) -> None: ...
    @overload
    def setStart(self, start:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def setStart(self, x:float, y:float) -> None: ...
    def start(self) -> QtCore.QPointF: ...


class QMatrix2x2(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QMatrix2x2:QtGui.QMatrix2x2) -> None: ...
    @overload
    def __init__(self, arg__1:Iterable) -> None: ...

    def __call__(self, row:int, column:int) -> float: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, other:QtGui.QMatrix2x2) -> QtGui.QMatrix2x2: ...
    def __imul__(self, factor:float) -> QtGui.QMatrix2x2: ...
    def __isub__(self, other:QtGui.QMatrix2x2) -> QtGui.QMatrix2x2: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def data(self) -> float: ...
    def fill(self, value:float) -> None: ...
    def isIdentity(self) -> bool: ...
    def setToIdentity(self) -> None: ...
    def transposed(self) -> QtGui.QMatrix2x2: ...


class QMatrix2x3(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QMatrix2x3:QtGui.QMatrix2x3) -> None: ...
    @overload
    def __init__(self, arg__1:Iterable) -> None: ...

    def __call__(self, row:int, column:int) -> float: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, other:QtGui.QMatrix2x3) -> QtGui.QMatrix2x3: ...
    def __imul__(self, factor:float) -> QtGui.QMatrix2x3: ...
    def __isub__(self, other:QtGui.QMatrix2x3) -> QtGui.QMatrix2x3: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def data(self) -> float: ...
    def fill(self, value:float) -> None: ...
    def isIdentity(self) -> bool: ...
    def setToIdentity(self) -> None: ...
    def transposed(self) -> QtGui.QMatrix3x2: ...


class QMatrix2x4(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QMatrix2x4:QtGui.QMatrix2x4) -> None: ...
    @overload
    def __init__(self, arg__1:Iterable) -> None: ...

    def __call__(self, row:int, column:int) -> float: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, other:QtGui.QMatrix2x4) -> QtGui.QMatrix2x4: ...
    def __imul__(self, factor:float) -> QtGui.QMatrix2x4: ...
    def __isub__(self, other:QtGui.QMatrix2x4) -> QtGui.QMatrix2x4: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def data(self) -> float: ...
    def fill(self, value:float) -> None: ...
    def isIdentity(self) -> bool: ...
    def setToIdentity(self) -> None: ...
    def transposed(self) -> QtGui.QMatrix4x2: ...


class QMatrix3x2(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QMatrix3x2:QtGui.QMatrix3x2) -> None: ...
    @overload
    def __init__(self, arg__1:Iterable) -> None: ...

    def __call__(self, row:int, column:int) -> float: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, other:QtGui.QMatrix3x2) -> QtGui.QMatrix3x2: ...
    def __imul__(self, factor:float) -> QtGui.QMatrix3x2: ...
    def __isub__(self, other:QtGui.QMatrix3x2) -> QtGui.QMatrix3x2: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def data(self) -> float: ...
    def fill(self, value:float) -> None: ...
    def isIdentity(self) -> bool: ...
    def setToIdentity(self) -> None: ...
    def transposed(self) -> QtGui.QMatrix2x3: ...


class QMatrix3x3(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QMatrix3x3:QtGui.QMatrix3x3) -> None: ...
    @overload
    def __init__(self, arg__1:Iterable) -> None: ...

    def __call__(self, row:int, column:int) -> float: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, other:QtGui.QMatrix3x3) -> QtGui.QMatrix3x3: ...
    def __imul__(self, factor:float) -> QtGui.QMatrix3x3: ...
    def __isub__(self, other:QtGui.QMatrix3x3) -> QtGui.QMatrix3x3: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def data(self) -> float: ...
    def fill(self, value:float) -> None: ...
    def isIdentity(self) -> bool: ...
    def setToIdentity(self) -> None: ...
    def transposed(self) -> QtGui.QMatrix3x3: ...


class QMatrix3x4(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QMatrix3x4:QtGui.QMatrix3x4) -> None: ...
    @overload
    def __init__(self, arg__1:Iterable) -> None: ...

    def __call__(self, row:int, column:int) -> float: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, other:QtGui.QMatrix3x4) -> QtGui.QMatrix3x4: ...
    def __imul__(self, factor:float) -> QtGui.QMatrix3x4: ...
    def __isub__(self, other:QtGui.QMatrix3x4) -> QtGui.QMatrix3x4: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def data(self) -> float: ...
    def fill(self, value:float) -> None: ...
    def isIdentity(self) -> bool: ...
    def setToIdentity(self) -> None: ...
    def transposed(self) -> QtGui.QMatrix4x3: ...


class QMatrix4x2(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QMatrix4x2:QtGui.QMatrix4x2) -> None: ...
    @overload
    def __init__(self, arg__1:Iterable) -> None: ...

    def __call__(self, row:int, column:int) -> float: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, other:QtGui.QMatrix4x2) -> QtGui.QMatrix4x2: ...
    def __imul__(self, factor:float) -> QtGui.QMatrix4x2: ...
    def __isub__(self, other:QtGui.QMatrix4x2) -> QtGui.QMatrix4x2: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def data(self) -> float: ...
    def fill(self, value:float) -> None: ...
    def isIdentity(self) -> bool: ...
    def setToIdentity(self) -> None: ...
    def transposed(self) -> QtGui.QMatrix2x4: ...


class QMatrix4x3(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QMatrix4x3:QtGui.QMatrix4x3) -> None: ...
    @overload
    def __init__(self, arg__1:Iterable) -> None: ...

    def __call__(self, row:int, column:int) -> float: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, other:QtGui.QMatrix4x3) -> QtGui.QMatrix4x3: ...
    def __imul__(self, factor:float) -> QtGui.QMatrix4x3: ...
    def __isub__(self, other:QtGui.QMatrix4x3) -> QtGui.QMatrix4x3: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def data(self) -> float: ...
    def fill(self, value:float) -> None: ...
    def isIdentity(self) -> bool: ...
    def setToIdentity(self) -> None: ...
    def transposed(self) -> QtGui.QMatrix3x4: ...


class QMatrix4x4(Shiboken.Object):

    Identity                 : QMatrix4x4.Flag = ... # 0x0
    Translation              : QMatrix4x4.Flag = ... # 0x1
    Scale                    : QMatrix4x4.Flag = ... # 0x2
    Rotation2D               : QMatrix4x4.Flag = ... # 0x4
    Rotation                 : QMatrix4x4.Flag = ... # 0x8
    Perspective              : QMatrix4x4.Flag = ... # 0x10
    General                  : QMatrix4x4.Flag = ... # 0x1f

    class Flag(Enum):

        Identity                 : QMatrix4x4.Flag # 0x0
        Translation              : QMatrix4x4.Flag # 0x1
        Scale                    : QMatrix4x4.Flag # 0x2
        Rotation2D               : QMatrix4x4.Flag # 0x4
        Rotation                 : QMatrix4x4.Flag # 0x8
        Perspective              : QMatrix4x4.Flag # 0x10
        General                  : QMatrix4x4.Flag # 0x1f

    class Flags(object): ...


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, m11:float, m12:float, m13:float, m14:float, m21:float, m22:float, m23:float, m24:float, m31:float, m32:float, m33:float, m34:float, m41:float, m42:float, m43:float, m44:float) -> None: ...
    @overload
    def __init__(self, transform:QtGui.QTransform) -> None: ...
    @overload
    def __init__(self, values:Iterable) -> None: ...

    def __add__(self, m2:Union[QtGui.QMatrix4x4, QtGui.QTransform]) -> QtGui.QMatrix4x4: ...
    @staticmethod
    def __copy__() -> None: ...
    def __dummy(self, arg__1:Sequence[float]) -> None: ...
    def __iadd__(self, other:Union[QtGui.QMatrix4x4, QtGui.QTransform]) -> QtGui.QMatrix4x4: ...
    @overload
    def __imul__(self, factor:float) -> QtGui.QMatrix4x4: ...
    @overload
    def __imul__(self, other:Union[QtGui.QMatrix4x4, QtGui.QTransform]) -> QtGui.QMatrix4x4: ...
    def __isub__(self, other:Union[QtGui.QMatrix4x4, QtGui.QTransform]) -> QtGui.QMatrix4x4: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    @overload
    def __mul__(self, factor:float) -> QtGui.QMatrix4x4: ...
    @overload
    def __mul__(self, m2:Union[QtGui.QMatrix4x4, QtGui.QTransform]) -> QtGui.QMatrix4x4: ...
    @overload
    def __mul__(self, point:QtCore.QPoint) -> QtCore.QPoint: ...
    @overload
    def __mul__(self, point:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> QtCore.QPointF: ...
    def __neg__(self) -> QtGui.QMatrix4x4: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __sub__(self, m2:Union[QtGui.QMatrix4x4, QtGui.QTransform]) -> QtGui.QMatrix4x4: ...
    def column(self, index:int) -> QtGui.QVector4D: ...
    def copyDataTo(self) -> float: ...
    def data(self) -> List[float]: ...
    def determinant(self) -> float: ...
    def fill(self, value:float) -> None: ...
    def flags(self) -> QtGui.QMatrix4x4.Flags: ...
    def flipCoordinates(self) -> None: ...
    def frustum(self, left:float, right:float, bottom:float, top:float, nearPlane:float, farPlane:float) -> None: ...
    def inverted(self) -> Tuple[Tuple, bool]: ...
    def isAffine(self) -> bool: ...
    def isIdentity(self) -> bool: ...
    def lookAt(self, eye:QtGui.QVector3D, center:QtGui.QVector3D, up:QtGui.QVector3D) -> None: ...
    @overload
    def map(self, point:QtCore.QPoint) -> QtCore.QPoint: ...
    @overload
    def map(self, point:QtGui.QVector3D) -> QtGui.QVector3D: ...
    @overload
    def map(self, point:QtGui.QVector4D) -> QtGui.QVector4D: ...
    @overload
    def map(self, point:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> QtCore.QPointF: ...
    @overload
    def mapRect(self, rect:QtCore.QRect) -> QtCore.QRect: ...
    @overload
    def mapRect(self, rect:Union[QtCore.QRectF, QtCore.QRect]) -> QtCore.QRectF: ...
    def mapVector(self, vector:QtGui.QVector3D) -> QtGui.QVector3D: ...
    def normalMatrix(self) -> QtGui.QMatrix3x3: ...
    def optimize(self) -> None: ...
    @overload
    def ortho(self, left:float, right:float, bottom:float, top:float, nearPlane:float, farPlane:float) -> None: ...
    @overload
    def ortho(self, rect:QtCore.QRect) -> None: ...
    @overload
    def ortho(self, rect:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    def perspective(self, verticalAngle:float, aspectRatio:float, nearPlane:float, farPlane:float) -> None: ...
    def projectedRotate(self, angle:float, x:float, y:float, z:float) -> None: ...
    @overload
    def rotate(self, angle:float, vector:QtGui.QVector3D) -> None: ...
    @overload
    def rotate(self, angle:float, x:float, y:float, z:float=...) -> None: ...
    @overload
    def rotate(self, quaternion:QtGui.QQuaternion) -> None: ...
    def row(self, index:int) -> QtGui.QVector4D: ...
    @overload
    def scale(self, factor:float) -> None: ...
    @overload
    def scale(self, vector:QtGui.QVector3D) -> None: ...
    @overload
    def scale(self, x:float, y:float) -> None: ...
    @overload
    def scale(self, x:float, y:float, z:float) -> None: ...
    def setColumn(self, index:int, value:QtGui.QVector4D) -> None: ...
    def setRow(self, index:int, value:QtGui.QVector4D) -> None: ...
    def setToIdentity(self) -> None: ...
    @overload
    def toTransform(self) -> QtGui.QTransform: ...
    @overload
    def toTransform(self, distanceToPlane:float) -> QtGui.QTransform: ...
    @overload
    def translate(self, vector:QtGui.QVector3D) -> None: ...
    @overload
    def translate(self, x:float, y:float) -> None: ...
    @overload
    def translate(self, x:float, y:float, z:float) -> None: ...
    def transposed(self) -> QtGui.QMatrix4x4: ...
    @overload
    def viewport(self, left:float, bottom:float, width:float, height:float, nearPlane:float=..., farPlane:float=...) -> None: ...
    @overload
    def viewport(self, rect:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...


class QMouseEvent(QtGui.QSinglePointEvent):

    @overload
    def __init__(self, arg__1:QtGui.QMouseEvent) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, localPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], button:QtCore.Qt.MouseButton, buttons:QtCore.Qt.MouseButtons, modifiers:QtCore.Qt.KeyboardModifiers, device:QtGui.QPointingDevice=...) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, localPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], globalPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], button:QtCore.Qt.MouseButton, buttons:QtCore.Qt.MouseButtons, modifiers:QtCore.Qt.KeyboardModifiers, device:QtGui.QPointingDevice=...) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, localPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], scenePos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], globalPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], button:QtCore.Qt.MouseButton, buttons:QtCore.Qt.MouseButtons, modifiers:QtCore.Qt.KeyboardModifiers, device:QtGui.QPointingDevice=...) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, localPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], scenePos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], globalPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], button:QtCore.Qt.MouseButton, buttons:QtCore.Qt.MouseButtons, modifiers:QtCore.Qt.KeyboardModifiers, source:QtCore.Qt.MouseEventSource, device:QtGui.QPointingDevice=...) -> None: ...

    def clone(self) -> QtGui.QMouseEvent: ...
    def flags(self) -> QtCore.Qt.MouseEventFlags: ...
    def globalPos(self) -> QtCore.QPoint: ...
    def globalX(self) -> int: ...
    def globalY(self) -> int: ...
    def localPos(self) -> QtCore.QPointF: ...
    def pos(self) -> QtCore.QPoint: ...
    def screenPos(self) -> QtCore.QPointF: ...
    def source(self) -> QtCore.Qt.MouseEventSource: ...
    def windowPos(self) -> QtCore.QPointF: ...
    def x(self) -> int: ...
    def y(self) -> int: ...


class QMoveEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QMoveEvent) -> None: ...
    @overload
    def __init__(self, pos:QtCore.QPoint, oldPos:QtCore.QPoint) -> None: ...

    def clone(self) -> QtGui.QMoveEvent: ...
    def oldPos(self) -> QtCore.QPoint: ...
    def pos(self) -> QtCore.QPoint: ...


class QMovie(QtCore.QObject):

    CacheNone                : QMovie.CacheMode = ... # 0x0
    CacheAll                 : QMovie.CacheMode = ... # 0x1
    NotRunning               : QMovie.MovieState = ... # 0x0
    Paused                   : QMovie.MovieState = ... # 0x1
    Running                  : QMovie.MovieState = ... # 0x2

    class CacheMode(Enum):

        CacheNone                : QMovie.CacheMode # 0x0
        CacheAll                 : QMovie.CacheMode # 0x1

    class MovieState(Enum):

        NotRunning               : QMovie.MovieState # 0x0
        Paused                   : QMovie.MovieState # 0x1
        Running                  : QMovie.MovieState # 0x2


    @overload
    def __init__(self, device:QtCore.QIODevice, format:Union[QtCore.QByteArray, bytes]=..., parent:Optional[QtCore.QObject]=...) -> None: ...
    @overload
    def __init__(self, fileName:str, format:Union[QtCore.QByteArray, bytes]=..., parent:Optional[QtCore.QObject]=...) -> None: ...
    @overload
    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...

    def backgroundColor(self) -> QtGui.QColor: ...
    def cacheMode(self) -> QtGui.QMovie.CacheMode: ...
    def currentFrameNumber(self) -> int: ...
    def currentImage(self) -> QtGui.QImage: ...
    def currentPixmap(self) -> QtGui.QPixmap: ...
    def device(self) -> QtCore.QIODevice: ...
    def fileName(self) -> str: ...
    def format(self) -> QtCore.QByteArray: ...
    def frameCount(self) -> int: ...
    def frameRect(self) -> QtCore.QRect: ...
    def isValid(self) -> bool: ...
    def jumpToFrame(self, frameNumber:int) -> bool: ...
    def jumpToNextFrame(self) -> bool: ...
    def lastError(self) -> QtGui.QImageReader.ImageReaderError: ...
    def lastErrorString(self) -> str: ...
    def loopCount(self) -> int: ...
    def nextFrameDelay(self) -> int: ...
    def scaledSize(self) -> QtCore.QSize: ...
    def setBackgroundColor(self, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    def setCacheMode(self, mode:QtGui.QMovie.CacheMode) -> None: ...
    def setDevice(self, device:QtCore.QIODevice) -> None: ...
    def setFileName(self, fileName:str) -> None: ...
    def setFormat(self, format:Union[QtCore.QByteArray, bytes]) -> None: ...
    def setPaused(self, paused:bool) -> None: ...
    def setScaledSize(self, size:QtCore.QSize) -> None: ...
    def setSpeed(self, percentSpeed:int) -> None: ...
    def speed(self) -> int: ...
    def start(self) -> None: ...
    def state(self) -> QtGui.QMovie.MovieState: ...
    def stop(self) -> None: ...
    @staticmethod
    def supportedFormats() -> List[QtCore.QByteArray]: ...


class QNativeGestureEvent(QtGui.QSinglePointEvent):

    @overload
    def __init__(self, arg__1:QtGui.QNativeGestureEvent) -> None: ...
    @overload
    def __init__(self, type:QtCore.Qt.NativeGestureType, dev:QtGui.QPointingDevice, fingerCount:int, localPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], scenePos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], globalPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], value:float, delta:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], sequenceId:int=...) -> None: ...
    @overload
    def __init__(self, type:QtCore.Qt.NativeGestureType, dev:QtGui.QPointingDevice, localPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], scenePos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], globalPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], value:float, sequenceId:int, intArgument:int) -> None: ...

    def clone(self) -> QtGui.QNativeGestureEvent: ...
    def delta(self) -> QtCore.QPointF: ...
    def fingerCount(self) -> int: ...
    def gestureType(self) -> QtCore.Qt.NativeGestureType: ...
    def globalPos(self) -> QtCore.QPoint: ...
    def localPos(self) -> QtCore.QPointF: ...
    def pos(self) -> QtCore.QPoint: ...
    def screenPos(self) -> QtCore.QPointF: ...
    def value(self) -> float: ...
    def windowPos(self) -> QtCore.QPointF: ...


class QOffscreenSurface(QtCore.QObject, QtGui.QSurface):

    def __init__(self, screen:Optional[QtGui.QScreen]=..., parent:Optional[QtCore.QObject]=...) -> None: ...

    def create(self) -> None: ...
    def destroy(self) -> None: ...
    def format(self) -> QtGui.QSurfaceFormat: ...
    def isValid(self) -> bool: ...
    def requestedFormat(self) -> QtGui.QSurfaceFormat: ...
    def resolveInterface(self, name:bytes, revision:int) -> int: ...
    def screen(self) -> QtGui.QScreen: ...
    def setFormat(self, format:Union[QtGui.QSurfaceFormat, QtGui.QSurfaceFormat.FormatOptions]) -> None: ...
    def setScreen(self, screen:QtGui.QScreen) -> None: ...
    def size(self) -> QtCore.QSize: ...
    def surfaceHandle(self) -> int: ...
    def surfaceType(self) -> QtGui.QSurface.SurfaceType: ...


class QOpenGLContext(QtCore.QObject):

    LibGL                    : QOpenGLContext.OpenGLModuleType = ... # 0x0
    LibGLES                  : QOpenGLContext.OpenGLModuleType = ... # 0x1

    class OpenGLModuleType(Enum):

        LibGL                    : QOpenGLContext.OpenGLModuleType # 0x0
        LibGLES                  : QOpenGLContext.OpenGLModuleType # 0x1


    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...

    @staticmethod
    def areSharing(first:QtGui.QOpenGLContext, second:QtGui.QOpenGLContext) -> bool: ...
    def create(self) -> bool: ...
    @staticmethod
    def currentContext() -> QtGui.QOpenGLContext: ...
    def defaultFramebufferObject(self) -> int: ...
    def doneCurrent(self) -> None: ...
    def extensions(self) -> Set[QtCore.QByteArray]: ...
    def extraFunctions(self) -> QtGui.QOpenGLExtraFunctions: ...
    def format(self) -> QtGui.QSurfaceFormat: ...
    def functions(self) -> QtGui.QOpenGLFunctions: ...
    @staticmethod
    def globalShareContext() -> QtGui.QOpenGLContext: ...
    def hasExtension(self, extension:Union[QtCore.QByteArray, bytes]) -> bool: ...
    def isOpenGLES(self) -> bool: ...
    def isValid(self) -> bool: ...
    def makeCurrent(self, surface:QtGui.QSurface) -> bool: ...
    @staticmethod
    def openGLModuleType() -> QtGui.QOpenGLContext.OpenGLModuleType: ...
    def resolveInterface(self, name:bytes, revision:int) -> int: ...
    def screen(self) -> QtGui.QScreen: ...
    def setFormat(self, format:Union[QtGui.QSurfaceFormat, QtGui.QSurfaceFormat.FormatOptions]) -> None: ...
    def setScreen(self, screen:QtGui.QScreen) -> None: ...
    def setShareContext(self, shareContext:QtGui.QOpenGLContext) -> None: ...
    def shareContext(self) -> QtGui.QOpenGLContext: ...
    def shareGroup(self) -> QtGui.QOpenGLContextGroup: ...
    @staticmethod
    def supportsThreadedOpenGL() -> bool: ...
    def surface(self) -> QtGui.QSurface: ...
    def swapBuffers(self, surface:QtGui.QSurface) -> None: ...


class QOpenGLContextGroup(QtCore.QObject):
    @staticmethod
    def currentContextGroup() -> QtGui.QOpenGLContextGroup: ...
    def shares(self) -> List[QtGui.QOpenGLContext]: ...


class QOpenGLExtraFunctions(QtGui.QOpenGLFunctions):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, context:QtGui.QOpenGLContext) -> None: ...

    def glActiveShaderProgram(self, pipeline:int, program:int) -> None: ...
    def glBeginQuery(self, target:int, id:int) -> None: ...
    def glBeginTransformFeedback(self, primitiveMode:int) -> None: ...
    def glBindBufferBase(self, target:int, index:int, buffer:int) -> None: ...
    def glBindImageTexture(self, unit:int, texture:int, level:int, layered:int, layer:int, access:int, format:int) -> None: ...
    def glBindProgramPipeline(self, pipeline:int) -> None: ...
    def glBindSampler(self, unit:int, sampler:int) -> None: ...
    def glBindTransformFeedback(self, target:int, id:int) -> None: ...
    def glBindVertexArray(self, array:int) -> None: ...
    def glBlendBarrier(self) -> None: ...
    def glBlendEquationSeparatei(self, buf:int, modeRGB:int, modeAlpha:int) -> None: ...
    def glBlendEquationi(self, buf:int, mode:int) -> None: ...
    def glBlendFuncSeparatei(self, buf:int, srcRGB:int, dstRGB:int, srcAlpha:int, dstAlpha:int) -> None: ...
    def glBlendFunci(self, buf:int, src:int, dst:int) -> None: ...
    def glBlitFramebuffer(self, srcX0:int, srcY0:int, srcX1:int, srcY1:int, dstX0:int, dstY0:int, dstX1:int, dstY1:int, mask:int, filter:int) -> None: ...
    def glClearBufferfi(self, buffer:int, drawbuffer:int, depth:float, stencil:int) -> None: ...
    def glClearBufferfv(self, buffer:int, drawbuffer:int, value:Sequence[float]) -> None: ...
    def glClearBufferiv(self, buffer:int, drawbuffer:int, value:Sequence[int]) -> None: ...
    def glClearBufferuiv(self, buffer:int, drawbuffer:int, value:Sequence[int]) -> None: ...
    def glColorMaski(self, index:int, r:int, g:int, b:int, a:int) -> None: ...
    def glCompressedTexImage3D(self, target:int, level:int, internalformat:int, width:int, height:int, depth:int, border:int, imageSize:int, data:int) -> None: ...
    def glCompressedTexSubImage3D(self, target:int, level:int, xoffset:int, yoffset:int, zoffset:int, width:int, height:int, depth:int, format:int, imageSize:int, data:int) -> None: ...
    def glCopyImageSubData(self, srcName:int, srcTarget:int, srcLevel:int, srcX:int, srcY:int, srcZ:int, dstName:int, dstTarget:int, dstLevel:int, dstX:int, dstY:int, dstZ:int, srcWidth:int, srcHeight:int, srcDepth:int) -> None: ...
    def glCopyTexSubImage3D(self, target:int, level:int, xoffset:int, yoffset:int, zoffset:int, x:int, y:int, width:int, height:int) -> None: ...
    def glDebugMessageControl(self, source:int, type:int, severity:int, count:int, ids:Sequence[int], enabled:int) -> None: ...
    def glDebugMessageInsert(self, source:int, type:int, id:int, severity:int, length:int, buf:bytes) -> None: ...
    def glDeleteProgramPipelines(self, n:int, pipelines:Sequence[int]) -> None: ...
    def glDeleteQueries(self, n:int, ids:Sequence[int]) -> None: ...
    def glDeleteSamplers(self, count:int, samplers:Sequence[int]) -> None: ...
    def glDeleteTransformFeedbacks(self, n:int, ids:Sequence[int]) -> None: ...
    def glDeleteVertexArrays(self, n:int, arrays:Sequence[int]) -> None: ...
    def glDisablei(self, target:int, index:int) -> None: ...
    def glDispatchCompute(self, num_groups_x:int, num_groups_y:int, num_groups_z:int) -> None: ...
    def glDrawArraysIndirect(self, mode:int, indirect:int) -> None: ...
    def glDrawArraysInstanced(self, mode:int, first:int, count:int, instancecount:int) -> None: ...
    def glDrawBuffers(self, n:int, bufs:Sequence[int]) -> None: ...
    def glDrawElementsBaseVertex(self, mode:int, count:int, type:int, indices:int, basevertex:int) -> None: ...
    def glDrawElementsIndirect(self, mode:int, type:int, indirect:int) -> None: ...
    def glDrawElementsInstanced(self, mode:int, count:int, type:int, indices:int, instancecount:int) -> None: ...
    def glDrawElementsInstancedBaseVertex(self, mode:int, count:int, type:int, indices:int, instancecount:int, basevertex:int) -> None: ...
    def glDrawRangeElements(self, mode:int, start:int, end:int, count:int, type:int, indices:int) -> None: ...
    def glDrawRangeElementsBaseVertex(self, mode:int, start:int, end:int, count:int, type:int, indices:int, basevertex:int) -> None: ...
    def glEnablei(self, target:int, index:int) -> None: ...
    def glEndQuery(self, target:int) -> None: ...
    def glFramebufferParameteri(self, target:int, pname:int, param:int) -> None: ...
    def glFramebufferTexture(self, target:int, attachment:int, texture:int, level:int) -> None: ...
    def glFramebufferTextureLayer(self, target:int, attachment:int, texture:int, level:int, layer:int) -> None: ...
    def glGenProgramPipelines(self, n:int, pipelines:Sequence[int]) -> None: ...
    def glGenQueries(self, n:int, ids:Sequence[int]) -> None: ...
    def glGenSamplers(self, count:int, samplers:Sequence[int]) -> None: ...
    def glGenTransformFeedbacks(self, n:int, ids:Sequence[int]) -> None: ...
    def glGenVertexArrays(self, n:int, arrays:Sequence[int]) -> None: ...
    def glGetActiveUniformBlockiv(self, program:int, uniformBlockIndex:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetActiveUniformsiv(self, program:int, uniformCount:int, uniformIndices:Sequence[int], pname:int, params:Sequence[int]) -> None: ...
    def glGetFragDataLocation(self, program:int, name:bytes) -> int: ...
    def glGetFramebufferParameteriv(self, target:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetGraphicsResetStatus(self) -> int: ...
    def glGetIntegeri_v(self, target:int, index:int, data:Sequence[int]) -> None: ...
    def glGetInternalformativ(self, target:int, internalformat:int, pname:int, bufSize:int, params:Sequence[int]) -> None: ...
    def glGetMultisamplefv(self, pname:int, index:int, val:Sequence[float]) -> None: ...
    def glGetProgramInterfaceiv(self, program:int, programInterface:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetProgramPipelineiv(self, pipeline:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetProgramResourceIndex(self, program:int, programInterface:int, name:bytes) -> int: ...
    def glGetProgramResourceLocation(self, program:int, programInterface:int, name:bytes) -> int: ...
    def glGetProgramResourceiv(self, program:int, programInterface:int, index:int, propCount:int, props:Sequence[int], bufSize:int, length:Sequence[int], params:Sequence[int]) -> None: ...
    def glGetQueryObjectuiv(self, id:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetQueryiv(self, target:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetSamplerParameterIiv(self, sampler:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetSamplerParameterIuiv(self, sampler:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetSamplerParameterfv(self, sampler:int, pname:int, params:Sequence[float]) -> None: ...
    def glGetSamplerParameteriv(self, sampler:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetStringi(self, name:int, index:int) -> str: ...
    def glGetTexLevelParameterfv(self, target:int, level:int, pname:int, params:Sequence[float]) -> None: ...
    def glGetTexLevelParameteriv(self, target:int, level:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetTexParameterIiv(self, target:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetTexParameterIuiv(self, target:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetUniformBlockIndex(self, program:int, uniformBlockName:bytes) -> int: ...
    def glGetUniformuiv(self, program:int, location:int, params:Sequence[int]) -> None: ...
    def glGetVertexAttribIiv(self, index:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetVertexAttribIuiv(self, index:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetnUniformfv(self, program:int, location:int, bufSize:int) -> float: ...
    def glGetnUniformiv(self, program:int, location:int, bufSize:int, params:Sequence[int]) -> None: ...
    def glGetnUniformuiv(self, program:int, location:int, bufSize:int, params:Sequence[int]) -> None: ...
    def glInvalidateFramebuffer(self, target:int, numAttachments:int, attachments:Sequence[int]) -> None: ...
    def glInvalidateSubFramebuffer(self, target:int, numAttachments:int, attachments:Sequence[int], x:int, y:int, width:int, height:int) -> None: ...
    def glIsEnabledi(self, target:int, index:int) -> int: ...
    def glIsProgramPipeline(self, pipeline:int) -> int: ...
    def glIsQuery(self, id:int) -> int: ...
    def glIsSampler(self, sampler:int) -> int: ...
    def glIsTransformFeedback(self, id:int) -> int: ...
    def glIsVertexArray(self, array:int) -> int: ...
    def glMemoryBarrier(self, barriers:int) -> None: ...
    def glMemoryBarrierByRegion(self, barriers:int) -> None: ...
    def glMinSampleShading(self, value:float) -> None: ...
    def glObjectLabel(self, identifier:int, name:int, length:int, label:bytes) -> None: ...
    def glObjectPtrLabel(self, ptr:int, length:int, label:bytes) -> None: ...
    def glPatchParameteri(self, pname:int, value:int) -> None: ...
    def glPopDebugGroup(self) -> None: ...
    def glPrimitiveBoundingBox(self, minX:float, minY:float, minZ:float, minW:float, maxX:float, maxY:float, maxZ:float, maxW:float) -> None: ...
    def glProgramBinary(self, program:int, binaryFormat:int, binary:int, length:int) -> None: ...
    def glProgramParameteri(self, program:int, pname:int, value:int) -> None: ...
    def glProgramUniform1f(self, program:int, location:int, v0:float) -> None: ...
    def glProgramUniform1fv(self, program:int, location:int, count:int, value:Sequence[float]) -> None: ...
    def glProgramUniform1i(self, program:int, location:int, v0:int) -> None: ...
    def glProgramUniform1iv(self, program:int, location:int, count:int, value:Sequence[int]) -> None: ...
    def glProgramUniform1ui(self, program:int, location:int, v0:int) -> None: ...
    def glProgramUniform1uiv(self, program:int, location:int, count:int, value:Sequence[int]) -> None: ...
    def glProgramUniform2f(self, program:int, location:int, v0:float, v1:float) -> None: ...
    def glProgramUniform2fv(self, program:int, location:int, count:int, value:Sequence[float]) -> None: ...
    def glProgramUniform2i(self, program:int, location:int, v0:int, v1:int) -> None: ...
    def glProgramUniform2iv(self, program:int, location:int, count:int, value:Sequence[int]) -> None: ...
    def glProgramUniform2ui(self, program:int, location:int, v0:int, v1:int) -> None: ...
    def glProgramUniform2uiv(self, program:int, location:int, count:int, value:Sequence[int]) -> None: ...
    def glProgramUniform3f(self, program:int, location:int, v0:float, v1:float, v2:float) -> None: ...
    def glProgramUniform3fv(self, program:int, location:int, count:int, value:Sequence[float]) -> None: ...
    def glProgramUniform3i(self, program:int, location:int, v0:int, v1:int, v2:int) -> None: ...
    def glProgramUniform3iv(self, program:int, location:int, count:int, value:Sequence[int]) -> None: ...
    def glProgramUniform3ui(self, program:int, location:int, v0:int, v1:int, v2:int) -> None: ...
    def glProgramUniform3uiv(self, program:int, location:int, count:int, value:Sequence[int]) -> None: ...
    def glProgramUniform4f(self, program:int, location:int, v0:float, v1:float, v2:float, v3:float) -> None: ...
    def glProgramUniform4fv(self, program:int, location:int, count:int, value:Sequence[float]) -> None: ...
    def glProgramUniform4i(self, program:int, location:int, v0:int, v1:int, v2:int, v3:int) -> None: ...
    def glProgramUniform4iv(self, program:int, location:int, count:int, value:Sequence[int]) -> None: ...
    def glProgramUniform4ui(self, program:int, location:int, v0:int, v1:int, v2:int, v3:int) -> None: ...
    def glProgramUniform4uiv(self, program:int, location:int, count:int, value:Sequence[int]) -> None: ...
    def glProgramUniformMatrix2fv(self, program:int, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glProgramUniformMatrix2x3fv(self, program:int, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glProgramUniformMatrix2x4fv(self, program:int, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glProgramUniformMatrix3fv(self, program:int, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glProgramUniformMatrix3x2fv(self, program:int, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glProgramUniformMatrix3x4fv(self, program:int, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glProgramUniformMatrix4fv(self, program:int, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glProgramUniformMatrix4x2fv(self, program:int, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glProgramUniformMatrix4x3fv(self, program:int, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glPushDebugGroup(self, source:int, id:int, length:int, message:bytes) -> None: ...
    def glReadBuffer(self, mode:int) -> None: ...
    def glReadnPixels(self, x:int, y:int, width:int, height:int, format:int, type:int, bufSize:int, data:int) -> None: ...
    def glRenderbufferStorageMultisample(self, target:int, samples:int, internalformat:int, width:int, height:int) -> None: ...
    def glSampleMaski(self, maskNumber:int, mask:int) -> None: ...
    def glSamplerParameterIiv(self, sampler:int, pname:int, param:Sequence[int]) -> None: ...
    def glSamplerParameterIuiv(self, sampler:int, pname:int, param:Sequence[int]) -> None: ...
    def glSamplerParameterf(self, sampler:int, pname:int, param:float) -> None: ...
    def glSamplerParameterfv(self, sampler:int, pname:int, param:Sequence[float]) -> None: ...
    def glSamplerParameteri(self, sampler:int, pname:int, param:int) -> None: ...
    def glSamplerParameteriv(self, sampler:int, pname:int, param:Sequence[int]) -> None: ...
    def glTexBuffer(self, target:int, internalformat:int, buffer:int) -> None: ...
    def glTexImage3D(self, target:int, level:int, internalformat:int, width:int, height:int, depth:int, border:int, format:int, type:int, pixels:int) -> None: ...
    def glTexParameterIiv(self, target:int, pname:int, params:Sequence[int]) -> None: ...
    def glTexParameterIuiv(self, target:int, pname:int, params:Sequence[int]) -> None: ...
    def glTexStorage2D(self, target:int, levels:int, internalformat:int, width:int, height:int) -> None: ...
    def glTexStorage2DMultisample(self, target:int, samples:int, internalformat:int, width:int, height:int, fixedsamplelocations:int) -> None: ...
    def glTexStorage3D(self, target:int, levels:int, internalformat:int, width:int, height:int, depth:int) -> None: ...
    def glTexStorage3DMultisample(self, target:int, samples:int, internalformat:int, width:int, height:int, depth:int, fixedsamplelocations:int) -> None: ...
    def glTexSubImage3D(self, target:int, level:int, xoffset:int, yoffset:int, zoffset:int, width:int, height:int, depth:int, format:int, type:int, pixels:int) -> None: ...
    def glUniform1ui(self, location:int, v0:int) -> None: ...
    def glUniform1uiv(self, location:int, count:int, value:Sequence[int]) -> None: ...
    def glUniform2ui(self, location:int, v0:int, v1:int) -> None: ...
    def glUniform2uiv(self, location:int, count:int, value:Sequence[int]) -> None: ...
    def glUniform3ui(self, location:int, v0:int, v1:int, v2:int) -> None: ...
    def glUniform3uiv(self, location:int, count:int, value:Sequence[int]) -> None: ...
    def glUniform4ui(self, location:int, v0:int, v1:int, v2:int, v3:int) -> None: ...
    def glUniform4uiv(self, location:int, count:int, value:Sequence[int]) -> None: ...
    def glUniformBlockBinding(self, program:int, uniformBlockIndex:int, uniformBlockBinding:int) -> None: ...
    def glUniformMatrix2x3fv(self, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glUniformMatrix2x4fv(self, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glUniformMatrix3x2fv(self, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glUniformMatrix3x4fv(self, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glUniformMatrix4x2fv(self, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glUniformMatrix4x3fv(self, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glUnmapBuffer(self, target:int) -> int: ...
    def glUseProgramStages(self, pipeline:int, stages:int, program:int) -> None: ...
    def glValidateProgramPipeline(self, pipeline:int) -> None: ...
    def glVertexAttribBinding(self, attribindex:int, bindingindex:int) -> None: ...
    def glVertexAttribDivisor(self, index:int, divisor:int) -> None: ...
    def glVertexAttribFormat(self, attribindex:int, size:int, type:int, normalized:int, relativeoffset:int) -> None: ...
    def glVertexAttribI4i(self, index:int, x:int, y:int, z:int, w:int) -> None: ...
    def glVertexAttribI4iv(self, index:int, v:Sequence[int]) -> None: ...
    def glVertexAttribI4ui(self, index:int, x:int, y:int, z:int, w:int) -> None: ...
    def glVertexAttribI4uiv(self, index:int, v:Sequence[int]) -> None: ...
    def glVertexAttribIFormat(self, attribindex:int, size:int, type:int, relativeoffset:int) -> None: ...
    def glVertexAttribIPointer(self, index:int, size:int, type:int, stride:int, pointer:int) -> None: ...
    def glVertexBindingDivisor(self, bindingindex:int, divisor:int) -> None: ...


class QOpenGLFunctions(Shiboken.Object):

    Multitexture             : QOpenGLFunctions.OpenGLFeature = ... # 0x1
    Shaders                  : QOpenGLFunctions.OpenGLFeature = ... # 0x2
    Buffers                  : QOpenGLFunctions.OpenGLFeature = ... # 0x4
    Framebuffers             : QOpenGLFunctions.OpenGLFeature = ... # 0x8
    BlendColor               : QOpenGLFunctions.OpenGLFeature = ... # 0x10
    BlendEquation            : QOpenGLFunctions.OpenGLFeature = ... # 0x20
    BlendEquationSeparate    : QOpenGLFunctions.OpenGLFeature = ... # 0x40
    BlendFuncSeparate        : QOpenGLFunctions.OpenGLFeature = ... # 0x80
    BlendSubtract            : QOpenGLFunctions.OpenGLFeature = ... # 0x100
    CompressedTextures       : QOpenGLFunctions.OpenGLFeature = ... # 0x200
    Multisample              : QOpenGLFunctions.OpenGLFeature = ... # 0x400
    StencilSeparate          : QOpenGLFunctions.OpenGLFeature = ... # 0x800
    NPOTTextures             : QOpenGLFunctions.OpenGLFeature = ... # 0x1000
    NPOTTextureRepeat        : QOpenGLFunctions.OpenGLFeature = ... # 0x2000
    FixedFunctionPipeline    : QOpenGLFunctions.OpenGLFeature = ... # 0x4000
    TextureRGFormats         : QOpenGLFunctions.OpenGLFeature = ... # 0x8000
    MultipleRenderTargets    : QOpenGLFunctions.OpenGLFeature = ... # 0x10000
    BlendEquationAdvanced    : QOpenGLFunctions.OpenGLFeature = ... # 0x20000

    class OpenGLFeature(Enum):

        Multitexture             : QOpenGLFunctions.OpenGLFeature # 0x1
        Shaders                  : QOpenGLFunctions.OpenGLFeature # 0x2
        Buffers                  : QOpenGLFunctions.OpenGLFeature # 0x4
        Framebuffers             : QOpenGLFunctions.OpenGLFeature # 0x8
        BlendColor               : QOpenGLFunctions.OpenGLFeature # 0x10
        BlendEquation            : QOpenGLFunctions.OpenGLFeature # 0x20
        BlendEquationSeparate    : QOpenGLFunctions.OpenGLFeature # 0x40
        BlendFuncSeparate        : QOpenGLFunctions.OpenGLFeature # 0x80
        BlendSubtract            : QOpenGLFunctions.OpenGLFeature # 0x100
        CompressedTextures       : QOpenGLFunctions.OpenGLFeature # 0x200
        Multisample              : QOpenGLFunctions.OpenGLFeature # 0x400
        StencilSeparate          : QOpenGLFunctions.OpenGLFeature # 0x800
        NPOTTextures             : QOpenGLFunctions.OpenGLFeature # 0x1000
        NPOTTextureRepeat        : QOpenGLFunctions.OpenGLFeature # 0x2000
        FixedFunctionPipeline    : QOpenGLFunctions.OpenGLFeature # 0x4000
        TextureRGFormats         : QOpenGLFunctions.OpenGLFeature # 0x8000
        MultipleRenderTargets    : QOpenGLFunctions.OpenGLFeature # 0x10000
        BlendEquationAdvanced    : QOpenGLFunctions.OpenGLFeature # 0x20000

    class OpenGLFeatures(object): ...


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, context:QtGui.QOpenGLContext) -> None: ...

    def glActiveTexture(self, texture:int) -> None: ...
    def glAttachShader(self, program:int, shader:int) -> None: ...
    def glBindAttribLocation(self, program:int, index:int, name:bytes) -> None: ...
    def glBindBuffer(self, target:int, buffer:int) -> None: ...
    def glBindFramebuffer(self, target:int, framebuffer:int) -> None: ...
    def glBindRenderbuffer(self, target:int, renderbuffer:int) -> None: ...
    def glBindTexture(self, target:int, texture:int) -> None: ...
    def glBlendColor(self, red:float, green:float, blue:float, alpha:float) -> None: ...
    def glBlendEquation(self, mode:int) -> None: ...
    def glBlendEquationSeparate(self, modeRGB:int, modeAlpha:int) -> None: ...
    def glBlendFunc(self, sfactor:int, dfactor:int) -> None: ...
    def glBlendFuncSeparate(self, srcRGB:int, dstRGB:int, srcAlpha:int, dstAlpha:int) -> None: ...
    def glCheckFramebufferStatus(self, target:int) -> int: ...
    def glClear(self, mask:int) -> None: ...
    def glClearColor(self, red:float, green:float, blue:float, alpha:float) -> None: ...
    def glClearDepthf(self, depth:float) -> None: ...
    def glClearStencil(self, s:int) -> None: ...
    def glColorMask(self, red:int, green:int, blue:int, alpha:int) -> None: ...
    def glCompileShader(self, shader:int) -> None: ...
    def glCompressedTexImage2D(self, target:int, level:int, internalformat:int, width:int, height:int, border:int, imageSize:int, data:int) -> None: ...
    def glCompressedTexSubImage2D(self, target:int, level:int, xoffset:int, yoffset:int, width:int, height:int, format:int, imageSize:int, data:int) -> None: ...
    def glCopyTexImage2D(self, target:int, level:int, internalformat:int, x:int, y:int, width:int, height:int, border:int) -> None: ...
    def glCopyTexSubImage2D(self, target:int, level:int, xoffset:int, yoffset:int, x:int, y:int, width:int, height:int) -> None: ...
    def glCreateProgram(self) -> int: ...
    def glCreateShader(self, type:int) -> int: ...
    def glCullFace(self, mode:int) -> None: ...
    def glDeleteBuffers(self, n:int, buffers:Sequence[int]) -> None: ...
    def glDeleteFramebuffers(self, n:int, framebuffers:Sequence[int]) -> None: ...
    def glDeleteProgram(self, program:int) -> None: ...
    def glDeleteRenderbuffers(self, n:int, renderbuffers:Sequence[int]) -> None: ...
    def glDeleteShader(self, shader:int) -> None: ...
    def glDeleteTextures(self, n:int, textures:Sequence[int]) -> None: ...
    def glDepthFunc(self, func:int) -> None: ...
    def glDepthMask(self, flag:int) -> None: ...
    def glDepthRangef(self, zNear:float, zFar:float) -> None: ...
    def glDetachShader(self, program:int, shader:int) -> None: ...
    def glDisable(self, cap:int) -> None: ...
    def glDisableVertexAttribArray(self, index:int) -> None: ...
    def glDrawArrays(self, mode:int, first:int, count:int) -> None: ...
    def glDrawElements(self, mode:int, count:int, type:int, indices:int) -> None: ...
    def glEnable(self, cap:int) -> None: ...
    def glEnableVertexAttribArray(self, index:int) -> None: ...
    def glFinish(self) -> None: ...
    def glFlush(self) -> None: ...
    def glFramebufferRenderbuffer(self, target:int, attachment:int, renderbuffertarget:int, renderbuffer:int) -> None: ...
    def glFramebufferTexture2D(self, target:int, attachment:int, textarget:int, texture:int, level:int) -> None: ...
    def glFrontFace(self, mode:int) -> None: ...
    def glGenBuffers(self, n:int, buffers:Sequence[int]) -> None: ...
    def glGenFramebuffers(self, n:int, framebuffers:Sequence[int]) -> None: ...
    def glGenRenderbuffers(self, n:int, renderbuffers:Sequence[int]) -> None: ...
    def glGenTextures(self, n:int, textures:Sequence[int]) -> None: ...
    def glGenerateMipmap(self, target:int) -> None: ...
    def glGetAttachedShaders(self, program:int, maxcount:int, count:Sequence[int], shaders:Sequence[int]) -> None: ...
    def glGetAttribLocation(self, program:int, name:bytes) -> int: ...
    def glGetBufferParameteriv(self, target:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetError(self) -> int: ...
    def glGetFloatv(self, pname:int, params:Sequence[float]) -> None: ...
    def glGetFramebufferAttachmentParameteriv(self, target:int, attachment:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetIntegerv(self, pname:int, params:Sequence[int]) -> None: ...
    def glGetProgramiv(self, program:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetRenderbufferParameteriv(self, target:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetShaderPrecisionFormat(self, shadertype:int, precisiontype:int, range:Sequence[int], precision:Sequence[int]) -> None: ...
    def glGetShaderSource(self, shader:int) -> bytes: ...
    def glGetShaderiv(self, shader:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetString(self, name:int) -> str: ...
    def glGetTexParameterfv(self, target:int, pname:int, params:Sequence[float]) -> None: ...
    def glGetTexParameteriv(self, target:int, pname:int, params:Sequence[int]) -> None: ...
    def glGetUniformLocation(self, program:int, name:bytes) -> int: ...
    def glGetUniformfv(self, program:int, location:int, params:Sequence[float]) -> None: ...
    def glGetUniformiv(self, program:int, location:int, params:Sequence[int]) -> None: ...
    def glGetVertexAttribfv(self, index:int, pname:int, params:Sequence[float]) -> None: ...
    def glGetVertexAttribiv(self, index:int, pname:int, params:Sequence[int]) -> None: ...
    def glHint(self, target:int, mode:int) -> None: ...
    def glIsBuffer(self, buffer:int) -> int: ...
    def glIsEnabled(self, cap:int) -> int: ...
    def glIsFramebuffer(self, framebuffer:int) -> int: ...
    def glIsProgram(self, program:int) -> int: ...
    def glIsRenderbuffer(self, renderbuffer:int) -> int: ...
    def glIsShader(self, shader:int) -> int: ...
    def glIsTexture(self, texture:int) -> int: ...
    def glLineWidth(self, width:float) -> None: ...
    def glLinkProgram(self, program:int) -> None: ...
    def glPixelStorei(self, pname:int, param:int) -> None: ...
    def glPolygonOffset(self, factor:float, units:float) -> None: ...
    def glReadPixels(self, x:int, y:int, width:int, height:int, format:int, type:int, pixels:int) -> None: ...
    def glReleaseShaderCompiler(self) -> None: ...
    def glRenderbufferStorage(self, target:int, internalformat:int, width:int, height:int) -> None: ...
    def glSampleCoverage(self, value:float, invert:int) -> None: ...
    def glScissor(self, x:int, y:int, width:int, height:int) -> None: ...
    def glShaderBinary(self, n:int, shaders:Sequence[int], binaryformat:int, binary:int, length:int) -> None: ...
    def glShaderSource(self, shader:int, source:str) -> None: ...
    def glStencilFunc(self, func:int, ref:int, mask:int) -> None: ...
    def glStencilFuncSeparate(self, face:int, func:int, ref:int, mask:int) -> None: ...
    def glStencilMask(self, mask:int) -> None: ...
    def glStencilMaskSeparate(self, face:int, mask:int) -> None: ...
    def glStencilOp(self, fail:int, zfail:int, zpass:int) -> None: ...
    def glStencilOpSeparate(self, face:int, fail:int, zfail:int, zpass:int) -> None: ...
    def glTexImage2D(self, target:int, level:int, internalformat:int, width:int, height:int, border:int, format:int, type:int, pixels:int) -> None: ...
    def glTexParameterf(self, target:int, pname:int, param:float) -> None: ...
    def glTexParameterfv(self, target:int, pname:int, params:Sequence[float]) -> None: ...
    def glTexParameteri(self, target:int, pname:int, param:int) -> None: ...
    def glTexParameteriv(self, target:int, pname:int, params:Sequence[int]) -> None: ...
    def glTexSubImage2D(self, target:int, level:int, xoffset:int, yoffset:int, width:int, height:int, format:int, type:int, pixels:int) -> None: ...
    def glUniform1f(self, location:int, x:float) -> None: ...
    def glUniform1fv(self, location:int, count:int, v:Sequence[float]) -> None: ...
    def glUniform1i(self, location:int, x:int) -> None: ...
    def glUniform1iv(self, location:int, count:int, v:Sequence[int]) -> None: ...
    def glUniform2f(self, location:int, x:float, y:float) -> None: ...
    def glUniform2fv(self, location:int, count:int, v:Sequence[float]) -> None: ...
    def glUniform2i(self, location:int, x:int, y:int) -> None: ...
    def glUniform2iv(self, location:int, count:int, v:Sequence[int]) -> None: ...
    def glUniform3f(self, location:int, x:float, y:float, z:float) -> None: ...
    def glUniform3fv(self, location:int, count:int, v:Sequence[float]) -> None: ...
    def glUniform3i(self, location:int, x:int, y:int, z:int) -> None: ...
    def glUniform3iv(self, location:int, count:int, v:Sequence[int]) -> None: ...
    def glUniform4f(self, location:int, x:float, y:float, z:float, w:float) -> None: ...
    def glUniform4fv(self, location:int, count:int, v:Sequence[float]) -> None: ...
    def glUniform4i(self, location:int, x:int, y:int, z:int, w:int) -> None: ...
    def glUniform4iv(self, location:int, count:int, v:Sequence[int]) -> None: ...
    def glUniformMatrix2fv(self, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glUniformMatrix3fv(self, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glUniformMatrix4fv(self, location:int, count:int, transpose:int, value:Sequence[float]) -> None: ...
    def glUseProgram(self, program:int) -> None: ...
    def glValidateProgram(self, program:int) -> None: ...
    def glVertexAttrib1f(self, indx:int, x:float) -> None: ...
    def glVertexAttrib1fv(self, indx:int, values:Sequence[float]) -> None: ...
    def glVertexAttrib2f(self, indx:int, x:float, y:float) -> None: ...
    def glVertexAttrib2fv(self, indx:int, values:Sequence[float]) -> None: ...
    def glVertexAttrib3f(self, indx:int, x:float, y:float, z:float) -> None: ...
    def glVertexAttrib3fv(self, indx:int, values:Sequence[float]) -> None: ...
    def glVertexAttrib4f(self, indx:int, x:float, y:float, z:float, w:float) -> None: ...
    def glVertexAttrib4fv(self, indx:int, values:Sequence[float]) -> None: ...
    def glVertexAttribPointer(self, indx:int, size:int, type:int, normalized:int, stride:int, ptr:int) -> None: ...
    def glViewport(self, x:int, y:int, width:int, height:int) -> None: ...
    def hasOpenGLFeature(self, feature:QtGui.QOpenGLFunctions.OpenGLFeature) -> bool: ...
    def initializeOpenGLFunctions(self) -> None: ...
    def openGLFeatures(self) -> QtGui.QOpenGLFunctions.OpenGLFeatures: ...


class QPageLayout(Shiboken.Object):

    StandardMode             : QPageLayout.Mode = ... # 0x0
    FullPageMode             : QPageLayout.Mode = ... # 0x1
    Portrait                 : QPageLayout.Orientation = ... # 0x0
    Landscape                : QPageLayout.Orientation = ... # 0x1
    Millimeter               : QPageLayout.Unit = ... # 0x0
    Point                    : QPageLayout.Unit = ... # 0x1
    Inch                     : QPageLayout.Unit = ... # 0x2
    Pica                     : QPageLayout.Unit = ... # 0x3
    Didot                    : QPageLayout.Unit = ... # 0x4
    Cicero                   : QPageLayout.Unit = ... # 0x5

    class Mode(Enum):

        StandardMode             : QPageLayout.Mode # 0x0
        FullPageMode             : QPageLayout.Mode # 0x1

    class Orientation(Enum):

        Portrait                 : QPageLayout.Orientation # 0x0
        Landscape                : QPageLayout.Orientation # 0x1

    class Unit(Enum):

        Millimeter               : QPageLayout.Unit # 0x0
        Point                    : QPageLayout.Unit # 0x1
        Inch                     : QPageLayout.Unit # 0x2
        Pica                     : QPageLayout.Unit # 0x3
        Didot                    : QPageLayout.Unit # 0x4
        Cicero                   : QPageLayout.Unit # 0x5


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, other:QtGui.QPageLayout) -> None: ...
    @overload
    def __init__(self, pageSize:Union[QtGui.QPageSize, QtGui.QPageSize.PageSizeId, QtCore.QSize], orientation:QtGui.QPageLayout.Orientation, margins:Union[QtCore.QMarginsF, QtCore.QMargins], units:QtGui.QPageLayout.Unit=..., minMargins:Union[QtCore.QMarginsF, QtCore.QMargins]=...) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    @overload
    def fullRect(self) -> QtCore.QRectF: ...
    @overload
    def fullRect(self, units:QtGui.QPageLayout.Unit) -> QtCore.QRectF: ...
    def fullRectPixels(self, resolution:int) -> QtCore.QRect: ...
    def fullRectPoints(self) -> QtCore.QRect: ...
    def isEquivalentTo(self, other:QtGui.QPageLayout) -> bool: ...
    def isValid(self) -> bool: ...
    @overload
    def margins(self) -> QtCore.QMarginsF: ...
    @overload
    def margins(self, units:QtGui.QPageLayout.Unit) -> QtCore.QMarginsF: ...
    def marginsPixels(self, resolution:int) -> QtCore.QMargins: ...
    def marginsPoints(self) -> QtCore.QMargins: ...
    def maximumMargins(self) -> QtCore.QMarginsF: ...
    def minimumMargins(self) -> QtCore.QMarginsF: ...
    def mode(self) -> QtGui.QPageLayout.Mode: ...
    def orientation(self) -> QtGui.QPageLayout.Orientation: ...
    def pageSize(self) -> QtGui.QPageSize: ...
    @overload
    def paintRect(self) -> QtCore.QRectF: ...
    @overload
    def paintRect(self, units:QtGui.QPageLayout.Unit) -> QtCore.QRectF: ...
    def paintRectPixels(self, resolution:int) -> QtCore.QRect: ...
    def paintRectPoints(self) -> QtCore.QRect: ...
    def setBottomMargin(self, bottomMargin:float) -> bool: ...
    def setLeftMargin(self, leftMargin:float) -> bool: ...
    def setMargins(self, margins:Union[QtCore.QMarginsF, QtCore.QMargins]) -> bool: ...
    def setMinimumMargins(self, minMargins:Union[QtCore.QMarginsF, QtCore.QMargins]) -> None: ...
    def setMode(self, mode:QtGui.QPageLayout.Mode) -> None: ...
    def setOrientation(self, orientation:QtGui.QPageLayout.Orientation) -> None: ...
    def setPageSize(self, pageSize:Union[QtGui.QPageSize, QtGui.QPageSize.PageSizeId, QtCore.QSize], minMargins:Union[QtCore.QMarginsF, QtCore.QMargins]=...) -> None: ...
    def setRightMargin(self, rightMargin:float) -> bool: ...
    def setTopMargin(self, topMargin:float) -> bool: ...
    def setUnits(self, units:QtGui.QPageLayout.Unit) -> None: ...
    def swap(self, other:QtGui.QPageLayout) -> None: ...
    def units(self) -> QtGui.QPageLayout.Unit: ...


class QPageRanges(Shiboken.Object):

    class Range(Shiboken.Object):

        @overload
        def __init__(self) -> None: ...
        @overload
        def __init__(self, Range:QtGui.QPageRanges.Range) -> None: ...

        @staticmethod
        def __copy__() -> None: ...
        def contains(self, pageNumber:int) -> bool: ...


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, other:QtGui.QPageRanges) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def addPage(self, pageNumber:int) -> None: ...
    def addRange(self, from_:int, to:int) -> None: ...
    def clear(self) -> None: ...
    def contains(self, pageNumber:int) -> bool: ...
    def firstPage(self) -> int: ...
    @staticmethod
    def fromString(ranges:str) -> QtGui.QPageRanges: ...
    def isEmpty(self) -> bool: ...
    def lastPage(self) -> int: ...
    def swap(self, other:QtGui.QPageRanges) -> None: ...
    def toRangeList(self) -> List[QtGui.QPageRanges.Range]: ...
    def toString(self) -> str: ...


class QPageSize(Shiboken.Object):

    AnsiA                    : QPageSize.PageSizeId = ... # 0x0
    Letter                   : QPageSize.PageSizeId = ... # 0x0
    Legal                    : QPageSize.PageSizeId = ... # 0x1
    Executive                : QPageSize.PageSizeId = ... # 0x2
    A0                       : QPageSize.PageSizeId = ... # 0x3
    A1                       : QPageSize.PageSizeId = ... # 0x4
    A2                       : QPageSize.PageSizeId = ... # 0x5
    A3                       : QPageSize.PageSizeId = ... # 0x6
    A4                       : QPageSize.PageSizeId = ... # 0x7
    A5                       : QPageSize.PageSizeId = ... # 0x8
    A6                       : QPageSize.PageSizeId = ... # 0x9
    A7                       : QPageSize.PageSizeId = ... # 0xa
    A8                       : QPageSize.PageSizeId = ... # 0xb
    A9                       : QPageSize.PageSizeId = ... # 0xc
    A10                      : QPageSize.PageSizeId = ... # 0xd
    B0                       : QPageSize.PageSizeId = ... # 0xe
    B1                       : QPageSize.PageSizeId = ... # 0xf
    B2                       : QPageSize.PageSizeId = ... # 0x10
    B3                       : QPageSize.PageSizeId = ... # 0x11
    B4                       : QPageSize.PageSizeId = ... # 0x12
    B5                       : QPageSize.PageSizeId = ... # 0x13
    B6                       : QPageSize.PageSizeId = ... # 0x14
    B7                       : QPageSize.PageSizeId = ... # 0x15
    B8                       : QPageSize.PageSizeId = ... # 0x16
    B9                       : QPageSize.PageSizeId = ... # 0x17
    B10                      : QPageSize.PageSizeId = ... # 0x18
    C5E                      : QPageSize.PageSizeId = ... # 0x19
    EnvelopeC5               : QPageSize.PageSizeId = ... # 0x19
    Comm10E                  : QPageSize.PageSizeId = ... # 0x1a
    Envelope10               : QPageSize.PageSizeId = ... # 0x1a
    DLE                      : QPageSize.PageSizeId = ... # 0x1b
    EnvelopeDL               : QPageSize.PageSizeId = ... # 0x1b
    Folio                    : QPageSize.PageSizeId = ... # 0x1c
    AnsiB                    : QPageSize.PageSizeId = ... # 0x1d
    Ledger                   : QPageSize.PageSizeId = ... # 0x1d
    Tabloid                  : QPageSize.PageSizeId = ... # 0x1e
    Custom                   : QPageSize.PageSizeId = ... # 0x1f
    A3Extra                  : QPageSize.PageSizeId = ... # 0x20
    A4Extra                  : QPageSize.PageSizeId = ... # 0x21
    A4Plus                   : QPageSize.PageSizeId = ... # 0x22
    A4Small                  : QPageSize.PageSizeId = ... # 0x23
    A5Extra                  : QPageSize.PageSizeId = ... # 0x24
    B5Extra                  : QPageSize.PageSizeId = ... # 0x25
    JisB0                    : QPageSize.PageSizeId = ... # 0x26
    JisB1                    : QPageSize.PageSizeId = ... # 0x27
    JisB2                    : QPageSize.PageSizeId = ... # 0x28
    JisB3                    : QPageSize.PageSizeId = ... # 0x29
    JisB4                    : QPageSize.PageSizeId = ... # 0x2a
    JisB5                    : QPageSize.PageSizeId = ... # 0x2b
    JisB6                    : QPageSize.PageSizeId = ... # 0x2c
    JisB7                    : QPageSize.PageSizeId = ... # 0x2d
    JisB8                    : QPageSize.PageSizeId = ... # 0x2e
    JisB9                    : QPageSize.PageSizeId = ... # 0x2f
    JisB10                   : QPageSize.PageSizeId = ... # 0x30
    AnsiC                    : QPageSize.PageSizeId = ... # 0x31
    AnsiD                    : QPageSize.PageSizeId = ... # 0x32
    AnsiE                    : QPageSize.PageSizeId = ... # 0x33
    LegalExtra               : QPageSize.PageSizeId = ... # 0x34
    LetterExtra              : QPageSize.PageSizeId = ... # 0x35
    LetterPlus               : QPageSize.PageSizeId = ... # 0x36
    LetterSmall              : QPageSize.PageSizeId = ... # 0x37
    TabloidExtra             : QPageSize.PageSizeId = ... # 0x38
    ArchA                    : QPageSize.PageSizeId = ... # 0x39
    ArchB                    : QPageSize.PageSizeId = ... # 0x3a
    ArchC                    : QPageSize.PageSizeId = ... # 0x3b
    ArchD                    : QPageSize.PageSizeId = ... # 0x3c
    ArchE                    : QPageSize.PageSizeId = ... # 0x3d
    Imperial7x9              : QPageSize.PageSizeId = ... # 0x3e
    Imperial8x10             : QPageSize.PageSizeId = ... # 0x3f
    Imperial9x11             : QPageSize.PageSizeId = ... # 0x40
    Imperial9x12             : QPageSize.PageSizeId = ... # 0x41
    Imperial10x11            : QPageSize.PageSizeId = ... # 0x42
    Imperial10x13            : QPageSize.PageSizeId = ... # 0x43
    Imperial10x14            : QPageSize.PageSizeId = ... # 0x44
    Imperial12x11            : QPageSize.PageSizeId = ... # 0x45
    Imperial15x11            : QPageSize.PageSizeId = ... # 0x46
    ExecutiveStandard        : QPageSize.PageSizeId = ... # 0x47
    Note                     : QPageSize.PageSizeId = ... # 0x48
    Quarto                   : QPageSize.PageSizeId = ... # 0x49
    Statement                : QPageSize.PageSizeId = ... # 0x4a
    SuperA                   : QPageSize.PageSizeId = ... # 0x4b
    SuperB                   : QPageSize.PageSizeId = ... # 0x4c
    Postcard                 : QPageSize.PageSizeId = ... # 0x4d
    DoublePostcard           : QPageSize.PageSizeId = ... # 0x4e
    Prc16K                   : QPageSize.PageSizeId = ... # 0x4f
    Prc32K                   : QPageSize.PageSizeId = ... # 0x50
    Prc32KBig                : QPageSize.PageSizeId = ... # 0x51
    FanFoldUS                : QPageSize.PageSizeId = ... # 0x52
    FanFoldGerman            : QPageSize.PageSizeId = ... # 0x53
    FanFoldGermanLegal       : QPageSize.PageSizeId = ... # 0x54
    EnvelopeB4               : QPageSize.PageSizeId = ... # 0x55
    EnvelopeB5               : QPageSize.PageSizeId = ... # 0x56
    EnvelopeB6               : QPageSize.PageSizeId = ... # 0x57
    EnvelopeC0               : QPageSize.PageSizeId = ... # 0x58
    EnvelopeC1               : QPageSize.PageSizeId = ... # 0x59
    EnvelopeC2               : QPageSize.PageSizeId = ... # 0x5a
    EnvelopeC3               : QPageSize.PageSizeId = ... # 0x5b
    EnvelopeC4               : QPageSize.PageSizeId = ... # 0x5c
    EnvelopeC6               : QPageSize.PageSizeId = ... # 0x5d
    EnvelopeC65              : QPageSize.PageSizeId = ... # 0x5e
    EnvelopeC7               : QPageSize.PageSizeId = ... # 0x5f
    Envelope9                : QPageSize.PageSizeId = ... # 0x60
    Envelope11               : QPageSize.PageSizeId = ... # 0x61
    Envelope12               : QPageSize.PageSizeId = ... # 0x62
    Envelope14               : QPageSize.PageSizeId = ... # 0x63
    EnvelopeMonarch          : QPageSize.PageSizeId = ... # 0x64
    EnvelopePersonal         : QPageSize.PageSizeId = ... # 0x65
    EnvelopeChou3            : QPageSize.PageSizeId = ... # 0x66
    EnvelopeChou4            : QPageSize.PageSizeId = ... # 0x67
    EnvelopeInvite           : QPageSize.PageSizeId = ... # 0x68
    EnvelopeItalian          : QPageSize.PageSizeId = ... # 0x69
    EnvelopeKaku2            : QPageSize.PageSizeId = ... # 0x6a
    EnvelopeKaku3            : QPageSize.PageSizeId = ... # 0x6b
    EnvelopePrc1             : QPageSize.PageSizeId = ... # 0x6c
    EnvelopePrc2             : QPageSize.PageSizeId = ... # 0x6d
    EnvelopePrc3             : QPageSize.PageSizeId = ... # 0x6e
    EnvelopePrc4             : QPageSize.PageSizeId = ... # 0x6f
    EnvelopePrc5             : QPageSize.PageSizeId = ... # 0x70
    EnvelopePrc6             : QPageSize.PageSizeId = ... # 0x71
    EnvelopePrc7             : QPageSize.PageSizeId = ... # 0x72
    EnvelopePrc8             : QPageSize.PageSizeId = ... # 0x73
    EnvelopePrc9             : QPageSize.PageSizeId = ... # 0x74
    EnvelopePrc10            : QPageSize.PageSizeId = ... # 0x75
    EnvelopeYou4             : QPageSize.PageSizeId = ... # 0x76
    LastPageSize             : QPageSize.PageSizeId = ... # 0x76
    FuzzyMatch               : QPageSize.SizeMatchPolicy = ... # 0x0
    FuzzyOrientationMatch    : QPageSize.SizeMatchPolicy = ... # 0x1
    ExactMatch               : QPageSize.SizeMatchPolicy = ... # 0x2
    Millimeter               : QPageSize.Unit = ... # 0x0
    Point                    : QPageSize.Unit = ... # 0x1
    Inch                     : QPageSize.Unit = ... # 0x2
    Pica                     : QPageSize.Unit = ... # 0x3
    Didot                    : QPageSize.Unit = ... # 0x4
    Cicero                   : QPageSize.Unit = ... # 0x5

    class PageSizeId(Enum):

        AnsiA                    : QPageSize.PageSizeId # 0x0
        Letter                   : QPageSize.PageSizeId # 0x0
        Legal                    : QPageSize.PageSizeId # 0x1
        Executive                : QPageSize.PageSizeId # 0x2
        A0                       : QPageSize.PageSizeId # 0x3
        A1                       : QPageSize.PageSizeId # 0x4
        A2                       : QPageSize.PageSizeId # 0x5
        A3                       : QPageSize.PageSizeId # 0x6
        A4                       : QPageSize.PageSizeId # 0x7
        A5                       : QPageSize.PageSizeId # 0x8
        A6                       : QPageSize.PageSizeId # 0x9
        A7                       : QPageSize.PageSizeId # 0xa
        A8                       : QPageSize.PageSizeId # 0xb
        A9                       : QPageSize.PageSizeId # 0xc
        A10                      : QPageSize.PageSizeId # 0xd
        B0                       : QPageSize.PageSizeId # 0xe
        B1                       : QPageSize.PageSizeId # 0xf
        B2                       : QPageSize.PageSizeId # 0x10
        B3                       : QPageSize.PageSizeId # 0x11
        B4                       : QPageSize.PageSizeId # 0x12
        B5                       : QPageSize.PageSizeId # 0x13
        B6                       : QPageSize.PageSizeId # 0x14
        B7                       : QPageSize.PageSizeId # 0x15
        B8                       : QPageSize.PageSizeId # 0x16
        B9                       : QPageSize.PageSizeId # 0x17
        B10                      : QPageSize.PageSizeId # 0x18
        C5E                      : QPageSize.PageSizeId # 0x19
        EnvelopeC5               : QPageSize.PageSizeId # 0x19
        Comm10E                  : QPageSize.PageSizeId # 0x1a
        Envelope10               : QPageSize.PageSizeId # 0x1a
        DLE                      : QPageSize.PageSizeId # 0x1b
        EnvelopeDL               : QPageSize.PageSizeId # 0x1b
        Folio                    : QPageSize.PageSizeId # 0x1c
        AnsiB                    : QPageSize.PageSizeId # 0x1d
        Ledger                   : QPageSize.PageSizeId # 0x1d
        Tabloid                  : QPageSize.PageSizeId # 0x1e
        Custom                   : QPageSize.PageSizeId # 0x1f
        A3Extra                  : QPageSize.PageSizeId # 0x20
        A4Extra                  : QPageSize.PageSizeId # 0x21
        A4Plus                   : QPageSize.PageSizeId # 0x22
        A4Small                  : QPageSize.PageSizeId # 0x23
        A5Extra                  : QPageSize.PageSizeId # 0x24
        B5Extra                  : QPageSize.PageSizeId # 0x25
        JisB0                    : QPageSize.PageSizeId # 0x26
        JisB1                    : QPageSize.PageSizeId # 0x27
        JisB2                    : QPageSize.PageSizeId # 0x28
        JisB3                    : QPageSize.PageSizeId # 0x29
        JisB4                    : QPageSize.PageSizeId # 0x2a
        JisB5                    : QPageSize.PageSizeId # 0x2b
        JisB6                    : QPageSize.PageSizeId # 0x2c
        JisB7                    : QPageSize.PageSizeId # 0x2d
        JisB8                    : QPageSize.PageSizeId # 0x2e
        JisB9                    : QPageSize.PageSizeId # 0x2f
        JisB10                   : QPageSize.PageSizeId # 0x30
        AnsiC                    : QPageSize.PageSizeId # 0x31
        AnsiD                    : QPageSize.PageSizeId # 0x32
        AnsiE                    : QPageSize.PageSizeId # 0x33
        LegalExtra               : QPageSize.PageSizeId # 0x34
        LetterExtra              : QPageSize.PageSizeId # 0x35
        LetterPlus               : QPageSize.PageSizeId # 0x36
        LetterSmall              : QPageSize.PageSizeId # 0x37
        TabloidExtra             : QPageSize.PageSizeId # 0x38
        ArchA                    : QPageSize.PageSizeId # 0x39
        ArchB                    : QPageSize.PageSizeId # 0x3a
        ArchC                    : QPageSize.PageSizeId # 0x3b
        ArchD                    : QPageSize.PageSizeId # 0x3c
        ArchE                    : QPageSize.PageSizeId # 0x3d
        Imperial7x9              : QPageSize.PageSizeId # 0x3e
        Imperial8x10             : QPageSize.PageSizeId # 0x3f
        Imperial9x11             : QPageSize.PageSizeId # 0x40
        Imperial9x12             : QPageSize.PageSizeId # 0x41
        Imperial10x11            : QPageSize.PageSizeId # 0x42
        Imperial10x13            : QPageSize.PageSizeId # 0x43
        Imperial10x14            : QPageSize.PageSizeId # 0x44
        Imperial12x11            : QPageSize.PageSizeId # 0x45
        Imperial15x11            : QPageSize.PageSizeId # 0x46
        ExecutiveStandard        : QPageSize.PageSizeId # 0x47
        Note                     : QPageSize.PageSizeId # 0x48
        Quarto                   : QPageSize.PageSizeId # 0x49
        Statement                : QPageSize.PageSizeId # 0x4a
        SuperA                   : QPageSize.PageSizeId # 0x4b
        SuperB                   : QPageSize.PageSizeId # 0x4c
        Postcard                 : QPageSize.PageSizeId # 0x4d
        DoublePostcard           : QPageSize.PageSizeId # 0x4e
        Prc16K                   : QPageSize.PageSizeId # 0x4f
        Prc32K                   : QPageSize.PageSizeId # 0x50
        Prc32KBig                : QPageSize.PageSizeId # 0x51
        FanFoldUS                : QPageSize.PageSizeId # 0x52
        FanFoldGerman            : QPageSize.PageSizeId # 0x53
        FanFoldGermanLegal       : QPageSize.PageSizeId # 0x54
        EnvelopeB4               : QPageSize.PageSizeId # 0x55
        EnvelopeB5               : QPageSize.PageSizeId # 0x56
        EnvelopeB6               : QPageSize.PageSizeId # 0x57
        EnvelopeC0               : QPageSize.PageSizeId # 0x58
        EnvelopeC1               : QPageSize.PageSizeId # 0x59
        EnvelopeC2               : QPageSize.PageSizeId # 0x5a
        EnvelopeC3               : QPageSize.PageSizeId # 0x5b
        EnvelopeC4               : QPageSize.PageSizeId # 0x5c
        EnvelopeC6               : QPageSize.PageSizeId # 0x5d
        EnvelopeC65              : QPageSize.PageSizeId # 0x5e
        EnvelopeC7               : QPageSize.PageSizeId # 0x5f
        Envelope9                : QPageSize.PageSizeId # 0x60
        Envelope11               : QPageSize.PageSizeId # 0x61
        Envelope12               : QPageSize.PageSizeId # 0x62
        Envelope14               : QPageSize.PageSizeId # 0x63
        EnvelopeMonarch          : QPageSize.PageSizeId # 0x64
        EnvelopePersonal         : QPageSize.PageSizeId # 0x65
        EnvelopeChou3            : QPageSize.PageSizeId # 0x66
        EnvelopeChou4            : QPageSize.PageSizeId # 0x67
        EnvelopeInvite           : QPageSize.PageSizeId # 0x68
        EnvelopeItalian          : QPageSize.PageSizeId # 0x69
        EnvelopeKaku2            : QPageSize.PageSizeId # 0x6a
        EnvelopeKaku3            : QPageSize.PageSizeId # 0x6b
        EnvelopePrc1             : QPageSize.PageSizeId # 0x6c
        EnvelopePrc2             : QPageSize.PageSizeId # 0x6d
        EnvelopePrc3             : QPageSize.PageSizeId # 0x6e
        EnvelopePrc4             : QPageSize.PageSizeId # 0x6f
        EnvelopePrc5             : QPageSize.PageSizeId # 0x70
        EnvelopePrc6             : QPageSize.PageSizeId # 0x71
        EnvelopePrc7             : QPageSize.PageSizeId # 0x72
        EnvelopePrc8             : QPageSize.PageSizeId # 0x73
        EnvelopePrc9             : QPageSize.PageSizeId # 0x74
        EnvelopePrc10            : QPageSize.PageSizeId # 0x75
        EnvelopeYou4             : QPageSize.PageSizeId # 0x76
        LastPageSize             : QPageSize.PageSizeId # 0x76

    class SizeMatchPolicy(Enum):

        FuzzyMatch               : QPageSize.SizeMatchPolicy # 0x0
        FuzzyOrientationMatch    : QPageSize.SizeMatchPolicy # 0x1
        ExactMatch               : QPageSize.SizeMatchPolicy # 0x2

    class Unit(Enum):

        Millimeter               : QPageSize.Unit # 0x0
        Point                    : QPageSize.Unit # 0x1
        Inch                     : QPageSize.Unit # 0x2
        Pica                     : QPageSize.Unit # 0x3
        Didot                    : QPageSize.Unit # 0x4
        Cicero                   : QPageSize.Unit # 0x5


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, other:Union[QtGui.QPageSize, QtGui.QPageSize.PageSizeId, QtCore.QSize]) -> None: ...
    @overload
    def __init__(self, pageSizeId:QtGui.QPageSize.PageSizeId) -> None: ...
    @overload
    def __init__(self, pointSize:QtCore.QSize, name:str=..., matchPolicy:QtGui.QPageSize.SizeMatchPolicy=...) -> None: ...
    @overload
    def __init__(self, size:Union[QtCore.QSizeF, QtCore.QSize], units:QtGui.QPageSize.Unit, name:str=..., matchPolicy:QtGui.QPageSize.SizeMatchPolicy=...) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    @overload
    @staticmethod
    def definitionSize(pageSizeId:QtGui.QPageSize.PageSizeId) -> QtCore.QSizeF: ...
    @overload
    def definitionSize(self) -> QtCore.QSizeF: ...
    @overload
    @staticmethod
    def definitionUnits(pageSizeId:QtGui.QPageSize.PageSizeId) -> QtGui.QPageSize.Unit: ...
    @overload
    def definitionUnits(self) -> QtGui.QPageSize.Unit: ...
    @overload
    @staticmethod
    def id(pointSize:QtCore.QSize, matchPolicy:QtGui.QPageSize.SizeMatchPolicy=...) -> QtGui.QPageSize.PageSizeId: ...
    @overload
    def id(self) -> QtGui.QPageSize.PageSizeId: ...
    @overload
    @staticmethod
    def id(size:Union[QtCore.QSizeF, QtCore.QSize], units:QtGui.QPageSize.Unit, matchPolicy:QtGui.QPageSize.SizeMatchPolicy=...) -> QtGui.QPageSize.PageSizeId: ...
    @overload
    @staticmethod
    def id(windowsId:int) -> QtGui.QPageSize.PageSizeId: ...
    def isEquivalentTo(self, other:Union[QtGui.QPageSize, QtGui.QPageSize.PageSizeId, QtCore.QSize]) -> bool: ...
    def isValid(self) -> bool: ...
    @overload
    @staticmethod
    def key(pageSizeId:QtGui.QPageSize.PageSizeId) -> str: ...
    @overload
    def key(self) -> str: ...
    @overload
    @staticmethod
    def name(pageSizeId:QtGui.QPageSize.PageSizeId) -> str: ...
    @overload
    def name(self) -> str: ...
    def rect(self, units:QtGui.QPageSize.Unit) -> QtCore.QRectF: ...
    def rectPixels(self, resolution:int) -> QtCore.QRect: ...
    def rectPoints(self) -> QtCore.QRect: ...
    @overload
    @staticmethod
    def size(pageSizeId:QtGui.QPageSize.PageSizeId, units:QtGui.QPageSize.Unit) -> QtCore.QSizeF: ...
    @overload
    def size(self, units:QtGui.QPageSize.Unit) -> QtCore.QSizeF: ...
    @overload
    @staticmethod
    def sizePixels(pageSizeId:QtGui.QPageSize.PageSizeId, resolution:int) -> QtCore.QSize: ...
    @overload
    def sizePixels(self, resolution:int) -> QtCore.QSize: ...
    @overload
    @staticmethod
    def sizePoints(pageSizeId:QtGui.QPageSize.PageSizeId) -> QtCore.QSize: ...
    @overload
    def sizePoints(self) -> QtCore.QSize: ...
    def swap(self, other:Union[QtGui.QPageSize, QtGui.QPageSize.PageSizeId, QtCore.QSize]) -> None: ...
    @overload
    @staticmethod
    def windowsId(pageSizeId:QtGui.QPageSize.PageSizeId) -> int: ...
    @overload
    def windowsId(self) -> int: ...


class QPagedPaintDevice(QtGui.QPaintDevice):

    PdfVersion_1_4           : QPagedPaintDevice.PdfVersion = ... # 0x0
    PdfVersion_A1b           : QPagedPaintDevice.PdfVersion = ... # 0x1
    PdfVersion_1_6           : QPagedPaintDevice.PdfVersion = ... # 0x2

    class PdfVersion(Enum):

        PdfVersion_1_4           : QPagedPaintDevice.PdfVersion # 0x0
        PdfVersion_A1b           : QPagedPaintDevice.PdfVersion # 0x1
        PdfVersion_1_6           : QPagedPaintDevice.PdfVersion # 0x2


    def newPage(self) -> bool: ...
    def pageLayout(self) -> QtGui.QPageLayout: ...
    def pageRanges(self) -> QtGui.QPageRanges: ...
    def setPageLayout(self, pageLayout:QtGui.QPageLayout) -> bool: ...
    def setPageMargins(self, margins:Union[QtCore.QMarginsF, QtCore.QMargins], units:QtGui.QPageLayout.Unit=...) -> bool: ...
    def setPageOrientation(self, orientation:QtGui.QPageLayout.Orientation) -> bool: ...
    def setPageRanges(self, ranges:QtGui.QPageRanges) -> None: ...
    def setPageSize(self, pageSize:Union[QtGui.QPageSize, QtGui.QPageSize.PageSizeId, QtCore.QSize]) -> bool: ...


class QPaintDevice(Shiboken.Object):

    PdmWidth                 : QPaintDevice.PaintDeviceMetric = ... # 0x1
    PdmHeight                : QPaintDevice.PaintDeviceMetric = ... # 0x2
    PdmWidthMM               : QPaintDevice.PaintDeviceMetric = ... # 0x3
    PdmHeightMM              : QPaintDevice.PaintDeviceMetric = ... # 0x4
    PdmNumColors             : QPaintDevice.PaintDeviceMetric = ... # 0x5
    PdmDepth                 : QPaintDevice.PaintDeviceMetric = ... # 0x6
    PdmDpiX                  : QPaintDevice.PaintDeviceMetric = ... # 0x7
    PdmDpiY                  : QPaintDevice.PaintDeviceMetric = ... # 0x8
    PdmPhysicalDpiX          : QPaintDevice.PaintDeviceMetric = ... # 0x9
    PdmPhysicalDpiY          : QPaintDevice.PaintDeviceMetric = ... # 0xa
    PdmDevicePixelRatio      : QPaintDevice.PaintDeviceMetric = ... # 0xb
    PdmDevicePixelRatioScaled: QPaintDevice.PaintDeviceMetric = ... # 0xc

    class PaintDeviceMetric(Enum):

        PdmWidth                 : QPaintDevice.PaintDeviceMetric # 0x1
        PdmHeight                : QPaintDevice.PaintDeviceMetric # 0x2
        PdmWidthMM               : QPaintDevice.PaintDeviceMetric # 0x3
        PdmHeightMM              : QPaintDevice.PaintDeviceMetric # 0x4
        PdmNumColors             : QPaintDevice.PaintDeviceMetric # 0x5
        PdmDepth                 : QPaintDevice.PaintDeviceMetric # 0x6
        PdmDpiX                  : QPaintDevice.PaintDeviceMetric # 0x7
        PdmDpiY                  : QPaintDevice.PaintDeviceMetric # 0x8
        PdmPhysicalDpiX          : QPaintDevice.PaintDeviceMetric # 0x9
        PdmPhysicalDpiY          : QPaintDevice.PaintDeviceMetric # 0xa
        PdmDevicePixelRatio      : QPaintDevice.PaintDeviceMetric # 0xb
        PdmDevicePixelRatioScaled: QPaintDevice.PaintDeviceMetric # 0xc


    def __init__(self) -> None: ...

    def colorCount(self) -> int: ...
    def depth(self) -> int: ...
    def devType(self) -> int: ...
    def devicePixelRatio(self) -> float: ...
    def devicePixelRatioF(self) -> float: ...
    @staticmethod
    def devicePixelRatioFScale() -> float: ...
    def height(self) -> int: ...
    def heightMM(self) -> int: ...
    def initPainter(self, painter:QtGui.QPainter) -> None: ...
    def logicalDpiX(self) -> int: ...
    def logicalDpiY(self) -> int: ...
    def metric(self, metric:QtGui.QPaintDevice.PaintDeviceMetric) -> int: ...
    def paintEngine(self) -> QtGui.QPaintEngine: ...
    def paintingActive(self) -> bool: ...
    def physicalDpiX(self) -> int: ...
    def physicalDpiY(self) -> int: ...
    def redirected(self, offset:QtCore.QPoint) -> QtGui.QPaintDevice: ...
    def sharedPainter(self) -> QtGui.QPainter: ...
    def width(self) -> int: ...
    def widthMM(self) -> int: ...


class QPaintDeviceWindow(QtGui.QWindow, QtGui.QPaintDevice):
    def event(self, event:QtCore.QEvent) -> bool: ...
    def exposeEvent(self, arg__1:QtGui.QExposeEvent) -> None: ...
    def metric(self, metric:QtGui.QPaintDevice.PaintDeviceMetric) -> int: ...
    def paintEngine(self) -> QtGui.QPaintEngine: ...
    def paintEvent(self, event:QtGui.QPaintEvent) -> None: ...
    @overload
    def update(self) -> None: ...
    @overload
    def update(self, rect:QtCore.QRect) -> None: ...
    @overload
    def update(self, region:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> None: ...


class QPaintEngine(Shiboken.Object):

    DirtyPen                 : QPaintEngine.DirtyFlag = ... # 0x1
    DirtyBrush               : QPaintEngine.DirtyFlag = ... # 0x2
    DirtyBrushOrigin         : QPaintEngine.DirtyFlag = ... # 0x4
    DirtyFont                : QPaintEngine.DirtyFlag = ... # 0x8
    DirtyBackground          : QPaintEngine.DirtyFlag = ... # 0x10
    DirtyBackgroundMode      : QPaintEngine.DirtyFlag = ... # 0x20
    DirtyTransform           : QPaintEngine.DirtyFlag = ... # 0x40
    DirtyClipRegion          : QPaintEngine.DirtyFlag = ... # 0x80
    DirtyClipPath            : QPaintEngine.DirtyFlag = ... # 0x100
    DirtyHints               : QPaintEngine.DirtyFlag = ... # 0x200
    DirtyCompositionMode     : QPaintEngine.DirtyFlag = ... # 0x400
    DirtyClipEnabled         : QPaintEngine.DirtyFlag = ... # 0x800
    DirtyOpacity             : QPaintEngine.DirtyFlag = ... # 0x1000
    AllDirty                 : QPaintEngine.DirtyFlag = ... # 0xffff
    AllFeatures              : QPaintEngine.PaintEngineFeature = ... # -0x1
    PrimitiveTransform       : QPaintEngine.PaintEngineFeature = ... # 0x1
    PatternTransform         : QPaintEngine.PaintEngineFeature = ... # 0x2
    PixmapTransform          : QPaintEngine.PaintEngineFeature = ... # 0x4
    PatternBrush             : QPaintEngine.PaintEngineFeature = ... # 0x8
    LinearGradientFill       : QPaintEngine.PaintEngineFeature = ... # 0x10
    RadialGradientFill       : QPaintEngine.PaintEngineFeature = ... # 0x20
    ConicalGradientFill      : QPaintEngine.PaintEngineFeature = ... # 0x40
    AlphaBlend               : QPaintEngine.PaintEngineFeature = ... # 0x80
    PorterDuff               : QPaintEngine.PaintEngineFeature = ... # 0x100
    PainterPaths             : QPaintEngine.PaintEngineFeature = ... # 0x200
    Antialiasing             : QPaintEngine.PaintEngineFeature = ... # 0x400
    BrushStroke              : QPaintEngine.PaintEngineFeature = ... # 0x800
    ConstantOpacity          : QPaintEngine.PaintEngineFeature = ... # 0x1000
    MaskedBrush              : QPaintEngine.PaintEngineFeature = ... # 0x2000
    PerspectiveTransform     : QPaintEngine.PaintEngineFeature = ... # 0x4000
    BlendModes               : QPaintEngine.PaintEngineFeature = ... # 0x8000
    ObjectBoundingModeGradients: QPaintEngine.PaintEngineFeature = ... # 0x10000
    RasterOpModes            : QPaintEngine.PaintEngineFeature = ... # 0x20000
    PaintOutsidePaintEvent   : QPaintEngine.PaintEngineFeature = ... # 0x20000000
    OddEvenMode              : QPaintEngine.PolygonDrawMode = ... # 0x0
    WindingMode              : QPaintEngine.PolygonDrawMode = ... # 0x1
    ConvexMode               : QPaintEngine.PolygonDrawMode = ... # 0x2
    PolylineMode             : QPaintEngine.PolygonDrawMode = ... # 0x3
    X11                      : QPaintEngine.Type = ... # 0x0
    Windows                  : QPaintEngine.Type = ... # 0x1
    QuickDraw                : QPaintEngine.Type = ... # 0x2
    CoreGraphics             : QPaintEngine.Type = ... # 0x3
    MacPrinter               : QPaintEngine.Type = ... # 0x4
    QWindowSystem            : QPaintEngine.Type = ... # 0x5
    OpenGL                   : QPaintEngine.Type = ... # 0x6
    Picture                  : QPaintEngine.Type = ... # 0x7
    SVG                      : QPaintEngine.Type = ... # 0x8
    Raster                   : QPaintEngine.Type = ... # 0x9
    Direct3D                 : QPaintEngine.Type = ... # 0xa
    Pdf                      : QPaintEngine.Type = ... # 0xb
    OpenVG                   : QPaintEngine.Type = ... # 0xc
    OpenGL2                  : QPaintEngine.Type = ... # 0xd
    PaintBuffer              : QPaintEngine.Type = ... # 0xe
    Blitter                  : QPaintEngine.Type = ... # 0xf
    Direct2D                 : QPaintEngine.Type = ... # 0x10
    User                     : QPaintEngine.Type = ... # 0x32
    MaxUser                  : QPaintEngine.Type = ... # 0x64

    class DirtyFlag(Enum):

        DirtyPen                 : QPaintEngine.DirtyFlag # 0x1
        DirtyBrush               : QPaintEngine.DirtyFlag # 0x2
        DirtyBrushOrigin         : QPaintEngine.DirtyFlag # 0x4
        DirtyFont                : QPaintEngine.DirtyFlag # 0x8
        DirtyBackground          : QPaintEngine.DirtyFlag # 0x10
        DirtyBackgroundMode      : QPaintEngine.DirtyFlag # 0x20
        DirtyTransform           : QPaintEngine.DirtyFlag # 0x40
        DirtyClipRegion          : QPaintEngine.DirtyFlag # 0x80
        DirtyClipPath            : QPaintEngine.DirtyFlag # 0x100
        DirtyHints               : QPaintEngine.DirtyFlag # 0x200
        DirtyCompositionMode     : QPaintEngine.DirtyFlag # 0x400
        DirtyClipEnabled         : QPaintEngine.DirtyFlag # 0x800
        DirtyOpacity             : QPaintEngine.DirtyFlag # 0x1000
        AllDirty                 : QPaintEngine.DirtyFlag # 0xffff

    class DirtyFlags(object): ...

    class PaintEngineFeature(Enum):

        AllFeatures              : QPaintEngine.PaintEngineFeature # -0x1
        PrimitiveTransform       : QPaintEngine.PaintEngineFeature # 0x1
        PatternTransform         : QPaintEngine.PaintEngineFeature # 0x2
        PixmapTransform          : QPaintEngine.PaintEngineFeature # 0x4
        PatternBrush             : QPaintEngine.PaintEngineFeature # 0x8
        LinearGradientFill       : QPaintEngine.PaintEngineFeature # 0x10
        RadialGradientFill       : QPaintEngine.PaintEngineFeature # 0x20
        ConicalGradientFill      : QPaintEngine.PaintEngineFeature # 0x40
        AlphaBlend               : QPaintEngine.PaintEngineFeature # 0x80
        PorterDuff               : QPaintEngine.PaintEngineFeature # 0x100
        PainterPaths             : QPaintEngine.PaintEngineFeature # 0x200
        Antialiasing             : QPaintEngine.PaintEngineFeature # 0x400
        BrushStroke              : QPaintEngine.PaintEngineFeature # 0x800
        ConstantOpacity          : QPaintEngine.PaintEngineFeature # 0x1000
        MaskedBrush              : QPaintEngine.PaintEngineFeature # 0x2000
        PerspectiveTransform     : QPaintEngine.PaintEngineFeature # 0x4000
        BlendModes               : QPaintEngine.PaintEngineFeature # 0x8000
        ObjectBoundingModeGradients: QPaintEngine.PaintEngineFeature # 0x10000
        RasterOpModes            : QPaintEngine.PaintEngineFeature # 0x20000
        PaintOutsidePaintEvent   : QPaintEngine.PaintEngineFeature # 0x20000000

    class PaintEngineFeatures(object): ...

    class PolygonDrawMode(Enum):

        OddEvenMode              : QPaintEngine.PolygonDrawMode # 0x0
        WindingMode              : QPaintEngine.PolygonDrawMode # 0x1
        ConvexMode               : QPaintEngine.PolygonDrawMode # 0x2
        PolylineMode             : QPaintEngine.PolygonDrawMode # 0x3

    class Type(Enum):

        X11                      : QPaintEngine.Type # 0x0
        Windows                  : QPaintEngine.Type # 0x1
        QuickDraw                : QPaintEngine.Type # 0x2
        CoreGraphics             : QPaintEngine.Type # 0x3
        MacPrinter               : QPaintEngine.Type # 0x4
        QWindowSystem            : QPaintEngine.Type # 0x5
        OpenGL                   : QPaintEngine.Type # 0x6
        Picture                  : QPaintEngine.Type # 0x7
        SVG                      : QPaintEngine.Type # 0x8
        Raster                   : QPaintEngine.Type # 0x9
        Direct3D                 : QPaintEngine.Type # 0xa
        Pdf                      : QPaintEngine.Type # 0xb
        OpenVG                   : QPaintEngine.Type # 0xc
        OpenGL2                  : QPaintEngine.Type # 0xd
        PaintBuffer              : QPaintEngine.Type # 0xe
        Blitter                  : QPaintEngine.Type # 0xf
        Direct2D                 : QPaintEngine.Type # 0x10
        User                     : QPaintEngine.Type # 0x32
        MaxUser                  : QPaintEngine.Type # 0x64


    def __init__(self, features:QtGui.QPaintEngine.PaintEngineFeatures=...) -> None: ...

    def begin(self, pdev:QtGui.QPaintDevice) -> bool: ...
    def clearDirty(self, df:QtGui.QPaintEngine.DirtyFlags) -> None: ...
    def coordinateOffset(self) -> QtCore.QPoint: ...
    def createPixmap(self, size:QtCore.QSize) -> QtGui.QPixmap: ...
    def createPixmapFromImage(self, image:Union[QtGui.QImage, str], flags:QtCore.Qt.ImageConversionFlags=...) -> QtGui.QPixmap: ...
    @overload
    def drawEllipse(self, r:QtCore.QRect) -> None: ...
    @overload
    def drawEllipse(self, r:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    def drawImage(self, r:Union[QtCore.QRectF, QtCore.QRect], pm:Union[QtGui.QImage, str], sr:Union[QtCore.QRectF, QtCore.QRect], flags:QtCore.Qt.ImageConversionFlags=...) -> None: ...
    @overload
    def drawLines(self, lines:QtCore.QLine, lineCount:int) -> None: ...
    @overload
    def drawLines(self, lines:Union[QtCore.QLineF, QtCore.QLine], lineCount:int) -> None: ...
    def drawPath(self, path:QtGui.QPainterPath) -> None: ...
    def drawPixmap(self, r:Union[QtCore.QRectF, QtCore.QRect], pm:Union[QtGui.QPixmap, QtGui.QImage, str], sr:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    @overload
    def drawPoints(self, points:QtCore.QPoint, pointCount:int) -> None: ...
    @overload
    def drawPoints(self, points:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], pointCount:int) -> None: ...
    @overload
    def drawPolygon(self, points:QtCore.QPoint, pointCount:int, mode:QtGui.QPaintEngine.PolygonDrawMode) -> None: ...
    @overload
    def drawPolygon(self, points:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], pointCount:int, mode:QtGui.QPaintEngine.PolygonDrawMode) -> None: ...
    @overload
    def drawRects(self, rects:QtCore.QRect, rectCount:int) -> None: ...
    @overload
    def drawRects(self, rects:Union[QtCore.QRectF, QtCore.QRect], rectCount:int) -> None: ...
    def drawTextItem(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], textItem:QtGui.QTextItem) -> None: ...
    def drawTiledPixmap(self, r:Union[QtCore.QRectF, QtCore.QRect], pixmap:Union[QtGui.QPixmap, QtGui.QImage, str], s:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def end(self) -> bool: ...
    def hasFeature(self, feature:QtGui.QPaintEngine.PaintEngineFeatures) -> bool: ...
    def isActive(self) -> bool: ...
    def isExtended(self) -> bool: ...
    def paintDevice(self) -> QtGui.QPaintDevice: ...
    def painter(self) -> QtGui.QPainter: ...
    def setActive(self, newState:bool) -> None: ...
    def setDirty(self, df:QtGui.QPaintEngine.DirtyFlags) -> None: ...
    def setSystemClip(self, baseClip:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> None: ...
    def setSystemRect(self, rect:QtCore.QRect) -> None: ...
    def syncState(self) -> None: ...
    def systemClip(self) -> QtGui.QRegion: ...
    def systemRect(self) -> QtCore.QRect: ...
    def testDirty(self, df:QtGui.QPaintEngine.DirtyFlags) -> bool: ...
    def type(self) -> QtGui.QPaintEngine.Type: ...
    def updateState(self, state:QtGui.QPaintEngineState) -> None: ...


class QPaintEngineState(Shiboken.Object):

    def __init__(self) -> None: ...

    def backgroundBrush(self) -> QtGui.QBrush: ...
    def backgroundMode(self) -> QtCore.Qt.BGMode: ...
    def brush(self) -> QtGui.QBrush: ...
    def brushNeedsResolving(self) -> bool: ...
    def brushOrigin(self) -> QtCore.QPointF: ...
    def clipOperation(self) -> QtCore.Qt.ClipOperation: ...
    def clipPath(self) -> QtGui.QPainterPath: ...
    def clipRegion(self) -> QtGui.QRegion: ...
    def compositionMode(self) -> QtGui.QPainter.CompositionMode: ...
    def font(self) -> QtGui.QFont: ...
    def isClipEnabled(self) -> bool: ...
    def opacity(self) -> float: ...
    def painter(self) -> QtGui.QPainter: ...
    def pen(self) -> QtGui.QPen: ...
    def penNeedsResolving(self) -> bool: ...
    def renderHints(self) -> QtGui.QPainter.RenderHints: ...
    def state(self) -> QtGui.QPaintEngine.DirtyFlags: ...
    def transform(self) -> QtGui.QTransform: ...


class QPaintEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QPaintEvent) -> None: ...
    @overload
    def __init__(self, paintRect:QtCore.QRect) -> None: ...
    @overload
    def __init__(self, paintRegion:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> None: ...

    def clone(self) -> QtGui.QPaintEvent: ...
    def rect(self) -> QtCore.QRect: ...
    def region(self) -> QtGui.QRegion: ...


class QPainter(Shiboken.Object):

    CompositionMode_SourceOver: QPainter.CompositionMode = ... # 0x0
    CompositionMode_DestinationOver: QPainter.CompositionMode = ... # 0x1
    CompositionMode_Clear    : QPainter.CompositionMode = ... # 0x2
    CompositionMode_Source   : QPainter.CompositionMode = ... # 0x3
    CompositionMode_Destination: QPainter.CompositionMode = ... # 0x4
    CompositionMode_SourceIn : QPainter.CompositionMode = ... # 0x5
    CompositionMode_DestinationIn: QPainter.CompositionMode = ... # 0x6
    CompositionMode_SourceOut: QPainter.CompositionMode = ... # 0x7
    CompositionMode_DestinationOut: QPainter.CompositionMode = ... # 0x8
    CompositionMode_SourceAtop: QPainter.CompositionMode = ... # 0x9
    CompositionMode_DestinationAtop: QPainter.CompositionMode = ... # 0xa
    CompositionMode_Xor      : QPainter.CompositionMode = ... # 0xb
    CompositionMode_Plus     : QPainter.CompositionMode = ... # 0xc
    CompositionMode_Multiply : QPainter.CompositionMode = ... # 0xd
    CompositionMode_Screen   : QPainter.CompositionMode = ... # 0xe
    CompositionMode_Overlay  : QPainter.CompositionMode = ... # 0xf
    CompositionMode_Darken   : QPainter.CompositionMode = ... # 0x10
    CompositionMode_Lighten  : QPainter.CompositionMode = ... # 0x11
    CompositionMode_ColorDodge: QPainter.CompositionMode = ... # 0x12
    CompositionMode_ColorBurn: QPainter.CompositionMode = ... # 0x13
    CompositionMode_HardLight: QPainter.CompositionMode = ... # 0x14
    CompositionMode_SoftLight: QPainter.CompositionMode = ... # 0x15
    CompositionMode_Difference: QPainter.CompositionMode = ... # 0x16
    CompositionMode_Exclusion: QPainter.CompositionMode = ... # 0x17
    RasterOp_SourceOrDestination: QPainter.CompositionMode = ... # 0x18
    RasterOp_SourceAndDestination: QPainter.CompositionMode = ... # 0x19
    RasterOp_SourceXorDestination: QPainter.CompositionMode = ... # 0x1a
    RasterOp_NotSourceAndNotDestination: QPainter.CompositionMode = ... # 0x1b
    RasterOp_NotSourceOrNotDestination: QPainter.CompositionMode = ... # 0x1c
    RasterOp_NotSourceXorDestination: QPainter.CompositionMode = ... # 0x1d
    RasterOp_NotSource       : QPainter.CompositionMode = ... # 0x1e
    RasterOp_NotSourceAndDestination: QPainter.CompositionMode = ... # 0x1f
    RasterOp_SourceAndNotDestination: QPainter.CompositionMode = ... # 0x20
    RasterOp_NotSourceOrDestination: QPainter.CompositionMode = ... # 0x21
    RasterOp_SourceOrNotDestination: QPainter.CompositionMode = ... # 0x22
    RasterOp_ClearDestination: QPainter.CompositionMode = ... # 0x23
    RasterOp_SetDestination  : QPainter.CompositionMode = ... # 0x24
    RasterOp_NotDestination  : QPainter.CompositionMode = ... # 0x25
    OpaqueHint               : QPainter.PixmapFragmentHint = ... # 0x1
    Antialiasing             : QPainter.RenderHint = ... # 0x1
    TextAntialiasing         : QPainter.RenderHint = ... # 0x2
    SmoothPixmapTransform    : QPainter.RenderHint = ... # 0x4
    VerticalSubpixelPositioning: QPainter.RenderHint = ... # 0x8
    LosslessImageRendering   : QPainter.RenderHint = ... # 0x40

    class CompositionMode(Enum):

        CompositionMode_SourceOver: QPainter.CompositionMode # 0x0
        CompositionMode_DestinationOver: QPainter.CompositionMode # 0x1
        CompositionMode_Clear    : QPainter.CompositionMode # 0x2
        CompositionMode_Source   : QPainter.CompositionMode # 0x3
        CompositionMode_Destination: QPainter.CompositionMode # 0x4
        CompositionMode_SourceIn : QPainter.CompositionMode # 0x5
        CompositionMode_DestinationIn: QPainter.CompositionMode # 0x6
        CompositionMode_SourceOut: QPainter.CompositionMode # 0x7
        CompositionMode_DestinationOut: QPainter.CompositionMode # 0x8
        CompositionMode_SourceAtop: QPainter.CompositionMode # 0x9
        CompositionMode_DestinationAtop: QPainter.CompositionMode # 0xa
        CompositionMode_Xor      : QPainter.CompositionMode # 0xb
        CompositionMode_Plus     : QPainter.CompositionMode # 0xc
        CompositionMode_Multiply : QPainter.CompositionMode # 0xd
        CompositionMode_Screen   : QPainter.CompositionMode # 0xe
        CompositionMode_Overlay  : QPainter.CompositionMode # 0xf
        CompositionMode_Darken   : QPainter.CompositionMode # 0x10
        CompositionMode_Lighten  : QPainter.CompositionMode # 0x11
        CompositionMode_ColorDodge: QPainter.CompositionMode # 0x12
        CompositionMode_ColorBurn: QPainter.CompositionMode # 0x13
        CompositionMode_HardLight: QPainter.CompositionMode # 0x14
        CompositionMode_SoftLight: QPainter.CompositionMode # 0x15
        CompositionMode_Difference: QPainter.CompositionMode # 0x16
        CompositionMode_Exclusion: QPainter.CompositionMode # 0x17
        RasterOp_SourceOrDestination: QPainter.CompositionMode # 0x18
        RasterOp_SourceAndDestination: QPainter.CompositionMode # 0x19
        RasterOp_SourceXorDestination: QPainter.CompositionMode # 0x1a
        RasterOp_NotSourceAndNotDestination: QPainter.CompositionMode # 0x1b
        RasterOp_NotSourceOrNotDestination: QPainter.CompositionMode # 0x1c
        RasterOp_NotSourceXorDestination: QPainter.CompositionMode # 0x1d
        RasterOp_NotSource       : QPainter.CompositionMode # 0x1e
        RasterOp_NotSourceAndDestination: QPainter.CompositionMode # 0x1f
        RasterOp_SourceAndNotDestination: QPainter.CompositionMode # 0x20
        RasterOp_NotSourceOrDestination: QPainter.CompositionMode # 0x21
        RasterOp_SourceOrNotDestination: QPainter.CompositionMode # 0x22
        RasterOp_ClearDestination: QPainter.CompositionMode # 0x23
        RasterOp_SetDestination  : QPainter.CompositionMode # 0x24
        RasterOp_NotDestination  : QPainter.CompositionMode # 0x25

    class PixmapFragment(Shiboken.Object):

        @overload
        def __init__(self) -> None: ...
        @overload
        def __init__(self, PixmapFragment:QtGui.QPainter.PixmapFragment) -> None: ...

        @staticmethod
        def __copy__() -> None: ...
        @staticmethod
        def create(pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], sourceRect:Union[QtCore.QRectF, QtCore.QRect], scaleX:float=..., scaleY:float=..., rotation:float=..., opacity:float=...) -> QtGui.QPainter.PixmapFragment: ...

    class PixmapFragmentHint(Enum):

        OpaqueHint               : QPainter.PixmapFragmentHint # 0x1

    class PixmapFragmentHints(object): ...

    class RenderHint(Enum):

        Antialiasing             : QPainter.RenderHint # 0x1
        TextAntialiasing         : QPainter.RenderHint # 0x2
        SmoothPixmapTransform    : QPainter.RenderHint # 0x4
        VerticalSubpixelPositioning: QPainter.RenderHint # 0x8
        LosslessImageRendering   : QPainter.RenderHint # 0x40

    class RenderHints(object): ...


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:QtGui.QPaintDevice) -> None: ...

    def background(self) -> QtGui.QBrush: ...
    def backgroundMode(self) -> QtCore.Qt.BGMode: ...
    def begin(self, arg__1:QtGui.QPaintDevice) -> bool: ...
    def beginNativePainting(self) -> None: ...
    @overload
    def boundingRect(self, rect:QtCore.QRect, flags:int, text:str) -> QtCore.QRect: ...
    @overload
    def boundingRect(self, rect:Union[QtCore.QRectF, QtCore.QRect], flags:int, text:str) -> QtCore.QRectF: ...
    @overload
    def boundingRect(self, rect:Union[QtCore.QRectF, QtCore.QRect], text:str, o:Union[QtGui.QTextOption, QtCore.Qt.Alignment]=...) -> QtCore.QRectF: ...
    @overload
    def boundingRect(self, x:int, y:int, w:int, h:int, flags:int, text:str) -> QtCore.QRect: ...
    def brush(self) -> QtGui.QBrush: ...
    def brushOrigin(self) -> QtCore.QPoint: ...
    def clipBoundingRect(self) -> QtCore.QRectF: ...
    def clipPath(self) -> QtGui.QPainterPath: ...
    def clipRegion(self) -> QtGui.QRegion: ...
    def combinedTransform(self) -> QtGui.QTransform: ...
    def compositionMode(self) -> QtGui.QPainter.CompositionMode: ...
    def device(self) -> QtGui.QPaintDevice: ...
    def deviceTransform(self) -> QtGui.QTransform: ...
    @overload
    def drawArc(self, arg__1:QtCore.QRect, a:int, alen:int) -> None: ...
    @overload
    def drawArc(self, rect:Union[QtCore.QRectF, QtCore.QRect], a:int, alen:int) -> None: ...
    @overload
    def drawArc(self, x:int, y:int, w:int, h:int, a:int, alen:int) -> None: ...
    @overload
    def drawChord(self, arg__1:QtCore.QRect, a:int, alen:int) -> None: ...
    @overload
    def drawChord(self, rect:Union[QtCore.QRectF, QtCore.QRect], a:int, alen:int) -> None: ...
    @overload
    def drawChord(self, x:int, y:int, w:int, h:int, a:int, alen:int) -> None: ...
    @overload
    def drawConvexPolygon(self, arg__1:Sequence[QtCore.QPointF]) -> None: ...
    @overload
    def drawConvexPolygon(self, arg__1:Sequence[QtCore.QPoint]) -> None: ...
    @overload
    def drawConvexPolygon(self, polygon:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect]) -> None: ...
    @overload
    def drawConvexPolygon(self, polygon:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> None: ...
    @overload
    def drawEllipse(self, center:QtCore.QPoint, rx:int, ry:int) -> None: ...
    @overload
    def drawEllipse(self, center:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], rx:float, ry:float) -> None: ...
    @overload
    def drawEllipse(self, r:QtCore.QRect) -> None: ...
    @overload
    def drawEllipse(self, r:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    @overload
    def drawEllipse(self, x:int, y:int, w:int, h:int) -> None: ...
    def drawGlyphRun(self, position:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], glyphRun:QtGui.QGlyphRun) -> None: ...
    @overload
    def drawImage(self, p:QtCore.QPoint, image:Union[QtGui.QImage, str]) -> None: ...
    @overload
    def drawImage(self, p:QtCore.QPoint, image:Union[QtGui.QImage, str], sr:QtCore.QRect, flags:QtCore.Qt.ImageConversionFlags=...) -> None: ...
    @overload
    def drawImage(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], image:Union[QtGui.QImage, str]) -> None: ...
    @overload
    def drawImage(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], image:Union[QtGui.QImage, str], sr:Union[QtCore.QRectF, QtCore.QRect], flags:QtCore.Qt.ImageConversionFlags=...) -> None: ...
    @overload
    def drawImage(self, r:QtCore.QRect, image:Union[QtGui.QImage, str]) -> None: ...
    @overload
    def drawImage(self, r:Union[QtCore.QRectF, QtCore.QRect], image:Union[QtGui.QImage, str]) -> None: ...
    @overload
    def drawImage(self, targetRect:QtCore.QRect, image:Union[QtGui.QImage, str], sourceRect:QtCore.QRect, flags:QtCore.Qt.ImageConversionFlags=...) -> None: ...
    @overload
    def drawImage(self, targetRect:Union[QtCore.QRectF, QtCore.QRect], image:Union[QtGui.QImage, str], sourceRect:Union[QtCore.QRectF, QtCore.QRect], flags:QtCore.Qt.ImageConversionFlags=...) -> None: ...
    @overload
    def drawImage(self, x:int, y:int, image:Union[QtGui.QImage, str], sx:int=..., sy:int=..., sw:int=..., sh:int=..., flags:QtCore.Qt.ImageConversionFlags=...) -> None: ...
    @overload
    def drawLine(self, line:QtCore.QLine) -> None: ...
    @overload
    def drawLine(self, line:Union[QtCore.QLineF, QtCore.QLine]) -> None: ...
    @overload
    def drawLine(self, p1:QtCore.QPoint, p2:QtCore.QPoint) -> None: ...
    @overload
    def drawLine(self, p1:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], p2:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def drawLine(self, x1:int, y1:int, x2:int, y2:int) -> None: ...
    @overload
    def drawLines(self, lines:Sequence[QtCore.QLineF]) -> None: ...
    @overload
    def drawLines(self, lines:Sequence[QtCore.QLine]) -> None: ...
    @overload
    def drawLines(self, pointPairs:Sequence[QtCore.QPointF]) -> None: ...
    @overload
    def drawLines(self, pointPairs:Sequence[QtCore.QPoint]) -> None: ...
    def drawPath(self, path:QtGui.QPainterPath) -> None: ...
    @overload
    def drawPicture(self, p:QtCore.QPoint, picture:Union[QtGui.QPicture, int]) -> None: ...
    @overload
    def drawPicture(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], picture:Union[QtGui.QPicture, int]) -> None: ...
    @overload
    def drawPicture(self, x:int, y:int, picture:Union[QtGui.QPicture, int]) -> None: ...
    @overload
    def drawPie(self, arg__1:QtCore.QRect, a:int, alen:int) -> None: ...
    @overload
    def drawPie(self, rect:Union[QtCore.QRectF, QtCore.QRect], a:int, alen:int) -> None: ...
    @overload
    def drawPie(self, x:int, y:int, w:int, h:int, a:int, alen:int) -> None: ...
    @overload
    def drawPixmap(self, p:QtCore.QPoint, pm:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    @overload
    def drawPixmap(self, p:QtCore.QPoint, pm:Union[QtGui.QPixmap, QtGui.QImage, str], sr:QtCore.QRect) -> None: ...
    @overload
    def drawPixmap(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], pm:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    @overload
    def drawPixmap(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], pm:Union[QtGui.QPixmap, QtGui.QImage, str], sr:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    @overload
    def drawPixmap(self, r:QtCore.QRect, pm:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    @overload
    def drawPixmap(self, targetRect:QtCore.QRect, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str], sourceRect:QtCore.QRect) -> None: ...
    @overload
    def drawPixmap(self, targetRect:Union[QtCore.QRectF, QtCore.QRect], pixmap:Union[QtGui.QPixmap, QtGui.QImage, str], sourceRect:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    @overload
    def drawPixmap(self, x:int, y:int, pm:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    @overload
    def drawPixmap(self, x:int, y:int, pm:Union[QtGui.QPixmap, QtGui.QImage, str], sx:int, sy:int, sw:int, sh:int) -> None: ...
    @overload
    def drawPixmap(self, x:int, y:int, w:int, h:int, pm:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    @overload
    def drawPixmap(self, x:int, y:int, w:int, h:int, pm:Union[QtGui.QPixmap, QtGui.QImage, str], sx:int, sy:int, sw:int, sh:int) -> None: ...
    def drawPixmapFragments(self, fragments:QtGui.QPainter.PixmapFragment, fragmentCount:int, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str], hints:QtGui.QPainter.PixmapFragmentHints=...) -> None: ...
    @overload
    def drawPoint(self, p:QtCore.QPoint) -> None: ...
    @overload
    def drawPoint(self, pt:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def drawPoint(self, x:int, y:int) -> None: ...
    @overload
    def drawPoints(self, arg__1:Sequence[QtCore.QPointF]) -> None: ...
    @overload
    def drawPoints(self, arg__1:Sequence[QtCore.QPoint]) -> None: ...
    @overload
    def drawPoints(self, points:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect]) -> None: ...
    @overload
    def drawPoints(self, points:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> None: ...
    def drawPointsNp(self, x:shibokensupport.signature.mapping.ArrayLikeVariable, y:shibokensupport.signature.mapping.ArrayLikeVariable) -> None: ...
    @overload
    def drawPolygon(self, arg__1:Sequence[QtCore.QPointF], arg__2:QtCore.Qt.FillRule) -> None: ...
    @overload
    def drawPolygon(self, arg__1:Sequence[QtCore.QPoint], arg__2:QtCore.Qt.FillRule) -> None: ...
    @overload
    def drawPolygon(self, polygon:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect], fillRule:QtCore.Qt.FillRule=...) -> None: ...
    @overload
    def drawPolygon(self, polygon:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF], fillRule:QtCore.Qt.FillRule=...) -> None: ...
    @overload
    def drawPolyline(self, arg__1:Sequence[QtCore.QPointF]) -> None: ...
    @overload
    def drawPolyline(self, arg__1:Sequence[QtCore.QPoint]) -> None: ...
    @overload
    def drawPolyline(self, polygon:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect]) -> None: ...
    @overload
    def drawPolyline(self, polyline:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> None: ...
    @overload
    def drawRect(self, rect:QtCore.QRect) -> None: ...
    @overload
    def drawRect(self, rect:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    @overload
    def drawRect(self, x1:int, y1:int, w:int, h:int) -> None: ...
    @overload
    def drawRects(self, rectangles:Sequence[QtCore.QRectF]) -> None: ...
    @overload
    def drawRects(self, rectangles:Sequence[QtCore.QRect]) -> None: ...
    @overload
    def drawRoundedRect(self, rect:QtCore.QRect, xRadius:float, yRadius:float, mode:QtCore.Qt.SizeMode=...) -> None: ...
    @overload
    def drawRoundedRect(self, rect:Union[QtCore.QRectF, QtCore.QRect], xRadius:float, yRadius:float, mode:QtCore.Qt.SizeMode=...) -> None: ...
    @overload
    def drawRoundedRect(self, x:int, y:int, w:int, h:int, xRadius:float, yRadius:float, mode:QtCore.Qt.SizeMode=...) -> None: ...
    @overload
    def drawStaticText(self, left:int, top:int, staticText:QtGui.QStaticText) -> None: ...
    @overload
    def drawStaticText(self, topLeftPosition:QtCore.QPoint, staticText:QtGui.QStaticText) -> None: ...
    @overload
    def drawStaticText(self, topLeftPosition:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], staticText:QtGui.QStaticText) -> None: ...
    @overload
    def drawText(self, p:QtCore.QPoint, s:str) -> None: ...
    @overload
    def drawText(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], s:str) -> None: ...
    @overload
    def drawText(self, r:QtCore.QRect, flags:int, text:str, br:QtCore.QRect) -> None: ...
    @overload
    def drawText(self, r:Union[QtCore.QRectF, QtCore.QRect], flags:int, text:str, br:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    @overload
    def drawText(self, r:Union[QtCore.QRectF, QtCore.QRect], text:str, o:Union[QtGui.QTextOption, QtCore.Qt.Alignment]=...) -> None: ...
    @overload
    def drawText(self, x:int, y:int, s:str) -> None: ...
    @overload
    def drawText(self, x:int, y:int, w:int, h:int, flags:int, text:str, br:QtCore.QRect) -> None: ...
    @overload
    def drawTextItem(self, p:QtCore.QPoint, ti:QtGui.QTextItem) -> None: ...
    @overload
    def drawTextItem(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], ti:QtGui.QTextItem) -> None: ...
    @overload
    def drawTextItem(self, x:int, y:int, ti:QtGui.QTextItem) -> None: ...
    @overload
    def drawTiledPixmap(self, arg__1:QtCore.QRect, arg__2:Union[QtGui.QPixmap, QtGui.QImage, str], pos:QtCore.QPoint=...) -> None: ...
    @overload
    def drawTiledPixmap(self, rect:Union[QtCore.QRectF, QtCore.QRect], pm:Union[QtGui.QPixmap, QtGui.QImage, str], offset:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]=...) -> None: ...
    @overload
    def drawTiledPixmap(self, x:int, y:int, w:int, h:int, arg__5:Union[QtGui.QPixmap, QtGui.QImage, str], sx:int=..., sy:int=...) -> None: ...
    def end(self) -> bool: ...
    def endNativePainting(self) -> None: ...
    @overload
    def eraseRect(self, arg__1:QtCore.QRect) -> None: ...
    @overload
    def eraseRect(self, arg__1:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    @overload
    def eraseRect(self, x:int, y:int, w:int, h:int) -> None: ...
    def fillPath(self, path:QtGui.QPainterPath, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    @overload
    def fillRect(self, arg__1:QtCore.QRect, arg__2:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    @overload
    def fillRect(self, arg__1:QtCore.QRect, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    @overload
    def fillRect(self, arg__1:Union[QtCore.QRectF, QtCore.QRect], arg__2:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    @overload
    def fillRect(self, arg__1:Union[QtCore.QRectF, QtCore.QRect], color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    @overload
    def fillRect(self, r:QtCore.QRect, c:QtCore.Qt.GlobalColor) -> None: ...
    @overload
    def fillRect(self, r:QtCore.QRect, preset:QtGui.QGradient.Preset) -> None: ...
    @overload
    def fillRect(self, r:QtCore.QRect, style:QtCore.Qt.BrushStyle) -> None: ...
    @overload
    def fillRect(self, r:Union[QtCore.QRectF, QtCore.QRect], c:QtCore.Qt.GlobalColor) -> None: ...
    @overload
    def fillRect(self, r:Union[QtCore.QRectF, QtCore.QRect], preset:QtGui.QGradient.Preset) -> None: ...
    @overload
    def fillRect(self, r:Union[QtCore.QRectF, QtCore.QRect], style:QtCore.Qt.BrushStyle) -> None: ...
    @overload
    def fillRect(self, x:int, y:int, w:int, h:int, arg__5:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    @overload
    def fillRect(self, x:int, y:int, w:int, h:int, c:QtCore.Qt.GlobalColor) -> None: ...
    @overload
    def fillRect(self, x:int, y:int, w:int, h:int, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    @overload
    def fillRect(self, x:int, y:int, w:int, h:int, preset:QtGui.QGradient.Preset) -> None: ...
    @overload
    def fillRect(self, x:int, y:int, w:int, h:int, style:QtCore.Qt.BrushStyle) -> None: ...
    def font(self) -> QtGui.QFont: ...
    def fontInfo(self) -> QtGui.QFontInfo: ...
    def fontMetrics(self) -> QtGui.QFontMetrics: ...
    def hasClipping(self) -> bool: ...
    def isActive(self) -> bool: ...
    def layoutDirection(self) -> QtCore.Qt.LayoutDirection: ...
    def opacity(self) -> float: ...
    def paintEngine(self) -> QtGui.QPaintEngine: ...
    def pen(self) -> QtGui.QPen: ...
    def renderHints(self) -> QtGui.QPainter.RenderHints: ...
    def resetTransform(self) -> None: ...
    def restore(self) -> None: ...
    def rotate(self, a:float) -> None: ...
    def save(self) -> None: ...
    def scale(self, sx:float, sy:float) -> None: ...
    def setBackground(self, bg:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setBackgroundMode(self, mode:QtCore.Qt.BGMode) -> None: ...
    @overload
    def setBrush(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    @overload
    def setBrush(self, style:QtCore.Qt.BrushStyle) -> None: ...
    @overload
    def setBrushOrigin(self, arg__1:QtCore.QPoint) -> None: ...
    @overload
    def setBrushOrigin(self, arg__1:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def setBrushOrigin(self, x:int, y:int) -> None: ...
    def setClipPath(self, path:QtGui.QPainterPath, op:QtCore.Qt.ClipOperation=...) -> None: ...
    @overload
    def setClipRect(self, arg__1:QtCore.QRect, op:QtCore.Qt.ClipOperation=...) -> None: ...
    @overload
    def setClipRect(self, arg__1:Union[QtCore.QRectF, QtCore.QRect], op:QtCore.Qt.ClipOperation=...) -> None: ...
    @overload
    def setClipRect(self, x:int, y:int, w:int, h:int, op:QtCore.Qt.ClipOperation=...) -> None: ...
    def setClipRegion(self, arg__1:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect], op:QtCore.Qt.ClipOperation=...) -> None: ...
    def setClipping(self, enable:bool) -> None: ...
    def setCompositionMode(self, mode:QtGui.QPainter.CompositionMode) -> None: ...
    def setFont(self, f:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    def setLayoutDirection(self, direction:QtCore.Qt.LayoutDirection) -> None: ...
    def setOpacity(self, opacity:float) -> None: ...
    @overload
    def setPen(self, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    @overload
    def setPen(self, pen:Union[QtGui.QPen, QtCore.Qt.PenStyle, QtGui.QColor]) -> None: ...
    @overload
    def setPen(self, style:QtCore.Qt.PenStyle) -> None: ...
    def setRenderHint(self, hint:QtGui.QPainter.RenderHint, on:bool=...) -> None: ...
    def setRenderHints(self, hints:QtGui.QPainter.RenderHints, on:bool=...) -> None: ...
    def setTransform(self, transform:QtGui.QTransform, combine:bool=...) -> None: ...
    def setViewTransformEnabled(self, enable:bool) -> None: ...
    @overload
    def setViewport(self, viewport:QtCore.QRect) -> None: ...
    @overload
    def setViewport(self, x:int, y:int, w:int, h:int) -> None: ...
    @overload
    def setWindow(self, window:QtCore.QRect) -> None: ...
    @overload
    def setWindow(self, x:int, y:int, w:int, h:int) -> None: ...
    def setWorldMatrixEnabled(self, enabled:bool) -> None: ...
    def setWorldTransform(self, matrix:QtGui.QTransform, combine:bool=...) -> None: ...
    def shear(self, sh:float, sv:float) -> None: ...
    def strokePath(self, path:QtGui.QPainterPath, pen:Union[QtGui.QPen, QtCore.Qt.PenStyle, QtGui.QColor]) -> None: ...
    def testRenderHint(self, hint:QtGui.QPainter.RenderHint) -> bool: ...
    def transform(self) -> QtGui.QTransform: ...
    @overload
    def translate(self, dx:float, dy:float) -> None: ...
    @overload
    def translate(self, offset:QtCore.QPoint) -> None: ...
    @overload
    def translate(self, offset:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def viewTransformEnabled(self) -> bool: ...
    def viewport(self) -> QtCore.QRect: ...
    def window(self) -> QtCore.QRect: ...
    def worldMatrixEnabled(self) -> bool: ...
    def worldTransform(self) -> QtGui.QTransform: ...


class QPainterPath(Shiboken.Object):

    MoveToElement            : QPainterPath.ElementType = ... # 0x0
    LineToElement            : QPainterPath.ElementType = ... # 0x1
    CurveToElement           : QPainterPath.ElementType = ... # 0x2
    CurveToDataElement       : QPainterPath.ElementType = ... # 0x3

    class Element(Shiboken.Object):

        @overload
        def __init__(self) -> None: ...
        @overload
        def __init__(self, Element:QtGui.QPainterPath.Element) -> None: ...

        @staticmethod
        def __copy__() -> None: ...
        def isCurveTo(self) -> bool: ...
        def isLineTo(self) -> bool: ...
        def isMoveTo(self) -> bool: ...

    class ElementType(Enum):

        MoveToElement            : QPainterPath.ElementType # 0x0
        LineToElement            : QPainterPath.ElementType # 0x1
        CurveToElement           : QPainterPath.ElementType # 0x2
        CurveToDataElement       : QPainterPath.ElementType # 0x3


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, other:QtGui.QPainterPath) -> None: ...
    @overload
    def __init__(self, startPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...

    def __add__(self, other:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    def __and__(self, other:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, other:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    def __iand__(self, other:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    def __ior__(self, other:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    def __isub__(self, other:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __mul__(self, m:QtGui.QTransform) -> QtGui.QPainterPath: ...
    def __or__(self, other:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __sub__(self, other:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    @overload
    def addEllipse(self, center:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], rx:float, ry:float) -> None: ...
    @overload
    def addEllipse(self, rect:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    @overload
    def addEllipse(self, x:float, y:float, w:float, h:float) -> None: ...
    def addPath(self, path:QtGui.QPainterPath) -> None: ...
    def addPolygon(self, polygon:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> None: ...
    @overload
    def addRect(self, rect:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    @overload
    def addRect(self, x:float, y:float, w:float, h:float) -> None: ...
    def addRegion(self, region:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> None: ...
    @overload
    def addRoundedRect(self, rect:Union[QtCore.QRectF, QtCore.QRect], xRadius:float, yRadius:float, mode:QtCore.Qt.SizeMode=...) -> None: ...
    @overload
    def addRoundedRect(self, x:float, y:float, w:float, h:float, xRadius:float, yRadius:float, mode:QtCore.Qt.SizeMode=...) -> None: ...
    @overload
    def addText(self, point:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], f:Union[QtGui.QFont, str, Sequence[str]], text:str) -> None: ...
    @overload
    def addText(self, x:float, y:float, f:Union[QtGui.QFont, str, Sequence[str]], text:str) -> None: ...
    def angleAtPercent(self, t:float) -> float: ...
    @overload
    def arcMoveTo(self, rect:Union[QtCore.QRectF, QtCore.QRect], angle:float) -> None: ...
    @overload
    def arcMoveTo(self, x:float, y:float, w:float, h:float, angle:float) -> None: ...
    @overload
    def arcTo(self, rect:Union[QtCore.QRectF, QtCore.QRect], startAngle:float, arcLength:float) -> None: ...
    @overload
    def arcTo(self, x:float, y:float, w:float, h:float, startAngle:float, arcLength:float) -> None: ...
    def boundingRect(self) -> QtCore.QRectF: ...
    def capacity(self) -> int: ...
    def clear(self) -> None: ...
    def closeSubpath(self) -> None: ...
    def connectPath(self, path:QtGui.QPainterPath) -> None: ...
    @overload
    def contains(self, p:QtGui.QPainterPath) -> bool: ...
    @overload
    def contains(self, pt:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> bool: ...
    @overload
    def contains(self, rect:Union[QtCore.QRectF, QtCore.QRect]) -> bool: ...
    def controlPointRect(self) -> QtCore.QRectF: ...
    @overload
    def cubicTo(self, ctrlPt1:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], ctrlPt2:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], endPt:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def cubicTo(self, ctrlPt1x:float, ctrlPt1y:float, ctrlPt2x:float, ctrlPt2y:float, endPtx:float, endPty:float) -> None: ...
    def currentPosition(self) -> QtCore.QPointF: ...
    def elementAt(self, i:int) -> QtGui.QPainterPath.Element: ...
    def elementCount(self) -> int: ...
    def fillRule(self) -> QtCore.Qt.FillRule: ...
    def intersected(self, r:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    @overload
    def intersects(self, p:QtGui.QPainterPath) -> bool: ...
    @overload
    def intersects(self, rect:Union[QtCore.QRectF, QtCore.QRect]) -> bool: ...
    def isEmpty(self) -> bool: ...
    def length(self) -> float: ...
    @overload
    def lineTo(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def lineTo(self, x:float, y:float) -> None: ...
    @overload
    def moveTo(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def moveTo(self, x:float, y:float) -> None: ...
    def percentAtLength(self, t:float) -> float: ...
    def pointAtPercent(self, t:float) -> QtCore.QPointF: ...
    @overload
    def quadTo(self, ctrlPt:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], endPt:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def quadTo(self, ctrlPtx:float, ctrlPty:float, endPtx:float, endPty:float) -> None: ...
    def reserve(self, size:int) -> None: ...
    def setElementPositionAt(self, i:int, x:float, y:float) -> None: ...
    def setFillRule(self, fillRule:QtCore.Qt.FillRule) -> None: ...
    def simplified(self) -> QtGui.QPainterPath: ...
    def slopeAtPercent(self, t:float) -> float: ...
    def subtracted(self, r:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    def swap(self, other:QtGui.QPainterPath) -> None: ...
    def toFillPolygon(self, matrix:QtGui.QTransform=...) -> QtGui.QPolygonF: ...
    def toFillPolygons(self, matrix:QtGui.QTransform=...) -> List[QtGui.QPolygonF]: ...
    def toReversed(self) -> QtGui.QPainterPath: ...
    def toSubpathPolygons(self, matrix:QtGui.QTransform=...) -> List[QtGui.QPolygonF]: ...
    @overload
    def translate(self, dx:float, dy:float) -> None: ...
    @overload
    def translate(self, offset:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def translated(self, dx:float, dy:float) -> QtGui.QPainterPath: ...
    @overload
    def translated(self, offset:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> QtGui.QPainterPath: ...
    def united(self, r:QtGui.QPainterPath) -> QtGui.QPainterPath: ...


class QPainterPathStroker(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, pen:Union[QtGui.QPen, QtCore.Qt.PenStyle, QtGui.QColor]) -> None: ...

    def capStyle(self) -> QtCore.Qt.PenCapStyle: ...
    def createStroke(self, path:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    def curveThreshold(self) -> float: ...
    def dashOffset(self) -> float: ...
    def dashPattern(self) -> List[float]: ...
    def joinStyle(self) -> QtCore.Qt.PenJoinStyle: ...
    def miterLimit(self) -> float: ...
    def setCapStyle(self, style:QtCore.Qt.PenCapStyle) -> None: ...
    def setCurveThreshold(self, threshold:float) -> None: ...
    def setDashOffset(self, offset:float) -> None: ...
    @overload
    def setDashPattern(self, arg__1:QtCore.Qt.PenStyle) -> None: ...
    @overload
    def setDashPattern(self, dashPattern:Sequence[float]) -> None: ...
    def setJoinStyle(self, style:QtCore.Qt.PenJoinStyle) -> None: ...
    def setMiterLimit(self, length:float) -> None: ...
    def setWidth(self, width:float) -> None: ...
    def width(self) -> float: ...


class QPalette(Shiboken.Object):

    Active                   : QPalette.ColorGroup = ... # 0x0
    Normal                   : QPalette.ColorGroup = ... # 0x0
    Disabled                 : QPalette.ColorGroup = ... # 0x1
    Inactive                 : QPalette.ColorGroup = ... # 0x2
    NColorGroups             : QPalette.ColorGroup = ... # 0x3
    Current                  : QPalette.ColorGroup = ... # 0x4
    All                      : QPalette.ColorGroup = ... # 0x5
    WindowText               : QPalette.ColorRole = ... # 0x0
    Button                   : QPalette.ColorRole = ... # 0x1
    Light                    : QPalette.ColorRole = ... # 0x2
    Midlight                 : QPalette.ColorRole = ... # 0x3
    Dark                     : QPalette.ColorRole = ... # 0x4
    Mid                      : QPalette.ColorRole = ... # 0x5
    Text                     : QPalette.ColorRole = ... # 0x6
    BrightText               : QPalette.ColorRole = ... # 0x7
    ButtonText               : QPalette.ColorRole = ... # 0x8
    Base                     : QPalette.ColorRole = ... # 0x9
    Window                   : QPalette.ColorRole = ... # 0xa
    Shadow                   : QPalette.ColorRole = ... # 0xb
    Highlight                : QPalette.ColorRole = ... # 0xc
    HighlightedText          : QPalette.ColorRole = ... # 0xd
    Link                     : QPalette.ColorRole = ... # 0xe
    LinkVisited              : QPalette.ColorRole = ... # 0xf
    AlternateBase            : QPalette.ColorRole = ... # 0x10
    NoRole                   : QPalette.ColorRole = ... # 0x11
    ToolTipBase              : QPalette.ColorRole = ... # 0x12
    ToolTipText              : QPalette.ColorRole = ... # 0x13
    PlaceholderText          : QPalette.ColorRole = ... # 0x14
    NColorRoles              : QPalette.ColorRole = ... # 0x15

    class ColorGroup(Enum):

        Active                   : QPalette.ColorGroup # 0x0
        Normal                   : QPalette.ColorGroup # 0x0
        Disabled                 : QPalette.ColorGroup # 0x1
        Inactive                 : QPalette.ColorGroup # 0x2
        NColorGroups             : QPalette.ColorGroup # 0x3
        Current                  : QPalette.ColorGroup # 0x4
        All                      : QPalette.ColorGroup # 0x5

    class ColorRole(Enum):

        WindowText               : QPalette.ColorRole # 0x0
        Button                   : QPalette.ColorRole # 0x1
        Light                    : QPalette.ColorRole # 0x2
        Midlight                 : QPalette.ColorRole # 0x3
        Dark                     : QPalette.ColorRole # 0x4
        Mid                      : QPalette.ColorRole # 0x5
        Text                     : QPalette.ColorRole # 0x6
        BrightText               : QPalette.ColorRole # 0x7
        ButtonText               : QPalette.ColorRole # 0x8
        Base                     : QPalette.ColorRole # 0x9
        Window                   : QPalette.ColorRole # 0xa
        Shadow                   : QPalette.ColorRole # 0xb
        Highlight                : QPalette.ColorRole # 0xc
        HighlightedText          : QPalette.ColorRole # 0xd
        Link                     : QPalette.ColorRole # 0xe
        LinkVisited              : QPalette.ColorRole # 0xf
        AlternateBase            : QPalette.ColorRole # 0x10
        NoRole                   : QPalette.ColorRole # 0x11
        ToolTipBase              : QPalette.ColorRole # 0x12
        ToolTipText              : QPalette.ColorRole # 0x13
        PlaceholderText          : QPalette.ColorRole # 0x14
        NColorRoles              : QPalette.ColorRole # 0x15


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, button:QtCore.Qt.GlobalColor) -> None: ...
    @overload
    def __init__(self, button:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    @overload
    def __init__(self, button:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int], window:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    @overload
    def __init__(self, palette:Union[QtGui.QPalette, QtCore.Qt.GlobalColor, QtGui.QColor]) -> None: ...
    @overload
    def __init__(self, windowText:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], button:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], light:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], dark:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], mid:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], text:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], bright_text:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], base:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], window:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    @overload
    def __init__(self, windowText:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int], window:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int], light:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int], dark:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int], mid:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int], text:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int], base:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, ds:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, ds:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def alternateBase(self) -> QtGui.QBrush: ...
    def base(self) -> QtGui.QBrush: ...
    def brightText(self) -> QtGui.QBrush: ...
    @overload
    def brush(self, cg:QtGui.QPalette.ColorGroup, cr:QtGui.QPalette.ColorRole) -> QtGui.QBrush: ...
    @overload
    def brush(self, cr:QtGui.QPalette.ColorRole) -> QtGui.QBrush: ...
    def button(self) -> QtGui.QBrush: ...
    def buttonText(self) -> QtGui.QBrush: ...
    def cacheKey(self) -> int: ...
    @overload
    def color(self, cg:QtGui.QPalette.ColorGroup, cr:QtGui.QPalette.ColorRole) -> QtGui.QColor: ...
    @overload
    def color(self, cr:QtGui.QPalette.ColorRole) -> QtGui.QColor: ...
    def currentColorGroup(self) -> QtGui.QPalette.ColorGroup: ...
    def dark(self) -> QtGui.QBrush: ...
    def highlight(self) -> QtGui.QBrush: ...
    def highlightedText(self) -> QtGui.QBrush: ...
    def isBrushSet(self, cg:QtGui.QPalette.ColorGroup, cr:QtGui.QPalette.ColorRole) -> bool: ...
    def isCopyOf(self, p:Union[QtGui.QPalette, QtCore.Qt.GlobalColor, QtGui.QColor]) -> bool: ...
    def isEqual(self, cr1:QtGui.QPalette.ColorGroup, cr2:QtGui.QPalette.ColorGroup) -> bool: ...
    def light(self) -> QtGui.QBrush: ...
    def link(self) -> QtGui.QBrush: ...
    def linkVisited(self) -> QtGui.QBrush: ...
    def mid(self) -> QtGui.QBrush: ...
    def midlight(self) -> QtGui.QBrush: ...
    def placeholderText(self) -> QtGui.QBrush: ...
    def resolve(self, other:Union[QtGui.QPalette, QtCore.Qt.GlobalColor, QtGui.QColor]) -> QtGui.QPalette: ...
    def resolveMask(self) -> int: ...
    @overload
    def setBrush(self, cg:QtGui.QPalette.ColorGroup, cr:QtGui.QPalette.ColorRole, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    @overload
    def setBrush(self, cr:QtGui.QPalette.ColorRole, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    @overload
    def setColor(self, cg:QtGui.QPalette.ColorGroup, cr:QtGui.QPalette.ColorRole, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    @overload
    def setColor(self, cr:QtGui.QPalette.ColorRole, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    def setColorGroup(self, cr:QtGui.QPalette.ColorGroup, windowText:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], button:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], light:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], dark:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], mid:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], text:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], bright_text:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], base:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], window:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setCurrentColorGroup(self, cg:QtGui.QPalette.ColorGroup) -> None: ...
    def setResolveMask(self, mask:int) -> None: ...
    def shadow(self) -> QtGui.QBrush: ...
    def swap(self, other:Union[QtGui.QPalette, QtCore.Qt.GlobalColor, QtGui.QColor]) -> None: ...
    def text(self) -> QtGui.QBrush: ...
    def toolTipBase(self) -> QtGui.QBrush: ...
    def toolTipText(self) -> QtGui.QBrush: ...
    def window(self) -> QtGui.QBrush: ...
    def windowText(self) -> QtGui.QBrush: ...


class QPdfWriter(QtCore.QObject, QtGui.QPagedPaintDevice):

    @overload
    def __init__(self, device:QtCore.QIODevice) -> None: ...
    @overload
    def __init__(self, filename:str) -> None: ...

    def addFileAttachment(self, fileName:str, data:Union[QtCore.QByteArray, bytes], mimeType:str=...) -> None: ...
    def creator(self) -> str: ...
    def documentXmpMetadata(self) -> QtCore.QByteArray: ...
    def metric(self, id:QtGui.QPaintDevice.PaintDeviceMetric) -> int: ...
    def newPage(self) -> bool: ...
    def paintEngine(self) -> QtGui.QPaintEngine: ...
    def pdfVersion(self) -> QtGui.QPagedPaintDevice.PdfVersion: ...
    def resolution(self) -> int: ...
    def setCreator(self, creator:str) -> None: ...
    def setDocumentXmpMetadata(self, xmpMetadata:Union[QtCore.QByteArray, bytes]) -> None: ...
    def setPdfVersion(self, version:QtGui.QPagedPaintDevice.PdfVersion) -> None: ...
    def setResolution(self, resolution:int) -> None: ...
    def setTitle(self, title:str) -> None: ...
    def title(self) -> str: ...


class QPen(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:QtCore.Qt.PenStyle) -> None: ...
    @overload
    def __init__(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap], width:float, s:QtCore.Qt.PenStyle=..., c:QtCore.Qt.PenCapStyle=..., j:QtCore.Qt.PenJoinStyle=...) -> None: ...
    @overload
    def __init__(self, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    @overload
    def __init__(self, pen:Union[QtGui.QPen, QtCore.Qt.PenStyle, QtGui.QColor]) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def brush(self) -> QtGui.QBrush: ...
    def capStyle(self) -> QtCore.Qt.PenCapStyle: ...
    def color(self) -> QtGui.QColor: ...
    def dashOffset(self) -> float: ...
    def dashPattern(self) -> List[float]: ...
    def isCosmetic(self) -> bool: ...
    def isSolid(self) -> bool: ...
    def joinStyle(self) -> QtCore.Qt.PenJoinStyle: ...
    def miterLimit(self) -> float: ...
    def setBrush(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setCapStyle(self, pcs:QtCore.Qt.PenCapStyle) -> None: ...
    def setColor(self, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    def setCosmetic(self, cosmetic:bool) -> None: ...
    def setDashOffset(self, doffset:float) -> None: ...
    def setDashPattern(self, pattern:Sequence[float]) -> None: ...
    def setJoinStyle(self, pcs:QtCore.Qt.PenJoinStyle) -> None: ...
    def setMiterLimit(self, limit:float) -> None: ...
    def setStyle(self, arg__1:QtCore.Qt.PenStyle) -> None: ...
    def setWidth(self, width:int) -> None: ...
    def setWidthF(self, width:float) -> None: ...
    def style(self) -> QtCore.Qt.PenStyle: ...
    def swap(self, other:Union[QtGui.QPen, QtCore.Qt.PenStyle, QtGui.QColor]) -> None: ...
    def width(self) -> int: ...
    def widthF(self) -> float: ...


class QPicture(QtGui.QPaintDevice):

    @overload
    def __init__(self, arg__1:Union[QtGui.QPicture, int]) -> None: ...
    @overload
    def __init__(self, formatVersion:int=...) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def boundingRect(self) -> QtCore.QRect: ...
    def data(self) -> object: ...
    def devType(self) -> int: ...
    def isNull(self) -> bool: ...
    @overload
    def load(self, dev:QtCore.QIODevice) -> bool: ...
    @overload
    def load(self, fileName:str) -> bool: ...
    def metric(self, m:QtGui.QPaintDevice.PaintDeviceMetric) -> int: ...
    def paintEngine(self) -> QtGui.QPaintEngine: ...
    def play(self, p:QtGui.QPainter) -> bool: ...
    @overload
    def save(self, dev:QtCore.QIODevice) -> bool: ...
    @overload
    def save(self, fileName:str) -> bool: ...
    def setBoundingRect(self, r:QtCore.QRect) -> None: ...
    def setData(self, data:bytes, size:int) -> None: ...
    def size(self) -> int: ...
    def swap(self, other:Union[QtGui.QPicture, int]) -> None: ...


class QPixelFormat(Shiboken.Object):

    AtBeginning              : QPixelFormat.AlphaPosition = ... # 0x0
    AtEnd                    : QPixelFormat.AlphaPosition = ... # 0x1
    NotPremultiplied         : QPixelFormat.AlphaPremultiplied = ... # 0x0
    Premultiplied            : QPixelFormat.AlphaPremultiplied = ... # 0x1
    UsesAlpha                : QPixelFormat.AlphaUsage = ... # 0x0
    IgnoresAlpha             : QPixelFormat.AlphaUsage = ... # 0x1
    LittleEndian             : QPixelFormat.ByteOrder = ... # 0x0
    BigEndian                : QPixelFormat.ByteOrder = ... # 0x1
    CurrentSystemEndian      : QPixelFormat.ByteOrder = ... # 0x2
    RGB                      : QPixelFormat.ColorModel = ... # 0x0
    BGR                      : QPixelFormat.ColorModel = ... # 0x1
    Indexed                  : QPixelFormat.ColorModel = ... # 0x2
    Grayscale                : QPixelFormat.ColorModel = ... # 0x3
    CMYK                     : QPixelFormat.ColorModel = ... # 0x4
    HSL                      : QPixelFormat.ColorModel = ... # 0x5
    HSV                      : QPixelFormat.ColorModel = ... # 0x6
    YUV                      : QPixelFormat.ColorModel = ... # 0x7
    Alpha                    : QPixelFormat.ColorModel = ... # 0x8
    UnsignedInteger          : QPixelFormat.TypeInterpretation = ... # 0x0
    UnsignedShort            : QPixelFormat.TypeInterpretation = ... # 0x1
    UnsignedByte             : QPixelFormat.TypeInterpretation = ... # 0x2
    FloatingPoint            : QPixelFormat.TypeInterpretation = ... # 0x3
    YUV444                   : QPixelFormat.YUVLayout = ... # 0x0
    YUV422                   : QPixelFormat.YUVLayout = ... # 0x1
    YUV411                   : QPixelFormat.YUVLayout = ... # 0x2
    YUV420P                  : QPixelFormat.YUVLayout = ... # 0x3
    YUV420SP                 : QPixelFormat.YUVLayout = ... # 0x4
    YV12                     : QPixelFormat.YUVLayout = ... # 0x5
    UYVY                     : QPixelFormat.YUVLayout = ... # 0x6
    YUYV                     : QPixelFormat.YUVLayout = ... # 0x7
    NV12                     : QPixelFormat.YUVLayout = ... # 0x8
    NV21                     : QPixelFormat.YUVLayout = ... # 0x9
    IMC1                     : QPixelFormat.YUVLayout = ... # 0xa
    IMC2                     : QPixelFormat.YUVLayout = ... # 0xb
    IMC3                     : QPixelFormat.YUVLayout = ... # 0xc
    IMC4                     : QPixelFormat.YUVLayout = ... # 0xd
    Y8                       : QPixelFormat.YUVLayout = ... # 0xe
    Y16                      : QPixelFormat.YUVLayout = ... # 0xf

    class AlphaPosition(Enum):

        AtBeginning              : QPixelFormat.AlphaPosition # 0x0
        AtEnd                    : QPixelFormat.AlphaPosition # 0x1

    class AlphaPremultiplied(Enum):

        NotPremultiplied         : QPixelFormat.AlphaPremultiplied # 0x0
        Premultiplied            : QPixelFormat.AlphaPremultiplied # 0x1

    class AlphaUsage(Enum):

        UsesAlpha                : QPixelFormat.AlphaUsage # 0x0
        IgnoresAlpha             : QPixelFormat.AlphaUsage # 0x1

    class ByteOrder(Enum):

        LittleEndian             : QPixelFormat.ByteOrder # 0x0
        BigEndian                : QPixelFormat.ByteOrder # 0x1
        CurrentSystemEndian      : QPixelFormat.ByteOrder # 0x2

    class ColorModel(Enum):

        RGB                      : QPixelFormat.ColorModel # 0x0
        BGR                      : QPixelFormat.ColorModel # 0x1
        Indexed                  : QPixelFormat.ColorModel # 0x2
        Grayscale                : QPixelFormat.ColorModel # 0x3
        CMYK                     : QPixelFormat.ColorModel # 0x4
        HSL                      : QPixelFormat.ColorModel # 0x5
        HSV                      : QPixelFormat.ColorModel # 0x6
        YUV                      : QPixelFormat.ColorModel # 0x7
        Alpha                    : QPixelFormat.ColorModel # 0x8

    class TypeInterpretation(Enum):

        UnsignedInteger          : QPixelFormat.TypeInterpretation # 0x0
        UnsignedShort            : QPixelFormat.TypeInterpretation # 0x1
        UnsignedByte             : QPixelFormat.TypeInterpretation # 0x2
        FloatingPoint            : QPixelFormat.TypeInterpretation # 0x3

    class YUVLayout(Enum):

        YUV444                   : QPixelFormat.YUVLayout # 0x0
        YUV422                   : QPixelFormat.YUVLayout # 0x1
        YUV411                   : QPixelFormat.YUVLayout # 0x2
        YUV420P                  : QPixelFormat.YUVLayout # 0x3
        YUV420SP                 : QPixelFormat.YUVLayout # 0x4
        YV12                     : QPixelFormat.YUVLayout # 0x5
        UYVY                     : QPixelFormat.YUVLayout # 0x6
        YUYV                     : QPixelFormat.YUVLayout # 0x7
        NV12                     : QPixelFormat.YUVLayout # 0x8
        NV21                     : QPixelFormat.YUVLayout # 0x9
        IMC1                     : QPixelFormat.YUVLayout # 0xa
        IMC2                     : QPixelFormat.YUVLayout # 0xb
        IMC3                     : QPixelFormat.YUVLayout # 0xc
        IMC4                     : QPixelFormat.YUVLayout # 0xd
        Y8                       : QPixelFormat.YUVLayout # 0xe
        Y16                      : QPixelFormat.YUVLayout # 0xf


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QPixelFormat:QtGui.QPixelFormat) -> None: ...
    @overload
    def __init__(self, colorModel:QtGui.QPixelFormat.ColorModel, firstSize:int, secondSize:int, thirdSize:int, fourthSize:int, fifthSize:int, alphaSize:int, alphaUsage:QtGui.QPixelFormat.AlphaUsage, alphaPosition:QtGui.QPixelFormat.AlphaPosition, premultiplied:QtGui.QPixelFormat.AlphaPremultiplied, typeInterpretation:QtGui.QPixelFormat.TypeInterpretation, byteOrder:QtGui.QPixelFormat.ByteOrder=..., subEnum:int=...) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def alphaPosition(self) -> QtGui.QPixelFormat.AlphaPosition: ...
    def alphaSize(self) -> int: ...
    def alphaUsage(self) -> QtGui.QPixelFormat.AlphaUsage: ...
    def bitsPerPixel(self) -> int: ...
    def blackSize(self) -> int: ...
    def blueSize(self) -> int: ...
    def brightnessSize(self) -> int: ...
    def byteOrder(self) -> QtGui.QPixelFormat.ByteOrder: ...
    def channelCount(self) -> int: ...
    def colorModel(self) -> QtGui.QPixelFormat.ColorModel: ...
    def cyanSize(self) -> int: ...
    def greenSize(self) -> int: ...
    def hueSize(self) -> int: ...
    def lightnessSize(self) -> int: ...
    def magentaSize(self) -> int: ...
    def premultiplied(self) -> QtGui.QPixelFormat.AlphaPremultiplied: ...
    def redSize(self) -> int: ...
    def saturationSize(self) -> int: ...
    def subEnum(self) -> int: ...
    def typeInterpretation(self) -> QtGui.QPixelFormat.TypeInterpretation: ...
    def yellowSize(self) -> int: ...
    def yuvLayout(self) -> QtGui.QPixelFormat.YUVLayout: ...


class QPixmap(QtGui.QPaintDevice):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:QtCore.QSize) -> None: ...
    @overload
    def __init__(self, arg__1:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    @overload
    def __init__(self, fileName:Union[str, bytes, os.PathLike], format:Optional[bytes]=..., flags:QtCore.Qt.ImageConversionFlags=...) -> None: ...
    @overload
    def __init__(self, image:Union[QtGui.QImage, str]) -> None: ...
    @overload
    def __init__(self, w:int, h:int) -> None: ...
    @overload
    def __init__(self, xpm:Iterable) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def cacheKey(self) -> int: ...
    def convertFromImage(self, img:Union[QtGui.QImage, str], flags:QtCore.Qt.ImageConversionFlags=...) -> bool: ...
    @overload
    def copy(self, rect:QtCore.QRect=...) -> QtGui.QPixmap: ...
    @overload
    def copy(self, x:int | float, y:int| float, width:int| float, height:int| float) -> QtGui.QPixmap: ...
    def createHeuristicMask(self, clipTight:bool=...) -> QtGui.QBitmap: ...
    def createMaskFromColor(self, maskColor:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int], mode:QtCore.Qt.MaskMode=...) -> QtGui.QBitmap: ...
    @staticmethod
    def defaultDepth() -> int: ...
    def depth(self) -> int: ...
    def devType(self) -> int: ...
    def deviceIndependentSize(self) -> QtCore.QSizeF: ...
    def devicePixelRatio(self) -> float: ...
    def fill(self, fillColor:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]=...) -> None: ...
    @staticmethod
    def fromImage(image:Union[QtGui.QImage, str], flags:QtCore.Qt.ImageConversionFlags=...) -> QtGui.QPixmap: ...
    @staticmethod
    def fromImageInPlace(image:Union[QtGui.QImage, str], flags:QtCore.Qt.ImageConversionFlags=...) -> QtGui.QPixmap: ...
    @staticmethod
    def fromImageReader(imageReader:QtGui.QImageReader, flags:QtCore.Qt.ImageConversionFlags=...) -> QtGui.QPixmap: ...
    def hasAlpha(self) -> bool: ...
    def hasAlphaChannel(self) -> bool: ...
    def height(self) -> int: ...
    def isNull(self) -> bool: ...
    def isQBitmap(self) -> bool: ...
    def load(self, fileName:str, format:Optional[bytes]=..., flags:QtCore.Qt.ImageConversionFlags=...) -> bool: ...
    @overload
    def loadFromData(self, buf:bytes, len:int, format:Optional[bytes]=..., flags:QtCore.Qt.ImageConversionFlags=...) -> bool: ...
    @overload
    def loadFromData(self, data:Union[QtCore.QByteArray, bytes], format:Optional[bytes]=..., flags:QtCore.Qt.ImageConversionFlags=...) -> bool: ...
    def mask(self) -> QtGui.QBitmap: ...
    def metric(self, arg__1:QtGui.QPaintDevice.PaintDeviceMetric) -> int: ...
    def paintEngine(self) -> QtGui.QPaintEngine: ...
    def rect(self) -> QtCore.QRect: ...
    @overload
    def save(self, device:QtCore.QIODevice, format:Optional[bytes]=..., quality:int=...) -> bool: ...
    @overload
    def save(self, fileName:str, format:Optional[bytes]=..., quality:int=...) -> bool: ...
    @overload
    def scaled(self, s:QtCore.QSize, aspectMode:QtCore.Qt.AspectRatioMode=..., mode:QtCore.Qt.TransformationMode=...) -> QtGui.QPixmap: ...
    @overload
    def scaled(self, w:int, h:int, aspectMode:QtCore.Qt.AspectRatioMode=..., mode:QtCore.Qt.TransformationMode=...) -> QtGui.QPixmap: ...
    def scaledToHeight(self, h:int, mode:QtCore.Qt.TransformationMode=...) -> QtGui.QPixmap: ...
    def scaledToWidth(self, w:int, mode:QtCore.Qt.TransformationMode=...) -> QtGui.QPixmap: ...
    @overload
    def scroll(
        self,
        dx:int,
        dy:int,
        rect:QtCore.QRect,
        exposed:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect] = ...
    ) -> None: ...
    @overload
    def scroll(
        self,
        dx:int,
        dy:int,
        x:int,
        y:int,
        width:int,
        height:int,
        exposed:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect] = ...,
    ) -> None: ...
    def setDevicePixelRatio(self, scaleFactor:float) -> None: ...
    def setMask(self, arg__1:Union[QtGui.QBitmap, str]) -> None: ...
    def size(self) -> QtCore.QSize: ...
    def swap(self, other:Union[QtGui.QPixmap, QtGui.QImage, str]) -> None: ...
    def toImage(self) -> QtGui.QImage: ...
    def transformed(self, arg__1:QtGui.QTransform, mode:QtCore.Qt.TransformationMode=...) -> QtGui.QPixmap: ...
    @staticmethod
    def trueMatrix(m:QtGui.QTransform, w:int, h:int) -> QtGui.QTransform: ...
    def width(self) -> int: ...


class QPixmapCache(Shiboken.Object):

    class Key(Shiboken.Object):

        @overload
        def __init__(self) -> None: ...
        @overload
        def __init__(self, other:QtGui.QPixmapCache.Key) -> None: ...

        @staticmethod
        def __copy__() -> None: ...
        def isValid(self) -> bool: ...
        def swap(self, other:QtGui.QPixmapCache.Key) -> None: ...


    def __init__(self) -> None: ...

    @staticmethod
    def cacheLimit() -> int: ...
    @staticmethod
    def clear() -> None: ...
    @overload
    @staticmethod
    def find(key:QtGui.QPixmapCache.Key, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str]) -> bool: ...
    @overload
    @staticmethod
    def find(key:str, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str]) -> bool: ...
    @overload
    def find(self, arg__1:QtGui.QPixmapCache.Key) -> None: ...
    @overload
    def find(self, arg__1:str) -> None: ...
    @overload
    @staticmethod
    def insert(key:str, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str]) -> bool: ...
    @overload
    @staticmethod
    def insert(pixmap:Union[QtGui.QPixmap, QtGui.QImage, str]) -> QtGui.QPixmapCache.Key: ...
    @overload
    @staticmethod
    def remove(key:QtGui.QPixmapCache.Key) -> None: ...
    @overload
    @staticmethod
    def remove(key:str) -> None: ...
    @staticmethod
    def replace(key:QtGui.QPixmapCache.Key, pixmap:Union[QtGui.QPixmap, QtGui.QImage, str]) -> bool: ...
    @staticmethod
    def setCacheLimit(arg__1:int) -> None: ...


class QPlatformSurfaceEvent(QtCore.QEvent):

    SurfaceCreated           : QPlatformSurfaceEvent.SurfaceEventType = ... # 0x0
    SurfaceAboutToBeDestroyed: QPlatformSurfaceEvent.SurfaceEventType = ... # 0x1

    class SurfaceEventType(Enum):

        SurfaceCreated           : QPlatformSurfaceEvent.SurfaceEventType # 0x0
        SurfaceAboutToBeDestroyed: QPlatformSurfaceEvent.SurfaceEventType # 0x1


    def __init__(self, arg__1:QtGui.QPlatformSurfaceEvent) -> None: ...

    def clone(self) -> QtGui.QPlatformSurfaceEvent: ...
    def surfaceEventType(self) -> QtGui.QPlatformSurfaceEvent.SurfaceEventType: ...


class QPointFList(object): ...


class QPointList(object): ...


class QPointerEvent(QtGui.QInputEvent):

    @overload
    def __init__(self, arg__1:QtGui.QPointerEvent) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, dev:QtGui.QPointingDevice, modifiers:QtCore.Qt.KeyboardModifiers=..., points:Sequence[QtGui.QEventPoint]=...) -> None: ...

    def addPassiveGrabber(self, point:QtGui.QEventPoint, grabber:QtCore.QObject) -> bool: ...
    def allPointsAccepted(self) -> bool: ...
    def allPointsGrabbed(self) -> bool: ...
    def clearPassiveGrabbers(self, point:QtGui.QEventPoint) -> None: ...
    def clone(self) -> QtGui.QPointerEvent: ...
    def exclusiveGrabber(self, point:QtGui.QEventPoint) -> QtCore.QObject: ...
    def isBeginEvent(self) -> bool: ...
    def isEndEvent(self) -> bool: ...
    def isUpdateEvent(self) -> bool: ...
    def point(self, i:int) -> QtGui.QEventPoint: ...
    def pointById(self, id:int) -> QtGui.QEventPoint: ...
    def pointCount(self) -> int: ...
    def pointerType(self) -> QtGui.QPointingDevice.PointerType: ...
    def pointingDevice(self) -> QtGui.QPointingDevice: ...
    def points(self) -> List[QtGui.QEventPoint]: ...
    def removePassiveGrabber(self, point:QtGui.QEventPoint, grabber:QtCore.QObject) -> bool: ...
    def setAccepted(self, accepted:bool) -> None: ...
    def setExclusiveGrabber(self, point:QtGui.QEventPoint, exclusiveGrabber:QtCore.QObject) -> None: ...
    def setTimestamp(self, timestamp:int) -> None: ...


class QPointingDevice(QtGui.QInputDevice):

    GrabPassive              : QPointingDevice.GrabTransition = ... # 0x1
    UngrabPassive            : QPointingDevice.GrabTransition = ... # 0x2
    CancelGrabPassive        : QPointingDevice.GrabTransition = ... # 0x3
    OverrideGrabPassive      : QPointingDevice.GrabTransition = ... # 0x4
    GrabExclusive            : QPointingDevice.GrabTransition = ... # 0x10
    UngrabExclusive          : QPointingDevice.GrabTransition = ... # 0x20
    CancelGrabExclusive      : QPointingDevice.GrabTransition = ... # 0x30

    class GrabTransition(Enum):

        GrabPassive              : QPointingDevice.GrabTransition # 0x1
        UngrabPassive            : QPointingDevice.GrabTransition # 0x2
        CancelGrabPassive        : QPointingDevice.GrabTransition # 0x3
        OverrideGrabPassive      : QPointingDevice.GrabTransition # 0x4
        GrabExclusive            : QPointingDevice.GrabTransition # 0x10
        UngrabExclusive          : QPointingDevice.GrabTransition # 0x20
        CancelGrabExclusive      : QPointingDevice.GrabTransition # 0x30

    class PointerType(Enum):

        Unknown                  : QPointingDevice.PointerType # 0x0
        Generic                  : QPointingDevice.PointerType # 0x1
        Finger                   : QPointingDevice.PointerType # 0x2
        Pen                      : QPointingDevice.PointerType # 0x4
        Eraser                   : QPointingDevice.PointerType # 0x8
        Cursor                   : QPointingDevice.PointerType # 0x10
        AllPointerTypes          : QPointingDevice.PointerType # 0x7fff

    class PointerTypes(object): ...


    @overload
    def __init__(self, name:str, systemId:int, devType:QtGui.QInputDevice.DeviceType, pType:QtGui.QPointingDevice.PointerType, caps:QtGui.QInputDevice.Capabilities, maxPoints:int, buttonCount:int, seatName:str=..., uniqueId:QtGui.QPointingDeviceUniqueId=..., parent:Optional[QtCore.QObject]=...) -> None: ...
    @overload
    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...

    def buttonCount(self) -> int: ...
    def maximumPoints(self) -> int: ...
    def pointerType(self) -> QtGui.QPointingDevice.PointerType: ...
    @staticmethod
    def primaryPointingDevice(seatName:str=...) -> QtGui.QPointingDevice: ...
    def setCapabilities(self, caps:QtGui.QInputDevice.Capabilities) -> None: ...
    def setMaximumTouchPoints(self, c:int) -> None: ...
    def setType(self, devType:QtGui.QInputDevice.DeviceType) -> None: ...
    def uniqueId(self) -> QtGui.QPointingDeviceUniqueId: ...


class QPointingDeviceUniqueId(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QPointingDeviceUniqueId:QtGui.QPointingDeviceUniqueId) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    @staticmethod
    def fromNumericId(id:int) -> QtGui.QPointingDeviceUniqueId: ...
    def isValid(self) -> bool: ...
    def numericId(self) -> int: ...


class QPolygon(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QPolygon:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect]) -> None: ...
    @overload
    def __init__(self, r:QtCore.QRect, closed:bool=...) -> None: ...
    @overload
    def __init__(self, v:Sequence[QtCore.QPoint]) -> None: ...

    def __add__(self, l:Sequence[QtCore.QPoint]) -> List[QtCore.QPoint]: ...
    @staticmethod
    def __copy__() -> None: ...
    @overload
    def __lshift__(self, arg__1:QtCore.QPoint) -> None: ...
    @overload
    def __lshift__(self, arg__1:Sequence[QtCore.QPoint]) -> None: ...
    @overload
    def __lshift__(self, stream:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __mul__(self, m:QtGui.QTransform) -> QtGui.QPolygon: ...
    def __reduce__(self) -> object: ...
    def __rshift__(self, stream:QtCore.QDataStream) -> QtCore.QDataStream: ...
    @overload
    def append(self, arg__1:QtCore.QPoint) -> None: ...
    @overload
    def append(self, l:Sequence[QtCore.QPoint]) -> None: ...
    def at(self, i:int) -> QtCore.QPoint: ...
    def back(self) -> QtCore.QPoint: ...
    def boundingRect(self) -> QtCore.QRect: ...
    def capacity(self) -> int: ...
    def clear(self) -> None: ...
    def constData(self) -> QtCore.QPoint: ...
    def constFirst(self) -> QtCore.QPoint: ...
    def constLast(self) -> QtCore.QPoint: ...
    def containsPoint(self, pt:QtCore.QPoint, fillRule:QtCore.Qt.FillRule) -> bool: ...
    def count(self) -> int: ...
    def data(self) -> QtCore.QPoint: ...
    def empty(self) -> bool: ...
    @overload
    def first(self) -> QtCore.QPoint: ...
    @overload
    def first(self, n:int) -> List[QtCore.QPoint]: ...
    @staticmethod
    def fromList(list:Sequence[QtCore.QPoint]) -> List[QtCore.QPoint]: ...
    @staticmethod
    def fromVector(vector:Sequence[QtCore.QPoint]) -> List[QtCore.QPoint]: ...
    def front(self) -> QtCore.QPoint: ...
    def insert(self, arg__1:int, arg__2:QtCore.QPoint) -> None: ...
    def intersected(self, r:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect]) -> QtGui.QPolygon: ...
    def intersects(self, r:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect]) -> bool: ...
    def isEmpty(self) -> bool: ...
    def isSharedWith(self, other:Sequence[QtCore.QPoint]) -> bool: ...
    @overload
    def last(self) -> QtCore.QPoint: ...
    @overload
    def last(self, n:int) -> List[QtCore.QPoint]: ...
    def length(self) -> int: ...
    def mid(self, pos:int, len:int=...) -> List[QtCore.QPoint]: ...
    def move(self, from_:int, to:int) -> None: ...
    def pop_back(self) -> None: ...
    def pop_front(self) -> None: ...
    def prepend(self, arg__1:QtCore.QPoint) -> None: ...
    def push_back(self, arg__1:QtCore.QPoint) -> None: ...
    def push_front(self, arg__1:QtCore.QPoint) -> None: ...
    def remove(self, i:int, n:int=...) -> None: ...
    def removeAll(self, arg__1:QtCore.QPoint) -> None: ...
    def removeAt(self, i:int) -> None: ...
    def removeFirst(self) -> None: ...
    def removeLast(self) -> None: ...
    def removeOne(self, arg__1:QtCore.QPoint) -> None: ...
    def reserve(self, size:int) -> None: ...
    def resize(self, size:int) -> None: ...
    def shrink_to_fit(self) -> None: ...
    def size(self) -> int: ...
    @overload
    def sliced(self, pos:int) -> List[QtCore.QPoint]: ...
    @overload
    def sliced(self, pos:int, n:int) -> List[QtCore.QPoint]: ...
    def squeeze(self) -> None: ...
    def subtracted(self, r:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect]) -> QtGui.QPolygon: ...
    def swap(self, other:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect]) -> None: ...
    def swapItemsAt(self, i:int, j:int) -> None: ...
    def takeAt(self, i:int) -> QtCore.QPoint: ...
    def toList(self) -> List[QtCore.QPoint]: ...
    def toVector(self) -> List[QtCore.QPoint]: ...
    @overload
    def translate(self, dx:int, dy:int) -> None: ...
    @overload
    def translate(self, offset:QtCore.QPoint) -> None: ...
    @overload
    def translated(self, dx:int, dy:int) -> QtGui.QPolygon: ...
    @overload
    def translated(self, offset:QtCore.QPoint) -> QtGui.QPolygon: ...
    def united(self, r:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect]) -> QtGui.QPolygon: ...
    def value(self, i:int) -> QtCore.QPoint: ...


class QPolygonF(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QPolygonF:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> None: ...
    @overload
    def __init__(self, a:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect]) -> None: ...
    @overload
    def __init__(self, r:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    @overload
    def __init__(self, v:Sequence[QtCore.QPointF]) -> None: ...

    def __add__(self, l:Sequence[QtCore.QPointF]) -> List[QtCore.QPointF]: ...
    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, stream:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __mul__(self, m:QtGui.QTransform) -> QtGui.QPolygonF: ...
    def __rshift__(self, stream:QtCore.QDataStream) -> QtCore.QDataStream: ...
    @overload
    def append(self, arg__1:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def append(self, l:Sequence[QtCore.QPointF]) -> None: ...
    def at(self, i:int) -> QtCore.QPointF: ...
    def back(self) -> QtCore.QPointF: ...
    def boundingRect(self) -> QtCore.QRectF: ...
    def capacity(self) -> int: ...
    def clear(self) -> None: ...
    def constData(self) -> QtCore.QPointF: ...
    def constFirst(self) -> QtCore.QPointF: ...
    def constLast(self) -> QtCore.QPointF: ...
    def containsPoint(self, pt:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], fillRule:QtCore.Qt.FillRule) -> bool: ...
    def count(self) -> int: ...
    def data(self) -> QtCore.QPointF: ...
    def empty(self) -> bool: ...
    @overload
    def first(self) -> QtCore.QPointF: ...
    @overload
    def first(self, n:int) -> List[QtCore.QPointF]: ...
    @staticmethod
    def fromList(list:Sequence[QtCore.QPointF]) -> List[QtCore.QPointF]: ...
    @staticmethod
    def fromVector(vector:Sequence[QtCore.QPointF]) -> List[QtCore.QPointF]: ...
    def front(self) -> QtCore.QPointF: ...
    def insert(self, arg__1:int, arg__2:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def intersected(self, r:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> QtGui.QPolygonF: ...
    def intersects(self, r:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> bool: ...
    def isClosed(self) -> bool: ...
    def isEmpty(self) -> bool: ...
    def isSharedWith(self, other:Sequence[QtCore.QPointF]) -> bool: ...
    @overload
    def last(self) -> QtCore.QPointF: ...
    @overload
    def last(self, n:int) -> List[QtCore.QPointF]: ...
    def length(self) -> int: ...
    def mid(self, pos:int, len:int=...) -> List[QtCore.QPointF]: ...
    def move(self, from_:int, to:int) -> None: ...
    def pop_back(self) -> None: ...
    def pop_front(self) -> None: ...
    def prepend(self, arg__1:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def push_back(self, arg__1:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def push_front(self, arg__1:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def remove(self, i:int, n:int=...) -> None: ...
    def removeAll(self, arg__1:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def removeAt(self, i:int) -> None: ...
    def removeFirst(self) -> None: ...
    def removeLast(self) -> None: ...
    def removeOne(self, arg__1:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def reserve(self, size:int) -> None: ...
    def resize(self, size:int) -> None: ...
    def shrink_to_fit(self) -> None: ...
    def size(self) -> int: ...
    @overload
    def sliced(self, pos:int) -> List[QtCore.QPointF]: ...
    @overload
    def sliced(self, pos:int, n:int) -> List[QtCore.QPointF]: ...
    def squeeze(self) -> None: ...
    def subtracted(self, r:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> QtGui.QPolygonF: ...
    def swap(self, other:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> None: ...
    def swapItemsAt(self, i:int, j:int) -> None: ...
    def takeAt(self, i:int) -> QtCore.QPointF: ...
    def toList(self) -> List[QtCore.QPointF]: ...
    def toPolygon(self) -> QtGui.QPolygon: ...
    def toVector(self) -> List[QtCore.QPointF]: ...
    @overload
    def translate(self, dx:float, dy:float) -> None: ...
    @overload
    def translate(self, offset:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def translated(self, dx:float, dy:float) -> QtGui.QPolygonF: ...
    @overload
    def translated(self, offset:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> QtGui.QPolygonF: ...
    def united(self, r:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> QtGui.QPolygonF: ...
    def value(self, i:int) -> QtCore.QPointF: ...


class QPyTextObject(QtCore.QObject, QtGui.QTextObjectInterface):

    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...


class QQuaternion(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, scalar:float, vector:QtGui.QVector3D) -> None: ...
    @overload
    def __init__(self, scalar:float, xpos:float, ypos:float, zpos:float) -> None: ...
    @overload
    def __init__(self, vector:QtGui.QVector4D) -> None: ...

    def __add__(self, q2:QtGui.QQuaternion) -> QtGui.QQuaternion: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, quaternion:QtGui.QQuaternion) -> QtGui.QQuaternion: ...
    @overload
    def __imul__(self, factor:float) -> QtGui.QQuaternion: ...
    @overload
    def __imul__(self, quaternion:QtGui.QQuaternion) -> QtGui.QQuaternion: ...
    def __isub__(self, quaternion:QtGui.QQuaternion) -> QtGui.QQuaternion: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    @overload
    def __mul__(self, factor:float) -> QtGui.QQuaternion: ...
    @overload
    def __mul__(self, q2:QtGui.QQuaternion) -> QtGui.QQuaternion: ...
    def __neg__(self) -> QtGui.QQuaternion: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __sub__(self, q2:QtGui.QQuaternion) -> QtGui.QQuaternion: ...
    def conjugated(self) -> QtGui.QQuaternion: ...
    @staticmethod
    def dotProduct(q1:QtGui.QQuaternion, q2:QtGui.QQuaternion) -> float: ...
    @staticmethod
    def fromAxes(xAxis:QtGui.QVector3D, yAxis:QtGui.QVector3D, zAxis:QtGui.QVector3D) -> QtGui.QQuaternion: ...
    @overload
    @staticmethod
    def fromAxisAndAngle(axis:QtGui.QVector3D, angle:float) -> QtGui.QQuaternion: ...
    @overload
    @staticmethod
    def fromAxisAndAngle(x:float, y:float, z:float, angle:float) -> QtGui.QQuaternion: ...
    @staticmethod
    def fromDirection(direction:QtGui.QVector3D, up:QtGui.QVector3D) -> QtGui.QQuaternion: ...
    @overload
    @staticmethod
    def fromEulerAngles(eulerAngles:QtGui.QVector3D) -> QtGui.QQuaternion: ...
    @overload
    @staticmethod
    def fromEulerAngles(pitch:float, yaw:float, roll:float) -> QtGui.QQuaternion: ...
    @staticmethod
    def fromRotationMatrix(rot3x3:QtGui.QMatrix3x3) -> QtGui.QQuaternion: ...
    def getAxes(self, xAxis:QtGui.QVector3D, yAxis:QtGui.QVector3D, zAxis:QtGui.QVector3D) -> None: ...
    def inverted(self) -> QtGui.QQuaternion: ...
    def isIdentity(self) -> bool: ...
    def isNull(self) -> bool: ...
    def length(self) -> float: ...
    def lengthSquared(self) -> float: ...
    @staticmethod
    def nlerp(q1:QtGui.QQuaternion, q2:QtGui.QQuaternion, t:float) -> QtGui.QQuaternion: ...
    def normalize(self) -> None: ...
    def normalized(self) -> QtGui.QQuaternion: ...
    def rotatedVector(self, vector:QtGui.QVector3D) -> QtGui.QVector3D: ...
    @staticmethod
    def rotationTo(from_:QtGui.QVector3D, to:QtGui.QVector3D) -> QtGui.QQuaternion: ...
    def scalar(self) -> float: ...
    def setScalar(self, scalar:float) -> None: ...
    @overload
    def setVector(self, vector:QtGui.QVector3D) -> None: ...
    @overload
    def setVector(self, x:float, y:float, z:float) -> None: ...
    def setX(self, x:float) -> None: ...
    def setY(self, y:float) -> None: ...
    def setZ(self, z:float) -> None: ...
    @staticmethod
    def slerp(q1:QtGui.QQuaternion, q2:QtGui.QQuaternion, t:float) -> QtGui.QQuaternion: ...
    def toEulerAngles(self) -> QtGui.QVector3D: ...
    def toRotationMatrix(self) -> QtGui.QMatrix3x3: ...
    def toVector4D(self) -> QtGui.QVector4D: ...
    def vector(self) -> QtGui.QVector3D: ...
    def x(self) -> float: ...
    def y(self) -> float: ...
    def z(self) -> float: ...


class QRadialGradient(QtGui.QGradient):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QRadialGradient:QtGui.QRadialGradient) -> None: ...
    @overload
    def __init__(self, center:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], centerRadius:float, focalPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], focalRadius:float) -> None: ...
    @overload
    def __init__(self, center:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], radius:float) -> None: ...
    @overload
    def __init__(self, center:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], radius:float, focalPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def __init__(self, cx:float, cy:float, centerRadius:float, fx:float, fy:float, focalRadius:float) -> None: ...
    @overload
    def __init__(self, cx:float, cy:float, radius:float) -> None: ...
    @overload
    def __init__(self, cx:float, cy:float, radius:float, fx:float, fy:float) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def center(self) -> QtCore.QPointF: ...
    def centerRadius(self) -> float: ...
    def focalPoint(self) -> QtCore.QPointF: ...
    def focalRadius(self) -> float: ...
    def radius(self) -> float: ...
    @overload
    def setCenter(self, center:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def setCenter(self, x:float, y:float) -> None: ...
    def setCenterRadius(self, radius:float) -> None: ...
    @overload
    def setFocalPoint(self, focalPoint:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def setFocalPoint(self, x:float, y:float) -> None: ...
    def setFocalRadius(self, radius:float) -> None: ...
    def setRadius(self, radius:float) -> None: ...


class QRasterWindow(QtGui.QPaintDeviceWindow):

    def __init__(self, parent:Optional[QtGui.QWindow]=...) -> None: ...

    def metric(self, metric:QtGui.QPaintDevice.PaintDeviceMetric) -> int: ...
    def redirected(self, arg__1:QtCore.QPoint) -> QtGui.QPaintDevice: ...


class QRawFont(Shiboken.Object):

    PixelAntialiasing        : QRawFont.AntialiasingType = ... # 0x0
    SubPixelAntialiasing     : QRawFont.AntialiasingType = ... # 0x1
    SeparateAdvances         : QRawFont.LayoutFlag = ... # 0x0
    KernedAdvances           : QRawFont.LayoutFlag = ... # 0x1
    UseDesignMetrics         : QRawFont.LayoutFlag = ... # 0x2

    class AntialiasingType(Enum):

        PixelAntialiasing        : QRawFont.AntialiasingType # 0x0
        SubPixelAntialiasing     : QRawFont.AntialiasingType # 0x1

    class LayoutFlag(Enum):

        SeparateAdvances         : QRawFont.LayoutFlag # 0x0
        KernedAdvances           : QRawFont.LayoutFlag # 0x1
        UseDesignMetrics         : QRawFont.LayoutFlag # 0x2

    class LayoutFlags(object): ...


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, fileName:str, pixelSize:float, hintingPreference:QtGui.QFont.HintingPreference=...) -> None: ...
    @overload
    def __init__(self, fontData:Union[QtCore.QByteArray, bytes], pixelSize:float, hintingPreference:QtGui.QFont.HintingPreference=...) -> None: ...
    @overload
    def __init__(self, other:QtGui.QRawFont) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    @overload
    def advancesForGlyphIndexes(self, glyphIndexes:Sequence[int]) -> List[QtCore.QPointF]: ...
    @overload
    def advancesForGlyphIndexes(self, glyphIndexes:Sequence[int], layoutFlags:QtGui.QRawFont.LayoutFlags) -> List[QtCore.QPointF]: ...
    def alphaMapForGlyph(self, glyphIndex:int, antialiasingType:QtGui.QRawFont.AntialiasingType=..., transform:QtGui.QTransform=...) -> QtGui.QImage: ...
    def ascent(self) -> float: ...
    def averageCharWidth(self) -> float: ...
    def boundingRect(self, glyphIndex:int) -> QtCore.QRectF: ...
    def capHeight(self) -> float: ...
    def descent(self) -> float: ...
    def familyName(self) -> str: ...
    def fontTable(self, tagName:bytes) -> QtCore.QByteArray: ...
    @staticmethod
    def fromFont(font:Union[QtGui.QFont, str, Sequence[str]], writingSystem:QtGui.QFontDatabase.WritingSystem=...) -> QtGui.QRawFont: ...
    def glyphIndexesForString(self, text:str) -> List[int]: ...
    def hintingPreference(self) -> QtGui.QFont.HintingPreference: ...
    def isValid(self) -> bool: ...
    def leading(self) -> float: ...
    def lineThickness(self) -> float: ...
    def loadFromData(self, fontData:Union[QtCore.QByteArray, bytes], pixelSize:float, hintingPreference:QtGui.QFont.HintingPreference) -> None: ...
    def loadFromFile(self, fileName:str, pixelSize:float, hintingPreference:QtGui.QFont.HintingPreference) -> None: ...
    def maxCharWidth(self) -> float: ...
    def pathForGlyph(self, glyphIndex:int) -> QtGui.QPainterPath: ...
    def pixelSize(self) -> float: ...
    def setPixelSize(self, pixelSize:float) -> None: ...
    def style(self) -> QtGui.QFont.Style: ...
    def styleName(self) -> str: ...
    def supportedWritingSystems(self) -> List[QtGui.QFontDatabase.WritingSystem]: ...
    @overload
    def supportsCharacter(self, character:str) -> bool: ...
    @overload
    def supportsCharacter(self, ucs4:int) -> bool: ...
    def swap(self, other:QtGui.QRawFont) -> None: ...
    def underlinePosition(self) -> float: ...
    def unitsPerEm(self) -> float: ...
    def weight(self) -> int: ...
    def xHeight(self) -> float: ...


class QRegion(Shiboken.Object):

    Rectangle                : QRegion.RegionType = ... # 0x0
    Ellipse                  : QRegion.RegionType = ... # 0x1

    class RegionType(Enum):

        Rectangle                : QRegion.RegionType # 0x0
        Ellipse                  : QRegion.RegionType # 0x1


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, bitmap:Union[QtGui.QBitmap, str]) -> None: ...
    @overload
    def __init__(self, pa:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect], fillRule:QtCore.Qt.FillRule=...) -> None: ...
    @overload
    def __init__(self, r:QtCore.QRect, t:QtGui.QRegion.RegionType=...) -> None: ...
    @overload
    def __init__(self, region:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> None: ...
    @overload
    def __init__(self, x:int, y:int, w:int, h:int, t:QtGui.QRegion.RegionType=...) -> None: ...

    @overload
    def __add__(self, r:QtCore.QRect) -> QtGui.QRegion: ...
    @overload
    def __add__(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    @overload
    def __and__(self, r:QtCore.QRect) -> QtGui.QRegion: ...
    @overload
    def __and__(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    @staticmethod
    def __copy__() -> None: ...
    @overload
    def __iadd__(self, r:QtCore.QRect) -> QtGui.QRegion: ...
    @overload
    def __iadd__(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    def __ior__(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    def __isub__(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    def __ixor__(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __mul__(self, m:QtGui.QTransform) -> QtGui.QRegion: ...
    def __or__(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __sub__(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    def __xor__(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    def begin(self) -> QtCore.QRect: ...
    def boundingRect(self) -> QtCore.QRect: ...
    def cbegin(self) -> QtCore.QRect: ...
    def cend(self) -> QtCore.QRect: ...
    @overload
    def contains(self, p:QtCore.QPoint) -> bool: ...
    @overload
    def contains(self, r:QtCore.QRect) -> bool: ...
    def end(self) -> QtCore.QRect: ...
    @overload
    def intersected(self, r:QtCore.QRect) -> QtGui.QRegion: ...
    @overload
    def intersected(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    @overload
    def intersects(self, r:QtCore.QRect) -> bool: ...
    @overload
    def intersects(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> bool: ...
    def isEmpty(self) -> bool: ...
    def isNull(self) -> bool: ...
    def rectCount(self) -> int: ...
    def setRects(self, rect:QtCore.QRect, num:int) -> None: ...
    def subtracted(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    def swap(self, other:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> None: ...
    @overload
    def translate(self, dx:int, dy:int) -> None: ...
    @overload
    def translate(self, p:QtCore.QPoint) -> None: ...
    @overload
    def translated(self, dx:int, dy:int) -> QtGui.QRegion: ...
    @overload
    def translated(self, p:QtCore.QPoint) -> QtGui.QRegion: ...
    @overload
    def united(self, r:QtCore.QRect) -> QtGui.QRegion: ...
    @overload
    def united(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    def xored(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...


class QRegularExpressionValidator(QtGui.QValidator):

    @overload
    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...
    @overload
    def __init__(self, re:Union[QtCore.QRegularExpression, str], parent:Optional[QtCore.QObject]=...) -> None: ...

    def regularExpression(self) -> QtCore.QRegularExpression: ...
    def setRegularExpression(self, re:Union[QtCore.QRegularExpression, str]) -> None: ...
    def validate(self, input:str, pos:int) -> object: ...


class QResizeEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QResizeEvent) -> None: ...
    @overload
    def __init__(self, size:QtCore.QSize, oldSize:QtCore.QSize) -> None: ...

    def clone(self) -> QtGui.QResizeEvent: ...
    def oldSize(self) -> QtCore.QSize: ...
    def size(self) -> QtCore.QSize: ...


class QRgba64(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QRgba64:QtGui.QRgba64) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def alpha(self) -> int: ...
    def alpha8(self) -> int: ...
    def blue(self) -> int: ...
    def blue8(self) -> int: ...
    @staticmethod
    def fromArgb32(rgb:int) -> QtGui.QRgba64: ...
    @staticmethod
    def fromRgba(red:int, green:int, blue:int, alpha:int) -> QtGui.QRgba64: ...
    @overload
    @staticmethod
    def fromRgba64(c:int) -> QtGui.QRgba64: ...
    @overload
    @staticmethod
    def fromRgba64(red:int, green:int, blue:int, alpha:int) -> QtGui.QRgba64: ...
    def green(self) -> int: ...
    def green8(self) -> int: ...
    def isOpaque(self) -> bool: ...
    def isTransparent(self) -> bool: ...
    def premultiplied(self) -> QtGui.QRgba64: ...
    def red(self) -> int: ...
    def red8(self) -> int: ...
    def setAlpha(self, _alpha:int) -> None: ...
    def setBlue(self, _blue:int) -> None: ...
    def setGreen(self, _green:int) -> None: ...
    def setRed(self, _red:int) -> None: ...
    def toArgb32(self) -> int: ...
    def toRgb16(self) -> int: ...
    def unpremultiplied(self) -> QtGui.QRgba64: ...


class QScreen(QtCore.QObject):
    def angleBetween(self, a:QtCore.Qt.ScreenOrientation, b:QtCore.Qt.ScreenOrientation) -> int: ...
    def availableGeometry(self) -> QtCore.QRect: ...
    def availableSize(self) -> QtCore.QSize: ...
    def availableVirtualGeometry(self) -> QtCore.QRect: ...
    def availableVirtualSize(self) -> QtCore.QSize: ...
    def depth(self) -> int: ...
    def devicePixelRatio(self) -> float: ...
    def geometry(self) -> QtCore.QRect: ...
    def grabWindow(self, window:int, x:int=..., y:int=..., w:int=..., h:int=...) -> QtGui.QPixmap: ...
    def isLandscape(self, orientation:QtCore.Qt.ScreenOrientation) -> bool: ...
    def isPortrait(self, orientation:QtCore.Qt.ScreenOrientation) -> bool: ...
    def logicalDotsPerInch(self) -> float: ...
    def logicalDotsPerInchX(self) -> float: ...
    def logicalDotsPerInchY(self) -> float: ...
    def manufacturer(self) -> str: ...
    def mapBetween(self, a:QtCore.Qt.ScreenOrientation, b:QtCore.Qt.ScreenOrientation, rect:QtCore.QRect) -> QtCore.QRect: ...
    def model(self) -> str: ...
    def name(self) -> str: ...
    def nativeOrientation(self) -> QtCore.Qt.ScreenOrientation: ...
    def orientation(self) -> QtCore.Qt.ScreenOrientation: ...
    def physicalDotsPerInch(self) -> float: ...
    def physicalDotsPerInchX(self) -> float: ...
    def physicalDotsPerInchY(self) -> float: ...
    def physicalSize(self) -> QtCore.QSizeF: ...
    def primaryOrientation(self) -> QtCore.Qt.ScreenOrientation: ...
    def refreshRate(self) -> float: ...
    def resolveInterface(self, name:bytes, revision:int) -> int: ...
    def serialNumber(self) -> str: ...
    def size(self) -> QtCore.QSize: ...
    def transformBetween(self, a:QtCore.Qt.ScreenOrientation, b:QtCore.Qt.ScreenOrientation, target:QtCore.QRect) -> QtGui.QTransform: ...
    def virtualGeometry(self) -> QtCore.QRect: ...
    def virtualSiblingAt(self, point:QtCore.QPoint) -> QtGui.QScreen: ...
    def virtualSiblings(self) -> List[QtGui.QScreen]: ...
    def virtualSize(self) -> QtCore.QSize: ...


class QScrollEvent(QtCore.QEvent):

    ScrollStarted            : QScrollEvent.ScrollState = ... # 0x0
    ScrollUpdated            : QScrollEvent.ScrollState = ... # 0x1
    ScrollFinished           : QScrollEvent.ScrollState = ... # 0x2

    class ScrollState(Enum):

        ScrollStarted            : QScrollEvent.ScrollState # 0x0
        ScrollUpdated            : QScrollEvent.ScrollState # 0x1
        ScrollFinished           : QScrollEvent.ScrollState # 0x2


    @overload
    def __init__(self, arg__1:QtGui.QScrollEvent) -> None: ...
    @overload
    def __init__(self, contentPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], overshoot:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], scrollState:QtGui.QScrollEvent.ScrollState) -> None: ...

    def clone(self) -> QtGui.QScrollEvent: ...
    def contentPos(self) -> QtCore.QPointF: ...
    def overshootDistance(self) -> QtCore.QPointF: ...
    def scrollState(self) -> QtGui.QScrollEvent.ScrollState: ...


class QScrollPrepareEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QScrollPrepareEvent) -> None: ...
    @overload
    def __init__(self, startPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...

    def clone(self) -> QtGui.QScrollPrepareEvent: ...
    def contentPos(self) -> QtCore.QPointF: ...
    def contentPosRange(self) -> QtCore.QRectF: ...
    def setContentPos(self, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def setContentPosRange(self, rect:Union[QtCore.QRectF, QtCore.QRect]) -> None: ...
    def setViewportSize(self, size:Union[QtCore.QSizeF, QtCore.QSize]) -> None: ...
    def startPos(self) -> QtCore.QPointF: ...
    def viewportSize(self) -> QtCore.QSizeF: ...


class QSessionManager(QtCore.QObject):

    RestartIfRunning         : QSessionManager.RestartHint = ... # 0x0
    RestartAnyway            : QSessionManager.RestartHint = ... # 0x1
    RestartImmediately       : QSessionManager.RestartHint = ... # 0x2
    RestartNever             : QSessionManager.RestartHint = ... # 0x3

    class RestartHint(Enum):

        RestartIfRunning         : QSessionManager.RestartHint # 0x0
        RestartAnyway            : QSessionManager.RestartHint # 0x1
        RestartImmediately       : QSessionManager.RestartHint # 0x2
        RestartNever             : QSessionManager.RestartHint # 0x3


    def allowsErrorInteraction(self) -> bool: ...
    def allowsInteraction(self) -> bool: ...
    def cancel(self) -> None: ...
    def discardCommand(self) -> List[str]: ...
    def isPhase2(self) -> bool: ...
    def release(self) -> None: ...
    def requestPhase2(self) -> None: ...
    def restartCommand(self) -> List[str]: ...
    def restartHint(self) -> QtGui.QSessionManager.RestartHint: ...
    def sessionId(self) -> str: ...
    def sessionKey(self) -> str: ...
    def setDiscardCommand(self, arg__1:Sequence[str]) -> None: ...
    @overload
    def setManagerProperty(self, name:str, value:str) -> None: ...
    @overload
    def setManagerProperty(self, name:str, value:Sequence[str]) -> None: ...
    def setRestartCommand(self, arg__1:Sequence[str]) -> None: ...
    def setRestartHint(self, arg__1:QtGui.QSessionManager.RestartHint) -> None: ...


class QShortcut(QtCore.QObject):

    @overload
    def __init__(self, arg__1:QtGui.QKeySequence.StandardKey, arg__2:QtCore.QObject, arg__3:Callable, arg__4:QtCore.Qt.ShortcutContext=...) -> None: ...
    @overload
    def __init__(self, arg__1:Union[QtGui.QKeySequence, QtCore.QKeyCombination, QtGui.QKeySequence.StandardKey, str, int], arg__2:QtCore.QObject, arg__3:Callable, arg__4:QtCore.Qt.ShortcutContext=...) -> None: ...
    @overload
    def __init__(self, key:QtGui.QKeySequence.StandardKey, parent:QtCore.QObject, member:Optional[bytes]=..., ambiguousMember:Optional[bytes]=..., context:QtCore.Qt.ShortcutContext=...) -> None: ...
    @overload
    def __init__(self, key:Union[QtGui.QKeySequence, QtCore.QKeyCombination, QtGui.QKeySequence.StandardKey, str, int], parent:QtCore.QObject, member:Optional[bytes]=..., ambiguousMember:Optional[bytes]=..., context:QtCore.Qt.ShortcutContext=...) -> None: ...
    @overload
    def __init__(self, parent:QtCore.QObject) -> None: ...

    def autoRepeat(self) -> bool: ...
    def context(self) -> QtCore.Qt.ShortcutContext: ...
    def event(self, e:QtCore.QEvent) -> bool: ...
    def id(self) -> int: ...
    def isEnabled(self) -> bool: ...
    def key(self) -> QtGui.QKeySequence: ...
    def keys(self) -> List[QtGui.QKeySequence]: ...
    def setAutoRepeat(self, on:bool) -> None: ...
    def setContext(self, context:QtCore.Qt.ShortcutContext) -> None: ...
    def setEnabled(self, enable:bool) -> None: ...
    def setKey(self, key:Union[QtGui.QKeySequence, QtCore.QKeyCombination, QtGui.QKeySequence.StandardKey, str, int]) -> None: ...
    @overload
    def setKeys(self, key:QtGui.QKeySequence.StandardKey) -> None: ...
    @overload
    def setKeys(self, keys:Sequence[QtGui.QKeySequence]) -> None: ...
    def setWhatsThis(self, text:str) -> None: ...
    def whatsThis(self) -> str: ...


class QShortcutEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QShortcutEvent) -> None: ...
    @overload
    def __init__(self, key:Union[QtGui.QKeySequence, QtCore.QKeyCombination, QtGui.QKeySequence.StandardKey, str, int], id:int, ambiguous:bool=...) -> None: ...

    def clone(self) -> QtGui.QShortcutEvent: ...
    def isAmbiguous(self) -> bool: ...
    def key(self) -> QtGui.QKeySequence: ...
    def shortcutId(self) -> int: ...


class QShowEvent(QtCore.QEvent):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, arg__1:QtGui.QShowEvent) -> None: ...


class QSinglePointEvent(QtGui.QPointerEvent):

    @overload
    def __init__(self, arg__1:QtGui.QSinglePointEvent) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, dev:QtGui.QPointingDevice, localPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], scenePos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], globalPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], button:QtCore.Qt.MouseButton, buttons:QtCore.Qt.MouseButtons, modifiers:QtCore.Qt.KeyboardModifiers, source:QtCore.Qt.MouseEventSource=...) -> None: ...
    @overload
    def __init__(self, type:QtCore.QEvent.Type, dev:QtGui.QPointingDevice, point:QtGui.QEventPoint, button:QtCore.Qt.MouseButton, buttons:QtCore.Qt.MouseButtons, modifiers:QtCore.Qt.KeyboardModifiers, source:QtCore.Qt.MouseEventSource) -> None: ...

    def button(self) -> QtCore.Qt.MouseButton: ...
    def buttons(self) -> QtCore.Qt.MouseButtons: ...
    def clone(self) -> QtGui.QSinglePointEvent: ...
    def exclusivePointGrabber(self) -> QtCore.QObject: ...
    def globalPosition(self) -> QtCore.QPointF: ...
    def isBeginEvent(self) -> bool: ...
    def isEndEvent(self) -> bool: ...
    def isUpdateEvent(self) -> bool: ...
    def position(self) -> QtCore.QPointF: ...
    def scenePosition(self) -> QtCore.QPointF: ...
    def setExclusivePointGrabber(self, exclusiveGrabber:QtCore.QObject) -> None: ...


class QStandardItem(Shiboken.Object):

    Type                     : QStandardItem.ItemType = ... # 0x0
    UserType                 : QStandardItem.ItemType = ... # 0x3e8

    class ItemType(Enum):

        Type                     : QStandardItem.ItemType # 0x0
        UserType                 : QStandardItem.ItemType # 0x3e8


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, icon:Union[QtGui.QIcon, QtGui.QPixmap], text:str) -> None: ...
    @overload
    def __init__(self, other:QtGui.QStandardItem) -> None: ...
    @overload
    def __init__(self, rows:int, columns:int=...) -> None: ...
    @overload
    def __init__(self, text:str) -> None: ...

    def __lshift__(self, out:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, in_:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def accessibleDescription(self) -> str: ...
    def accessibleText(self) -> str: ...
    def appendColumn(self, items:Sequence[QtGui.QStandardItem]) -> None: ...
    @overload
    def appendRow(self, item:QtGui.QStandardItem) -> None: ...
    @overload
    def appendRow(self, items:Sequence[QtGui.QStandardItem]) -> None: ...
    def appendRows(self, items:Sequence[QtGui.QStandardItem]) -> None: ...
    def background(self) -> QtGui.QBrush: ...
    def checkState(self) -> QtCore.Qt.CheckState: ...
    def child(self, row:int, column:int=...) -> QtGui.QStandardItem: ...
    def clearData(self) -> None: ...
    def clone(self) -> QtGui.QStandardItem: ...
    def column(self) -> int: ...
    def columnCount(self) -> int: ...
    def data(self, role:int=...) -> Any: ...
    def emitDataChanged(self) -> None: ...
    def flags(self) -> QtCore.Qt.ItemFlags: ...
    def font(self) -> QtGui.QFont: ...
    def foreground(self) -> QtGui.QBrush: ...
    def hasChildren(self) -> bool: ...
    def icon(self) -> QtGui.QIcon: ...
    def index(self) -> QtCore.QModelIndex: ...
    def insertColumn(self, column:int, items:Sequence[QtGui.QStandardItem]) -> None: ...
    def insertColumns(self, column:int, count:int) -> None: ...
    @overload
    def insertRow(self, row:int, item:QtGui.QStandardItem) -> None: ...
    @overload
    def insertRow(self, row:int, items:Sequence[QtGui.QStandardItem]) -> None: ...
    @overload
    def insertRows(self, row:int, count:int) -> None: ...
    @overload
    def insertRows(self, row:int, items:Sequence[QtGui.QStandardItem]) -> None: ...
    def isAutoTristate(self) -> bool: ...
    def isCheckable(self) -> bool: ...
    def isDragEnabled(self) -> bool: ...
    def isDropEnabled(self) -> bool: ...
    def isEditable(self) -> bool: ...
    def isEnabled(self) -> bool: ...
    def isSelectable(self) -> bool: ...
    def isUserTristate(self) -> bool: ...
    def model(self) -> QtGui.QStandardItemModel: ...
    def parent(self) -> QtGui.QStandardItem: ...
    def read(self, in_:QtCore.QDataStream) -> None: ...
    def removeColumn(self, column:int) -> None: ...
    def removeColumns(self, column:int, count:int) -> None: ...
    def removeRow(self, row:int) -> None: ...
    def removeRows(self, row:int, count:int) -> None: ...
    def row(self) -> int: ...
    def rowCount(self) -> int: ...
    def setAccessibleDescription(self, accessibleDescription:str) -> None: ...
    def setAccessibleText(self, accessibleText:str) -> None: ...
    def setAutoTristate(self, tristate:bool) -> None: ...
    def setBackground(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setCheckState(self, checkState:QtCore.Qt.CheckState) -> None: ...
    def setCheckable(self, checkable:bool) -> None: ...
    @overload
    def setChild(self, row:int, column:int, item:QtGui.QStandardItem) -> None: ...
    @overload
    def setChild(self, row:int, item:QtGui.QStandardItem) -> None: ...
    def setColumnCount(self, columns:int) -> None: ...
    def setData(self, value:Any, role:int=...) -> None: ...
    def setDragEnabled(self, dragEnabled:bool) -> None: ...
    def setDropEnabled(self, dropEnabled:bool) -> None: ...
    def setEditable(self, editable:bool) -> None: ...
    def setEnabled(self, enabled:bool) -> None: ...
    def setFlags(self, flags:QtCore.Qt.ItemFlags) -> None: ...
    def setFont(self, font:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    def setForeground(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setIcon(self, icon:Union[QtGui.QIcon, QtGui.QPixmap]) -> None: ...
    def setRowCount(self, rows:int) -> None: ...
    def setSelectable(self, selectable:bool) -> None: ...
    def setSizeHint(self, sizeHint:QtCore.QSize) -> None: ...
    def setStatusTip(self, statusTip:str) -> None: ...
    def setText(self, text:str) -> None: ...
    def setTextAlignment(self, textAlignment:QtCore.Qt.Alignment) -> None: ...
    def setToolTip(self, toolTip:str) -> None: ...
    def setUserTristate(self, tristate:bool) -> None: ...
    def setWhatsThis(self, whatsThis:str) -> None: ...
    def sizeHint(self) -> QtCore.QSize: ...
    def sortChildren(self, column:int, order:QtCore.Qt.SortOrder=...) -> None: ...
    def statusTip(self) -> str: ...
    def takeChild(self, row:int, column:int=...) -> QtGui.QStandardItem: ...
    def takeColumn(self, column:int) -> List[QtGui.QStandardItem]: ...
    def takeRow(self, row:int) -> List[QtGui.QStandardItem]: ...
    def text(self) -> str: ...
    def textAlignment(self) -> QtCore.Qt.Alignment: ...
    def toolTip(self) -> str: ...
    def type(self) -> int: ...
    def whatsThis(self) -> str: ...
    def write(self, out:QtCore.QDataStream) -> None: ...


class QStandardItemModel(QtCore.QAbstractItemModel):

    @overload
    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...
    @overload
    def __init__(self, rows:int, columns:int, parent:Optional[QtCore.QObject]=...) -> None: ...

    def appendColumn(self, items:Sequence[QtGui.QStandardItem]) -> None: ...
    @overload
    def appendRow(self, item:QtGui.QStandardItem) -> None: ...
    @overload
    def appendRow(self, items:Sequence[QtGui.QStandardItem]) -> None: ...
    def clear(self) -> None: ...
    def clearItemData(self, index:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]) -> bool: ...
    def columnCount(self, parent:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]=...) -> int: ...
    def data(self, index:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex], role:int=...) -> Any: ...
    def dropMimeData(self, data:QtCore.QMimeData, action:QtCore.Qt.DropAction, row:int, column:int, parent:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]) -> bool: ...
    def findItems(self, text:str, flags:QtCore.Qt.MatchFlags=..., column:int=...) -> List[QtGui.QStandardItem]: ...
    def flags(self, index:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]) -> QtCore.Qt.ItemFlags: ...
    def hasChildren(self, parent:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]=...) -> bool: ...
    def headerData(self, section:int, orientation:QtCore.Qt.Orientation, role:int=...) -> Any: ...
    def horizontalHeaderItem(self, column:int) -> QtGui.QStandardItem: ...
    def index(self, row:int, column:int, parent:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]=...) -> QtCore.QModelIndex: ...
    def indexFromItem(self, item:QtGui.QStandardItem) -> QtCore.QModelIndex: ...
    @overload
    def insertColumn(self, column:int, items:Sequence[QtGui.QStandardItem]) -> None: ...
    @overload
    def insertColumn(self, column:int, parent:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]=...) -> bool: ...
    def insertColumns(self, column:int, count:int, parent:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]=...) -> bool: ...
    @overload
    def insertRow(self, row:int, item:QtGui.QStandardItem) -> None: ...
    @overload
    def insertRow(self, row:int, items:Sequence[QtGui.QStandardItem]) -> None: ...
    @overload
    def insertRow(self, row:int, parent:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]=...) -> bool: ...
    def insertRows(self, row:int, count:int, parent:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]=...) -> bool: ...
    def invisibleRootItem(self) -> QtGui.QStandardItem: ...
    def item(self, row:int, column:int=...) -> QtGui.QStandardItem: ...
    def itemData(self, index:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]) -> Dict[int, Any]: ...
    def itemFromIndex(self, index:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]) -> QtGui.QStandardItem: ...
    def itemPrototype(self) -> QtGui.QStandardItem: ...
    def mimeData(self, indexes:List[int]) -> QtCore.QMimeData: ...
    def mimeTypes(self) -> List[str]: ...
    @overload
    def parent(self) -> QtCore.QObject: ...
    @overload
    def parent(self, child:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]) -> QtCore.QModelIndex: ...
    def removeColumns(self, column:int, count:int, parent:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]=...) -> bool: ...
    def removeRows(self, row:int, count:int, parent:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]=...) -> bool: ...
    def roleNames(self) -> Dict[int, QtCore.QByteArray]: ...
    def rowCount(self, parent:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]=...) -> int: ...
    def setColumnCount(self, columns:int) -> None: ...
    def setData(self, index:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex], value:Any, role:int=...) -> bool: ...
    def setHeaderData(self, section:int, orientation:QtCore.Qt.Orientation, value:Any, role:int=...) -> bool: ...
    def setHorizontalHeaderItem(self, column:int, item:QtGui.QStandardItem) -> None: ...
    def setHorizontalHeaderLabels(self, labels:Sequence[str]) -> None: ...
    @overload
    def setItem(self, row:int, column:int, item:QtGui.QStandardItem) -> None: ...
    @overload
    def setItem(self, row:int, item:QtGui.QStandardItem) -> None: ...
    def setItemData(self, index:Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex], roles:Dict[int, Any]) -> bool: ...
    def setItemPrototype(self, item:QtGui.QStandardItem) -> None: ...
    def setItemRoleNames(self, roleNames:Dict[int, QtCore.QByteArray]) -> None: ...
    def setRowCount(self, rows:int) -> None: ...
    def setSortRole(self, role:int) -> None: ...
    def setVerticalHeaderItem(self, row:int, item:QtGui.QStandardItem) -> None: ...
    def setVerticalHeaderLabels(self, labels:Sequence[str]) -> None: ...
    def sort(self, column:int, order:QtCore.Qt.SortOrder=...) -> None: ...
    def sortRole(self) -> int: ...
    def supportedDropActions(self) -> QtCore.Qt.DropActions: ...
    def takeColumn(self, column:int) -> List[QtGui.QStandardItem]: ...
    def takeHorizontalHeaderItem(self, column:int) -> QtGui.QStandardItem: ...
    def takeItem(self, row:int, column:int=...) -> QtGui.QStandardItem: ...
    def takeRow(self, row:int) -> List[QtGui.QStandardItem]: ...
    def takeVerticalHeaderItem(self, row:int) -> QtGui.QStandardItem: ...
    def verticalHeaderItem(self, row:int) -> QtGui.QStandardItem: ...


class QStaticText(Shiboken.Object):

    ModerateCaching          : QStaticText.PerformanceHint = ... # 0x0
    AggressiveCaching        : QStaticText.PerformanceHint = ... # 0x1

    class PerformanceHint(Enum):

        ModerateCaching          : QStaticText.PerformanceHint # 0x0
        AggressiveCaching        : QStaticText.PerformanceHint # 0x1


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, other:QtGui.QStaticText) -> None: ...
    @overload
    def __init__(self, text:str) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def performanceHint(self) -> QtGui.QStaticText.PerformanceHint: ...
    def prepare(self, matrix:QtGui.QTransform=..., font:Union[QtGui.QFont, str, Sequence[str]]=...) -> None: ...
    def setPerformanceHint(self, performanceHint:QtGui.QStaticText.PerformanceHint) -> None: ...
    def setText(self, text:str) -> None: ...
    def setTextFormat(self, textFormat:QtCore.Qt.TextFormat) -> None: ...
    def setTextOption(self, textOption:Union[QtGui.QTextOption, QtCore.Qt.Alignment]) -> None: ...
    def setTextWidth(self, textWidth:float) -> None: ...
    def size(self) -> QtCore.QSizeF: ...
    def swap(self, other:QtGui.QStaticText) -> None: ...
    def text(self) -> str: ...
    def textFormat(self) -> QtCore.Qt.TextFormat: ...
    def textOption(self) -> QtGui.QTextOption: ...
    def textWidth(self) -> float: ...


class QStatusTipEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QStatusTipEvent) -> None: ...
    @overload
    def __init__(self, tip:str) -> None: ...

    def clone(self) -> QtGui.QStatusTipEvent: ...
    def tip(self) -> str: ...


class QStyleHints(QtCore.QObject):
    def cursorFlashTime(self) -> int: ...
    def fontSmoothingGamma(self) -> float: ...
    def keyboardAutoRepeatRate(self) -> int: ...
    def keyboardInputInterval(self) -> int: ...
    def mouseDoubleClickDistance(self) -> int: ...
    def mouseDoubleClickInterval(self) -> int: ...
    def mousePressAndHoldInterval(self) -> int: ...
    def mouseQuickSelectionThreshold(self) -> int: ...
    def passwordMaskCharacter(self) -> str: ...
    def passwordMaskDelay(self) -> int: ...
    def setCursorFlashTime(self, cursorFlashTime:int) -> None: ...
    def setFocusOnTouchRelease(self) -> bool: ...
    def setKeyboardInputInterval(self, keyboardInputInterval:int) -> None: ...
    def setMouseDoubleClickInterval(self, mouseDoubleClickInterval:int) -> None: ...
    def setMousePressAndHoldInterval(self, mousePressAndHoldInterval:int) -> None: ...
    def setMouseQuickSelectionThreshold(self, threshold:int) -> None: ...
    def setShowShortcutsInContextMenus(self, showShortcutsInContextMenus:bool) -> None: ...
    def setStartDragDistance(self, startDragDistance:int) -> None: ...
    def setStartDragTime(self, startDragTime:int) -> None: ...
    def setTabFocusBehavior(self, tabFocusBehavior:QtCore.Qt.TabFocusBehavior) -> None: ...
    def setUseHoverEffects(self, useHoverEffects:bool) -> None: ...
    def setWheelScrollLines(self, scrollLines:int) -> None: ...
    def showIsFullScreen(self) -> bool: ...
    def showIsMaximized(self) -> bool: ...
    def showShortcutsInContextMenus(self) -> bool: ...
    def singleClickActivation(self) -> bool: ...
    def startDragDistance(self) -> int: ...
    def startDragTime(self) -> int: ...
    def startDragVelocity(self) -> int: ...
    def tabFocusBehavior(self) -> QtCore.Qt.TabFocusBehavior: ...
    def touchDoubleTapDistance(self) -> int: ...
    def useHoverEffects(self) -> bool: ...
    def useRtlExtensions(self) -> bool: ...
    def wheelScrollLines(self) -> int: ...


class QSurface(Shiboken.Object):

    Window                   : QSurface.SurfaceClass = ... # 0x0
    Offscreen                : QSurface.SurfaceClass = ... # 0x1
    RasterSurface            : QSurface.SurfaceType = ... # 0x0
    OpenGLSurface            : QSurface.SurfaceType = ... # 0x1
    RasterGLSurface          : QSurface.SurfaceType = ... # 0x2
    OpenVGSurface            : QSurface.SurfaceType = ... # 0x3
    VulkanSurface            : QSurface.SurfaceType = ... # 0x4
    MetalSurface             : QSurface.SurfaceType = ... # 0x5
    Direct3DSurface          : QSurface.SurfaceType = ... # 0x6

    class SurfaceClass(Enum):

        Window                   : QSurface.SurfaceClass # 0x0
        Offscreen                : QSurface.SurfaceClass # 0x1

    class SurfaceType(Enum):

        RasterSurface            : QSurface.SurfaceType # 0x0
        OpenGLSurface            : QSurface.SurfaceType # 0x1
        RasterGLSurface          : QSurface.SurfaceType # 0x2
        OpenVGSurface            : QSurface.SurfaceType # 0x3
        VulkanSurface            : QSurface.SurfaceType # 0x4
        MetalSurface             : QSurface.SurfaceType # 0x5
        Direct3DSurface          : QSurface.SurfaceType # 0x6


    def __init__(self, type:QtGui.QSurface.SurfaceClass) -> None: ...

    def format(self) -> QtGui.QSurfaceFormat: ...
    def size(self) -> QtCore.QSize: ...
    def supportsOpenGL(self) -> bool: ...
    def surfaceClass(self) -> QtGui.QSurface.SurfaceClass: ...
    def surfaceHandle(self) -> int: ...
    def surfaceType(self) -> QtGui.QSurface.SurfaceType: ...


class QSurfaceFormat(Shiboken.Object):

    DefaultColorSpace        : QSurfaceFormat.ColorSpace = ... # 0x0
    sRGBColorSpace           : QSurfaceFormat.ColorSpace = ... # 0x1
    StereoBuffers            : QSurfaceFormat.FormatOption = ... # 0x1
    DebugContext             : QSurfaceFormat.FormatOption = ... # 0x2
    DeprecatedFunctions      : QSurfaceFormat.FormatOption = ... # 0x4
    ResetNotification        : QSurfaceFormat.FormatOption = ... # 0x8
    ProtectedContent         : QSurfaceFormat.FormatOption = ... # 0x10
    NoProfile                : QSurfaceFormat.OpenGLContextProfile = ... # 0x0
    CoreProfile              : QSurfaceFormat.OpenGLContextProfile = ... # 0x1
    CompatibilityProfile     : QSurfaceFormat.OpenGLContextProfile = ... # 0x2
    DefaultRenderableType    : QSurfaceFormat.RenderableType = ... # 0x0
    OpenGL                   : QSurfaceFormat.RenderableType = ... # 0x1
    OpenGLES                 : QSurfaceFormat.RenderableType = ... # 0x2
    OpenVG                   : QSurfaceFormat.RenderableType = ... # 0x4
    DefaultSwapBehavior      : QSurfaceFormat.SwapBehavior = ... # 0x0
    SingleBuffer             : QSurfaceFormat.SwapBehavior = ... # 0x1
    DoubleBuffer             : QSurfaceFormat.SwapBehavior = ... # 0x2
    TripleBuffer             : QSurfaceFormat.SwapBehavior = ... # 0x3

    class ColorSpace(Enum):

        DefaultColorSpace        : QSurfaceFormat.ColorSpace # 0x0
        sRGBColorSpace           : QSurfaceFormat.ColorSpace # 0x1

    class FormatOption(Enum):

        StereoBuffers            : QSurfaceFormat.FormatOption # 0x1
        DebugContext             : QSurfaceFormat.FormatOption # 0x2
        DeprecatedFunctions      : QSurfaceFormat.FormatOption # 0x4
        ResetNotification        : QSurfaceFormat.FormatOption # 0x8
        ProtectedContent         : QSurfaceFormat.FormatOption # 0x10

    class FormatOptions(object): ...

    class OpenGLContextProfile(Enum):

        NoProfile                : QSurfaceFormat.OpenGLContextProfile # 0x0
        CoreProfile              : QSurfaceFormat.OpenGLContextProfile # 0x1
        CompatibilityProfile     : QSurfaceFormat.OpenGLContextProfile # 0x2

    class RenderableType(Enum):

        DefaultRenderableType    : QSurfaceFormat.RenderableType # 0x0
        OpenGL                   : QSurfaceFormat.RenderableType # 0x1
        OpenGLES                 : QSurfaceFormat.RenderableType # 0x2
        OpenVG                   : QSurfaceFormat.RenderableType # 0x4

    class SwapBehavior(Enum):

        DefaultSwapBehavior      : QSurfaceFormat.SwapBehavior # 0x0
        SingleBuffer             : QSurfaceFormat.SwapBehavior # 0x1
        DoubleBuffer             : QSurfaceFormat.SwapBehavior # 0x2
        TripleBuffer             : QSurfaceFormat.SwapBehavior # 0x3


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, options:QtGui.QSurfaceFormat.FormatOptions) -> None: ...
    @overload
    def __init__(self, other:Union[QtGui.QSurfaceFormat, QtGui.QSurfaceFormat.FormatOptions]) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def alphaBufferSize(self) -> int: ...
    def blueBufferSize(self) -> int: ...
    def colorSpace(self) -> QtGui.QColorSpace: ...
    @staticmethod
    def defaultFormat() -> QtGui.QSurfaceFormat: ...
    def depthBufferSize(self) -> int: ...
    def greenBufferSize(self) -> int: ...
    def hasAlpha(self) -> bool: ...
    def majorVersion(self) -> int: ...
    def minorVersion(self) -> int: ...
    def options(self) -> QtGui.QSurfaceFormat.FormatOptions: ...
    def profile(self) -> QtGui.QSurfaceFormat.OpenGLContextProfile: ...
    def redBufferSize(self) -> int: ...
    def renderableType(self) -> QtGui.QSurfaceFormat.RenderableType: ...
    def samples(self) -> int: ...
    def setAlphaBufferSize(self, size:int) -> None: ...
    def setBlueBufferSize(self, size:int) -> None: ...
    @overload
    def setColorSpace(self, colorSpace:QtGui.QSurfaceFormat.ColorSpace) -> None: ...
    @overload
    def setColorSpace(self, colorSpace:Union[QtGui.QColorSpace, QtGui.QColorSpace.NamedColorSpace]) -> None: ...
    @staticmethod
    def setDefaultFormat(format:Union[QtGui.QSurfaceFormat, QtGui.QSurfaceFormat.FormatOptions]) -> None: ...
    def setDepthBufferSize(self, size:int) -> None: ...
    def setGreenBufferSize(self, size:int) -> None: ...
    def setMajorVersion(self, majorVersion:int) -> None: ...
    def setMinorVersion(self, minorVersion:int) -> None: ...
    def setOption(self, option:QtGui.QSurfaceFormat.FormatOption, on:bool=...) -> None: ...
    def setOptions(self, options:QtGui.QSurfaceFormat.FormatOptions) -> None: ...
    def setProfile(self, profile:QtGui.QSurfaceFormat.OpenGLContextProfile) -> None: ...
    def setRedBufferSize(self, size:int) -> None: ...
    def setRenderableType(self, type:QtGui.QSurfaceFormat.RenderableType) -> None: ...
    def setSamples(self, numSamples:int) -> None: ...
    def setStencilBufferSize(self, size:int) -> None: ...
    def setStereo(self, enable:bool) -> None: ...
    def setSwapBehavior(self, behavior:QtGui.QSurfaceFormat.SwapBehavior) -> None: ...
    def setSwapInterval(self, interval:int) -> None: ...
    def setVersion(self, major:int, minor:int) -> None: ...
    def stencilBufferSize(self) -> int: ...
    def stereo(self) -> bool: ...
    def swapBehavior(self) -> QtGui.QSurfaceFormat.SwapBehavior: ...
    def swapInterval(self) -> int: ...
    def testOption(self, option:QtGui.QSurfaceFormat.FormatOption) -> bool: ...
    def version(self) -> Tuple[int, int]: ...


class QSyntaxHighlighter(QtCore.QObject):

    @overload
    def __init__(self, parent:QtCore.QObject) -> None: ...
    @overload
    def __init__(self, parent:QtGui.QTextDocument) -> None: ...

    def currentBlock(self) -> QtGui.QTextBlock: ...
    def currentBlockState(self) -> int: ...
    def currentBlockUserData(self) -> QtGui.QTextBlockUserData: ...
    def document(self) -> QtGui.QTextDocument: ...
    def format(self, pos:int) -> QtGui.QTextCharFormat: ...
    def highlightBlock(self, text:str) -> None: ...
    def previousBlockState(self) -> int: ...
    def rehighlight(self) -> None: ...
    def rehighlightBlock(self, block:QtGui.QTextBlock) -> None: ...
    def setCurrentBlockState(self, newState:int) -> None: ...
    def setCurrentBlockUserData(self, data:QtGui.QTextBlockUserData) -> None: ...
    def setDocument(self, doc:QtGui.QTextDocument) -> None: ...
    @overload
    def setFormat(self, start:int, count:int, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    @overload
    def setFormat(self, start:int, count:int, font:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    @overload
    def setFormat(self, start:int, count:int, format:QtGui.QTextCharFormat) -> None: ...


class QTabletEvent(QtGui.QSinglePointEvent):

    @overload
    def __init__(self, arg__1:QtGui.QTabletEvent) -> None: ...
    @overload
    def __init__(self, t:QtCore.QEvent.Type, device:QtGui.QPointingDevice, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], globalPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], pressure:float, xTilt:float, yTilt:float, tangentialPressure:float, rotation:float, z:float, keyState:QtCore.Qt.KeyboardModifiers, button:QtCore.Qt.MouseButton, buttons:QtCore.Qt.MouseButtons) -> None: ...

    def clone(self) -> QtGui.QTabletEvent: ...
    def globalPos(self) -> QtCore.QPoint: ...
    def globalPosF(self) -> QtCore.QPointF: ...
    def globalX(self) -> int: ...
    def globalY(self) -> int: ...
    def hiResGlobalX(self) -> float: ...
    def hiResGlobalY(self) -> float: ...
    def pos(self) -> QtCore.QPoint: ...
    def posF(self) -> QtCore.QPointF: ...
    def pressure(self) -> float: ...
    def rotation(self) -> float: ...
    def tangentialPressure(self) -> float: ...
    def uniqueId(self) -> int: ...
    def x(self) -> int: ...
    def xTilt(self) -> float: ...
    def y(self) -> int: ...
    def yTilt(self) -> float: ...
    def z(self) -> float: ...


class QTextBlock(Shiboken.Object):

    class iterator(Shiboken.Object):

        def __init__(self) -> None: ...

        @staticmethod
        def __copy__() -> None: ...
        def __iadd__(self, arg__1:int) -> QtGui.QTextBlock.iterator: ...
        def __isub__(self, arg__1:int) -> QtGui.QTextBlock.iterator: ...
        def __iter__(self) -> object: ...
        def __next__(self) -> object: ...
        def atEnd(self) -> bool: ...
        def fragment(self) -> QtGui.QTextFragment: ...


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, o:QtGui.QTextBlock) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __iter__(self) -> object: ...
    def begin(self) -> QtGui.QTextBlock.iterator: ...
    def blockFormat(self) -> QtGui.QTextBlockFormat: ...
    def blockFormatIndex(self) -> int: ...
    def blockNumber(self) -> int: ...
    def charFormat(self) -> QtGui.QTextCharFormat: ...
    def charFormatIndex(self) -> int: ...
    def clearLayout(self) -> None: ...
    def contains(self, position:int) -> bool: ...
    def document(self) -> QtGui.QTextDocument: ...
    def end(self) -> QtGui.QTextBlock.iterator: ...
    def firstLineNumber(self) -> int: ...
    def fragmentIndex(self) -> int: ...
    def isValid(self) -> bool: ...
    def isVisible(self) -> bool: ...
    def layout(self) -> QtGui.QTextLayout: ...
    def length(self) -> int: ...
    def lineCount(self) -> int: ...
    def next(self) -> QtGui.QTextBlock: ...
    def position(self) -> int: ...
    def previous(self) -> QtGui.QTextBlock: ...
    def revision(self) -> int: ...
    def setLineCount(self, count:int) -> None: ...
    def setRevision(self, rev:int) -> None: ...
    def setUserData(self, data:QtGui.QTextBlockUserData) -> None: ...
    def setUserState(self, state:int) -> None: ...
    def setVisible(self, visible:bool) -> None: ...
    def text(self) -> str: ...
    def textDirection(self) -> QtCore.Qt.LayoutDirection: ...
    def textFormats(self) -> List[QtGui.QTextLayout.FormatRange]: ...
    def textList(self) -> QtGui.QTextList: ...
    def userData(self) -> QtGui.QTextBlockUserData: ...
    def userState(self) -> int: ...


class QTextBlockFormat(QtGui.QTextFormat):

    SingleHeight             : QTextBlockFormat.LineHeightTypes = ... # 0x0
    ProportionalHeight       : QTextBlockFormat.LineHeightTypes = ... # 0x1
    FixedHeight              : QTextBlockFormat.LineHeightTypes = ... # 0x2
    MinimumHeight            : QTextBlockFormat.LineHeightTypes = ... # 0x3
    LineDistanceHeight       : QTextBlockFormat.LineHeightTypes = ... # 0x4

    class LineHeightTypes(Enum):

        SingleHeight             : QTextBlockFormat.LineHeightTypes # 0x0
        ProportionalHeight       : QTextBlockFormat.LineHeightTypes # 0x1
        FixedHeight              : QTextBlockFormat.LineHeightTypes # 0x2
        MinimumHeight            : QTextBlockFormat.LineHeightTypes # 0x3
        LineDistanceHeight       : QTextBlockFormat.LineHeightTypes # 0x4

    class MarkerType(Enum):

        NoMarker                 : QTextBlockFormat.MarkerType # 0x0
        Unchecked                : QTextBlockFormat.MarkerType # 0x1
        Checked                  : QTextBlockFormat.MarkerType # 0x2


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QTextBlockFormat:QtGui.QTextBlockFormat) -> None: ...
    @overload
    def __init__(self, fmt:QtGui.QTextFormat) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def alignment(self) -> QtCore.Qt.Alignment: ...
    def bottomMargin(self) -> float: ...
    def headingLevel(self) -> int: ...
    def indent(self) -> int: ...
    def isValid(self) -> bool: ...
    def leftMargin(self) -> float: ...
    @overload
    def lineHeight(self) -> float: ...
    @overload
    def lineHeight(self, scriptLineHeight:float, scaling:float) -> float: ...
    def lineHeightType(self) -> int: ...
    def marker(self) -> QtGui.QTextBlockFormat.MarkerType: ...
    def nonBreakableLines(self) -> bool: ...
    def pageBreakPolicy(self) -> QtGui.QTextFormat.PageBreakFlags: ...
    def rightMargin(self) -> float: ...
    def setAlignment(self, alignment:QtCore.Qt.Alignment) -> None: ...
    def setBottomMargin(self, margin:float) -> None: ...
    def setHeadingLevel(self, alevel:int) -> None: ...
    def setIndent(self, indent:int) -> None: ...
    def setLeftMargin(self, margin:float) -> None: ...
    def setLineHeight(self, height:float, heightType:int) -> None: ...
    def setMarker(self, marker:QtGui.QTextBlockFormat.MarkerType) -> None: ...
    def setNonBreakableLines(self, b:bool) -> None: ...
    def setPageBreakPolicy(self, flags:QtGui.QTextFormat.PageBreakFlags) -> None: ...
    def setRightMargin(self, margin:float) -> None: ...
    def setTabPositions(self, tabs:Sequence[QtGui.QTextOption.Tab]) -> None: ...
    def setTextIndent(self, aindent:float) -> None: ...
    def setTopMargin(self, margin:float) -> None: ...
    def tabPositions(self) -> List[QtGui.QTextOption.Tab]: ...
    def textIndent(self) -> float: ...
    def topMargin(self) -> float: ...


class QTextBlockGroup(QtGui.QTextObject):

    def __init__(self, doc:QtGui.QTextDocument) -> None: ...

    def blockFormatChanged(self, block:QtGui.QTextBlock) -> None: ...
    def blockInserted(self, block:QtGui.QTextBlock) -> None: ...
    def blockList(self) -> List[QtGui.QTextBlock]: ...
    def blockRemoved(self, block:QtGui.QTextBlock) -> None: ...


class QTextBlockUserData(Shiboken.Object):

    def __init__(self) -> None: ...


class QTextCharFormat(QtGui.QTextFormat):

    FontPropertiesSpecifiedOnly: QTextCharFormat.FontPropertiesInheritanceBehavior = ... # 0x0
    FontPropertiesAll        : QTextCharFormat.FontPropertiesInheritanceBehavior = ... # 0x1
    NoUnderline              : QTextCharFormat.UnderlineStyle = ... # 0x0
    SingleUnderline          : QTextCharFormat.UnderlineStyle = ... # 0x1
    DashUnderline            : QTextCharFormat.UnderlineStyle = ... # 0x2
    DotLine                  : QTextCharFormat.UnderlineStyle = ... # 0x3
    DashDotLine              : QTextCharFormat.UnderlineStyle = ... # 0x4
    DashDotDotLine           : QTextCharFormat.UnderlineStyle = ... # 0x5
    WaveUnderline            : QTextCharFormat.UnderlineStyle = ... # 0x6
    SpellCheckUnderline      : QTextCharFormat.UnderlineStyle = ... # 0x7
    AlignNormal              : QTextCharFormat.VerticalAlignment = ... # 0x0
    AlignSuperScript         : QTextCharFormat.VerticalAlignment = ... # 0x1
    AlignSubScript           : QTextCharFormat.VerticalAlignment = ... # 0x2
    AlignMiddle              : QTextCharFormat.VerticalAlignment = ... # 0x3
    AlignTop                 : QTextCharFormat.VerticalAlignment = ... # 0x4
    AlignBottom              : QTextCharFormat.VerticalAlignment = ... # 0x5
    AlignBaseline            : QTextCharFormat.VerticalAlignment = ... # 0x6

    class FontPropertiesInheritanceBehavior(Enum):

        FontPropertiesSpecifiedOnly: QTextCharFormat.FontPropertiesInheritanceBehavior # 0x0
        FontPropertiesAll        : QTextCharFormat.FontPropertiesInheritanceBehavior # 0x1

    class UnderlineStyle(Enum):

        NoUnderline              : QTextCharFormat.UnderlineStyle # 0x0
        SingleUnderline          : QTextCharFormat.UnderlineStyle # 0x1
        DashUnderline            : QTextCharFormat.UnderlineStyle # 0x2
        DotLine                  : QTextCharFormat.UnderlineStyle # 0x3
        DashDotLine              : QTextCharFormat.UnderlineStyle # 0x4
        DashDotDotLine           : QTextCharFormat.UnderlineStyle # 0x5
        WaveUnderline            : QTextCharFormat.UnderlineStyle # 0x6
        SpellCheckUnderline      : QTextCharFormat.UnderlineStyle # 0x7

    class VerticalAlignment(Enum):

        AlignNormal              : QTextCharFormat.VerticalAlignment # 0x0
        AlignSuperScript         : QTextCharFormat.VerticalAlignment # 0x1
        AlignSubScript           : QTextCharFormat.VerticalAlignment # 0x2
        AlignMiddle              : QTextCharFormat.VerticalAlignment # 0x3
        AlignTop                 : QTextCharFormat.VerticalAlignment # 0x4
        AlignBottom              : QTextCharFormat.VerticalAlignment # 0x5
        AlignBaseline            : QTextCharFormat.VerticalAlignment # 0x6


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QTextCharFormat:QtGui.QTextCharFormat) -> None: ...
    @overload
    def __init__(self, fmt:QtGui.QTextFormat) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def anchorHref(self) -> str: ...
    def anchorNames(self) -> List[str]: ...
    def baselineOffset(self) -> float: ...
    def font(self) -> QtGui.QFont: ...
    def fontCapitalization(self) -> QtGui.QFont.Capitalization: ...
    def fontFamilies(self) -> Any: ...
    def fontFamily(self) -> str: ...
    def fontFixedPitch(self) -> bool: ...
    def fontHintingPreference(self) -> QtGui.QFont.HintingPreference: ...
    def fontItalic(self) -> bool: ...
    def fontKerning(self) -> bool: ...
    def fontLetterSpacing(self) -> float: ...
    def fontLetterSpacingType(self) -> QtGui.QFont.SpacingType: ...
    def fontOverline(self) -> bool: ...
    def fontPointSize(self) -> float: ...
    def fontStretch(self) -> int: ...
    def fontStrikeOut(self) -> bool: ...
    def fontStyleHint(self) -> QtGui.QFont.StyleHint: ...
    def fontStyleName(self) -> Any: ...
    def fontStyleStrategy(self) -> QtGui.QFont.StyleStrategy: ...
    def fontUnderline(self) -> bool: ...
    def fontWeight(self) -> int: ...
    def fontWordSpacing(self) -> float: ...
    def isAnchor(self) -> bool: ...
    def isValid(self) -> bool: ...
    def setAnchor(self, anchor:bool) -> None: ...
    def setAnchorHref(self, value:str) -> None: ...
    def setAnchorNames(self, names:Sequence[str]) -> None: ...
    def setBaselineOffset(self, baseline:float) -> None: ...
    def setFont(self, font:Union[QtGui.QFont, str, Sequence[str]], behavior:QtGui.QTextCharFormat.FontPropertiesInheritanceBehavior=...) -> None: ...
    def setFontCapitalization(self, capitalization:QtGui.QFont.Capitalization) -> None: ...
    def setFontFamilies(self, families:Sequence[str]) -> None: ...
    def setFontFamily(self, family:str) -> None: ...
    def setFontFixedPitch(self, fixedPitch:bool) -> None: ...
    def setFontHintingPreference(self, hintingPreference:QtGui.QFont.HintingPreference) -> None: ...
    def setFontItalic(self, italic:bool) -> None: ...
    def setFontKerning(self, enable:bool) -> None: ...
    def setFontLetterSpacing(self, spacing:float) -> None: ...
    def setFontLetterSpacingType(self, letterSpacingType:QtGui.QFont.SpacingType) -> None: ...
    def setFontOverline(self, overline:bool) -> None: ...
    def setFontPointSize(self, size:float) -> None: ...
    def setFontStretch(self, factor:int) -> None: ...
    def setFontStrikeOut(self, strikeOut:bool) -> None: ...
    def setFontStyleHint(self, hint:QtGui.QFont.StyleHint, strategy:QtGui.QFont.StyleStrategy=...) -> None: ...
    def setFontStyleName(self, styleName:str) -> None: ...
    def setFontStyleStrategy(self, strategy:QtGui.QFont.StyleStrategy) -> None: ...
    def setFontUnderline(self, underline:bool) -> None: ...
    def setFontWeight(self, weight:int) -> None: ...
    def setFontWordSpacing(self, spacing:float) -> None: ...
    def setSubScriptBaseline(self, baseline:float) -> None: ...
    def setSuperScriptBaseline(self, baseline:float) -> None: ...
    def setTableCellColumnSpan(self, tableCellColumnSpan:int) -> None: ...
    def setTableCellRowSpan(self, tableCellRowSpan:int) -> None: ...
    def setTextOutline(self, pen:Union[QtGui.QPen, QtCore.Qt.PenStyle, QtGui.QColor]) -> None: ...
    def setToolTip(self, tip:str) -> None: ...
    def setUnderlineColor(self, color:Union[QtGui.QColor, QtGui.QRgba64, Any, QtCore.Qt.GlobalColor, str, int]) -> None: ...
    def setUnderlineStyle(self, style:QtGui.QTextCharFormat.UnderlineStyle) -> None: ...
    def setVerticalAlignment(self, alignment:QtGui.QTextCharFormat.VerticalAlignment) -> None: ...
    def subScriptBaseline(self) -> float: ...
    def superScriptBaseline(self) -> float: ...
    def tableCellColumnSpan(self) -> int: ...
    def tableCellRowSpan(self) -> int: ...
    def textOutline(self) -> QtGui.QPen: ...
    def toolTip(self) -> str: ...
    def underlineColor(self) -> QtGui.QColor: ...
    def underlineStyle(self) -> QtGui.QTextCharFormat.UnderlineStyle: ...
    def verticalAlignment(self) -> QtGui.QTextCharFormat.VerticalAlignment: ...


class QTextCursor(Shiboken.Object):

    MoveAnchor               : QTextCursor.MoveMode = ... # 0x0
    KeepAnchor               : QTextCursor.MoveMode = ... # 0x1
    NoMove                   : QTextCursor.MoveOperation = ... # 0x0
    Start                    : QTextCursor.MoveOperation = ... # 0x1
    Up                       : QTextCursor.MoveOperation = ... # 0x2
    StartOfLine              : QTextCursor.MoveOperation = ... # 0x3
    StartOfBlock             : QTextCursor.MoveOperation = ... # 0x4
    StartOfWord              : QTextCursor.MoveOperation = ... # 0x5
    PreviousBlock            : QTextCursor.MoveOperation = ... # 0x6
    PreviousCharacter        : QTextCursor.MoveOperation = ... # 0x7
    PreviousWord             : QTextCursor.MoveOperation = ... # 0x8
    Left                     : QTextCursor.MoveOperation = ... # 0x9
    WordLeft                 : QTextCursor.MoveOperation = ... # 0xa
    End                      : QTextCursor.MoveOperation = ... # 0xb
    Down                     : QTextCursor.MoveOperation = ... # 0xc
    EndOfLine                : QTextCursor.MoveOperation = ... # 0xd
    EndOfWord                : QTextCursor.MoveOperation = ... # 0xe
    EndOfBlock               : QTextCursor.MoveOperation = ... # 0xf
    NextBlock                : QTextCursor.MoveOperation = ... # 0x10
    NextCharacter            : QTextCursor.MoveOperation = ... # 0x11
    NextWord                 : QTextCursor.MoveOperation = ... # 0x12
    Right                    : QTextCursor.MoveOperation = ... # 0x13
    WordRight                : QTextCursor.MoveOperation = ... # 0x14
    NextCell                 : QTextCursor.MoveOperation = ... # 0x15
    PreviousCell             : QTextCursor.MoveOperation = ... # 0x16
    NextRow                  : QTextCursor.MoveOperation = ... # 0x17
    PreviousRow              : QTextCursor.MoveOperation = ... # 0x18
    WordUnderCursor          : QTextCursor.SelectionType = ... # 0x0
    LineUnderCursor          : QTextCursor.SelectionType = ... # 0x1
    BlockUnderCursor         : QTextCursor.SelectionType = ... # 0x2
    Document                 : QTextCursor.SelectionType = ... # 0x3

    class MoveMode(Enum):

        MoveAnchor               : QTextCursor.MoveMode # 0x0
        KeepAnchor               : QTextCursor.MoveMode # 0x1

    class MoveOperation(Enum):

        NoMove                   : QTextCursor.MoveOperation # 0x0
        Start                    : QTextCursor.MoveOperation # 0x1
        Up                       : QTextCursor.MoveOperation # 0x2
        StartOfLine              : QTextCursor.MoveOperation # 0x3
        StartOfBlock             : QTextCursor.MoveOperation # 0x4
        StartOfWord              : QTextCursor.MoveOperation # 0x5
        PreviousBlock            : QTextCursor.MoveOperation # 0x6
        PreviousCharacter        : QTextCursor.MoveOperation # 0x7
        PreviousWord             : QTextCursor.MoveOperation # 0x8
        Left                     : QTextCursor.MoveOperation # 0x9
        WordLeft                 : QTextCursor.MoveOperation # 0xa
        End                      : QTextCursor.MoveOperation # 0xb
        Down                     : QTextCursor.MoveOperation # 0xc
        EndOfLine                : QTextCursor.MoveOperation # 0xd
        EndOfWord                : QTextCursor.MoveOperation # 0xe
        EndOfBlock               : QTextCursor.MoveOperation # 0xf
        NextBlock                : QTextCursor.MoveOperation # 0x10
        NextCharacter            : QTextCursor.MoveOperation # 0x11
        NextWord                 : QTextCursor.MoveOperation # 0x12
        Right                    : QTextCursor.MoveOperation # 0x13
        WordRight                : QTextCursor.MoveOperation # 0x14
        NextCell                 : QTextCursor.MoveOperation # 0x15
        PreviousCell             : QTextCursor.MoveOperation # 0x16
        NextRow                  : QTextCursor.MoveOperation # 0x17
        PreviousRow              : QTextCursor.MoveOperation # 0x18

    class SelectionType(Enum):

        WordUnderCursor          : QTextCursor.SelectionType # 0x0
        LineUnderCursor          : QTextCursor.SelectionType # 0x1
        BlockUnderCursor         : QTextCursor.SelectionType # 0x2
        Document                 : QTextCursor.SelectionType # 0x3


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, block:QtGui.QTextBlock) -> None: ...
    @overload
    def __init__(self, cursor:QtGui.QTextCursor) -> None: ...
    @overload
    def __init__(self, document:QtGui.QTextDocument) -> None: ...
    @overload
    def __init__(self, frame:QtGui.QTextFrame) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def anchor(self) -> int: ...
    def atBlockEnd(self) -> bool: ...
    def atBlockStart(self) -> bool: ...
    def atEnd(self) -> bool: ...
    def atStart(self) -> bool: ...
    def beginEditBlock(self) -> None: ...
    def block(self) -> QtGui.QTextBlock: ...
    def blockCharFormat(self) -> QtGui.QTextCharFormat: ...
    def blockFormat(self) -> QtGui.QTextBlockFormat: ...
    def blockNumber(self) -> int: ...
    def charFormat(self) -> QtGui.QTextCharFormat: ...
    def clearSelection(self) -> None: ...
    def columnNumber(self) -> int: ...
    @overload
    def createList(self, format:QtGui.QTextListFormat) -> QtGui.QTextList: ...
    @overload
    def createList(self, style:QtGui.QTextListFormat.Style) -> QtGui.QTextList: ...
    def currentFrame(self) -> QtGui.QTextFrame: ...
    def currentList(self) -> QtGui.QTextList: ...
    def currentTable(self) -> QtGui.QTextTable: ...
    def deleteChar(self) -> None: ...
    def deletePreviousChar(self) -> None: ...
    def document(self) -> QtGui.QTextDocument: ...
    def endEditBlock(self) -> None: ...
    def hasComplexSelection(self) -> bool: ...
    def hasSelection(self) -> bool: ...
    @overload
    def insertBlock(self) -> None: ...
    @overload
    def insertBlock(self, format:QtGui.QTextBlockFormat) -> None: ...
    @overload
    def insertBlock(self, format:QtGui.QTextBlockFormat, charFormat:QtGui.QTextCharFormat) -> None: ...
    def insertFragment(self, fragment:QtGui.QTextDocumentFragment) -> None: ...
    def insertFrame(self, format:QtGui.QTextFrameFormat) -> QtGui.QTextFrame: ...
    def insertHtml(self, html:str) -> None: ...
    @overload
    def insertImage(self, format:QtGui.QTextImageFormat) -> None: ...
    @overload
    def insertImage(self, format:QtGui.QTextImageFormat, alignment:QtGui.QTextFrameFormat.Position) -> None: ...
    @overload
    def insertImage(self, image:Union[QtGui.QImage, str], name:str=...) -> None: ...
    @overload
    def insertImage(self, name:str) -> None: ...
    @overload
    def insertList(self, format:QtGui.QTextListFormat) -> QtGui.QTextList: ...
    @overload
    def insertList(self, style:QtGui.QTextListFormat.Style) -> QtGui.QTextList: ...
    @overload
    def insertTable(self, rows:int, cols:int) -> QtGui.QTextTable: ...
    @overload
    def insertTable(self, rows:int, cols:int, format:QtGui.QTextTableFormat) -> QtGui.QTextTable: ...
    @overload
    def insertText(self, text:str) -> None: ...
    @overload
    def insertText(self, text:str, format:QtGui.QTextCharFormat) -> None: ...
    def isCopyOf(self, other:QtGui.QTextCursor) -> bool: ...
    def isNull(self) -> bool: ...
    def joinPreviousEditBlock(self) -> None: ...
    def keepPositionOnInsert(self) -> bool: ...
    def mergeBlockCharFormat(self, modifier:QtGui.QTextCharFormat) -> None: ...
    def mergeBlockFormat(self, modifier:QtGui.QTextBlockFormat) -> None: ...
    def mergeCharFormat(self, modifier:QtGui.QTextCharFormat) -> None: ...
    def movePosition(self, op:QtGui.QTextCursor.MoveOperation, arg__2:QtGui.QTextCursor.MoveMode=..., n:int=...) -> bool: ...
    def position(self) -> int: ...
    def positionInBlock(self) -> int: ...
    def removeSelectedText(self) -> None: ...
    def select(self, selection:QtGui.QTextCursor.SelectionType) -> None: ...
    def selectedTableCells(self) -> Tuple[int, int, int, int]: ...
    def selectedText(self) -> str: ...
    def selection(self) -> QtGui.QTextDocumentFragment: ...
    def selectionEnd(self) -> int: ...
    def selectionStart(self) -> int: ...
    def setBlockCharFormat(self, format:QtGui.QTextCharFormat) -> None: ...
    def setBlockFormat(self, format:QtGui.QTextBlockFormat) -> None: ...
    def setCharFormat(self, format:QtGui.QTextCharFormat) -> None: ...
    def setKeepPositionOnInsert(self, b:bool) -> None: ...
    def setPosition(self, pos:int, mode:QtGui.QTextCursor.MoveMode=...) -> None: ...
    def setVerticalMovementX(self, x:int) -> None: ...
    def setVisualNavigation(self, b:bool) -> None: ...
    def swap(self, other:QtGui.QTextCursor) -> None: ...
    def verticalMovementX(self) -> int: ...
    def visualNavigation(self) -> bool: ...


class QTextDocument(QtCore.QObject):

    FindBackward             : QTextDocument.FindFlag = ... # 0x1
    FindCaseSensitively      : QTextDocument.FindFlag = ... # 0x2
    FindWholeWords           : QTextDocument.FindFlag = ... # 0x4
    MarkdownDialectCommonMark: QTextDocument.MarkdownFeature = ... # 0x0
    MarkdownNoHTML           : QTextDocument.MarkdownFeature = ... # 0x60
    MarkdownDialectGitHub    : QTextDocument.MarkdownFeature = ... # 0x4f0c
    DocumentTitle            : QTextDocument.MetaInformation = ... # 0x0
    DocumentUrl              : QTextDocument.MetaInformation = ... # 0x1
    UnknownResource          : QTextDocument.ResourceType = ... # 0x0
    HtmlResource             : QTextDocument.ResourceType = ... # 0x1
    ImageResource            : QTextDocument.ResourceType = ... # 0x2
    StyleSheetResource       : QTextDocument.ResourceType = ... # 0x3
    MarkdownResource         : QTextDocument.ResourceType = ... # 0x4
    UserResource             : QTextDocument.ResourceType = ... # 0x64
    UndoStack                : QTextDocument.Stacks = ... # 0x1
    RedoStack                : QTextDocument.Stacks = ... # 0x2
    UndoAndRedoStacks        : QTextDocument.Stacks = ... # 0x3

    class FindFlag(Enum):

        FindBackward             : QTextDocument.FindFlag # 0x1
        FindCaseSensitively      : QTextDocument.FindFlag # 0x2
        FindWholeWords           : QTextDocument.FindFlag # 0x4

    class FindFlags: ...

    class MarkdownFeature(Enum):

        MarkdownDialectCommonMark: QTextDocument.MarkdownFeature # 0x0
        MarkdownNoHTML           : QTextDocument.MarkdownFeature # 0x60
        MarkdownDialectGitHub    : QTextDocument.MarkdownFeature # 0x4f0c

    class MarkdownFeatures: ...

    class MetaInformation(Enum):

        DocumentTitle            : QTextDocument.MetaInformation # 0x0
        DocumentUrl              : QTextDocument.MetaInformation # 0x1

    class ResourceType(Enum):

        UnknownResource          : QTextDocument.ResourceType # 0x0
        HtmlResource             : QTextDocument.ResourceType # 0x1
        ImageResource            : QTextDocument.ResourceType # 0x2
        StyleSheetResource       : QTextDocument.ResourceType # 0x3
        MarkdownResource         : QTextDocument.ResourceType # 0x4
        UserResource             : QTextDocument.ResourceType # 0x64

    class Stacks(Enum):

        UndoStack                : QTextDocument.Stacks # 0x1
        RedoStack                : QTextDocument.Stacks # 0x2
        UndoAndRedoStacks        : QTextDocument.Stacks # 0x3


    @overload
    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...
    @overload
    def __init__(self, text:str, parent:Optional[QtCore.QObject]=...) -> None: ...

    def addResource(self, type:int, name:Union[QtCore.QUrl, str], resource:Any) -> None: ...
    def adjustSize(self) -> None: ...
    def allFormats(self) -> List[QtGui.QTextFormat]: ...
    def availableRedoSteps(self) -> int: ...
    def availableUndoSteps(self) -> int: ...
    def baseUrl(self) -> QtCore.QUrl: ...
    def baselineOffset(self) -> float: ...
    def begin(self) -> QtGui.QTextBlock: ...
    def blockCount(self) -> int: ...
    def characterAt(self, pos:int) -> str: ...
    def characterCount(self) -> int: ...
    def clear(self) -> None: ...
    def clearUndoRedoStacks(self, historyToClear:QtGui.QTextDocument.Stacks=...) -> None: ...
    def clone(self, parent:Optional[QtCore.QObject]=...) -> QtGui.QTextDocument: ...
    def createObject(self, f:QtGui.QTextFormat) -> QtGui.QTextObject: ...
    def defaultCursorMoveStyle(self) -> QtCore.Qt.CursorMoveStyle: ...
    def defaultFont(self) -> QtGui.QFont: ...
    def defaultStyleSheet(self) -> str: ...
    def defaultTextOption(self) -> QtGui.QTextOption: ...
    def documentLayout(self) -> QtGui.QAbstractTextDocumentLayout: ...
    def documentMargin(self) -> float: ...
    def drawContents(self, painter:QtGui.QPainter, rect:Union[QtCore.QRectF, QtCore.QRect]=...) -> None: ...
    def end(self) -> QtGui.QTextBlock: ...
    @overload
    def find(self, expr:Union[QtCore.QRegularExpression, str], cursor:QtGui.QTextCursor, options:QtGui.QTextDocument.FindFlags=...) -> QtGui.QTextCursor: ...
    @overload
    def find(self, expr:Union[QtCore.QRegularExpression, str], from_:int=..., options:QtGui.QTextDocument.FindFlags=...) -> QtGui.QTextCursor: ...
    @overload
    def find(self, subString:str, cursor:QtGui.QTextCursor, options:QtGui.QTextDocument.FindFlags=...) -> QtGui.QTextCursor: ...
    @overload
    def find(self, subString:str, from_:int=..., options:QtGui.QTextDocument.FindFlags=...) -> QtGui.QTextCursor: ...
    def findBlock(self, pos:int) -> QtGui.QTextBlock: ...
    def findBlockByLineNumber(self, blockNumber:int) -> QtGui.QTextBlock: ...
    def findBlockByNumber(self, blockNumber:int) -> QtGui.QTextBlock: ...
    def firstBlock(self) -> QtGui.QTextBlock: ...
    def frameAt(self, pos:int) -> QtGui.QTextFrame: ...
    def idealWidth(self) -> float: ...
    def indentWidth(self) -> float: ...
    def isEmpty(self) -> bool: ...
    def isModified(self) -> bool: ...
    def isRedoAvailable(self) -> bool: ...
    def isUndoAvailable(self) -> bool: ...
    def isUndoRedoEnabled(self) -> bool: ...
    def lastBlock(self) -> QtGui.QTextBlock: ...
    def lineCount(self) -> int: ...
    def loadResource(self, type:int, name:Union[QtCore.QUrl, str]) -> Any: ...
    def markContentsDirty(self, from_:int, length:int) -> None: ...
    def maximumBlockCount(self) -> int: ...
    def metaInformation(self, info:QtGui.QTextDocument.MetaInformation) -> str: ...
    def object(self, objectIndex:int) -> QtGui.QTextObject: ...
    def objectForFormat(self, arg__1:QtGui.QTextFormat) -> QtGui.QTextObject: ...
    def pageCount(self) -> int: ...
    def pageSize(self) -> QtCore.QSizeF: ...
    def print_(self, printer:QtGui.QPagedPaintDevice) -> None: ...
    @overload
    def redo(self) -> None: ...
    @overload
    def redo(self, cursor:QtGui.QTextCursor) -> None: ...
    def resource(self, type:int, name:Union[QtCore.QUrl, str]) -> Any: ...
    def revision(self) -> int: ...
    def rootFrame(self) -> QtGui.QTextFrame: ...
    def setBaseUrl(self, url:Union[QtCore.QUrl, str]) -> None: ...
    def setBaselineOffset(self, baseline:float) -> None: ...
    def setDefaultCursorMoveStyle(self, style:QtCore.Qt.CursorMoveStyle) -> None: ...
    def setDefaultFont(self, font:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    def setDefaultStyleSheet(self, sheet:str) -> None: ...
    def setDefaultTextOption(self, option:Union[QtGui.QTextOption, QtCore.Qt.Alignment]) -> None: ...
    def setDocumentLayout(self, layout:QtGui.QAbstractTextDocumentLayout) -> None: ...
    def setDocumentMargin(self, margin:float) -> None: ...
    def setHtml(self, html:str) -> None: ...
    def setIndentWidth(self, width:float) -> None: ...
    def setMarkdown(self, markdown:str, features:QtGui.QTextDocument.MarkdownFeatures=...) -> None: ...
    def setMaximumBlockCount(self, maximum:int) -> None: ...
    def setMetaInformation(self, info:QtGui.QTextDocument.MetaInformation, arg__2:str) -> None: ...
    def setModified(self, m:bool=...) -> None: ...
    def setPageSize(self, size:Union[QtCore.QSizeF, QtCore.QSize]) -> None: ...
    def setPlainText(self, text:str) -> None: ...
    def setSubScriptBaseline(self, baseline:float) -> None: ...
    def setSuperScriptBaseline(self, baseline:float) -> None: ...
    def setTextWidth(self, width:float) -> None: ...
    def setUndoRedoEnabled(self, enable:bool) -> None: ...
    def setUseDesignMetrics(self, b:bool) -> None: ...
    def size(self) -> QtCore.QSizeF: ...
    def subScriptBaseline(self) -> float: ...
    def superScriptBaseline(self) -> float: ...
    def textWidth(self) -> float: ...
    def toHtml(self) -> str: ...
    def toMarkdown(self, features:QtGui.QTextDocument.MarkdownFeatures=...) -> str: ...
    def toPlainText(self) -> str: ...
    def toRawText(self) -> str: ...
    @overload
    def undo(self) -> None: ...
    @overload
    def undo(self, cursor:QtGui.QTextCursor) -> None: ...
    def useDesignMetrics(self) -> bool: ...


class QTextDocumentFragment(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, document:QtGui.QTextDocument) -> None: ...
    @overload
    def __init__(self, range:QtGui.QTextCursor) -> None: ...
    @overload
    def __init__(self, rhs:QtGui.QTextDocumentFragment) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    @staticmethod
    def fromHtml(html:str, resourceProvider:Optional[QtGui.QTextDocument]=...) -> QtGui.QTextDocumentFragment: ...
    @staticmethod
    def fromPlainText(plainText:str) -> QtGui.QTextDocumentFragment: ...
    def isEmpty(self) -> bool: ...
    def toHtml(self) -> str: ...
    def toPlainText(self) -> str: ...


class QTextDocumentWriter(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, device:QtCore.QIODevice, format:Union[QtCore.QByteArray, bytes]) -> None: ...
    @overload
    def __init__(self, fileName:str, format:Union[QtCore.QByteArray, bytes]=...) -> None: ...

    def device(self) -> QtCore.QIODevice: ...
    def fileName(self) -> str: ...
    def format(self) -> QtCore.QByteArray: ...
    def setDevice(self, device:QtCore.QIODevice) -> None: ...
    def setFileName(self, fileName:str) -> None: ...
    def setFormat(self, format:Union[QtCore.QByteArray, bytes]) -> None: ...
    @staticmethod
    def supportedDocumentFormats() -> List[QtCore.QByteArray]: ...
    @overload
    def write(self, document:QtGui.QTextDocument) -> bool: ...
    @overload
    def write(self, fragment:QtGui.QTextDocumentFragment) -> bool: ...


class QTextFormat(Shiboken.Object):

    InvalidFormat            : QTextFormat.FormatType = ... # -0x1
    BlockFormat              : QTextFormat.FormatType = ... # 0x1
    CharFormat               : QTextFormat.FormatType = ... # 0x2
    ListFormat               : QTextFormat.FormatType = ... # 0x3
    FrameFormat              : QTextFormat.FormatType = ... # 0x5
    UserFormat               : QTextFormat.FormatType = ... # 0x64
    NoObject                 : QTextFormat.ObjectTypes = ... # 0x0
    ImageObject              : QTextFormat.ObjectTypes = ... # 0x1
    TableObject              : QTextFormat.ObjectTypes = ... # 0x2
    TableCellObject          : QTextFormat.ObjectTypes = ... # 0x3
    UserObject               : QTextFormat.ObjectTypes = ... # 0x1000
    PageBreak_Auto           : QTextFormat.PageBreakFlag = ... # 0x0
    PageBreak_AlwaysBefore   : QTextFormat.PageBreakFlag = ... # 0x1
    PageBreak_AlwaysAfter    : QTextFormat.PageBreakFlag = ... # 0x10
    ObjectIndex              : QTextFormat.Property = ... # 0x0
    CssFloat                 : QTextFormat.Property = ... # 0x800
    LayoutDirection          : QTextFormat.Property = ... # 0x801
    OutlinePen               : QTextFormat.Property = ... # 0x810
    BackgroundBrush          : QTextFormat.Property = ... # 0x820
    ForegroundBrush          : QTextFormat.Property = ... # 0x821
    BackgroundImageUrl       : QTextFormat.Property = ... # 0x823
    BlockAlignment           : QTextFormat.Property = ... # 0x1010
    BlockTopMargin           : QTextFormat.Property = ... # 0x1030
    BlockBottomMargin        : QTextFormat.Property = ... # 0x1031
    BlockLeftMargin          : QTextFormat.Property = ... # 0x1032
    BlockRightMargin         : QTextFormat.Property = ... # 0x1033
    TextIndent               : QTextFormat.Property = ... # 0x1034
    TabPositions             : QTextFormat.Property = ... # 0x1035
    BlockIndent              : QTextFormat.Property = ... # 0x1040
    LineHeight               : QTextFormat.Property = ... # 0x1048
    LineHeightType           : QTextFormat.Property = ... # 0x1049
    BlockNonBreakableLines   : QTextFormat.Property = ... # 0x1050
    BlockTrailingHorizontalRulerWidth: QTextFormat.Property = ... # 0x1060
    HeadingLevel             : QTextFormat.Property = ... # 0x1070
    BlockQuoteLevel          : QTextFormat.Property = ... # 0x1080
    BlockCodeLanguage        : QTextFormat.Property = ... # 0x1090
    BlockCodeFence           : QTextFormat.Property = ... # 0x1091
    BlockMarker              : QTextFormat.Property = ... # 0x10a0
    FirstFontProperty        : QTextFormat.Property = ... # 0x1fe0
    FontCapitalization       : QTextFormat.Property = ... # 0x1fe0
    FontLetterSpacing        : QTextFormat.Property = ... # 0x1fe1
    FontWordSpacing          : QTextFormat.Property = ... # 0x1fe2
    FontStyleHint            : QTextFormat.Property = ... # 0x1fe3
    FontStyleStrategy        : QTextFormat.Property = ... # 0x1fe4
    FontKerning              : QTextFormat.Property = ... # 0x1fe5
    FontHintingPreference    : QTextFormat.Property = ... # 0x1fe6
    FontFamilies             : QTextFormat.Property = ... # 0x1fe7
    FontStyleName            : QTextFormat.Property = ... # 0x1fe8
    FontLetterSpacingType    : QTextFormat.Property = ... # 0x1fe9
    FontStretch              : QTextFormat.Property = ... # 0x1fea
    FontFamily               : QTextFormat.Property = ... # 0x2000
    FontPointSize            : QTextFormat.Property = ... # 0x2001
    FontSizeAdjustment       : QTextFormat.Property = ... # 0x2002
    FontSizeIncrement        : QTextFormat.Property = ... # 0x2002
    FontWeight               : QTextFormat.Property = ... # 0x2003
    FontItalic               : QTextFormat.Property = ... # 0x2004
    FontUnderline            : QTextFormat.Property = ... # 0x2005
    FontOverline             : QTextFormat.Property = ... # 0x2006
    FontStrikeOut            : QTextFormat.Property = ... # 0x2007
    FontFixedPitch           : QTextFormat.Property = ... # 0x2008
    FontPixelSize            : QTextFormat.Property = ... # 0x2009
    LastFontProperty         : QTextFormat.Property = ... # 0x2009
    OldTextUnderlineColor    : QTextFormat.Property = ... # 0x2010
    TextUnderlineColor       : QTextFormat.Property = ... # 0x2020
    TextVerticalAlignment    : QTextFormat.Property = ... # 0x2021
    TextOutline              : QTextFormat.Property = ... # 0x2022
    TextUnderlineStyle       : QTextFormat.Property = ... # 0x2023
    TextToolTip              : QTextFormat.Property = ... # 0x2024
    TextSuperScriptBaseline  : QTextFormat.Property = ... # 0x2025
    TextSubScriptBaseline    : QTextFormat.Property = ... # 0x2026
    TextBaselineOffset       : QTextFormat.Property = ... # 0x2027
    IsAnchor                 : QTextFormat.Property = ... # 0x2030
    AnchorHref               : QTextFormat.Property = ... # 0x2031
    AnchorName               : QTextFormat.Property = ... # 0x2032
    OldFontLetterSpacingType : QTextFormat.Property = ... # 0x2033
    OldFontStretch           : QTextFormat.Property = ... # 0x2034
    ObjectType               : QTextFormat.Property = ... # 0x2f00
    ListStyle                : QTextFormat.Property = ... # 0x3000
    ListIndent               : QTextFormat.Property = ... # 0x3001
    ListNumberPrefix         : QTextFormat.Property = ... # 0x3002
    ListNumberSuffix         : QTextFormat.Property = ... # 0x3003
    FrameBorder              : QTextFormat.Property = ... # 0x4000
    FrameMargin              : QTextFormat.Property = ... # 0x4001
    FramePadding             : QTextFormat.Property = ... # 0x4002
    FrameWidth               : QTextFormat.Property = ... # 0x4003
    FrameHeight              : QTextFormat.Property = ... # 0x4004
    FrameTopMargin           : QTextFormat.Property = ... # 0x4005
    FrameBottomMargin        : QTextFormat.Property = ... # 0x4006
    FrameLeftMargin          : QTextFormat.Property = ... # 0x4007
    FrameRightMargin         : QTextFormat.Property = ... # 0x4008
    FrameBorderBrush         : QTextFormat.Property = ... # 0x4009
    FrameBorderStyle         : QTextFormat.Property = ... # 0x4010
    TableColumns             : QTextFormat.Property = ... # 0x4100
    TableColumnWidthConstraints: QTextFormat.Property = ... # 0x4101
    TableCellSpacing         : QTextFormat.Property = ... # 0x4102
    TableCellPadding         : QTextFormat.Property = ... # 0x4103
    TableHeaderRowCount      : QTextFormat.Property = ... # 0x4104
    TableBorderCollapse      : QTextFormat.Property = ... # 0x4105
    TableCellRowSpan         : QTextFormat.Property = ... # 0x4810
    TableCellColumnSpan      : QTextFormat.Property = ... # 0x4811
    TableCellTopPadding      : QTextFormat.Property = ... # 0x4812
    TableCellBottomPadding   : QTextFormat.Property = ... # 0x4813
    TableCellLeftPadding     : QTextFormat.Property = ... # 0x4814
    TableCellRightPadding    : QTextFormat.Property = ... # 0x4815
    TableCellTopBorder       : QTextFormat.Property = ... # 0x4816
    TableCellBottomBorder    : QTextFormat.Property = ... # 0x4817
    TableCellLeftBorder      : QTextFormat.Property = ... # 0x4818
    TableCellRightBorder     : QTextFormat.Property = ... # 0x4819
    TableCellTopBorderStyle  : QTextFormat.Property = ... # 0x481a
    TableCellBottomBorderStyle: QTextFormat.Property = ... # 0x481b
    TableCellLeftBorderStyle : QTextFormat.Property = ... # 0x481c
    TableCellRightBorderStyle: QTextFormat.Property = ... # 0x481d
    TableCellTopBorderBrush  : QTextFormat.Property = ... # 0x481e
    TableCellBottomBorderBrush: QTextFormat.Property = ... # 0x481f
    TableCellLeftBorderBrush : QTextFormat.Property = ... # 0x4820
    TableCellRightBorderBrush: QTextFormat.Property = ... # 0x4821
    ImageName                : QTextFormat.Property = ... # 0x5000
    ImageTitle               : QTextFormat.Property = ... # 0x5001
    ImageAltText             : QTextFormat.Property = ... # 0x5002
    ImageWidth               : QTextFormat.Property = ... # 0x5010
    ImageHeight              : QTextFormat.Property = ... # 0x5011
    ImageQuality             : QTextFormat.Property = ... # 0x5014
    FullWidthSelection       : QTextFormat.Property = ... # 0x6000
    PageBreakPolicy          : QTextFormat.Property = ... # 0x7000
    UserProperty             : QTextFormat.Property = ... # 0x100000

    class FormatType(Enum):

        InvalidFormat            : QTextFormat.FormatType # -0x1
        BlockFormat              : QTextFormat.FormatType # 0x1
        CharFormat               : QTextFormat.FormatType # 0x2
        ListFormat               : QTextFormat.FormatType # 0x3
        FrameFormat              : QTextFormat.FormatType # 0x5
        UserFormat               : QTextFormat.FormatType # 0x64

    class ObjectTypes(Enum):

        NoObject                 : QTextFormat.ObjectTypes # 0x0
        ImageObject              : QTextFormat.ObjectTypes # 0x1
        TableObject              : QTextFormat.ObjectTypes # 0x2
        TableCellObject          : QTextFormat.ObjectTypes # 0x3
        UserObject               : QTextFormat.ObjectTypes # 0x1000

    class PageBreakFlag(Enum):

        PageBreak_Auto           : QTextFormat.PageBreakFlag # 0x0
        PageBreak_AlwaysBefore   : QTextFormat.PageBreakFlag # 0x1
        PageBreak_AlwaysAfter    : QTextFormat.PageBreakFlag # 0x10

    class PageBreakFlags(object): ...

    class Property(Enum):

        ObjectIndex              : QTextFormat.Property # 0x0
        CssFloat                 : QTextFormat.Property # 0x800
        LayoutDirection          : QTextFormat.Property # 0x801
        OutlinePen               : QTextFormat.Property # 0x810
        BackgroundBrush          : QTextFormat.Property # 0x820
        ForegroundBrush          : QTextFormat.Property # 0x821
        BackgroundImageUrl       : QTextFormat.Property # 0x823
        BlockAlignment           : QTextFormat.Property # 0x1010
        BlockTopMargin           : QTextFormat.Property # 0x1030
        BlockBottomMargin        : QTextFormat.Property # 0x1031
        BlockLeftMargin          : QTextFormat.Property # 0x1032
        BlockRightMargin         : QTextFormat.Property # 0x1033
        TextIndent               : QTextFormat.Property # 0x1034
        TabPositions             : QTextFormat.Property # 0x1035
        BlockIndent              : QTextFormat.Property # 0x1040
        LineHeight               : QTextFormat.Property # 0x1048
        LineHeightType           : QTextFormat.Property # 0x1049
        BlockNonBreakableLines   : QTextFormat.Property # 0x1050
        BlockTrailingHorizontalRulerWidth: QTextFormat.Property # 0x1060
        HeadingLevel             : QTextFormat.Property # 0x1070
        BlockQuoteLevel          : QTextFormat.Property # 0x1080
        BlockCodeLanguage        : QTextFormat.Property # 0x1090
        BlockCodeFence           : QTextFormat.Property # 0x1091
        BlockMarker              : QTextFormat.Property # 0x10a0
        FirstFontProperty        : QTextFormat.Property # 0x1fe0
        FontCapitalization       : QTextFormat.Property # 0x1fe0
        FontLetterSpacing        : QTextFormat.Property # 0x1fe1
        FontWordSpacing          : QTextFormat.Property # 0x1fe2
        FontStyleHint            : QTextFormat.Property # 0x1fe3
        FontStyleStrategy        : QTextFormat.Property # 0x1fe4
        FontKerning              : QTextFormat.Property # 0x1fe5
        FontHintingPreference    : QTextFormat.Property # 0x1fe6
        FontFamilies             : QTextFormat.Property # 0x1fe7
        FontStyleName            : QTextFormat.Property # 0x1fe8
        FontLetterSpacingType    : QTextFormat.Property # 0x1fe9
        FontStretch              : QTextFormat.Property # 0x1fea
        FontFamily               : QTextFormat.Property # 0x2000
        FontPointSize            : QTextFormat.Property # 0x2001
        FontSizeAdjustment       : QTextFormat.Property # 0x2002
        FontSizeIncrement        : QTextFormat.Property # 0x2002
        FontWeight               : QTextFormat.Property # 0x2003
        FontItalic               : QTextFormat.Property # 0x2004
        FontUnderline            : QTextFormat.Property # 0x2005
        FontOverline             : QTextFormat.Property # 0x2006
        FontStrikeOut            : QTextFormat.Property # 0x2007
        FontFixedPitch           : QTextFormat.Property # 0x2008
        FontPixelSize            : QTextFormat.Property # 0x2009
        LastFontProperty         : QTextFormat.Property # 0x2009
        OldTextUnderlineColor    : QTextFormat.Property # 0x2010
        TextUnderlineColor       : QTextFormat.Property # 0x2020
        TextVerticalAlignment    : QTextFormat.Property # 0x2021
        TextOutline              : QTextFormat.Property # 0x2022
        TextUnderlineStyle       : QTextFormat.Property # 0x2023
        TextToolTip              : QTextFormat.Property # 0x2024
        TextSuperScriptBaseline  : QTextFormat.Property # 0x2025
        TextSubScriptBaseline    : QTextFormat.Property # 0x2026
        TextBaselineOffset       : QTextFormat.Property # 0x2027
        IsAnchor                 : QTextFormat.Property # 0x2030
        AnchorHref               : QTextFormat.Property # 0x2031
        AnchorName               : QTextFormat.Property # 0x2032
        OldFontLetterSpacingType : QTextFormat.Property # 0x2033
        OldFontStretch           : QTextFormat.Property # 0x2034
        ObjectType               : QTextFormat.Property # 0x2f00
        ListStyle                : QTextFormat.Property # 0x3000
        ListIndent               : QTextFormat.Property # 0x3001
        ListNumberPrefix         : QTextFormat.Property # 0x3002
        ListNumberSuffix         : QTextFormat.Property # 0x3003
        FrameBorder              : QTextFormat.Property # 0x4000
        FrameMargin              : QTextFormat.Property # 0x4001
        FramePadding             : QTextFormat.Property # 0x4002
        FrameWidth               : QTextFormat.Property # 0x4003
        FrameHeight              : QTextFormat.Property # 0x4004
        FrameTopMargin           : QTextFormat.Property # 0x4005
        FrameBottomMargin        : QTextFormat.Property # 0x4006
        FrameLeftMargin          : QTextFormat.Property # 0x4007
        FrameRightMargin         : QTextFormat.Property # 0x4008
        FrameBorderBrush         : QTextFormat.Property # 0x4009
        FrameBorderStyle         : QTextFormat.Property # 0x4010
        TableColumns             : QTextFormat.Property # 0x4100
        TableColumnWidthConstraints: QTextFormat.Property # 0x4101
        TableCellSpacing         : QTextFormat.Property # 0x4102
        TableCellPadding         : QTextFormat.Property # 0x4103
        TableHeaderRowCount      : QTextFormat.Property # 0x4104
        TableBorderCollapse      : QTextFormat.Property # 0x4105
        TableCellRowSpan         : QTextFormat.Property # 0x4810
        TableCellColumnSpan      : QTextFormat.Property # 0x4811
        TableCellTopPadding      : QTextFormat.Property # 0x4812
        TableCellBottomPadding   : QTextFormat.Property # 0x4813
        TableCellLeftPadding     : QTextFormat.Property # 0x4814
        TableCellRightPadding    : QTextFormat.Property # 0x4815
        TableCellTopBorder       : QTextFormat.Property # 0x4816
        TableCellBottomBorder    : QTextFormat.Property # 0x4817
        TableCellLeftBorder      : QTextFormat.Property # 0x4818
        TableCellRightBorder     : QTextFormat.Property # 0x4819
        TableCellTopBorderStyle  : QTextFormat.Property # 0x481a
        TableCellBottomBorderStyle: QTextFormat.Property # 0x481b
        TableCellLeftBorderStyle : QTextFormat.Property # 0x481c
        TableCellRightBorderStyle: QTextFormat.Property # 0x481d
        TableCellTopBorderBrush  : QTextFormat.Property # 0x481e
        TableCellBottomBorderBrush: QTextFormat.Property # 0x481f
        TableCellLeftBorderBrush : QTextFormat.Property # 0x4820
        TableCellRightBorderBrush: QTextFormat.Property # 0x4821
        ImageName                : QTextFormat.Property # 0x5000
        ImageTitle               : QTextFormat.Property # 0x5001
        ImageAltText             : QTextFormat.Property # 0x5002
        ImageWidth               : QTextFormat.Property # 0x5010
        ImageHeight              : QTextFormat.Property # 0x5011
        ImageQuality             : QTextFormat.Property # 0x5014
        FullWidthSelection       : QTextFormat.Property # 0x6000
        PageBreakPolicy          : QTextFormat.Property # 0x7000
        UserProperty             : QTextFormat.Property # 0x100000


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, rhs:QtGui.QTextFormat) -> None: ...
    @overload
    def __init__(self, type:int) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def background(self) -> QtGui.QBrush: ...
    def boolProperty(self, propertyId:int) -> bool: ...
    def brushProperty(self, propertyId:int) -> QtGui.QBrush: ...
    def clearBackground(self) -> None: ...
    def clearForeground(self) -> None: ...
    def clearProperty(self, propertyId:int) -> None: ...
    def colorProperty(self, propertyId:int) -> QtGui.QColor: ...
    def doubleProperty(self, propertyId:int) -> float: ...
    def foreground(self) -> QtGui.QBrush: ...
    def hasProperty(self, propertyId:int) -> bool: ...
    def intProperty(self, propertyId:int) -> int: ...
    def isBlockFormat(self) -> bool: ...
    def isCharFormat(self) -> bool: ...
    def isEmpty(self) -> bool: ...
    def isFrameFormat(self) -> bool: ...
    def isImageFormat(self) -> bool: ...
    def isListFormat(self) -> bool: ...
    def isTableCellFormat(self) -> bool: ...
    def isTableFormat(self) -> bool: ...
    def isValid(self) -> bool: ...
    def layoutDirection(self) -> QtCore.Qt.LayoutDirection: ...
    def lengthProperty(self, propertyId:int) -> QtGui.QTextLength: ...
    def lengthVectorProperty(self, propertyId:int) -> List[QtGui.QTextLength]: ...
    def merge(self, other:QtGui.QTextFormat) -> None: ...
    def objectIndex(self) -> int: ...
    def objectType(self) -> int: ...
    def penProperty(self, propertyId:int) -> QtGui.QPen: ...
    def properties(self) -> Dict[int, Any]: ...
    def property(self, propertyId:int) -> Any: ...
    def propertyCount(self) -> int: ...
    def setBackground(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setForeground(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setLayoutDirection(self, direction:QtCore.Qt.LayoutDirection) -> None: ...
    def setObjectIndex(self, object:int) -> None: ...
    def setObjectType(self, type:int) -> None: ...
    @overload
    def setProperty(self, propertyId:int, lengths:Sequence[QtGui.QTextLength]) -> None: ...
    @overload
    def setProperty(self, propertyId:int, value:Any) -> None: ...
    def stringProperty(self, propertyId:int) -> str: ...
    def swap(self, other:QtGui.QTextFormat) -> None: ...
    def toBlockFormat(self) -> QtGui.QTextBlockFormat: ...
    def toCharFormat(self) -> QtGui.QTextCharFormat: ...
    def toFrameFormat(self) -> QtGui.QTextFrameFormat: ...
    def toImageFormat(self) -> QtGui.QTextImageFormat: ...
    def toListFormat(self) -> QtGui.QTextListFormat: ...
    def toTableCellFormat(self) -> QtGui.QTextTableCellFormat: ...
    def toTableFormat(self) -> QtGui.QTextTableFormat: ...
    def type(self) -> int: ...


class QTextFragment(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, o:QtGui.QTextFragment) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def charFormat(self) -> QtGui.QTextCharFormat: ...
    def charFormatIndex(self) -> int: ...
    def contains(self, position:int) -> bool: ...
    def glyphRuns(self, from_:int=..., length:int=...) -> List[QtGui.QGlyphRun]: ...
    def isValid(self) -> bool: ...
    def length(self) -> int: ...
    def position(self) -> int: ...
    def text(self) -> str: ...


class QTextFrame(QtGui.QTextObject):

    class iterator(Shiboken.Object):

        @overload
        def __init__(self) -> None: ...
        @overload
        def __init__(self, iterator:QtGui.QTextFrame.iterator) -> None: ...

        @staticmethod
        def __copy__() -> None: ...
        def __iadd__(self, arg__1:int) -> QtGui.QTextFrame.iterator: ...
        def __isub__(self, arg__1:int) -> QtGui.QTextFrame.iterator: ...
        def __iter__(self) -> object: ...
        def __next__(self) -> object: ...
        def atEnd(self) -> bool: ...
        def currentBlock(self) -> QtGui.QTextBlock: ...
        def currentFrame(self) -> QtGui.QTextFrame: ...
        def parentFrame(self) -> QtGui.QTextFrame: ...


    def __init__(self, doc:QtGui.QTextDocument) -> None: ...

    def __iter__(self) -> object: ...
    def begin(self) -> QtGui.QTextFrame.iterator: ...
    def childFrames(self) -> List[QtGui.QTextFrame]: ...
    def end(self) -> QtGui.QTextFrame.iterator: ...
    def firstCursorPosition(self) -> QtGui.QTextCursor: ...
    def firstPosition(self) -> int: ...
    def frameFormat(self) -> QtGui.QTextFrameFormat: ...
    def lastCursorPosition(self) -> QtGui.QTextCursor: ...
    def lastPosition(self) -> int: ...
    def parentFrame(self) -> QtGui.QTextFrame: ...
    def setFrameFormat(self, format:QtGui.QTextFrameFormat) -> None: ...


class QTextFrameFormat(QtGui.QTextFormat):

    BorderStyle_None         : QTextFrameFormat.BorderStyle = ... # 0x0
    BorderStyle_Dotted       : QTextFrameFormat.BorderStyle = ... # 0x1
    BorderStyle_Dashed       : QTextFrameFormat.BorderStyle = ... # 0x2
    BorderStyle_Solid        : QTextFrameFormat.BorderStyle = ... # 0x3
    BorderStyle_Double       : QTextFrameFormat.BorderStyle = ... # 0x4
    BorderStyle_DotDash      : QTextFrameFormat.BorderStyle = ... # 0x5
    BorderStyle_DotDotDash   : QTextFrameFormat.BorderStyle = ... # 0x6
    BorderStyle_Groove       : QTextFrameFormat.BorderStyle = ... # 0x7
    BorderStyle_Ridge        : QTextFrameFormat.BorderStyle = ... # 0x8
    BorderStyle_Inset        : QTextFrameFormat.BorderStyle = ... # 0x9
    BorderStyle_Outset       : QTextFrameFormat.BorderStyle = ... # 0xa
    InFlow                   : QTextFrameFormat.Position = ... # 0x0
    FloatLeft                : QTextFrameFormat.Position = ... # 0x1
    FloatRight               : QTextFrameFormat.Position = ... # 0x2

    class BorderStyle(Enum):

        BorderStyle_None         : QTextFrameFormat.BorderStyle # 0x0
        BorderStyle_Dotted       : QTextFrameFormat.BorderStyle # 0x1
        BorderStyle_Dashed       : QTextFrameFormat.BorderStyle # 0x2
        BorderStyle_Solid        : QTextFrameFormat.BorderStyle # 0x3
        BorderStyle_Double       : QTextFrameFormat.BorderStyle # 0x4
        BorderStyle_DotDash      : QTextFrameFormat.BorderStyle # 0x5
        BorderStyle_DotDotDash   : QTextFrameFormat.BorderStyle # 0x6
        BorderStyle_Groove       : QTextFrameFormat.BorderStyle # 0x7
        BorderStyle_Ridge        : QTextFrameFormat.BorderStyle # 0x8
        BorderStyle_Inset        : QTextFrameFormat.BorderStyle # 0x9
        BorderStyle_Outset       : QTextFrameFormat.BorderStyle # 0xa

    class Position(Enum):

        InFlow                   : QTextFrameFormat.Position # 0x0
        FloatLeft                : QTextFrameFormat.Position # 0x1
        FloatRight               : QTextFrameFormat.Position # 0x2


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QTextFrameFormat:QtGui.QTextFrameFormat) -> None: ...
    @overload
    def __init__(self, fmt:QtGui.QTextFormat) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def border(self) -> float: ...
    def borderBrush(self) -> QtGui.QBrush: ...
    def borderStyle(self) -> QtGui.QTextFrameFormat.BorderStyle: ...
    def bottomMargin(self) -> float: ...
    def height(self) -> QtGui.QTextLength: ...
    def isValid(self) -> bool: ...
    def leftMargin(self) -> float: ...
    def margin(self) -> float: ...
    def padding(self) -> float: ...
    def pageBreakPolicy(self) -> QtGui.QTextFormat.PageBreakFlags: ...
    def position(self) -> QtGui.QTextFrameFormat.Position: ...
    def rightMargin(self) -> float: ...
    def setBorder(self, border:float) -> None: ...
    def setBorderBrush(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setBorderStyle(self, style:QtGui.QTextFrameFormat.BorderStyle) -> None: ...
    def setBottomMargin(self, margin:float) -> None: ...
    @overload
    def setHeight(self, height:QtGui.QTextLength) -> None: ...
    @overload
    def setHeight(self, height:float) -> None: ...
    def setLeftMargin(self, margin:float) -> None: ...
    def setMargin(self, margin:float) -> None: ...
    def setPadding(self, padding:float) -> None: ...
    def setPageBreakPolicy(self, flags:QtGui.QTextFormat.PageBreakFlags) -> None: ...
    def setPosition(self, f:QtGui.QTextFrameFormat.Position) -> None: ...
    def setRightMargin(self, margin:float) -> None: ...
    def setTopMargin(self, margin:float) -> None: ...
    @overload
    def setWidth(self, length:QtGui.QTextLength) -> None: ...
    @overload
    def setWidth(self, width:float) -> None: ...
    def topMargin(self) -> float: ...
    def width(self) -> QtGui.QTextLength: ...


class QTextImageFormat(QtGui.QTextCharFormat):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QTextImageFormat:QtGui.QTextImageFormat) -> None: ...
    @overload
    def __init__(self, format:QtGui.QTextFormat) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def height(self) -> float: ...
    def isValid(self) -> bool: ...
    def name(self) -> str: ...
    def quality(self) -> int: ...
    def setHeight(self, height:float) -> None: ...
    def setName(self, name:str) -> None: ...
    def setQuality(self, quality:int=...) -> None: ...
    def setWidth(self, width:float) -> None: ...
    def width(self) -> float: ...


class QTextInlineObject(Shiboken.Object):

    def __init__(self) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def ascent(self) -> float: ...
    def descent(self) -> float: ...
    def format(self) -> QtGui.QTextFormat: ...
    def formatIndex(self) -> int: ...
    def height(self) -> float: ...
    def isValid(self) -> bool: ...
    def rect(self) -> QtCore.QRectF: ...
    def setAscent(self, a:float) -> None: ...
    def setDescent(self, d:float) -> None: ...
    def setWidth(self, w:float) -> None: ...
    def textDirection(self) -> QtCore.Qt.LayoutDirection: ...
    def textPosition(self) -> int: ...
    def width(self) -> float: ...


class QTextItem(Shiboken.Object):

    Dummy                    : QTextItem.RenderFlag = ... # -0x1
    RightToLeft              : QTextItem.RenderFlag = ... # 0x1
    Overline                 : QTextItem.RenderFlag = ... # 0x10
    Underline                : QTextItem.RenderFlag = ... # 0x20
    StrikeOut                : QTextItem.RenderFlag = ... # 0x40

    class RenderFlag(Enum):

        Dummy                    : QTextItem.RenderFlag # -0x1
        RightToLeft              : QTextItem.RenderFlag # 0x1
        Overline                 : QTextItem.RenderFlag # 0x10
        Underline                : QTextItem.RenderFlag # 0x20
        StrikeOut                : QTextItem.RenderFlag # 0x40

    class RenderFlags(object): ...


    def __init__(self) -> None: ...

    def ascent(self) -> float: ...
    def descent(self) -> float: ...
    def font(self) -> QtGui.QFont: ...
    def renderFlags(self) -> QtGui.QTextItem.RenderFlags: ...
    def text(self) -> str: ...
    def width(self) -> float: ...


class QTextLayout(Shiboken.Object):

    SkipCharacters           : QTextLayout.CursorMode = ... # 0x0
    SkipWords                : QTextLayout.CursorMode = ... # 0x1

    class CursorMode(Enum):

        SkipCharacters           : QTextLayout.CursorMode # 0x0
        SkipWords                : QTextLayout.CursorMode # 0x1

    class FormatRange(Shiboken.Object):

        @overload
        def __init__(self) -> None: ...
        @overload
        def __init__(self, FormatRange:QtGui.QTextLayout.FormatRange) -> None: ...

        @staticmethod
        def __copy__() -> None: ...


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, b:QtGui.QTextBlock) -> None: ...
    @overload
    def __init__(self, text:str) -> None: ...
    @overload
    def __init__(self, text:str, font:Union[QtGui.QFont, str, Sequence[str]], paintdevice:Optional[QtGui.QPaintDevice]=...) -> None: ...

    def beginLayout(self) -> None: ...
    def boundingRect(self) -> QtCore.QRectF: ...
    def cacheEnabled(self) -> bool: ...
    def clearFormats(self) -> None: ...
    def clearLayout(self) -> None: ...
    def createLine(self) -> QtGui.QTextLine: ...
    def cursorMoveStyle(self) -> QtCore.Qt.CursorMoveStyle: ...
    def draw(self, p:QtGui.QPainter, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], selections:Sequence[QtGui.QTextLayout.FormatRange]=..., clip:Union[QtCore.QRectF, QtCore.QRect]=...) -> None: ...
    @overload
    def drawCursor(self, p:QtGui.QPainter, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], cursorPosition:int) -> None: ...
    @overload
    def drawCursor(self, p:QtGui.QPainter, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], cursorPosition:int, width:int) -> None: ...
    def endLayout(self) -> None: ...
    def font(self) -> QtGui.QFont: ...
    def formats(self) -> List[QtGui.QTextLayout.FormatRange]: ...
    def glyphRuns(self, from_:int=..., length:int=...) -> List[QtGui.QGlyphRun]: ...
    def isValidCursorPosition(self, pos:int) -> bool: ...
    def leftCursorPosition(self, oldPos:int) -> int: ...
    def lineAt(self, i:int) -> QtGui.QTextLine: ...
    def lineCount(self) -> int: ...
    def lineForTextPosition(self, pos:int) -> QtGui.QTextLine: ...
    def maximumWidth(self) -> float: ...
    def minimumWidth(self) -> float: ...
    def nextCursorPosition(self, oldPos:int, mode:QtGui.QTextLayout.CursorMode=...) -> int: ...
    def position(self) -> QtCore.QPointF: ...
    def preeditAreaPosition(self) -> int: ...
    def preeditAreaText(self) -> str: ...
    def previousCursorPosition(self, oldPos:int, mode:QtGui.QTextLayout.CursorMode=...) -> int: ...
    def rightCursorPosition(self, oldPos:int) -> int: ...
    def setCacheEnabled(self, enable:bool) -> None: ...
    def setCursorMoveStyle(self, style:QtCore.Qt.CursorMoveStyle) -> None: ...
    def setFlags(self, flags:int) -> None: ...
    def setFont(self, f:Union[QtGui.QFont, str, Sequence[str]]) -> None: ...
    def setFormats(self, overrides:Sequence[QtGui.QTextLayout.FormatRange]) -> None: ...
    def setPosition(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def setPreeditArea(self, position:int, text:str) -> None: ...
    def setRawFont(self, rawFont:QtGui.QRawFont) -> None: ...
    def setText(self, string:str) -> None: ...
    def setTextOption(self, option:Union[QtGui.QTextOption, QtCore.Qt.Alignment]) -> None: ...
    def text(self) -> str: ...
    def textOption(self) -> QtGui.QTextOption: ...


class QTextLength(Shiboken.Object):

    VariableLength           : QTextLength.Type = ... # 0x0
    FixedLength              : QTextLength.Type = ... # 0x1
    PercentageLength         : QTextLength.Type = ... # 0x2

    class Type(Enum):

        VariableLength           : QTextLength.Type # 0x0
        FixedLength              : QTextLength.Type # 0x1
        PercentageLength         : QTextLength.Type # 0x2


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QTextLength:QtGui.QTextLength) -> None: ...
    @overload
    def __init__(self, type:QtGui.QTextLength.Type, value:float) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def rawValue(self) -> float: ...
    def type(self) -> QtGui.QTextLength.Type: ...
    def value(self, maximumLength:float) -> float: ...


class QTextLine(Shiboken.Object):

    CursorBetweenCharacters  : QTextLine.CursorPosition = ... # 0x0
    CursorOnCharacter        : QTextLine.CursorPosition = ... # 0x1
    Leading                  : QTextLine.Edge = ... # 0x0
    Trailing                 : QTextLine.Edge = ... # 0x1

    class CursorPosition(Enum):

        CursorBetweenCharacters  : QTextLine.CursorPosition # 0x0
        CursorOnCharacter        : QTextLine.CursorPosition # 0x1

    class Edge(Enum):

        Leading                  : QTextLine.Edge # 0x0
        Trailing                 : QTextLine.Edge # 0x1


    def __init__(self) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def ascent(self) -> float: ...
    def cursorToX(self, cursorPos:int, edge:QtGui.QTextLine.Edge=...) -> object: ...
    def descent(self) -> float: ...
    def draw(self, painter:QtGui.QPainter, position:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def glyphRuns(self, from_:int=..., length:int=...) -> List[QtGui.QGlyphRun]: ...
    def height(self) -> float: ...
    def horizontalAdvance(self) -> float: ...
    def isValid(self) -> bool: ...
    def leading(self) -> float: ...
    def leadingIncluded(self) -> bool: ...
    def lineNumber(self) -> int: ...
    def naturalTextRect(self) -> QtCore.QRectF: ...
    def naturalTextWidth(self) -> float: ...
    def position(self) -> QtCore.QPointF: ...
    def rect(self) -> QtCore.QRectF: ...
    def setLeadingIncluded(self, included:bool) -> None: ...
    def setLineWidth(self, width:float) -> None: ...
    @overload
    def setNumColumns(self, columns:int) -> None: ...
    @overload
    def setNumColumns(self, columns:int, alignmentWidth:float) -> None: ...
    def setPosition(self, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    def textLength(self) -> int: ...
    def textStart(self) -> int: ...
    def width(self) -> float: ...
    def x(self) -> float: ...
    def xToCursor(self, x:float, edge:QtGui.QTextLine.CursorPosition=...) -> int: ...
    def y(self) -> float: ...


class QTextList(QtGui.QTextBlockGroup):

    def __init__(self, doc:QtGui.QTextDocument) -> None: ...

    def add(self, block:QtGui.QTextBlock) -> None: ...
    def count(self) -> int: ...
    def format(self) -> QtGui.QTextListFormat: ...
    def item(self, i:int) -> QtGui.QTextBlock: ...
    def itemNumber(self, arg__1:QtGui.QTextBlock) -> int: ...
    def itemText(self, arg__1:QtGui.QTextBlock) -> str: ...
    def remove(self, arg__1:QtGui.QTextBlock) -> None: ...
    def removeItem(self, i:int) -> None: ...
    def setFormat(self, format:QtGui.QTextListFormat) -> None: ...


class QTextListFormat(QtGui.QTextFormat):

    ListUpperRoman           : QTextListFormat.Style = ... # -0x8
    ListLowerRoman           : QTextListFormat.Style = ... # -0x7
    ListUpperAlpha           : QTextListFormat.Style = ... # -0x6
    ListLowerAlpha           : QTextListFormat.Style = ... # -0x5
    ListDecimal              : QTextListFormat.Style = ... # -0x4
    ListSquare               : QTextListFormat.Style = ... # -0x3
    ListCircle               : QTextListFormat.Style = ... # -0x2
    ListDisc                 : QTextListFormat.Style = ... # -0x1
    ListStyleUndefined       : QTextListFormat.Style = ... # 0x0

    class Style(Enum):

        ListUpperRoman           : QTextListFormat.Style # -0x8
        ListLowerRoman           : QTextListFormat.Style # -0x7
        ListUpperAlpha           : QTextListFormat.Style # -0x6
        ListLowerAlpha           : QTextListFormat.Style # -0x5
        ListDecimal              : QTextListFormat.Style # -0x4
        ListSquare               : QTextListFormat.Style # -0x3
        ListCircle               : QTextListFormat.Style # -0x2
        ListDisc                 : QTextListFormat.Style # -0x1
        ListStyleUndefined       : QTextListFormat.Style # 0x0


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QTextListFormat:QtGui.QTextListFormat) -> None: ...
    @overload
    def __init__(self, fmt:QtGui.QTextFormat) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def indent(self) -> int: ...
    def isValid(self) -> bool: ...
    def numberPrefix(self) -> str: ...
    def numberSuffix(self) -> str: ...
    def setIndent(self, indent:int) -> None: ...
    def setNumberPrefix(self, numberPrefix:str) -> None: ...
    def setNumberSuffix(self, numberSuffix:str) -> None: ...
    def setStyle(self, style:QtGui.QTextListFormat.Style) -> None: ...
    def style(self) -> QtGui.QTextListFormat.Style: ...


class QTextObject(QtCore.QObject):

    def __init__(self, doc:QtGui.QTextDocument) -> None: ...

    def document(self) -> QtGui.QTextDocument: ...
    def format(self) -> QtGui.QTextFormat: ...
    def formatIndex(self) -> int: ...
    def objectIndex(self) -> int: ...
    def setFormat(self, format:QtGui.QTextFormat) -> None: ...


class QTextObjectInterface(Shiboken.Object):

    def __init__(self) -> None: ...

    def drawObject(self, painter:QtGui.QPainter, rect:Union[QtCore.QRectF, QtCore.QRect], doc:QtGui.QTextDocument, posInDocument:int, format:QtGui.QTextFormat) -> None: ...
    def intrinsicSize(self, doc:QtGui.QTextDocument, posInDocument:int, format:QtGui.QTextFormat) -> QtCore.QSizeF: ...


class QTextOption(Shiboken.Object):

    IncludeTrailingSpaces    : QTextOption.Flag = ... # -0x80000000
    ShowTabsAndSpaces        : QTextOption.Flag = ... # 0x1
    ShowLineAndParagraphSeparators: QTextOption.Flag = ... # 0x2
    AddSpaceForLineAndParagraphSeparators: QTextOption.Flag = ... # 0x4
    SuppressColors           : QTextOption.Flag = ... # 0x8
    ShowDocumentTerminator   : QTextOption.Flag = ... # 0x10
    LeftTab                  : QTextOption.TabType = ... # 0x0
    RightTab                 : QTextOption.TabType = ... # 0x1
    CenterTab                : QTextOption.TabType = ... # 0x2
    DelimiterTab             : QTextOption.TabType = ... # 0x3
    NoWrap                   : QTextOption.WrapMode = ... # 0x0
    WordWrap                 : QTextOption.WrapMode = ... # 0x1
    ManualWrap               : QTextOption.WrapMode = ... # 0x2
    WrapAnywhere             : QTextOption.WrapMode = ... # 0x3
    WrapAtWordBoundaryOrAnywhere: QTextOption.WrapMode = ... # 0x4

    class Flag(Enum):

        IncludeTrailingSpaces    : QTextOption.Flag # -0x80000000
        ShowTabsAndSpaces        : QTextOption.Flag # 0x1
        ShowLineAndParagraphSeparators: QTextOption.Flag # 0x2
        AddSpaceForLineAndParagraphSeparators: QTextOption.Flag # 0x4
        SuppressColors           : QTextOption.Flag # 0x8
        ShowDocumentTerminator   : QTextOption.Flag # 0x10

    class Flags(object): ...

    class Tab(Shiboken.Object):

        @overload
        def __init__(self) -> None: ...
        @overload
        def __init__(self, Tab:QtGui.QTextOption.Tab) -> None: ...
        @overload
        def __init__(self, pos:float, tabType:QtGui.QTextOption.TabType, delim:str=...) -> None: ...

        @staticmethod
        def __copy__() -> None: ...

    class TabType(Enum):

        LeftTab                  : QTextOption.TabType # 0x0
        RightTab                 : QTextOption.TabType # 0x1
        CenterTab                : QTextOption.TabType # 0x2
        DelimiterTab             : QTextOption.TabType # 0x3

    class WrapMode(Enum):

        NoWrap                   : QTextOption.WrapMode # 0x0
        WordWrap                 : QTextOption.WrapMode # 0x1
        ManualWrap               : QTextOption.WrapMode # 0x2
        WrapAnywhere             : QTextOption.WrapMode # 0x3
        WrapAtWordBoundaryOrAnywhere: QTextOption.WrapMode # 0x4


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, alignment:QtCore.Qt.Alignment) -> None: ...
    @overload
    def __init__(self, o:Union[QtGui.QTextOption, QtCore.Qt.Alignment]) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def alignment(self) -> QtCore.Qt.Alignment: ...
    def flags(self) -> QtGui.QTextOption.Flags: ...
    def setAlignment(self, alignment:QtCore.Qt.Alignment) -> None: ...
    def setFlags(self, flags:QtGui.QTextOption.Flags) -> None: ...
    def setTabArray(self, tabStops:Sequence[float]) -> None: ...
    def setTabStopDistance(self, tabStopDistance:float) -> None: ...
    def setTabs(self, tabStops:Sequence[QtGui.QTextOption.Tab]) -> None: ...
    def setTextDirection(self, aDirection:QtCore.Qt.LayoutDirection) -> None: ...
    def setUseDesignMetrics(self, b:bool) -> None: ...
    def setWrapMode(self, wrap:QtGui.QTextOption.WrapMode) -> None: ...
    def tabArray(self) -> List[float]: ...
    def tabStopDistance(self) -> float: ...
    def tabs(self) -> List[QtGui.QTextOption.Tab]: ...
    def textDirection(self) -> QtCore.Qt.LayoutDirection: ...
    def useDesignMetrics(self) -> bool: ...
    def wrapMode(self) -> QtGui.QTextOption.WrapMode: ...


class QTextTable(QtGui.QTextFrame):

    def __init__(self, doc:QtGui.QTextDocument) -> None: ...

    def appendColumns(self, count:int) -> None: ...
    def appendRows(self, count:int) -> None: ...
    @overload
    def cellAt(self, c:QtGui.QTextCursor) -> QtGui.QTextTableCell: ...
    @overload
    def cellAt(self, position:int) -> QtGui.QTextTableCell: ...
    @overload
    def cellAt(self, row:int, col:int) -> QtGui.QTextTableCell: ...
    def columns(self) -> int: ...
    def format(self) -> QtGui.QTextTableFormat: ...
    def insertColumns(self, pos:int, num:int) -> None: ...
    def insertRows(self, pos:int, num:int) -> None: ...
    @overload
    def mergeCells(self, cursor:QtGui.QTextCursor) -> None: ...
    @overload
    def mergeCells(self, row:int, col:int, numRows:int, numCols:int) -> None: ...
    def removeColumns(self, pos:int, num:int) -> None: ...
    def removeRows(self, pos:int, num:int) -> None: ...
    def resize(self, rows:int, cols:int) -> None: ...
    def rowEnd(self, c:QtGui.QTextCursor) -> QtGui.QTextCursor: ...
    def rowStart(self, c:QtGui.QTextCursor) -> QtGui.QTextCursor: ...
    def rows(self) -> int: ...
    def setFormat(self, format:QtGui.QTextTableFormat) -> None: ...
    def splitCell(self, row:int, col:int, numRows:int, numCols:int) -> None: ...


class QTextTableCell(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, o:QtGui.QTextTableCell) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def begin(self) -> QtGui.QTextFrame.iterator: ...
    def column(self) -> int: ...
    def columnSpan(self) -> int: ...
    def end(self) -> QtGui.QTextFrame.iterator: ...
    def firstCursorPosition(self) -> QtGui.QTextCursor: ...
    def firstPosition(self) -> int: ...
    def format(self) -> QtGui.QTextCharFormat: ...
    def isValid(self) -> bool: ...
    def lastCursorPosition(self) -> QtGui.QTextCursor: ...
    def lastPosition(self) -> int: ...
    def row(self) -> int: ...
    def rowSpan(self) -> int: ...
    def setFormat(self, format:QtGui.QTextCharFormat) -> None: ...
    def tableCellFormatIndex(self) -> int: ...


class QTextTableCellFormat(QtGui.QTextCharFormat):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QTextTableCellFormat:QtGui.QTextTableCellFormat) -> None: ...
    @overload
    def __init__(self, fmt:QtGui.QTextFormat) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def bottomBorder(self) -> float: ...
    def bottomBorderBrush(self) -> QtGui.QBrush: ...
    def bottomBorderStyle(self) -> QtGui.QTextFrameFormat.BorderStyle: ...
    def bottomPadding(self) -> float: ...
    def isValid(self) -> bool: ...
    def leftBorder(self) -> float: ...
    def leftBorderBrush(self) -> QtGui.QBrush: ...
    def leftBorderStyle(self) -> QtGui.QTextFrameFormat.BorderStyle: ...
    def leftPadding(self) -> float: ...
    def rightBorder(self) -> float: ...
    def rightBorderBrush(self) -> QtGui.QBrush: ...
    def rightBorderStyle(self) -> QtGui.QTextFrameFormat.BorderStyle: ...
    def rightPadding(self) -> float: ...
    def setBorder(self, width:float) -> None: ...
    def setBorderBrush(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setBorderStyle(self, style:QtGui.QTextFrameFormat.BorderStyle) -> None: ...
    def setBottomBorder(self, width:float) -> None: ...
    def setBottomBorderBrush(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setBottomBorderStyle(self, style:QtGui.QTextFrameFormat.BorderStyle) -> None: ...
    def setBottomPadding(self, padding:float) -> None: ...
    def setLeftBorder(self, width:float) -> None: ...
    def setLeftBorderBrush(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setLeftBorderStyle(self, style:QtGui.QTextFrameFormat.BorderStyle) -> None: ...
    def setLeftPadding(self, padding:float) -> None: ...
    def setPadding(self, padding:float) -> None: ...
    def setRightBorder(self, width:float) -> None: ...
    def setRightBorderBrush(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setRightBorderStyle(self, style:QtGui.QTextFrameFormat.BorderStyle) -> None: ...
    def setRightPadding(self, padding:float) -> None: ...
    def setTopBorder(self, width:float) -> None: ...
    def setTopBorderBrush(self, brush:Union[QtGui.QBrush, QtCore.Qt.BrushStyle, QtCore.Qt.GlobalColor, QtGui.QColor, QtGui.QGradient, QtGui.QImage, QtGui.QPixmap]) -> None: ...
    def setTopBorderStyle(self, style:QtGui.QTextFrameFormat.BorderStyle) -> None: ...
    def setTopPadding(self, padding:float) -> None: ...
    def topBorder(self) -> float: ...
    def topBorderBrush(self) -> QtGui.QBrush: ...
    def topBorderStyle(self) -> QtGui.QTextFrameFormat.BorderStyle: ...
    def topPadding(self) -> float: ...


class QTextTableFormat(QtGui.QTextFrameFormat):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, QTextTableFormat:QtGui.QTextTableFormat) -> None: ...
    @overload
    def __init__(self, fmt:QtGui.QTextFormat) -> None: ...

    @staticmethod
    def __copy__() -> None: ...
    def alignment(self) -> QtCore.Qt.Alignment: ...
    def borderCollapse(self) -> bool: ...
    def cellPadding(self) -> float: ...
    def cellSpacing(self) -> float: ...
    def clearColumnWidthConstraints(self) -> None: ...
    def columnWidthConstraints(self) -> List[QtGui.QTextLength]: ...
    def columns(self) -> int: ...
    def headerRowCount(self) -> int: ...
    def isValid(self) -> bool: ...
    def setAlignment(self, alignment:QtCore.Qt.Alignment) -> None: ...
    def setBorderCollapse(self, borderCollapse:bool) -> None: ...
    def setCellPadding(self, padding:float) -> None: ...
    def setCellSpacing(self, spacing:float) -> None: ...
    def setColumnWidthConstraints(self, constraints:Sequence[QtGui.QTextLength]) -> None: ...
    def setColumns(self, columns:int) -> None: ...
    def setHeaderRowCount(self, count:int) -> None: ...


class QToolBarChangeEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QToolBarChangeEvent) -> None: ...
    @overload
    def __init__(self, t:bool) -> None: ...

    def clone(self) -> QtGui.QToolBarChangeEvent: ...
    def toggle(self) -> bool: ...


class QTouchEvent(QtGui.QPointerEvent):

    @overload
    def __init__(self, arg__1:QtGui.QTouchEvent) -> None: ...
    @overload
    def __init__(self, eventType:QtCore.QEvent.Type, device:Optional[QtGui.QPointingDevice]=..., modifiers:QtCore.Qt.KeyboardModifiers=..., touchPoints:Sequence[QtGui.QEventPoint]=...) -> None: ...

    def clone(self) -> QtGui.QTouchEvent: ...
    def isBeginEvent(self) -> bool: ...
    def isEndEvent(self) -> bool: ...
    def isUpdateEvent(self) -> bool: ...
    def target(self) -> QtCore.QObject: ...
    def touchPoints(self) -> List[QtGui.QEventPoint]: ...


class QTransform(Shiboken.Object):

    TxNone                   : QTransform.TransformationType = ... # 0x0
    TxTranslate              : QTransform.TransformationType = ... # 0x1
    TxScale                  : QTransform.TransformationType = ... # 0x2
    TxRotate                 : QTransform.TransformationType = ... # 0x4
    TxShear                  : QTransform.TransformationType = ... # 0x8
    TxProject                : QTransform.TransformationType = ... # 0x10

    class TransformationType(Enum):

        TxNone                   : QTransform.TransformationType # 0x0
        TxTranslate              : QTransform.TransformationType # 0x1
        TxScale                  : QTransform.TransformationType # 0x2
        TxRotate                 : QTransform.TransformationType # 0x4
        TxShear                  : QTransform.TransformationType # 0x8
        TxProject                : QTransform.TransformationType # 0x10


    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, h11:float, h12:float, h13:float, h21:float, h22:float, h23:float, h31:float, h32:float, h33:float) -> None: ...
    @overload
    def __init__(self, h11:float, h12:float, h21:float, h22:float, dx:float, dy:float) -> None: ...
    @overload
    def __init__(self, other:QtGui.QTransform) -> None: ...

    def __add__(self, n:float) -> QtGui.QTransform: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, div:float) -> QtGui.QTransform: ...
    @overload
    def __imul__(self, arg__1:QtGui.QTransform) -> QtGui.QTransform: ...
    @overload
    def __imul__(self, div:float) -> QtGui.QTransform: ...
    def __isub__(self, div:float) -> QtGui.QTransform: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    @overload
    def __mul__(self, l:QtCore.QLine) -> QtCore.QLine: ...
    @overload
    def __mul__(self, l:Union[QtCore.QLineF, QtCore.QLine]) -> QtCore.QLineF: ...
    @overload
    def __mul__(self, n:float) -> QtGui.QTransform: ...
    @overload
    def __mul__(self, o:QtGui.QTransform) -> QtGui.QTransform: ...
    @overload
    def __mul__(self, p:QtCore.QPoint) -> QtCore.QPoint: ...
    @overload
    def __mul__(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> QtCore.QPointF: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __sub__(self, n:float) -> QtGui.QTransform: ...
    def adjoint(self) -> QtGui.QTransform: ...
    def determinant(self) -> float: ...
    def dx(self) -> float: ...
    def dy(self) -> float: ...
    @staticmethod
    def fromScale(dx:float, dy:float) -> QtGui.QTransform: ...
    @staticmethod
    def fromTranslate(dx:float, dy:float) -> QtGui.QTransform: ...
    def inverted(self) -> Tuple[Tuple, bool]: ...
    def isAffine(self) -> bool: ...
    def isIdentity(self) -> bool: ...
    def isInvertible(self) -> bool: ...
    def isRotating(self) -> bool: ...
    def isScaling(self) -> bool: ...
    def isTranslating(self) -> bool: ...
    def m11(self) -> float: ...
    def m12(self) -> float: ...
    def m13(self) -> float: ...
    def m21(self) -> float: ...
    def m22(self) -> float: ...
    def m23(self) -> float: ...
    def m31(self) -> float: ...
    def m32(self) -> float: ...
    def m33(self) -> float: ...
    @overload
    def map(self, a:Union[QtGui.QPolygon, Sequence[QtCore.QPoint], QtCore.QRect]) -> QtGui.QPolygon: ...
    @overload
    def map(self, a:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> QtGui.QPolygonF: ...
    @overload
    def map(self, l:QtCore.QLine) -> QtCore.QLine: ...
    @overload
    def map(self, l:Union[QtCore.QLineF, QtCore.QLine]) -> QtCore.QLineF: ...
    @overload
    def map(self, p:QtCore.QPoint) -> QtCore.QPoint: ...
    @overload
    def map(self, p:QtGui.QPainterPath) -> QtGui.QPainterPath: ...
    @overload
    def map(self, p:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> QtCore.QPointF: ...
    @overload
    def map(self, r:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> QtGui.QRegion: ...
    @overload
    def map(self, x:float, y:float) -> Tuple[float, float]: ...
    @overload
    def mapRect(self, arg__1:QtCore.QRect) -> QtCore.QRect: ...
    @overload
    def mapRect(self, arg__1:Union[QtCore.QRectF, QtCore.QRect]) -> QtCore.QRectF: ...
    def mapToPolygon(self, r:QtCore.QRect) -> QtGui.QPolygon: ...
    @overload
    @staticmethod
    def quadToQuad(arg__1:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF], arg__2:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> object: ...
    @overload
    @staticmethod
    def quadToQuad(one:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF], two:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF], result:QtGui.QTransform) -> bool: ...
    @overload
    @staticmethod
    def quadToSquare(arg__1:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> object: ...
    @overload
    @staticmethod
    def quadToSquare(quad:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF], result:QtGui.QTransform) -> bool: ...
    def reset(self) -> None: ...
    def rotate(self, a:float, axis:QtCore.Qt.Axis=...) -> QtGui.QTransform: ...
    def rotateRadians(self, a:float, axis:QtCore.Qt.Axis=...) -> QtGui.QTransform: ...
    def scale(self, sx:float, sy:float) -> QtGui.QTransform: ...
    def setMatrix(self, m11:float, m12:float, m13:float, m21:float, m22:float, m23:float, m31:float, m32:float, m33:float) -> None: ...
    def shear(self, sh:float, sv:float) -> QtGui.QTransform: ...
    @overload
    @staticmethod
    def squareToQuad(arg__1:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF]) -> object: ...
    @overload
    @staticmethod
    def squareToQuad(square:Union[QtGui.QPolygonF, Sequence[QtCore.QPointF], QtGui.QPolygon, QtCore.QRectF], result:QtGui.QTransform) -> bool: ...
    def translate(self, dx:float, dy:float) -> QtGui.QTransform: ...
    def transposed(self) -> QtGui.QTransform: ...
    def type(self) -> QtGui.QTransform.TransformationType: ...


class QUndoCommand(Shiboken.Object):

    @overload
    def __init__(self, parent:Optional[QtGui.QUndoCommand]=...) -> None: ...
    @overload
    def __init__(self, text:str, parent:Optional[QtGui.QUndoCommand]=...) -> None: ...

    def actionText(self) -> str: ...
    def child(self, index:int) -> QtGui.QUndoCommand: ...
    def childCount(self) -> int: ...
    def id(self) -> int: ...
    def isObsolete(self) -> bool: ...
    def mergeWith(self, other:QtGui.QUndoCommand) -> bool: ...
    def redo(self) -> None: ...
    def setObsolete(self, obsolete:bool) -> None: ...
    def setText(self, text:str) -> None: ...
    def text(self) -> str: ...
    def undo(self) -> None: ...


class QUndoGroup(QtCore.QObject):

    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...

    def activeStack(self) -> QtGui.QUndoStack: ...
    def addStack(self, stack:QtGui.QUndoStack) -> None: ...
    def canRedo(self) -> bool: ...
    def canUndo(self) -> bool: ...
    def createRedoAction(self, parent:QtCore.QObject, prefix:str=...) -> QtGui.QAction: ...
    def createUndoAction(self, parent:QtCore.QObject, prefix:str=...) -> QtGui.QAction: ...
    def isClean(self) -> bool: ...
    def redo(self) -> None: ...
    def redoText(self) -> str: ...
    def removeStack(self, stack:QtGui.QUndoStack) -> None: ...
    def setActiveStack(self, stack:QtGui.QUndoStack) -> None: ...
    def stacks(self) -> List[QtGui.QUndoStack]: ...
    def undo(self) -> None: ...
    def undoText(self) -> str: ...


class QUndoStack(QtCore.QObject):

    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...

    def beginMacro(self, text:str) -> None: ...
    def canRedo(self) -> bool: ...
    def canUndo(self) -> bool: ...
    def cleanIndex(self) -> int: ...
    def clear(self) -> None: ...
    def command(self, index:int) -> QtGui.QUndoCommand: ...
    def count(self) -> int: ...
    def createRedoAction(self, parent:QtCore.QObject, prefix:str=...) -> QtGui.QAction: ...
    def createUndoAction(self, parent:QtCore.QObject, prefix:str=...) -> QtGui.QAction: ...
    def endMacro(self) -> None: ...
    def index(self) -> int: ...
    def isActive(self) -> bool: ...
    def isClean(self) -> bool: ...
    def push(self, cmd:QtGui.QUndoCommand) -> None: ...
    def redo(self) -> None: ...
    def redoText(self) -> str: ...
    def resetClean(self) -> None: ...
    def setActive(self, active:bool=...) -> None: ...
    def setClean(self) -> None: ...
    def setIndex(self, idx:int) -> None: ...
    def setUndoLimit(self, limit:int) -> None: ...
    def text(self, idx:int) -> str: ...
    def undo(self) -> None: ...
    def undoLimit(self) -> int: ...
    def undoText(self) -> str: ...


class QValidator(QtCore.QObject):

    Invalid                  : QValidator.State = ... # 0x0
    Intermediate             : QValidator.State = ... # 0x1
    Acceptable               : QValidator.State = ... # 0x2

    class State(Enum):

        Invalid                  : QValidator.State # 0x0
        Intermediate             : QValidator.State # 0x1
        Acceptable               : QValidator.State # 0x2


    def __init__(self, parent:Optional[QtCore.QObject]=...) -> None: ...

    def fixup(self, arg__1:str) -> None: ...
    def locale(self) -> QtCore.QLocale: ...
    def setLocale(self, locale:Union[QtCore.QLocale, QtCore.QLocale.Language]) -> None: ...
    def validate(self, arg__1:str, arg__2:int) -> object: ...


class QVector2D(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, point:QtCore.QPoint) -> None: ...
    @overload
    def __init__(self, point:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def __init__(self, vector:QtGui.QVector3D) -> None: ...
    @overload
    def __init__(self, vector:QtGui.QVector4D) -> None: ...
    @overload
    def __init__(self, xpos:float, ypos:float) -> None: ...

    def __add__(self, v2:QtGui.QVector2D) -> QtGui.QVector2D: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, vector:QtGui.QVector2D) -> QtGui.QVector2D: ...
    @overload
    def __imul__(self, factor:float) -> QtGui.QVector2D: ...
    @overload
    def __imul__(self, vector:QtGui.QVector2D) -> QtGui.QVector2D: ...
    def __isub__(self, vector:QtGui.QVector2D) -> QtGui.QVector2D: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    @overload
    def __mul__(self, factor:float) -> QtGui.QVector2D: ...
    @overload
    def __mul__(self, v2:QtGui.QVector2D) -> QtGui.QVector2D: ...
    def __neg__(self) -> QtGui.QVector2D: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __sub__(self, v2:QtGui.QVector2D) -> QtGui.QVector2D: ...
    def distanceToLine(self, point:QtGui.QVector2D, direction:QtGui.QVector2D) -> float: ...
    def distanceToPoint(self, point:QtGui.QVector2D) -> float: ...
    @staticmethod
    def dotProduct(v1:QtGui.QVector2D, v2:QtGui.QVector2D) -> float: ...
    def isNull(self) -> bool: ...
    def length(self) -> float: ...
    def lengthSquared(self) -> float: ...
    def normalize(self) -> None: ...
    def normalized(self) -> QtGui.QVector2D: ...
    def setX(self, x:float) -> None: ...
    def setY(self, y:float) -> None: ...
    def toPoint(self) -> QtCore.QPoint: ...
    def toPointF(self) -> QtCore.QPointF: ...
    def toTuple(self) -> object: ...
    def toVector3D(self) -> QtGui.QVector3D: ...
    def toVector4D(self) -> QtGui.QVector4D: ...
    def x(self) -> float: ...
    def y(self) -> float: ...


class QVector3D(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, point:QtCore.QPoint) -> None: ...
    @overload
    def __init__(self, point:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def __init__(self, vector:QtGui.QVector2D) -> None: ...
    @overload
    def __init__(self, vector:QtGui.QVector2D, zpos:float) -> None: ...
    @overload
    def __init__(self, vector:QtGui.QVector4D) -> None: ...
    @overload
    def __init__(self, xpos:float, ypos:float, zpos:float) -> None: ...

    def __add__(self, v2:QtGui.QVector3D) -> QtGui.QVector3D: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, vector:QtGui.QVector3D) -> QtGui.QVector3D: ...
    @overload
    def __imul__(self, factor:float) -> QtGui.QVector3D: ...
    @overload
    def __imul__(self, vector:QtGui.QVector3D) -> QtGui.QVector3D: ...
    def __isub__(self, vector:QtGui.QVector3D) -> QtGui.QVector3D: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    @overload
    def __mul__(self, factor:float) -> QtGui.QVector3D: ...
    @overload
    def __mul__(self, matrix:Union[QtGui.QMatrix4x4, QtGui.QTransform]) -> QtGui.QVector3D: ...
    @overload
    def __mul__(self, quaternion:QtGui.QQuaternion) -> QtGui.QVector3D: ...
    @overload
    def __mul__(self, v2:QtGui.QVector3D) -> QtGui.QVector3D: ...
    def __neg__(self) -> QtGui.QVector3D: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __sub__(self, v2:QtGui.QVector3D) -> QtGui.QVector3D: ...
    @staticmethod
    def crossProduct(v1:QtGui.QVector3D, v2:QtGui.QVector3D) -> QtGui.QVector3D: ...
    def distanceToLine(self, point:QtGui.QVector3D, direction:QtGui.QVector3D) -> float: ...
    @overload
    def distanceToPlane(self, plane1:QtGui.QVector3D, plane2:QtGui.QVector3D, plane3:QtGui.QVector3D) -> float: ...
    @overload
    def distanceToPlane(self, plane:QtGui.QVector3D, normal:QtGui.QVector3D) -> float: ...
    def distanceToPoint(self, point:QtGui.QVector3D) -> float: ...
    @staticmethod
    def dotProduct(v1:QtGui.QVector3D, v2:QtGui.QVector3D) -> float: ...
    def isNull(self) -> bool: ...
    def length(self) -> float: ...
    def lengthSquared(self) -> float: ...
    @overload
    @staticmethod
    def normal(v1:QtGui.QVector3D, v2:QtGui.QVector3D) -> QtGui.QVector3D: ...
    @overload
    @staticmethod
    def normal(v1:QtGui.QVector3D, v2:QtGui.QVector3D, v3:QtGui.QVector3D) -> QtGui.QVector3D: ...
    def normalize(self) -> None: ...
    def normalized(self) -> QtGui.QVector3D: ...
    def project(self, modelView:Union[QtGui.QMatrix4x4, QtGui.QTransform], projection:Union[QtGui.QMatrix4x4, QtGui.QTransform], viewport:QtCore.QRect) -> QtGui.QVector3D: ...
    def setX(self, x:float) -> None: ...
    def setY(self, y:float) -> None: ...
    def setZ(self, z:float) -> None: ...
    def toPoint(self) -> QtCore.QPoint: ...
    def toPointF(self) -> QtCore.QPointF: ...
    def toTuple(self) -> object: ...
    def toVector2D(self) -> QtGui.QVector2D: ...
    def toVector4D(self) -> QtGui.QVector4D: ...
    def unproject(self, modelView:Union[QtGui.QMatrix4x4, QtGui.QTransform], projection:Union[QtGui.QMatrix4x4, QtGui.QTransform], viewport:QtCore.QRect) -> QtGui.QVector3D: ...
    def x(self) -> float: ...
    def y(self) -> float: ...
    def z(self) -> float: ...


class QVector4D(Shiboken.Object):

    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, point:QtCore.QPoint) -> None: ...
    @overload
    def __init__(self, point:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> None: ...
    @overload
    def __init__(self, vector:QtGui.QVector2D) -> None: ...
    @overload
    def __init__(self, vector:QtGui.QVector2D, zpos:float, wpos:float) -> None: ...
    @overload
    def __init__(self, vector:QtGui.QVector3D) -> None: ...
    @overload
    def __init__(self, vector:QtGui.QVector3D, wpos:float) -> None: ...
    @overload
    def __init__(self, xpos:float, ypos:float, zpos:float, wpos:float) -> None: ...

    def __add__(self, v2:QtGui.QVector4D) -> QtGui.QVector4D: ...
    @staticmethod
    def __copy__() -> None: ...
    def __iadd__(self, vector:QtGui.QVector4D) -> QtGui.QVector4D: ...
    @overload
    def __imul__(self, factor:float) -> QtGui.QVector4D: ...
    @overload
    def __imul__(self, vector:QtGui.QVector4D) -> QtGui.QVector4D: ...
    def __isub__(self, vector:QtGui.QVector4D) -> QtGui.QVector4D: ...
    def __lshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    @overload
    def __mul__(self, factor:float) -> QtGui.QVector4D: ...
    @overload
    def __mul__(self, matrix:Union[QtGui.QMatrix4x4, QtGui.QTransform]) -> QtGui.QVector4D: ...
    @overload
    def __mul__(self, v2:QtGui.QVector4D) -> QtGui.QVector4D: ...
    def __neg__(self) -> QtGui.QVector4D: ...
    def __reduce__(self) -> object: ...
    def __repr__(self) -> object: ...
    def __rshift__(self, arg__1:QtCore.QDataStream) -> QtCore.QDataStream: ...
    def __sub__(self, v2:QtGui.QVector4D) -> QtGui.QVector4D: ...
    @staticmethod
    def dotProduct(v1:QtGui.QVector4D, v2:QtGui.QVector4D) -> float: ...
    def isNull(self) -> bool: ...
    def length(self) -> float: ...
    def lengthSquared(self) -> float: ...
    def normalize(self) -> None: ...
    def normalized(self) -> QtGui.QVector4D: ...
    def setW(self, w:float) -> None: ...
    def setX(self, x:float) -> None: ...
    def setY(self, y:float) -> None: ...
    def setZ(self, z:float) -> None: ...
    def toPoint(self) -> QtCore.QPoint: ...
    def toPointF(self) -> QtCore.QPointF: ...
    def toTuple(self) -> object: ...
    def toVector2D(self) -> QtGui.QVector2D: ...
    def toVector2DAffine(self) -> QtGui.QVector2D: ...
    def toVector3D(self) -> QtGui.QVector3D: ...
    def toVector3DAffine(self) -> QtGui.QVector3D: ...
    def w(self) -> float: ...
    def x(self) -> float: ...
    def y(self) -> float: ...
    def z(self) -> float: ...


class QWhatsThisClickedEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QWhatsThisClickedEvent) -> None: ...
    @overload
    def __init__(self, href:str) -> None: ...

    def clone(self) -> QtGui.QWhatsThisClickedEvent: ...
    def href(self) -> str: ...


class QWheelEvent(QtGui.QSinglePointEvent):

    @overload
    def __init__(self, arg__1:QtGui.QWheelEvent) -> None: ...
    @overload
    def __init__(self, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], globalPos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element], pixelDelta:QtCore.QPoint, angleDelta:QtCore.QPoint, buttons:QtCore.Qt.MouseButtons, modifiers:QtCore.Qt.KeyboardModifiers, phase:QtCore.Qt.ScrollPhase, inverted:bool, source:QtCore.Qt.MouseEventSource=..., device:QtGui.QPointingDevice=...) -> None: ...

    def angleDelta(self) -> QtCore.QPoint: ...
    def clone(self) -> QtGui.QWheelEvent: ...
    def hasPixelDelta(self) -> bool: ...
    def inverted(self) -> bool: ...
    def isBeginEvent(self) -> bool: ...
    def isEndEvent(self) -> bool: ...
    def isInverted(self) -> bool: ...
    def isUpdateEvent(self) -> bool: ...
    def phase(self) -> QtCore.Qt.ScrollPhase: ...
    def pixelDelta(self) -> QtCore.QPoint: ...
    def source(self) -> QtCore.Qt.MouseEventSource: ...


class QWindow(QtCore.QObject, QtGui.QSurface):

    ExcludeTransients        : QWindow.AncestorMode = ... # 0x0
    IncludeTransients        : QWindow.AncestorMode = ... # 0x1
    Hidden                   : QWindow.Visibility = ... # 0x0
    AutomaticVisibility      : QWindow.Visibility = ... # 0x1
    Windowed                 : QWindow.Visibility = ... # 0x2
    Minimized                : QWindow.Visibility = ... # 0x3
    Maximized                : QWindow.Visibility = ... # 0x4
    FullScreen               : QWindow.Visibility = ... # 0x5

    class AncestorMode(Enum):

        ExcludeTransients        : QWindow.AncestorMode # 0x0
        IncludeTransients        : QWindow.AncestorMode # 0x1

    class Visibility(Enum):

        Hidden                   : QWindow.Visibility # 0x0
        AutomaticVisibility      : QWindow.Visibility # 0x1
        Windowed                 : QWindow.Visibility # 0x2
        Minimized                : QWindow.Visibility # 0x3
        Maximized                : QWindow.Visibility # 0x4
        FullScreen               : QWindow.Visibility # 0x5


    @overload
    def __init__(self, parent:QtGui.QWindow) -> None: ...
    @overload
    def __init__(self, screen:Optional[QtGui.QScreen]=...) -> None: ...

    def accessibleRoot(self) -> QtGui.QAccessibleInterface: ...
    def alert(self, msec:int) -> None: ...
    def baseSize(self) -> QtCore.QSize: ...
    def close(self) -> bool: ...
    def closeEvent(self, arg__1:QtGui.QCloseEvent) -> None: ...
    def contentOrientation(self) -> QtCore.Qt.ScreenOrientation: ...
    def create(self) -> None: ...
    def cursor(self) -> QtGui.QCursor: ...
    def destroy(self) -> None: ...
    def devicePixelRatio(self) -> float: ...
    def event(self, arg__1:QtCore.QEvent) -> bool: ...
    def exposeEvent(self, arg__1:QtGui.QExposeEvent) -> None: ...
    def filePath(self) -> str: ...
    def flags(self) -> QtCore.Qt.WindowFlags: ...
    def focusInEvent(self, arg__1:QtGui.QFocusEvent) -> None: ...
    def focusObject(self) -> QtCore.QObject: ...
    def focusOutEvent(self, arg__1:QtGui.QFocusEvent) -> None: ...
    def format(self) -> QtGui.QSurfaceFormat: ...
    def frameGeometry(self) -> QtCore.QRect: ...
    def frameMargins(self) -> QtCore.QMargins: ...
    def framePosition(self) -> QtCore.QPoint: ...
    @staticmethod
    def fromWinId(id:int) -> QtGui.QWindow: ...
    def geometry(self) -> QtCore.QRect: ...
    def height(self) -> int: ...
    def hide(self) -> None: ...
    def hideEvent(self, arg__1:QtGui.QHideEvent) -> None: ...
    def icon(self) -> QtGui.QIcon: ...
    def isActive(self) -> bool: ...
    def isAncestorOf(self, child:QtGui.QWindow, mode:QtGui.QWindow.AncestorMode=...) -> bool: ...
    def isExposed(self) -> bool: ...
    def isModal(self) -> bool: ...
    def isTopLevel(self) -> bool: ...
    def isVisible(self) -> bool: ...
    def keyPressEvent(self, arg__1:QtGui.QKeyEvent) -> None: ...
    def keyReleaseEvent(self, arg__1:QtGui.QKeyEvent) -> None: ...
    def lower(self) -> None: ...
    @overload
    def mapFromGlobal(self, pos:QtCore.QPoint) -> QtCore.QPoint: ...
    @overload
    def mapFromGlobal(self, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> QtCore.QPointF: ...
    @overload
    def mapToGlobal(self, pos:QtCore.QPoint) -> QtCore.QPoint: ...
    @overload
    def mapToGlobal(self, pos:Union[QtCore.QPointF, QtCore.QPoint, QtGui.QPainterPath.Element]) -> QtCore.QPointF: ...
    def mask(self) -> QtGui.QRegion: ...
    def maximumHeight(self) -> int: ...
    def maximumSize(self) -> QtCore.QSize: ...
    def maximumWidth(self) -> int: ...
    def minimumHeight(self) -> int: ...
    def minimumSize(self) -> QtCore.QSize: ...
    def minimumWidth(self) -> int: ...
    def modality(self) -> QtCore.Qt.WindowModality: ...
    def mouseDoubleClickEvent(self, arg__1:QtGui.QMouseEvent) -> None: ...
    def mouseMoveEvent(self, arg__1:QtGui.QMouseEvent) -> None: ...
    def mousePressEvent(self, arg__1:QtGui.QMouseEvent) -> None: ...
    def mouseReleaseEvent(self, arg__1:QtGui.QMouseEvent) -> None: ...
    def moveEvent(self, arg__1:QtGui.QMoveEvent) -> None: ...
    def nativeEvent(self, eventType:Union[QtCore.QByteArray, bytes], message:int) -> Tuple[object, int]: ...
    def opacity(self) -> float: ...
    def paintEvent(self, arg__1:QtGui.QPaintEvent) -> None: ...
    def parent(self, mode:QtGui.QWindow.AncestorMode=...) -> QtGui.QWindow: ...
    def position(self) -> QtCore.QPoint: ...
    def raise_(self) -> None: ...
    def reportContentOrientationChange(self, orientation:QtCore.Qt.ScreenOrientation) -> None: ...
    def requestActivate(self) -> None: ...
    def requestUpdate(self) -> None: ...
    def requestedFormat(self) -> QtGui.QSurfaceFormat: ...
    @overload
    def resize(self, newSize:QtCore.QSize) -> None: ...
    @overload
    def resize(self, w:int, h:int) -> None: ...
    def resizeEvent(self, arg__1:QtGui.QResizeEvent) -> None: ...
    def resolveInterface(self, name:bytes, revision:int) -> int: ...
    def screen(self) -> QtGui.QScreen: ...
    def setBaseSize(self, size:QtCore.QSize) -> None: ...
    def setCursor(self, arg__1:Union[QtGui.QCursor, QtCore.Qt.CursorShape, QtGui.QPixmap]) -> None: ...
    def setFilePath(self, filePath:str) -> None: ...
    def setFlag(self, arg__1:QtCore.Qt.WindowType, on:bool=...) -> None: ...
    def setFlags(self, flags:QtCore.Qt.WindowFlags) -> None: ...
    def setFormat(self, format:Union[QtGui.QSurfaceFormat, QtGui.QSurfaceFormat.FormatOptions]) -> None: ...
    def setFramePosition(self, point:QtCore.QPoint) -> None: ...
    @overload
    def setGeometry(self, posx:int, posy:int, w:int, h:int) -> None: ...
    @overload
    def setGeometry(self, rect:QtCore.QRect) -> None: ...
    def setHeight(self, arg:int) -> None: ...
    def setIcon(self, icon:Union[QtGui.QIcon, QtGui.QPixmap]) -> None: ...
    def setKeyboardGrabEnabled(self, grab:bool) -> bool: ...
    def setMask(self, region:Union[QtGui.QRegion, QtGui.QBitmap, QtGui.QPolygon, QtCore.QRect]) -> None: ...
    def setMaximumHeight(self, h:int) -> None: ...
    def setMaximumSize(self, size:QtCore.QSize) -> None: ...
    def setMaximumWidth(self, w:int) -> None: ...
    def setMinimumHeight(self, h:int) -> None: ...
    def setMinimumSize(self, size:QtCore.QSize) -> None: ...
    def setMinimumWidth(self, w:int) -> None: ...
    def setModality(self, modality:QtCore.Qt.WindowModality) -> None: ...
    def setMouseGrabEnabled(self, grab:bool) -> bool: ...
    def setOpacity(self, level:float) -> None: ...
    def setParent(self, parent:QtGui.QWindow) -> None: ...
    @overload
    def setPosition(self, posx:int, posy:int) -> None: ...
    @overload
    def setPosition(self, pt:QtCore.QPoint) -> None: ...
    def setScreen(self, screen:QtGui.QScreen) -> None: ...
    def setSizeIncrement(self, size:QtCore.QSize) -> None: ...
    def setSurfaceType(self, surfaceType:QtGui.QSurface.SurfaceType) -> None: ...
    def setTitle(self, arg__1:str) -> None: ...
    def setTransientParent(self, parent:QtGui.QWindow) -> None: ...
    def setVisibility(self, v:QtGui.QWindow.Visibility) -> None: ...
    def setVisible(self, visible:bool) -> None: ...
    def setWidth(self, arg:int) -> None: ...
    def setWindowState(self, state:QtCore.Qt.WindowState) -> None: ...
    def setWindowStates(self, states:QtCore.Qt.WindowStates) -> None: ...
    def setX(self, arg:int) -> None: ...
    def setY(self, arg:int) -> None: ...
    def show(self) -> None: ...
    def showEvent(self, arg__1:QtGui.QShowEvent) -> None: ...
    def showFullScreen(self) -> None: ...
    def showMaximized(self) -> None: ...
    def showMinimized(self) -> None: ...
    def showNormal(self) -> None: ...
    def size(self) -> QtCore.QSize: ...
    def sizeIncrement(self) -> QtCore.QSize: ...
    def startSystemMove(self) -> bool: ...
    def startSystemResize(self, edges:QtCore.Qt.Edges) -> bool: ...
    def surfaceHandle(self) -> int: ...
    def surfaceType(self) -> QtGui.QSurface.SurfaceType: ...
    def tabletEvent(self, arg__1:QtGui.QTabletEvent) -> None: ...
    def title(self) -> str: ...
    def touchEvent(self, arg__1:QtGui.QTouchEvent) -> None: ...
    def transientParent(self) -> QtGui.QWindow: ...
    def type(self) -> QtCore.Qt.WindowType: ...
    def unsetCursor(self) -> None: ...
    def visibility(self) -> QtGui.QWindow.Visibility: ...
    def wheelEvent(self, arg__1:QtGui.QWheelEvent) -> None: ...
    def width(self) -> int: ...
    def winId(self) -> int: ...
    def windowState(self) -> QtCore.Qt.WindowState: ...
    def windowStates(self) -> QtCore.Qt.WindowStates: ...
    def x(self) -> int: ...
    def y(self) -> int: ...


class QWindowStateChangeEvent(QtCore.QEvent):

    @overload
    def __init__(self, arg__1:QtGui.QWindowStateChangeEvent) -> None: ...
    @overload
    def __init__(self, oldState:QtCore.Qt.WindowStates, isOverride:bool=...) -> None: ...

    def clone(self) -> QtGui.QWindowStateChangeEvent: ...
    def isOverride(self) -> bool: ...
    def oldState(self) -> QtCore.Qt.WindowStates: ...


class Qt(QtCore.Qt):
    @staticmethod
    def convertFromPlainText(plain:str, mode:QtCore.Qt.WhiteSpaceMode=...) -> str: ...
    @staticmethod
    def mightBeRichText(arg__1:str) -> bool: ...


def qAlpha(rgb:int) -> int: ...
def qBlue(rgb:int) -> int: ...
@overload
def qGray(r:int, g:int, b:int) -> int: ...
@overload
def qGray(rgb:int) -> int: ...
def qGreen(rgb:int) -> int: ...
def qIsGray(rgb:int) -> bool: ...
def qPixelFormatAlpha(channelSize:int, typeInt:QtGui.QPixelFormat.TypeInterpretation=...) -> QtGui.QPixelFormat: ...
def qPixelFormatCmyk(channelSize:int, alfa:int=..., usage:QtGui.QPixelFormat.AlphaUsage=..., position:QtGui.QPixelFormat.AlphaPosition=..., typeInt:QtGui.QPixelFormat.TypeInterpretation=...) -> QtGui.QPixelFormat: ...
def qPixelFormatGrayscale(channelSize:int, typeInt:QtGui.QPixelFormat.TypeInterpretation=...) -> QtGui.QPixelFormat: ...
def qPixelFormatHsl(channelSize:int, alfa:int=..., usage:QtGui.QPixelFormat.AlphaUsage=..., position:QtGui.QPixelFormat.AlphaPosition=..., typeInt:QtGui.QPixelFormat.TypeInterpretation=...) -> QtGui.QPixelFormat: ...
def qPixelFormatHsv(channelSize:int, alfa:int=..., usage:QtGui.QPixelFormat.AlphaUsage=..., position:QtGui.QPixelFormat.AlphaPosition=..., typeInt:QtGui.QPixelFormat.TypeInterpretation=...) -> QtGui.QPixelFormat: ...
def qPixelFormatRgba(red:int, green:int, blue:int, alfa:int, usage:QtGui.QPixelFormat.AlphaUsage, position:QtGui.QPixelFormat.AlphaPosition, pmul:QtGui.QPixelFormat.AlphaPremultiplied=..., typeInt:QtGui.QPixelFormat.TypeInterpretation=...) -> QtGui.QPixelFormat: ...
def qPixelFormatYuv(layout:QtGui.QPixelFormat.YUVLayout, alfa:int=..., usage:QtGui.QPixelFormat.AlphaUsage=..., position:QtGui.QPixelFormat.AlphaPosition=..., p_mul:QtGui.QPixelFormat.AlphaPremultiplied=..., typeInt:QtGui.QPixelFormat.TypeInterpretation=..., b_order:QtGui.QPixelFormat.ByteOrder=...) -> QtGui.QPixelFormat: ...
def qRed(rgb:int) -> int: ...
def qRgb(r:int, g:int, b:int) -> int: ...
def qRgba(r:int, g:int, b:int, a:int) -> int: ...


# eof
