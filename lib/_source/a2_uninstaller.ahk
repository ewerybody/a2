...
outro(items) {
    ...
    if (!run_silent) {
        result := MsgBox(uninstall_msg . process_msg . continue_msg, NAME, "OKCancel")
        if result == "Cancel"
            ExitApp
    } else
...
ask_for_user_data_deletion(items) {
    ...
    if FileExist(user_data_path) {
        result := MsgBox("Do you also want to delete the user data?", "Delete User data?", "YesNo")
        if result == "Yes"
        {
...
delete_items(items) {
    ...
    if (!run_silent) {
        ...
        progui := Gui(,NAME)
        progtext := progui.AddText("w500")
        progtext.SetFont(,"Small Fonts")
        progress := progui.AddProgress("w500 h20 cb36EC95 vMyProgress Range0-" . num_items, 0)
        progui.Show()
    }
    ...
    if (!run_silent) {
        progress.Value := num_items
        progtext.Value := "Okithxbye! :)"
        Sleep(1500)
        progui.Destroy()
    }
