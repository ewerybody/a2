...
UpdateLayeredWindow(hwnd, hdcSrc, x:="", y:="", w:="", h:="", Alpha:=255) {
    ; Update layered window with handle to the DC of a gdi bitmap.
...
gdipbitmap_update_layered_win(hwnd, pBitmap) {
    Width := Gdip_GetImageWidth(pBitmap), Height := Gdip_GetImageHeight(pBitmap)
    rw := Width//1, rh := Height//1
    hbm := CreateDIBSection(rw, rh)
    hdc := CreateCompatibleDC()
    obm := SelectObject(hdc, hbm)
    G := Gdip_GraphicsFromHDC(hdc)
    ; Gdip_SetInterpolationMode(G, 7)
    Gdip_DrawImage(G, pBitmap, 0, 0, rw, rh, 0, 0, Width, Height)
    UpdateLayeredWindow(hwnd1, hdc, 0, 0, rw, rh)
    SelectObject(hdc, obm)
    DeleteObject(hbm)
    DeleteDC(hdc)
