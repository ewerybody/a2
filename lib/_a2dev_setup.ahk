...
start_ui_btn := A2DevUI.AddButton("xm Section", "Create A2UI`n shortcut")
start_ui_btn.OnEvent("Click", start_ui)
...
start_ui(*) {
    link_path := path_join(root_path, "a2ui.lnk")
    desc := "Start the a2 UI with Python"
    FileCreateShortcut(py_executable_path, link_path, root_path, "ui\a2app.py", desc, icon_path)
}